        ...
        const char *add_action, const char *csrf_token, bud_node *song_options)
{
	return lx_el("form", lx_attr("method", "POST"),
	             ...
	             lx_el("datalist", lx_attr("id", "choir-song-datalist"),
...
static bud_node *
ch_render_songbooks_section(ch_sb_entry_t *songbooks, int n_songbooks)
{
	bud_node *frag = bud_fragment();
	if (!frag)
		return NULL;
	...
	if (n_songbooks == 0) {
		...
		for (int i = 0; i < n_songbooks; i++)
			bud_append(
			        frag,
			        ch_render_songbook_link(
			                songbooks[i].title, songbooks[i].href));
	}
	return frag;
}
...
static bud_node *ch_render_add_song_section(
        ch_opt_entry_t *options, int n_options, const char *choir_id,
        const char *csrf_token)
{
	bud_node *frag = bud_fragment();
	if (!frag)
