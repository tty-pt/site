        ...
        bud_node **out_media)
{
	...
	bud_node *chord_pre =
	        lx_el("pre", lx_attr("data-songbook-chord-data", n_buf),
	...
	bud_node *media_node =
	        lx_el("div", lx_attr("data-songbook-media", n_buf),
	...
	return lx_el("div", lx_attr("data-songbook-item", ""),
	             lx_attr("class",
...
static bud_node *sb_render_header(const char *choir_href, const char *owner)
{
	...
	if (choir_href && choir_href[0]) {
		bud_append(
		        frag, lx_el("a", lx_attr("href", choir_href),
...
static bud_node *sb_build_body_content(void)
{
	...
	if (sb_app_state.choir_id[0])
		snprintf(
		        choir_href, sizeof(choir_href), "/choir/%s",
		        sb_app_state.choir_id);
	{
		bud_node *hdr =
		        sb_render_header(choir_href, sb_app_state.owner);
		if (hdr)
			bud_append(frag, hdr);
	}
	    ...
	    sb_app_state.n_song_options > 0)
	{
		...
		snprintf(
		        ...
		        "/api/songbook/%s/songs", sb_app_state.sb_id);
	...
	if (sb_app_state.n_songs == 0) {
		...
		for (int i = 0; i < sb_app_state.n_songs; i++) {
			...
			snprintf(
			        ...
			        "/songbook/%s/randomize", sb_app_state.sb_id);
