...
poem_render_detail_body(const char *content, const char *owner, const char *id)
{
	...
	if (content && content[0]) {
		...
		if (fixed) {
			inner =
			        bud_tpl("<div class='poem-body font-serif "
			                "leading-relaxed'>%raw</div>",
			                fixed);
			free(fixed);
		}
	}
	...
	bud_node *owner_node =
	        (owner && owner[0]) ? bud_tpl("<div class='text-sm text-muted "
	                                      "text-right'>By %s</div>",
	                                      owner)
	                            : NULL;

	return bud_tpl(
	        "<div class='flex flex-col gap-4'>%node %node</div>", inner,
	        owner_node);
}
