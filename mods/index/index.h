...
NDX_HOOK_DECL(unsigned, index_open, const char *, name, unsigned, mask,
              unsigned, flags, index_cleanup_fn, cleanup);

NDX_HOOK_DECL(unsigned, index_put, unsigned, hd, char *, key, char *, value);

NDX_HOOK_DECL(int, index_id, char *, result, size_t, result_len, const char *,
              title, size_t, title_len);
NDX_HOOK_DECL(int, index_add_item, int, fd, char *, body, char *, id_out,
              size_t, id_len);

NDX_HOOK_DECL(int, index_tsv_load, unsigned, hd, const char *, path,
              index_tsv_cb, cb, void *, user);
NDX_HOOK_DECL(int, index_tsv_save, unsigned, hd, const char *, path);
NDX_HOOK_DECL(int, index_tsv_rebuild, const char *, doc_root, const char *,
