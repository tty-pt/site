...
static bud_node *idx_list_layout(const list_state_t *state)
{
	...
	if (state->nids == 0) {
		form = bud_tpl(
		        "<form method='get' action='' class='list-form'>"
		        "  %node"
		        "  %node"
		        "</form>",
		        filter_wrap,
		        site_ui_empty_state(ui_t("No items found")));
	} else {
		...
		table = hyle_bud_table_actions(
		        col_keys, col_labels, state->ncols,
		        (const char **)state->ids, state->nids,
		        (const char **)state->values, state->module,
		        state->sort_field, state->sort_asc, state->query,
		        &act);
		pagination = hyle_bud_pagination(
		        state->page, state->per_page, state->total,
		        state->nids, state->query);
...
static inline bud_node *idx_list_empty_layout(const list_state_t *state)
{
	return idx_list_layout(state);
}
