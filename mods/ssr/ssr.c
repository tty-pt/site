...
static int ssr_get_handler(int fd, char *body) {
	return core_get(fd, body);
}

static int dispatch_render_result(int fd, struct RenderResult result) {
...
NDX_LISTENER(int, ssr_render_song_detail, int, fd,
             const struct SongDetailRenderFfi *, req) {
	return dispatch_render_result(fd, ssr_render_song_detail_ffi(req));
}

NDX_LISTENER(int, ssr_render_poem_detail, int, fd, const struct PoemRenderFfi *,
             req) {
	return dispatch_render_result(fd, ssr_render_poem_detail_ffi(req));
}

NDX_LISTENER(int, ssr_render_poem_edit, int, fd, const struct PoemRenderFfi *,
             req) {
	return dispatch_render_result(fd, ssr_render_poem_edit_ffi(req));
}

NDX_LISTENER(int, ssr_render_delete, int, fd, const char *, module,
