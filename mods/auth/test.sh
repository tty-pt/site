...
out=$(curl -s -X POST "$BASE/auth/login" -d "username=testuser&password=pass1234")
echo "$out" | grep -q "not confirmed" && pass "not confirmed" || fail "expected 'not confirmed', got: $out"

# 4. Confirm
echo -n "4. Confirm with rcode... "
code=$(curl -sw "%{http_code}" -o /dev/null "$BASE/auth/confirm?u=testuser&r=$RCODE")
...
out=$(curl -s -X POST "$BASE/auth/register" -d "username=testuser&password=pass1234&password2=pass1234&email=test2@test.com")
...
out=$(curl -s -X POST "$BASE/auth/login" -d "username=testuser&password=wrongpass")
...
out=$(curl -s -X POST "$BASE/auth/login" -d "username=nobody&password=pass1234")
