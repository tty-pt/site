...
static void song_meta_read(const char *path, song_meta_t *m) {
	meta_field_t f[] = {{"title",m->title,256},{"type",m->type,256},{"yt",m->yt,512},{"audio",m->audio,512},{"pdf",m->pdf,512},{"author",m->author,256}};
	memset(m, 0, sizeof(*m)); meta_fields_read(path, f, 6);
}

static int song_meta_write(const char *path, const song_meta_t *m) {
	meta_field_t f[] = {{"title",(char*)m->title,0},{"type",(char*)m->type,0},{"yt",(char*)m->yt,0},{"audio",(char*)m->audio,0},{"pdf",(char*)m->pdf,0},{"author",(char*)m->author,0}};
...
static void song_type_index_add(const char *type, const char *id) {
	...
	char l[8192] = {0};
	if (l_v) snprintf(l, sizeof(l), "%s", l_v);
	if (l[0] && strlen(l) < sizeof(l) - 2) strcat(l, ",");
	if (strlen(l) < sizeof(l) - strlen(id) - 2) strcat(l, id);
...
static void song_index_put_meta(const char *id, const song_meta_t *m) {
	char title[256], type[256], author[256], val[768];
...
static int song_index_upsert(const char *doc, const char *id, const char *item_path) {
	song_meta_t m;
...
static int song_viewer_pref_path(const char *user, const char *name, char *out, size_t sz) {
	char s[PATH_MAX]; snprintf(s, sizeof(s), ".tty/%s", name); return user_path_build(user, s, out, sz);
}

static int song_viewer_pref_write(const char *user, const char *name, const char *val) {
	char d[PATH_MAX], p[PATH_MAX]; user_path_build(user, ".tty", d, sizeof(d));
	ensure_dir_path(d); song_viewer_pref_path(user, name, p, sizeof(p));
	return write_file_path(p, val, val ? strlen(val) : 0);
}

static char* song_viewer_pref_read(const char *user, const char *name) {
	char p[PATH_MAX]; if (song_viewer_pref_path(user, name, p, sizeof(p)) != 0) return NULL;
	return slurp_file(p);
}

NDX_LISTENER(int, song_get_viewer_zoom, const char *, user) {
	char *r = song_viewer_pref_read(user, "chords-zoom");
	int v = r ? atoi(r) : VIEWER_ZOOM_DEFAULT; free(r);
	return (v < VIEWER_ZOOM_MIN || v > VIEWER_ZOOM_MAX) ? VIEWER_ZOOM_DEFAULT : v;
}

NDX_LISTENER(int, song_set_viewer_zoom, const char *, user, int, zoom) {
	char b[16]; snprintf(b, sizeof(b), "%d", zoom); return song_viewer_pref_write(user, "chords-zoom", b);
}

NDX_LISTENER(int, song_read_title, const char *, doc, const char *, id, char *, out, size_t, sz) {
	char p[PATH_MAX]; item_path_build_root(doc, "song", id, p, sizeof(p)); return read_meta_file(p, "title", out, sz);
}

static int song_read_type(const char *doc, const char *id, char *out, size_t sz) {
	char p[PATH_MAX]; item_path_build_root(doc, "song", id, p, sizeof(p)); return read_meta_file(p, "type", out, sz);
}

NDX_LISTENER(int, song_transpose_root, const char *, doc, const char *, id, int, semi, int, fl, char **, out, int *, key) {
	char p[PATH_MAX], dp[PATH_MAX]; item_path_build_root(doc, "song", id, p, sizeof(p));
	item_child_path(p, "data.txt", dp, sizeof(dp)); char *c = slurp_file(dp);
	if (!c) return -1;
...
NDX_LISTENER(int, song_get_random_by_type, const char *, t, char **, out) {
	...
	if (!l) l = (char *)qmap_get(type_index_hd, "any");
	if (!l) return -1;
	int c = 1; for (char *p = l; *p; p++) if (*p == ',') c++;
	int idx = rand() % c; char *cp = strdup(l), *tok = strtok(cp, ",");
	for (int i = 0; i < idx && tok; i++) tok = strtok(NULL, ",");
	if (tok && out) *out = strdup(tok);
	free(cp); return (*out) ? 0 : -1;
}

static int parse_params(const char *q, int *t, int *f, int *m) {
	*t = 0; *m = 0; if (!q || !*q) return 0;
	char c[1024]; snprintf(c, sizeof(c), "%s", q); ndc_query_parse(c);
	char b[32]; if (ndc_query_param("t", b, sizeof(b)) > 0) *t = atoi(b);
	if (ndc_query_param("b", b, sizeof(b)) >= 0) *f |= TRANSP_BEMOL;
	if (ndc_query_param("l", b, sizeof(b)) >= 0) *f |= TRANSP_LATIN;
	if (ndc_query_param("h", b, sizeof(b)) >= 0) *f |= TRANSP_HTML;
	if (ndc_query_param("m", b, sizeof(b)) >= 0) *m = 1;
	return 0;
}

static int api_song_viewer_prefs_handler(int fd, char *body) {
	const char *u = get_request_user(fd); if (!u || !u[0]) return respond_plain(fd, 204, "");
	ndc_query_parse(body); char b[16];
	if (ndc_query_param("v", b, sizeof(b)) >= 0) song_set_viewer_zoom(u, atoi(b));
	if (ndc_query_param("b", b, sizeof(b)) >= 0) song_viewer_pref_write(u, "chords-bemol", b);
	if (ndc_query_param("l", b, sizeof(b)) >= 0) song_viewer_pref_write(u, "chords-latin", b);
	return respond_plain(fd, 204, "");
}

static int song_details_auth(int fd, char *body, const item_ctx_t *ctx, void *user) {
	(void)body;
	...
	if (q[0]) {
		if (ssr_response && ctx->username && ctx->username[0]) {
			song_viewer_pref_write(ctx->username, "chords-bemol", (f & TRANSP_BEMOL) ? "1" : "0");
			song_viewer_pref_write(ctx->username, "chords-latin", (f & TRANSP_LATIN) ? "1" : "0");
		}
	} else {
		if (v_b)
			f |= TRANSP_BEMOL;
...
static int song_details_handler(int fd, char *body) {
	return with_item_access(fd, body, CHORDS_ITEMS_PATH, 0, "Not found", "Forbidden", song_details_auth, (void*)TRANSP_HTML);
}

static int api_song_transpose_handler(int fd, char *body) {
	return with_item_access(fd, body, CHORDS_ITEMS_PATH, 0, "Not found", "Forbidden", song_details_auth, NULL);
}

static int song_edit_get_auth(int fd, char *body, const item_ctx_t *ctx, void *u) {
	(void)body; (void)u; song_meta_t m; song_meta_read(ctx->item_path, &m);
	char dp[PATH_MAX]; item_child_path(ctx->item_path, "data.txt", dp, sizeof(dp));
	char *c = slurp_file(dp); form_body_t *fb = form_body_new(0);
	form_body_add(fb, "title", m.title); form_body_add(fb, "type", m.type);
	form_body_add(fb, "yt", m.yt); form_body_add(fb, "audio", m.audio);
	form_body_add(fb, "pdf", m.pdf); form_body_add(fb, "author", m.author);
	if (c) form_body_add(fb, "data", c); free(c); return core_post_form(fd, fb);
}

static int song_edit_get_handler(int fd, char *body) {
	return with_item_access(fd, body, CHORDS_ITEMS_PATH, ICTX_NEED_LOGIN|ICTX_NEED_OWNERSHIP, "Not found", "Forbidden", song_edit_get_auth, NULL);
}

static int song_edit_post_auth(int fd, char *body, const item_ctx_t *ctx, void *u) {
	(void)u; ndc_query_parse(body); song_meta_t m = {0};
	int tl = ndc_query_param("title", m.title, 255); if (tl > 0) m.title[tl] = 0;
	int yl = ndc_query_param("yt", m.yt, 511); if (yl > 0) m.yt[yl] = 0;
	int al = ndc_query_param("audio", m.audio, 511); if (al > 0) m.audio[al] = 0;
	int pl = ndc_query_param("pdf", m.pdf, 511); if (pl > 0) m.pdf[pl] = 0;
	int ul = ndc_query_param("author", m.author, 255); if (ul > 0) m.author[ul] = 0;
	int ty = ndc_query_param("type", m.type, 255); if (ty > 0) m.type[ty] = 0;
	...
	char db[65536]; int dl = ndc_query_param("data", db, 65535);
	char dp[PATH_MAX]; item_child_path(ctx->item_path, "data.txt", dp, sizeof(dp));
	if (write_file_path(dp, db, dl > 0 ? dl : 0) != 0)
		return server_error(fd, "Failed to write song data");
	char loc[256]; snprintf(loc, sizeof(loc), "/song/%s", ctx->id); return redirect(fd, loc);
}

static int song_edit_post_handler(int fd, char *body) {
	return with_item_access(fd, body, CHORDS_ITEMS_PATH, ICTX_NEED_LOGIN|ICTX_NEED_OWNERSHIP, "Not found", "Forbidden", song_edit_post_auth, NULL);
}
...
NDX_LISTENER(char *, song_get_types_json, int, dummy) {
	(void)dummy; json_array_t *ja = json_array_new(0); if (!ja) return NULL;
	unsigned c = qmap_iter(type_index_hd, NULL, 0); const void *k, *v;
	while (qmap_next(&k, &v, c)) {
		char e[256]; json_escape((const char*)k, e, sizeof(e));
		char en[260]; snprintf(en, sizeof(en), "\"%s\"", e); json_array_append_raw(ja, en);
	}
	return json_array_finish(ja);
}

NDX_LISTENER(int, song_get_original_key_root, const char *, doc, const char *, id) {
	int k = 0; char *o = NULL; song_transpose_root(doc, id, 0, 0, &o, &k); free(o); return k;
}

NDX_LISTENER(int, song_get_original_key, const char *, id) { return song_get_original_key_root(".", id); }

NDX_LISTENER(char *, build_all_songs_json, const char *, doc, int, inc_t) {
	(void)doc;
	...
	while (qmap_next(&k, &v, c)) {
		...
		*type++ = '\0';
		...
		if (inc_t) json_array_kv_str(ja, "type", type);
...
static int song_add_post_handler(int fd, char *body) {
	...
	if (item_path_build(fd, "song", id, item_path, sizeof(item_path)) == 0)
		song_index_upsert(".", id, item_path);
...
void ndx_install(void) {
	...
	ndc_register_handler("GET:/api/song/:id/transpose", api_song_transpose_handler);
	ndc_register_handler("POST:/api/song/prefs", api_song_viewer_prefs_handler);
}
