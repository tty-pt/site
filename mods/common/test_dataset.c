...
int ndc_respond_plain(int fd, int status, const char *msg) { return 0; }
void ndc_header_get(int fd, const char *name, char *buf, size_t len) {}
int ndc_query_parse(char *query) { return 0; }
int ndc_query_param(const char *name, char *buf, size_t len) { return -1; }
int csrf_validate(int fd, const char *token) { return 0; }
void ndc_register_handler(const char *pattern, int (*handler)(int, char *)) {}

/* Mock qmap */
unsigned qmap_open(const char *path, const char *name, int ktype, int vtype, int flags, int mode) { return 1; }
void qmap_close(unsigned hd) {}
int qmap_put(unsigned hd, const char *key, const char *val) { return 0; }
...
int qmap_next(const void **key, const void **value, unsigned cur) {
    if (mock_data[mock_iter_idx].key == NULL) return 0;
...
static int test_row_json_cb(json_object_t *row, const char *key, const void *value, void *user) {
