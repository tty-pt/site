...
> Slice 2B-5 of `external/libcorm`. Read these first, in order:
...
> machinery this dialect rides), then `external/libcorm/src/corm.c`
> (current shape, incl. `gen_put`/`gen_del`/`gen_del_all`,
> `corm_fanout_store`/`corm_fanout_unstore`, `corm_write_ref`).
>
> **D14 update (2026-09-16):** historical record of the mm dialect as
> built. The `stoma="… query=…"` leaf grammar here is still valid corm
...
> via `corm_open` aliasing; docs + CHANGELOG landed).

## 1. The built surface as found (2026-09-15)

All paths `external/libcorm/…`; primary type throughout is `:a:s`
(CM_AINDEX-keyed, string payload).

- **Pass-2 ops** (`src/corm.c:1951-1970`): `-p` → `gen_put`
  (`src/corm.c:695-734`); `-d` → `gen_del` (`src/corm.c:463-485`); `-D` →
  `gen_del_all` (`src/corm.c:673-693`); `-g .` armed → `corm_composed_get`,
  unarmed → `gen_get` classic; `-l`, `-m`, `-c`, `-r`, `-g KEY` classic;
  `--list-axes` long-only discovery.
- **Primary open** (`src/corm.c:872`): `corm_open(buf, "hd", ktype, vtype,
  corm_mask_effective(), (flags & CM_FLAGS_MASK) | CM_MIRROR)` — map name
  `"hd"`, mirror flag on. Note for §6: **everything opens `"hd"`**.
- **Roster**: explicit `@csv` in filespec (`mem.db@joint,stoma:a:s`) ∪
  stored `<primary>.roster` sidecar ∪ `-X` names ∪ `CORM_AXIS_LIBS`;
  inter-pass bind (`corm_axes_setup` after `gen_open`);
...
- **Ref-operand rule** (`corm_write_ref`, `src/corm.c:1218-1237`):
...
- **Discovery**: `CORM_AXIS_PATH` dir-list for `lib<name>.so` (default
  `/usr/lib` — never used in this slice; D4); `test-real.sh:66` now
  defaults `CORM_AXIS_REAL_LIBS` script-relative to the in-site
...
- **Stoma sidecar-scan** (`external/libstoma/src/libstoma.c:771-866`):
  ...
  `<base>` as `corm_open(primary, "hd", CM_HNDL, CM_STR, pmask,
  CM_AINDEX | CM_MIRROR)` (`:840`) and bulk-reindexes. **This second open
...
- **F4 — `-d` (`-D`) with stoma in the roster does not persist.**
  ...
  Root cause pinned: two `"hd"`-named handles on one file; libcorm's
...
corm -X '(joint="a=2026-09-14 b=2026-09-16" AND stoma="field=text query=beacon matched=1")' -g . mem.db -t 10
...
New `external/libcorm/test-mm.sh` (+ `Makefile:62-67` `test:` row),
self-contained shell like the sibling gates (`assert_eq/ok/fails`),
own `$td`, real axes via the `test-real.sh:66` `CORM_AXIS_REAL_LIBS`
...
- **(a) libcorm dedup of identical (file, dbid) opens [preferred].**
  A second `corm_open` of the same path+`"hd"` (same types) aliases the
  ...
  `corm_close` interplay, mask/shape mismatches (stoma uses D11-derived
...
libcorm suite stays green; otherwise (b)-style or re-scope as finding.
...
- `external/libcorm/CHANGELOG.md`: `## [Unreleased] — 2B-5 mm dialect`
  entry (recipes, test-mm.sh, F4 fix record).
- Sibling (`external/libstoma` or libcorm core) CHANGELOG only if touched.

## 8. Gates

- New red cases fail first (record exact output in §9), then green after fix.
- `make -C external/libcorm test` green (6 scripts) with
  `CORM_AXIS_REAL_LIBS` **and** `CORM_MASK` unset.
- Touched sibling suite(s) green (`external/libstoma`, and `libjoint` as
  insurance); root `make` green (W06).
- `grep -rn '~/lib\|/home/quirinpa/lib' mm-plan/ external/libcorm/`
...
- 2026-09-15: F4 fix landed in `external/libcorm/src/libcorm.c`
  (`corm_open` aliases the live handle on identical (file, map, shape)
...
- 2026-09-15: gates — `make -C external/libcorm test` all 6 scripts
...
- 2026-09-15: docs landed — `CLI-SURFACE-EXAMPLES.md` §8 + legend (U3
  ...
  §3/U3/§12, `external/libcorm/CHANGELOG.md` Unreleased 2B-5 entry.
  No sibling lib code touched (fix is libcorm-core) → no sibling
