...
| 9 | Axis-contributed CLI flags (D14) | `--query`/`--min-sim` plugin flags broadcast to every bound axis that declares them via `rec_axis_cli_options`/`rec_axis_config_arg` (D14 — collector mirror-scan + dynamic getopt table + post-bind flush in libqmap; inline `--name=value` only; credentials env-only); sepal adopts `query` + `min-sim`, stoma adopts `query`; `decode` merges CLI into leaf params (leaf spec > CLI > env); pi-mm emits a bare `sepal` leaf + static `stoma="field=text matched=1"` and appends `--query=… --min-sim=…` as extra args. Detail: `RECALL-KERNEL.md` §3 convention, `CLI-SURFACE-EXAMPLES.md` §11 | ✅ **DONE 2026-09-16** — libqmap collector + dynamic table + flush + 11 new test-cli rows (all green); libsepal `rec_axis_cli_options`/`rec_axis_config_arg` + decode merge + bare-leaf fallback (all green, 87465+ asserts); libstoma `stoma_cli_query` + decode merge (0 warnings, 242/242 + stress + property + rebuild tests green); pi-mm `scanExpr` drop `sepalLeafForText`, `buildScanInvocation` gains `extraArgs`, scan emits bare `sepal` + flags (deno 62/62 + lint + complexity + integration all green); 5-lib gates green, site `make` W06 PASS. No commits. |
...
2026-09-16 (detail §9 table row; `RECALL-KERNEL.md` convention section;
