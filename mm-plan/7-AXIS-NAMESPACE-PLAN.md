...
### 1. Directory-scoped axis spec (D9) — `corm.c:964-972`

```c
corm_axes_spec(const char *name, ...)  →  "<dir>%s/<name>.db"
```

`dirname(corm_path)` + `<name>.db` means the axis files are
...
### 3. `./` alias mismatch — `corm.c:872` vs `libstoma.c:840`

CLI opens the primary verbatim (`mem2.db`); stoma re-opens it as
`./mem2.db` (from `dirname("./stoma.db")`). `corm_open`'s live-handle
...
- `sepal.db` (3436 B) held vectors from prior runs/DBs → `corm: 3 refs
  skipped: no primary record`.
- First run of a fresh primary with pre-existing orphan `joint.db`:
  `corm: axis store found (./joint.db) beside mem2.db but no roster yet`.
...
1. **Per-primary axis store names.** `corm_axes_spec` becomes
...
2. **`CORM_AXIS_PRIMARY` binding env.** `corm_axes_setup` (after the
   primary is open, `corm.c:1941-1945`) sets
   `CORM_AXIS_PRIMARY=<corm_path verbatim>`. stoma's rebuild prefers it
...
### Phase A — libcorm (`external/libcorm`)

- **A-1** `corm_axes_spec` (`corm.c:964-972`): emit
...
- **A-2** `corm_axes_setup` (`corm.c:1085+`): `setenv("CORM_AXIS_PRIMARY",
  corm_path, 1)` before binding slots.
- **A-3** The orphan-store diagnostic (`corm.c:1103-1115`) uses the new
  spec automatically (it calls `corm_axes_spec` — no code change,
  verify message shows `garden.db-joint` now).
- Tests: unit test for `corm_axes_spec` output shape (new name + legacy
...
- **B-1** `rec_axis_open` (`libstoma.c:771-875`): if
  `getenv("CORM_AXIS_PRIMARY")` is set and `<it>.roster` exists, use that
...
- Per-repo `make test` green (libcorm, libstoma, libjoint, libsepal);
...
```sh
export CORM_AXIS_PATH=external/libjoint/lib:external/libstoma/lib:external/libsepal/lib
mkdir -p data
corm -p 1:"2026-09-14:I was with my grandfather under the cherry tree" "data/garden.db@joint,stoma,sepal:a:s"
corm -p 1:"2026-09-14:A boat ride with a panda can be deep" "data/park.db@joint,stoma,sepal:a:s"
ls data/                        # garden.db-joint garden.db-stoma garden.db-sepal park.db-*
corm -X 'stoma="field=text query=grandfather matched=1"' -g . "data/garden.db@joint,stoma,sepal:a:s" -t 10   # → ref 1
corm -X 'stoma="field=text query=grandfather matched=1"' -g . "data/park.db@joint,stoma,sepal:a:s"  -t 10   # → (nothing)
```
