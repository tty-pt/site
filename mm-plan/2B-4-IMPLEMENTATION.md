...
> Precise, normative spec for slice 2B-4 of `external/libcorm`. Read these
...
> D1/D11/D12/D13, the 2B-4 slice), `external/libcorm/docs/RECALL-KERNEL.md`
> (`rec_axis_store` / `rec_axis_store_typed` / `rec_axis_unstore` /
> `rec_axis_readback` conventional exports), then
> `external/libcorm/src/corm.c` (current shape).

## 0. Scope & file deltas

| File | Action |
|---|---|
| `src/corm.c` | `QDBE_MASK` 4095 + `CORM_MASK`; per-slot capability table at bind; fan-out store/unstore; `gen_put`/`gen_del`/`gen_del_all` → int; `corm_write_ref`; exit-code threading |
...
Ground rules: work only in `external/libcorm` (+ the one-line-shape
...
extension (`-X` names, `CORM_AXIS_LIBS`) binds query-only axes that are
never fan-out targets. The roster is the effective one: explicit `@`
(or the stored sidecar roster read back by `corm_axes_setup`).

Firing rule: the composed write path fires **only** when `roster_n > 0`
(effective roster) **and** the primary key type is `CM_HNDL` (`:a:`).
...
- The fan-out payload is what the primary actually stored:
  `corm_get(prim_hd, &ref)` (string fallback `value_ptr` only when the
  get misses). `len`/`qtype` come from `metas[prim_hd].types[1]` +
  `corm_type_len(vtype)` (0 for `CM_STR`, fixed size for the rest).
- `-d REF` / `-D REF`: `corm_write_ref` — whole-arg decimal u32, else
  the primary reverse-view name lookup (`corm_get(prim_hd + 1, name)`).
  Resolved **before** `gen_lookup` (it truncates the operand). In
  composed mode the primary is deleted **by u32 ref** (`corm_del` /
  `corm_del_all` on `prim_hd`) — this is the mm `forget`; the legacy
...
- Unresolvable write operand in composed mode: loud
  `corm: write ref 'X': no primary record`, `EXIT_FAILURE` (reads
...
|---|---|---|---|
| `CM_STR` | string store | `rec_axis_store(ctx, NULL, ref, payload)` | verbatim string |
| `!= CM_STR` | typed export present | `rec_axis_store_typed(ctx, NULL, ref, payload, len, qtype)` | binary payload |
| `!= CM_STR` | typed export absent | **loud skip** (`binary payload, text-only axis (no store_typed), skipped`) | no silent corruption |
...
`corm_fanout_store` / `corm_fanout_unstore` attempt **every** roster
...
```
corm: axis '<name>': store skipped (no ctx bound)
corm: axis '<name>': read-only (no store symbol), skipped
corm: axis '<name>': binary payload, text-only axis (no store_typed), skipped
corm: axis '<name>': store rejected ref <r>
corm: axis '<name>': unstore skipped (no ctx bound)
corm: axis '<name>': read-only (no unstore symbol), skipped
corm: axis '<name>': unstore rejected ref <r>
corm: write ref '<operand>': no primary record
corm: axis fan-out needs an :a:-type primary (refs must exist); primary written, axes skipped
corm: invalid CORM_MASK '<e>' (need 2^n-1)
```
...
(lets the gate prove *which* symbol the CLI chose), raw for `CM_STR`,
`-1`/`EINVAL` otherwise; `unstore` = `corm_del` (idempotent, returns
...
`corm_mask_effective()` derives `CORM_MASK`-or-`4095` (validated,
...
non-`:a:` + roster declined · `CORM_MASK=255` round-trip + invalid
...
1. **Forward declared the fan-out trio** (`static` defs live after
   `corm_axes_list`; `gen_*` call them earlier) — first build failed
