...
- WASM-safe vs native-only split holds: `site_ui.c` + `mods/index/ux/list.c` WASM-compilable; `mods/common/list_fill.c` + `mods/source/source.c` native-only. Sanctioned includes limited to pure C-isomorphic `mods/common/ux/site_ui.c`/`mods/index/ux/list.c`.
