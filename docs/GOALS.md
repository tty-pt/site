...
- **Global chrome mixed into page tree:** `mods/common/ux/site_ui.c:447`
  `site_ui_layout` returns `main.main + nav.menu` with
  `input#menu-functions`, overlay, toggle, and `site_ui_menu:162`
  back-link. `NAV.md:6` requires `#chrome-root` (global) separate from
  `#bud-root` (page-local). Splitting needs `site_ui_chrome()` vs
  `site_ui_layout()` (`NAV.md:535`).
...
- Keep WASM-safe vs native-only files split. `site_ui.c` and
  ...
  Including a native file into a WASM TU (as `mods/gig/ux/detail.c:19`
  does with `list_fill.c`) links but crashes only in the browser.
- Include-source reuse (`detail.c:12` `#include "../../common/ux/site_ui.c"`)
  ...
  minimal; a global/page split should become `site_ui_chrome.c` +
  `site_ui_layout.c` with headers if the TU grows (`NAV.md:998`).
- Cache bust is build-owned: bumping `SITE_CSS_V` (`mods/common/ux/site_ui.c:701`
  `"?v=20"`) and `bud-client.js` version must happen at build, not
  hand-edited four times (`docs/BUILD.md:57`, `NAV.md:837`).
...
- **Single-module loader:** `htdocs/bud-client.js:10` `mod.split(/\s+/)[0]`
  ...
  `list` (`NAV.md:181`).
- **Conditional global:** `mods/common/ux/site_ui.c:711`
  `if (module && module[0])` emits `data-modules`/`bud-client.js` only
  when a local module exists. `/` (`mods/index/index.c:464`),
  `poem`/`grp` detail (`mods/poem/poem.c:104`, `mods/grp/grp.c:628`),
  error pages (`mods/common/common_response.c:82`) get no global WASM.
