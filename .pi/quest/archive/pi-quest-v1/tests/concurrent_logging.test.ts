...
Deno.test("concurrent_logging: per-run log isolation and concurrency guarantees", async (t) => {
  ...
  try {
    await t.step(
      "1. Start run A -> log event -> run B -> log event -> A log contains only A, B log contains only B",
      async () => {
        ...
        await asyncContext.run(ctxA, async () => {
          logEvent("QUEST_CREATED", "created alpha quest", {
            logPath: logPathA,
            questId: questIdA,
          });
          logEvent("TURN_START", "turn 1 start in A", {
            logPath: logPathA,
            questId: questIdA,
          });
        });
        ...
        await asyncContext.run(ctxB, async () => {
          logEvent("QUEST_CREATED", "created beta quest", {
            logPath: logPathB,
            questId: questIdB,
          });
          logEvent("TURN_START", "turn 1 start in B", {
            logPath: logPathB,
            questId: questIdB,
          });
          logEvent("TURN_END", "turn 1 end in B", {
            logPath: logPathB,
            questId: questIdB,
          });
        });
        ...
        assert.ok(
          !contentA.includes("quest-beta"),
          "Log A must not contain run B events",
        );
        assert.ok(
          !contentA.includes("turn 1 start in B"),
          "Log A must not contain run B events",
        );
        ...
        assert.ok(
          !contentB.includes("quest-alpha"),
          "Log B must not contain run A events",
        );
        assert.ok(
          !contentB.includes("turn 1 start in A"),
          "Log B must not contain run A events",
        );
      },
    );
    ...
    await t.step(
      "2. Same root quest in two independent runs -> separate run IDs and separate logs",
      async () => {
        ...
        await asyncContext.run(ctx1, async () => {
          logEvent("QUEST_CREATED", "run 1 started common quest", {
            logPath: logPath1,
            questId: questId1,
          });
        });
        ...
        await asyncContext.run(ctx2, async () => {
          logEvent(
            "QUEST_CREATED",
            "run 2 started common quest independently",
            { logPath: logPath2, questId: questId2 },
          );
        });
    ...
    await t.step(
      "3. One run with multiple subquests -> exactly one run log, events identify active subquest",
      async () => {
        ...
        await asyncContext.run(ctx, async () => {
          ...
          logEvent("SUBQUEST_START", "sub-parser started", {
            quest: "sub-parser",
            parent: "root-engine",
            logPath,
            questId,
          ...
          logEvent("SUBQUEST_START", "sub-compiler started", {
            quest: "sub-compiler",
            parent: "root-engine",
            logPath,
            questId,
          ...
          s.active = "root-engine";
          s.stack = ["root-engine"];
          logEvent("SUBQUEST_RETURN", "sub-compiler returned", {
            quest: "root-engine",
            child: "sub-compiler",
            logPath,
            questId,
    ...
    await t.step(
      "4. Compaction/resume within a run -> continues in the same run log",
      async () => {
        ...
        await asyncContext.run(ctx, async () => {
          logEvent("TURN_START", "before compaction turn", {
            logPath,
            questId,
          });
          ...
          logEvent("COMPACTION_STARTED", "compaction started", {
            compactionId: tx.id,
            logPath,
            questId,
          });
          ...
          assert.strictEqual(
            preserved.questId,
            questId,
            "Compaction state must preserve questId",
          );

          // Post-compaction resume
          logEvent("COMPACTION_COMPLETED", "compaction completed", {
            compactionId: tx.id,
            logPath,
            questId,
          });
          logEvent("RESUME_DELIVERED", "resumed in same run", {
            compactionId: tx.id,
            logPath,
            questId,
          });
    ...
    await t.step(
      "5. Diagnostic packaging for run A -> includes A's log and quest state, does not include B's",
      async () => {
        ...
        await writeFile(
          join(currentDir, questIdA, "quest.md"),
          "# Quest: quest-A\n\n## Sub-Quests\n- [ ] [[sub-A]]\n",
          "utf8",
        );
        ...
        await writeFile(
          join(currentDir, questIdB, "quest.md"),
          "# Quest: quest-B\n\n## Sub-Quests\n- [ ] [[sub-B]]\n",
          "utf8",
        );
        ...
        await writeFile(
          logPathA,
          "2026-08-31T03:00:00.000Z | QUEST_CREATED | questId=" + questIdA +
            " root=quest-A quest=quest-A | init A\n" +
            "2026-08-31T03:01:00.000Z | SUBQUEST_START | questId=" + questIdA +
            " root=quest-A quest=sub-A parent=quest-A child=sub-A | sub A\n",
          "utf8",
        );
        ...
        await writeFile(
          logPathB,
          "2026-08-31T03:10:00.000Z | QUEST_CREATED | questId=" + questIdB +
            " root=quest-B quest=quest-B | init B\n" +
            "2026-08-31T03:11:00.000Z | SUBQUEST_START | questId=" + questIdB +
            " root=quest-B quest=sub-B parent=quest-B child=sub-B | sub B\n",
          "utf8",
        );
        ...
        assert.ok(
          entriesA.includes("run/manifest.txt") ||
            entriesA.includes("diagnostic/current-run/manifest.txt"),
        );
        assert.ok(entriesA.includes("run/initial-prompt.txt"));
        assert.ok(entriesA.includes("run/summary.md"));
        assert.ok(
          entriesA.includes("run/execution.log") ||
            entriesA.includes("diagnostic/current-run/run.log"),
        );
    ...
    await t.step(
      "6. Concurrent writes to separate run logs -> no interleaving or state cross-contamination",
      async () => {
        ...
        const task1 = async () => {
          for (let i = 0; i < count; i++) {
            await asyncContext.run(ctx1, async () => {
              logEvent("TURN_START", `worker 1 iteration ${i}`, {
                logPath: log1,
                questId: questId1,
              });
            });
        ...
        const task2 = async () => {
          for (let i = 0; i < count; i++) {
            await asyncContext.run(ctx2, async () => {
              logEvent("TURN_START", `worker 2 iteration ${i}`, {
                logPath: log2,
                questId: questId2,
              });
            });
        ...
        assert.strictEqual(
          lines1.length,
          count,
          "Worker 1 should have exact expected entry count",
        );
        assert.strictEqual(
          lines2.length,
          count,
          "Worker 2 should have exact expected entry count",
        );

        assert.ok(
          lines1.every((l) =>
            l.includes("worker 1") && l.includes(questId1) &&
            !l.includes("worker 2")
          ),
        );
        assert.ok(
          lines2.every((l) =>
            l.includes("worker 2") && l.includes(questId2) &&
            !l.includes("worker 1")
          ),
        );
      },
