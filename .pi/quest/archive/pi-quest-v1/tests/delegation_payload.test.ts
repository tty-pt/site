...
Deno.test("Structured Delegation Payload Suite: pi-subagents bridge contract (#59)", async (t) => {
  ...
  assert.ok(
    executor,
    "executor must be resolved when the subagent tool is registered",
  );
  ...
  await t.step(
    "1. structured payload carries ownerRunId/nodeId/result and no unsupported async",
    async () => {
      ...
      assert.ok(
        validId(ownerRunId),
        "ownerRunId must be a valid delegation id",
      );
      ...
      const emitted = bus.emitted.find((e) =>
        e.event === "prompt-template:subagent:request"
      );
      assert.ok(emitted, "must emit prompt-template:subagent:request");
      const payload = emitted.data;
      assert.ok(
        Object.hasOwn(payload, "ownerRunId"),
        "payload must carry ownerRunId marker",
      );
      assert.ok(
        Object.hasOwn(payload, "nodeId"),
        "payload must carry nodeId marker",
      );
      ...
      assert.ok(
        !Object.hasOwn(payload, "async"),
        "unsupported async field must be omitted",
      );
  ...
  await t.step(
    "2. requestId and nodeId are unique across invocations (same-turn safety)",
    async () => {
      ...
      assert.notStrictEqual(
        first.requestId,
        second.requestId,
        "requestId must be unique per invocation",
      );
      assert.notStrictEqual(
        first.nodeId,
        second.nodeId,
        "nodeId must be unique per invocation",
      );
    },
  );
});
