...
Deno.test("quest_journal_economy: periodic checkpoint (token thresholds removed) and subquest threshold", async () => {
  ...
  try {
    ...
    assert.ok(
      commands["quest-economy"],
      "/quest-economy command should be registered",
    );
    ...
    assert.ok(
      notifiedMessages.some((m) =>
        m.includes("periodic") || m.includes("Periodic") ||
        m.includes("subquest")
      ),
      `Should report periodic checkpoint: ${JSON.stringify(notifiedMessages)}`,
    );
    ...
    assert.ok(
      ...
      `Should support configuring subquest launch threshold to 40k via third arg, got: ${
        JSON.stringify(notifiedMessages)
      }`,
    );
    ...
    assert.ok(
      notifiedMessages.some((m) => m.includes("40k") || m.includes("40,000")),
      `subquest threshold command: ${JSON.stringify(notifiedMessages)}`,
    );
    ...
    assert.ok(
      notifiedMessages.some((m) =>
        m.toLowerCase().includes("disabled") || m.includes("off")
      ),
      `Should allow disabling subquest launch, got: ${
        JSON.stringify(notifiedMessages)
      }`,
    );
    ...
    assert.ok(
      notifiedMessages.some((m) =>
        m.toLowerCase().includes("default") || m.includes("periodic")
      ),
      `Should restore default, got: ${JSON.stringify(notifiedMessages)}`,
    );
  } finally {
...
Deno.test("quest_journal_economy: periodic constant and deprecated stubs", async () => {
  const { DEFAULT_CHECKPOINT_INTERVAL_TURNS } = await import(
    "../src/constants.ts"
  );
  assert.strictEqual(
    DEFAULT_CHECKPOINT_INTERVAL_TURNS,
    6,
    "periodic interval must be 6",
  );
  ...
  const {
    ...
    CompactionPressure,
  } = await import("../src/compaction/index.ts");
  // Deprecated stubs always return neutral values (periodic replaces pressure)
  const ctx: any = {
    cwd: process.cwd(),
    getContextUsage: () => ({ tokens: 999999, contextWindow: 100000 }),
    sessionManager: { getBranch: () => [] },
  };
