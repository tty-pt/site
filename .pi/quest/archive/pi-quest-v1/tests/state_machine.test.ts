...
Deno.test("quest_journal_state_machine: deterministic lifecycle, no implicit activation, verified saves only, and loop prevention", async () => {
  ...
  for (const cb of handlers["tool_result"] || []) {
    await cb({
      toolName: "write",
      input: {
        path: ".pi/quest/current/arbitrary/other.md",
        content: "arbitrary",
      },
      isError: false,
    }, mockCtx);
  ...
  assert.strictEqual(
    userMessages.length,
    0,
    "Clean state on turn_end must not emit save request",
  );
});
...
Deno.test("quest_journal_state_machine: periodic checkpoint, verified save gate, and compaction lifecycle", async () => {
  ...
  assert.strictEqual(
    userMessages.filter((m) =>
      (typeof m.msg === "string" ? m.msg : "").includes(
  ...
  for (let i = 0; i < 3; i++) {
    for (const cb of handlers["tool_result"] || []) {
      await cb(
        { toolName: "edit", input: { path: `mods/song/${i}a.c` } },
        mockCtx,
      );
    }
    for (const cb of handlers["turn_end"] || []) {
  ...
  for (const cb of handlers["tool_result"] || []) {
    await cb(
      { toolName: "edit", input: { path: "mods/song/final.c" } },
      mockCtx,
    );
  }
  for (const cb of handlers["turn_end"] || []) {
    await cb({
      toolResults: [{ toolName: "edit", input: { path: "mods/song/final.c" } }],
    }, mockCtx);
  }
  ...
  for (let i = 0; i < 6; i++) {
    for (const cb of handlers["tool_result"] || []) {
      await cb(
        { toolName: "edit", input: { path: `mods/song/b${i}.c` } },
        mockCtx,
      );
    }
    for (const cb of handlers["turn_end"] || []) {
  ...
  assert.ok(
    ...
    "6th turn after save should trigger periodic",
  );
