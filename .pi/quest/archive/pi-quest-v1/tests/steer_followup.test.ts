...
Deno.test("quest_journal_steer_followup: in-flight steering, post-compaction continuation, tool freedom, and root turn-1 confirmation", async () => {
  ...
  assert.ok(
    userMessages.length > 0,
    "Root quest initialization should send initial directive",
  );
  const rootInitMsg = userMessages[userMessages.length - 1].msg;
  assert.ok(
    rootInitMsg.includes("ASK FOR USER CONFIRMATION") ||
      rootInitMsg.includes("confirmation"),
  ...
  for (const cb of handlers["turn_end"] || []) {
    await cb({
      toolResults: [{ toolName: "edit", input: { path: "mods/song/5.c" } }],
    }, mockCtx);
  }
  ...
  assert.strictEqual(
    followUpEntry.options?.deliverAs,
    "steer",
    "Periodic checkpoint must use deliverAs: 'steer'",
  );
  assert.ok(
  ...
  assert.notStrictEqual(
    ...
    true,
    "session_before_compact must allow compaction after verified save",
  );
  ...
  assert.ok(
    userMessages.length > 0,
    "session_compact / session_before_compact must send post-compaction continuation",
  );
  const postCompactEntry = userMessages[0];
  assert.ok(
    postCompactEntry.options?.deliverAs === "followUp" ||
      !postCompactEntry.options,
    "Post-compaction continuation must be delivered to agent",
  );
  assert.ok(
    postCompactEntry.msg.includes(
  ...
  assert.strictEqual(
    compactInvocationCount,
    0,
    "Subquest creation under low tokens must NOT trigger launch compaction",
  );
  assert.ok(
    subToolRes.content[0].text.includes("Created sub-quest"),
    "Subquest creation succeeded",
  );
