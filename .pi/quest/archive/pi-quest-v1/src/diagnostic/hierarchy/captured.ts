...
export async function collectCapturedSubQuests(
    ...
    try {
      ...
      for (const sub of declared) {
        if (sub !== resolvedRoot && !seenSubquests.has(sub)) {
          if (questMap.has(sub)) {
            seenSubquests.add(sub);
            capturedSubQuests.push({
              name: sub,
              path: questMap.get(sub)!.path,
            });
          } else {
            ...
            if (archSub) {
              ...
              capturedSubQuests.push({
                name: sub,
                path: resolve(projectRoot, QUEST_CURRENT_DIR, sub, "quest.md"),
              });
            }
          }
        }
  ...
  for (const ev of detailedEvents) {
    if (
      ev.parent === resolvedRoot && ev.child && !seenSubquests.has(ev.child)
    ) {
      const sub = ev.child;
      if (questMap.has(sub)) {
        seenSubquests.add(sub);
