...
export async function handleQuestStatusCommand(
  _args: string,
  ...
  if (state.pendingRootQuest) {
    ...
    const line =
      `[PROVISIONAL ROOT INITIALIZATION] - Research required to establish quest identity and plan. Original request: "${reqPreview}..."`;
  ...
  const tokenStr = tokens !== null
    ? `${formatTokens(tokens)}`
    : `~${Math.round(usagePercent(ctx))}%`;
  const checkpointStr =
    `checkpoint every ${DEFAULT_CHECKPOINT_INTERVAL_TURNS} turns`;
  let parentInfo = "";
  const parentFromStack = state.stack && state.stack.length >= 2
  ...
  if (exists) {
    try {
      ...
      if (subQuests.length > 0) {
        subInfo = ` | sub-quests: ${subQuests.join(", ")}`;
      }
      const audit = auditQuestConsistency(content, {
        recentModifiedFiles: state.sessionModifiedFiles,
      });
  ...
  const line = exists
    ? `${path}${parentInfo} [${hier}] - ${
      fresh ? "fresh" : "SAVE PENDING"
    }${runInfo}, tokens ${tokenStr} (${checkpointStr}), prompts ${state.prompts.length}${subInfo}`
    : `${path || state.active} - MISSING on disk!`;
  if (ctx.hasUI) {
    ctx.ui.notify(`Active quest: ${line}`, fresh ? "info" : "warning");
  }
  return line;
}
...
export async function handleQuestsCommand(
  ...
  for (const r of currentRecords) {
    ...
    try {
      ...
      logError(
        `Failed to read quest file for parent linking at ${r.path}`,
        err,
        ctx,
      );
    }
  ...
  ctx.ui.setWidget("quest", [
    `Active: ${
      state.active ? (questPath(state.questId) || state.active) : "(none)"
    }`,
    "",
    "Current quests:",
    ...(renderedCurrent.length
      ? renderedCurrent
      : ["  (none - use /quest <name>)"]),
    "",
