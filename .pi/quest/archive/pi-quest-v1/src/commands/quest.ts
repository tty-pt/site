...
import {
  loadExistingQuestEpistemicState,
  loadExistingQuestIntent,
...
export async function resolveQuestTarget(
  args: string,
  ctx: ExtensionContext,
): Promise<{ name: string; goal: string } | null> {
  const trimmed = args.trim();
  if (!trimmed) {
    const choice = await promptForQuestChoice(
      ctx,
      "Which quest do you want to work on?",
    );
    if (!choice) {
  ...
  const firstRecord = spaceIdx > 0
    ? await resolveQuestRecordBySlug(firstSlug)
    : null;
  const fullFuturePath = `${FUTURE_DIR}/${fullSlug}.md`;
  const firstFuturePath = `${FUTURE_DIR}/${firstSlug}.md`;
  const existsFullFuture = await (async () => {
    try {
      await readFile(fullFuturePath, "utf8");
      return true;
    } catch {
      return false;
    }
  })();
  const existsFirstFuture = spaceIdx > 0
    ? await (async () => {
      try {
        await readFile(firstFuturePath, "utf8");
        return true;
      } catch {
        return false;
      }
    ...
    : false;
  if (fullRecord || existsFullFuture) {
    return { name: fullSlug, goal: spaceIdx > 0 ? trimmed : "" };
  }
  if (spaceIdx > 0 && (firstRecord || existsFirstFuture)) {
...
export async function ensureQuestFileOnDisk(
  name: string,
  goal: string,
  originalReq: string,
  ctx: ExtensionContext,
    ...
    try {
      ...
      try {
        ...
        if (qid) {
          ...
          try {
            const h = createHash("sha256").update(futureContent || name).digest(
              "hex",
            ).slice(0, 12);
            logEvent("DRAFT_DISCARDED", `draft discarded`, {
              quest: name,
              slug: name,
              hash: h,
              dest: destArch,
              reason: "ensureQuestFileOnDisk",
            });
          } catch {}
      ...
      if (!finalGoal && ctx.hasUI && ctx.mode === "tui") {
        finalGoal =
          ((await ctx.ui.input("Describe the goal for this quest:")) ?? "")
            .trim();
      }
...
export async function applyQuestSwitchEpistemicState(
  name: string,
  targetState?: any,
): Promise<void> {
  ...
  if (loaded.exists) {
    ...
    const isTargetConfirmed = Array.isArray(s.confirmedQuests) &&
      s.confirmedQuests.includes(name);
    s.awaitingUserConfirmation = isRootQuest(s)
      ? (!isTargetConfirmed && (loaded.awaitingUserConfirmation ?? false))
      : false;
    if (loaded.researchComplete) {
      ...
      startResearchEpoch(
        s,
        s.reassessmentRequired ? "reassessment" : "research",
      );
    }
  } else {
    s.researchRound = 1;
...
export function buildQuestStartInstructions(
  name: string,
  path: string,
  goal: string,
): string {
...
export async function handleQuestCommand(
  ...
  const originalReq = s.pendingRootRequest ||
    (s.prompts && s.prompts.length > 0 ? s.prompts[0] : "");
...
export async function handleQuestSaveCommand(
  _args: string,
  ctx: ExtensionContext,
  pi: ExtensionAPI,
): Promise<void> {
  ...
  sendSaveRequest(
    pi,
    "Quest-journal: /quest-save -- write a full state snapshot to the active quest file now.",
  );
  state.lastPromptAt = Date.now();
}

export async function handleQuestRefineCommand(
  args: string,
  ctx: ExtensionContext,
  pi: ExtensionAPI,
): Promise<void> {
  ...
  if (!refinement && ctx.mode === "tui") {
    refinement =
      (await ctx.ui.input("Enter quest refinement or new requirements:")) ?? "";
  }
  if (!refinement) {
  ...
  triggerReassessment(
    state,
    `User refinement received: "${refinement.slice(0, 100)}..."`,
    refinement,
  );
  persist(pi, ctx);
  sendSaveRequest(
    ...
    }\` now: expand ## Goal if needed, add entry under ## Quest Refinements & User Feedback Loops, update ## Remaining work and ## Test / Build Status, and record any new decisions.`,
  );
  state.lastPromptAt = Date.now();
  if (ctx.hasUI) {
    ctx.ui.notify(
      `Refinement queued for active quest '${state.active}'`,
      "info",
    );
  }
}

export async function handleQuestDelCommand(
  args: string,
  ctx: ExtensionContext,
  pi: ExtensionAPI,
): Promise<void> {
