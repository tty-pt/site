...
export function isToolExecutionError(tr: any): boolean {
  return (
    !!tr?.isError ||
    !!tr?.error ||
    !!(tr?.details && (tr.details.error || tr.details.success === false))
  );
}

export function detectBashToolFailure(tr: any): {
  hasFailure: boolean;
  reason: string;
  evidence: string;
} {
...
export function isPathToActiveQuest(
  targetPath: string,
  activeQuest: string,
): boolean {
...
export function classifyToolResultForTurn(
  ...
  if (
    tr?.details?.gateBlocked &&
    tr?.details?.code === "REVIEW_COALESCENCE_PENDING"
  ) {
    const toolFailed = isToolExecutionError(tr);
...
export function analyzeTurnToolResults(
  ...
  for (const tr of toolResults) {
    ...
    if (classified.failure) {
      ...
      const recoveryFor = state.lastFailureId ?? undefined;
      logVerificationTransition(
        isBuildOnly ? "BUILD_PASSED" : "TEST_PASSED",
        `command passed: ${cmd}`,
        {
          quest: activeQuest,
          command: cmd,
...
export function applyTurnEndStateTransitions(
  ...
  if (analysis.meaningfulFailureDetected) {
    ...
    targetState.substantiveTurnsSinceCheckpoint =
      (targetState.substantiveTurnsSinceCheckpoint || 0) + 1;
    persist(pi, ctx);
    updateUIStatus(ctx);
  }
}
