...
} from "./lifecycle/archive/context.ts";
...
} from "./lifecycle/archive/removal.ts";
import { createArchiveZip } from "./lifecycle/archive/zip.ts";
import {
  hydrateNextActive,
  popArchivedAndFindNextActive,
} from "./lifecycle/archive/stack.ts";
...
export type LifecycleStage =
  ...
  | "zip_creation"
  | "changelog_appended";
export type LifecycleStageObserver = (
  stage: LifecycleStage,
  details: any,
) => void;

export let onLifecycleStageTransition: LifecycleStageObserver | null = null;
export function setLifecycleStageObserver(
  observer: LifecycleStageObserver | null,
): void {
  onLifecycleStageTransition = observer;
}

export function initProvisionalRootQuest(
  pi: ExtensionAPI,
  ...
  return await withReviewFileLock(slug, async () => {
    ...
    if (!isExistingOnDisk && (await fileExists(futurePath))) {
      ...
      try {
        ...
        try {
          copyFileSync(futurePath, destArch);
        } catch {
          try {
        ...
        try {
          ...
          const h = createHash("sha256").update(content).digest("hex").slice(
            0,
            12,
          );
          logEvent("DRAFT_PROMOTED", `draft promoted`, {
            quest: state.active || slug,
            slug,
            hash: h,
            dest: destArch,
            reason: "activateExistingQuest",
          });
        } catch {}
      } catch {}
      await rename(futurePath, path);
      if (ctx?.hasUI) {
        ctx.ui.notify(`Promoted draft ${futurePath} → ${path}`, "info");
      }
    } else if (!isExistingOnDisk) {
      logQuestTransition(
        "QUEST_ACTIVATION_FAILED",
        `failed to activate quest '${slug}': file not found`,
        { quest: slug, reason: "file_not_found" },
      );
      return false;
    }
    ...
    if (isQuestSessionActive(targetQid, sessionId)) {
      ...
      logEvent(
        ...
        `quest already active in another live session; refusing duplicate mount`,
        {
          quest: targetQid,
          questId: targetQid,
          slug,
          sessionId,
          activeSession: activeInfo?.sessionId || "",
        },
      );
      reportAgentError(
        pi,
        ctx,
        `Quest '${questName}' is already active in another live session (${
          activeInfo?.sessionId || "unknown"
        }). This session will not mount a duplicate; no fresh turns, generation resets, or saved-state changes will be emitted here.`,
        {
          code: QuestErrorCode.QUEST_REUSED_COALESCED,
          requiredNextAction:
            `Do not mount '${questName}' again while the owning session is active. Continue with research-reads only, or if you must act on it, do so in the owning session.`,
          details: {
            Quest: questName,
            ActiveSession: activeInfo?.sessionId || "",
            Slug: slug,
    ...
    logQuestTransition(
      "QUEST_REUSED",
      `reusing existing quest '${questName}'`,
      { quest: questName },
    );
...
export function checkOrdinaryCompletionConditions(
  ...
  if (s.reassessmentRequired) {
    return {
      satisfied: false,
      reason: `Reassessment is pending (v${s.reassessmentVersion || 1}: ${
        s.reassessmentReason || "contradiction detected"
      }). Resolve reassessment before completing quest.`,
    };
  }
  ...
  if (s.dirty) {
    return {
      satisfied: false,
      reason:
        "Active quest has unsaved changes. Save and mark saved before completing quest.",
    };
  }
  ...
  if (markdownContent) {
    ...
    const remainingSec = sections.get("remaining work") ||
      sections.get("remaining tasks") || sections.get("remaining");
    if (remainingSec && remainingSec.body) {
      ...
      const unchecked = lines
        ...
        .filter((l) => {
          ...
          return taskText.length > 0 && !taskText.startsWith(">") &&
            taskText !== "-" &&
            !/^(none|n\/a|no remaining work|no remaining tasks)$/i.test(
              taskText,
            );
        });
      if (unchecked.length > 0) {
        const sample = unchecked.slice(0, 3).map((u) =>
          u.replace(/^[-*]?\s*\[\s*\]\s*/, "")
        ).join("; ");
        return {
          satisfied: false,
          reason: `Unfinished tasks in Remaining Work: ${sample}${
            unchecked.length > 3 ? ` (and ${unchecked.length - 3} more)` : ""
          }`,
        };
      }
    }
...
export async function archiveQuestFile(
  name: string,
  pi: ExtensionAPI,
  ctx?: ExtensionContext,
...
> {
  ...
  const { targetQid, path, questName, parentSlug, questContent, childSummary } =
    ctxRes as ArchiveContext;
  const stack = Array.isArray(s.stack)
    ? [...s.stack]
    : (s.active ? [s.active] : []);

  if (!parentSlug) {
    const gateRes = await runRootCompletionGates(
      questContent,
      s,
      pi,
  ...
  if (
    s.activeTransaction &&
    (s.activeTransaction.activeQuest === questName ||
      s.activeTransaction.questPath === path)
  ) {
  ...
  if (
    ...
    (state.activeTransaction.activeQuest === questName ||
      state.activeTransaction.questPath === path)
  ) {
  ...
  if (
    s.pendingResume &&
    (s.pendingResume.activeQuest === questName ||
      s.pendingResume.checkpointQuestPath === path)
  ) s.pendingResume = null;
  if (
    state.pendingResume &&
    (state.pendingResume.activeQuest === questName ||
      state.pendingResume.checkpointQuestPath === path)
  ) state.pendingResume = null;
  supersedeObligation(
    s,
    (obl) => obl.questId === questName || obl.questId === questId,
    "Quest archived",
  );
  if (s !== state) {
    supersedeObligation(
  ...
  if (isRoot) {
    const termRes = await finalizeTerminalState(
      pi,
      ctx,
      questName,
      questId,
      ...
      projectRoot,
      () =>
        existsSync(resolve(projectRoot, QUEST_CURRENT_DIR, targetQid)) ||
        existsSync(questDirPath(targetQid)),
      () =>
        existsSync(questArchivePath(questId, projectRoot)) ||
        existsSync(questArchivePath(questId)),
    );
    ...
    finalArchiveZipPath = await createArchiveZip(
      projectRoot,
      questId,
      questName,
    ...
    onLifecycleStageTransition?.("changelog_appended", {
      questId,
      questName,
      zipExists: existsSync(questArchivePath(questId, projectRoot)) ||
        existsSync(questArchivePath(questId)),
      activeDirExists:
        existsSync(resolve(projectRoot, QUEST_CURRENT_DIR, targetQid)) ||
        existsSync(questDirPath(targetQid)),
  ...
  const { nextActive, stack: newStack } = await popArchivedAndFindNextActive(
    stack,
    questName,
    parentSlug,
    s,
  );
  await hydrateNextActive(
    nextActive,
    newStack,
    s,
    questName,
  ...
  const returnMsg = nextActive
    ? ` Resumed parent/previous quest '${nextActive}' (LIFO stack).`
    : "";
...
export async function promptForQuestChoice(
  ctx: ExtensionContext,
  title = "Select quest:",
): Promise<QuestChoiceResult | null> {
  ...
  if (choice === "New quest…") {
    const nameInput = (await ctx.ui.input(
      "Enter short quest name / slug (e.g. expand-editor-textarea):",
    )) ?? "";
    const trimmedName = nameInput.trim();
    if (!trimmedName) return null;
    const name = slugify(trimmedName, 45);
    if (!name) return null;
    const goalInput = (await ctx.ui.input(
      "Describe what you want to accomplish (optional):",
    )) ?? "";
