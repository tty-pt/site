...
export async function resolveArchiveContext(
  name: string,
  ctx?: any,
): Promise<ArchiveContext | { error: string }> {
  ...
  if (record) {
    ...
    return {
      ...
      childSummary: "",
      isRoot: true,
      error: `No quest file found at ${path}`,
    };
  }
  ...
  if (!parentSlug) {
    ...
    if (targetRec && targetRec.parent) parentSlug = targetRec.parent;
  }
  ...
  try {
    ...
    logError(
      `Failed to read quest file for parent extraction at ${path}`,
      err,
      ctx,
