...
export function parseCriticalReviewResponse(responseText: string): {
  ...
  if (
    ...
    return {
      ...
      severity: "MAJOR",
      findings: [{
        issue: "Empty reviewer response",
        evidence: "No output received from critical reviewer",
      }],
  ...
  const promptComplianceItems: Array<
    {
      requirement: string;
      planHandling?: string;
  ...
  for (let i = lines.length - 1; i >= 0; i--) {
    ...
    const verdictMatch = line.match(
      /^VERDICT:\s*(APPROVE|PASS|REVISE|FAIL|UNCERTAIN)\b/i,
    );
    if (verdictMatch) {
  ...
  for (const line of lines) {
    ...
    if (
      trimmed.startsWith("PASS 1") || trimmed.startsWith("PASS 1 (") ||
      trimmed.startsWith("PASS 1:")
    ) {
      inPass1 = true;
      inPass2 = false;
      inReqCheck = false;
    ...
    if (
      trimmed.startsWith("PASS 2") || trimmed.startsWith("PASS 2 (") ||
      trimmed.startsWith("PASS 2:")
    ) {
      inPass1 = false;
      inPass2 = true;
      inReqCheck = false;
    ...
    if (
      ...
      if (currentIssue) {
        findings.push({
          issue: currentIssue,
          evidence: currentEvidence || "(none specified)",
        });
        currentIssue = "";
        currentEvidence = "";
      }
    ...
    if (inPass1) {
      const m = trimmed.match(
        /Provisional Judgment:\s*(APPROVE|PASS|REVISE|FAIL|UNCERTAIN)\b/i,
      );
      if (m) {
    ...
    if (inPass2) {
      const m = trimmed.match(
        /Revised Judgment:\s*(APPROVE|PASS|REVISE|FAIL|UNCERTAIN)\b/i,
      );
      if (m) {
    ...
    if (inReqCheck) {
      const statusMatch = trimmed.match(
        /(?:Status|Satisfied):\s*(SATISFIED|UNSATISFIED|UNCERTAIN|YES|NO)\b/i,
      );
      if (statusMatch) {
        const statusVal = statusMatch[1].toUpperCase();
        const reqMatch = trimmed.match(
          /Requirement:\s*(.*?)(?=\s*->|\s*(?:Status|Satisfied):|$)/i,
        );
        const planMatch = trimmed.match(
          /Plan Handling:\s*(.*?)(?=\s*->|\s*(?:Status|Satisfied):|$)/i,
        );
    ...
    if (inFindings) {
      if (
        trimmed.startsWith("- Issue:") || trimmed.startsWith("- issue:") ||
        trimmed.startsWith("- Problem:") || trimmed.startsWith("- problem:")
      ) {
        if (currentIssue) {
          findings.push({
            issue: currentIssue,
        ...
        currentIssue = trimmed.replace(/^-\s*(?:Issue|Problem):\s*/i, "")
          .trim();
      } else if (
        trimmed.startsWith("Evidence:") || trimmed.startsWith("evidence:")
      ) {
    ...
    if (inActions) {
      if (
        ...
        const act = trimmed.replace(/^[-*]\s*/, "").replace(/^\d+\.\s*/, "")
          .trim();
  ...
  if (
    (verdict === "APPROVE" || verdict === "PASS") && severity !== "NONE" &&
    severity !== "MINOR"
  ) {
    severity = "NONE";
  }
  ...
  return {
    ...
    findings,
    requiredActions,
    originalRequestCheck: {
      satisfied,
      unsatisfied,
      items: promptComplianceItems,
    },
    selfCritique,
  };
}
