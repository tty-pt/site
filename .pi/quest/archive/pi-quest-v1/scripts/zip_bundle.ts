...
async function main() {
  ...
  try {
    ...
    printLine(
      `Root quest:           ${result.hierarchy.activeRootQuest || "(none)"}`,
    );
    printLine(`Bundle:               ${displayPath}`);
    printLine(`SHA-256:              ${result.sha256}`);
    printLine(
      `Quest State Hash:     ${result.hierarchy.questHash || "(none)"}`,
    );
    if (result.bundleHash) {
      printLine(`Bundle Content SHA:   ${result.bundleHash}`);
    }
    printLine(
      `Current Code:         pi-quest/ (package.json, src/, tests/, scripts/)`,
    );
    printLine(
      `Active Sub-Quest:     ${result.hierarchy.activeSubQuest || "(none)"}`,
    );
    printLine(
      `Captured Sub-Quests:  ${
    ...
    printLine(
      `Run Start Time:       ${result.hierarchy.startTime || "(none)"}`,
    );
    ...
    printLine(
      `Execution Log:        ${
        result.hierarchy.logExists
          ? `${result.hierarchy.logPath} (${
            ((result.hierarchy.logSize || 0) / 1024).toFixed(1)
          } KB)`
          : "Not found (no run recorded)"
      }`,
    );
    printLine(
      `Verification:         PASSED (all ${result.verification.entries.length} archive entries verified)`,
    );
    printLine(
      "================================================================================\n",
    );
    printLine(
      `Upload or inspect ${displayPath} for current code + latest run diagnostics.\n`,
    );
  } catch (err: any) {
