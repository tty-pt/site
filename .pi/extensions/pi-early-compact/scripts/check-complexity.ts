// Complexity gate (same contract as pi-quest RD2, self-contained):
...
function estimateFnSize(text: string): Array<{ name: string; loc: number }> {
  ...
  const re =
    /^\s*(export\s+)?(async\s+)?function\s+(\w+)|^\s*(export\s+)?(async\s+)?(const|let)\s+(\w+)\s*=\s*\(\s*\)\s*=>/;
...
for await (const path of walkTs(SRC)) {
  ...
  for (const fn of estimateFnSize(text)) {
    if (fn.loc > BUDGET_FN_LOC) {
      console.error(`FAIL func ${fn.name} in ${short}: ${fn.loc} LOC > ${BUDGET_FN_LOC}`);
      failures++;
    }
  }
}
