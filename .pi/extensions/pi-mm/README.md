...
Optional embeddings: `QMAP_SEPAL_EMBED_URL` + `QMAP_SEPAL_EMBED_MODEL` (optional `QMAP_SEPAL_EMBED_KEY`, or `embedUrl`/`embedModel`/`embedKey` under `"pi-mm"`). When set, tools use `mem.db@joint,stoma,sepal:a:s` and `memory_scan(embed=true)` adds semantic ranking via a bare `sepal` leaf plus `--query=<topic> --min-sim=0.2` flags that libsepal merges server-side at decode (no client curl, no temp files). Sepal stores vectors only (no text); unconfigured degrades soft (`details.embed` = `unconfigured`), while a configured-but-unreachable endpoint fails the query loud (exit 1). Operator guide: repo-root `RUNNING.md`.
...
`src/config.ts` (defaults + resolution), `src/window.ts` (level→joint window, always bounded per F2), `src/resolve.ts` (bare-ref parse + sentinel skip + nextRef), `src/qmap.ts` (QmapRunner + invocation builders pinned to §8, incl. the bare-`sepal` + `--query`/`--min-sim` flags path), `src/tools/*` (store/scan/think/forget/reset), `skills/pi-mm/SKILL.md`, `scripts/{zip_bundle,check-complexity,integration-mm}.`.
