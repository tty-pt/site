...
- **Query-time semantic recall via the `query=` text leaf.** `memory_scan(embed=true)` merges a `sepal="query='<topic>' m=10 min_sim=0.2"` AND-leaf when configured (libsepal embeds server-side at decode; no curl, no temp files, no `ToolEnv.exec`) — else the soft `unconfigured` fallback. An unreachable endpoint fails the query loud (exit 1), never silent-empty.
