...
- **Query-time semantic recall via `--query`/`--min-sim` flags.** `memory_scan(embed=true)` emits a bare `sepal` AND-leaf plus `--query=<topic> --min-sim=0.2` args when configured (broadcast to every bound axis declaring `query` — stoma + sepal; libsepal embeds server-side at decode — `m` omitted, the library default clamps to the store size and the CLI `-t` caps output; a per-embed on-disk cache under the store dir avoids repeat HTTP; no curl, no temp files, no `ToolEnv.exec`); the stoma leaf is static `field=text matched=1`. Without sepal, only `--query=<topic>` rides (stoma text). Else the soft `unconfigured` fallback. An unreachable endpoint fails the query loud (exit 1), never silent-empty.
