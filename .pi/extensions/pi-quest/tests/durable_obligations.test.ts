...
import { createMockContext, createMockExtensionAPI } from "./compaction_test_helpers.ts";
...
Deno.test("Durable Obligations Lifecycle Suite: 10 Core Architectural Invariants", async (t) => {
	...
	// -----------------------------------------------------------------------
	await t.step("1. Five independent obligations remain five independent obligations", () => {
		...
		assert.strictEqual(state.pendingNotifications?.length, 5, "Pending notifications must contain exactly 5 obligations");
		const ids = state.pendingNotifications!.map((o) => o.id);
		assert.deepStrictEqual(ids, ["obl_1", "obl_2", "obl_3", "obl_4", "obl_5"], "All 5 obligation identities must be preserved independently");

		for (const obl of state.pendingNotifications!) {
			assert.strictEqual(obl.status, "pending", `Obligation ${obl.id} must have status pending`);
		}
	});
	...
	await t.step("2. Delivering all five together does not merge them", () => {
		...
		const o1 = createAgentObligation(state, { id: "obl_1", kind: "error", code: "TEST_FAILED", message: "rerun tests" });
		const o2 = createAgentObligation(state, { id: "obl_2", kind: "error", code: "COMPILER_FAILED", message: "investigate compiler failure" });
		const o3 = createAgentObligation(state, { id: "obl_3", kind: "error", code: "REASSESS", message: "reassess plan" });
		const o4 = createAgentObligation(state, { id: "obl_4", kind: "error", code: "REVIEW", message: "perform review" });
		const o5 = createAgentObligation(state, { id: "obl_5", kind: "error", code: "SAVE", message: "checkpoint state" });
		...
		assert.strictEqual(delivered, true, "drainAgentObligations must report delivery success");

		// ASSERTIONS:
		// 1. All 5 obligations still exist independently in pending queue
		assert.strictEqual(state.pendingNotifications?.length, 5, "All 5 obligations must remain in pending queue after delivery");
		const ids = state.pendingNotifications!.map((o) => o.id);
		assert.deepStrictEqual(ids, ["obl_1", "obl_2", "obl_3", "obl_4", "obl_5"], "Identities must not be merged");

		// 2. Delivery metadata updated on each individual obligation
		for (const obl of state.pendingNotifications!) {
			assert.strictEqual(obl.status, "delivering", `Obligation ${obl.id} must have status delivering`);
			assert.ok(typeof obl.deliveredAt === "number", `Obligation ${obl.id} must have deliveredAt timestamp`);
			assert.strictEqual(obl.attempts, 1, `Obligation ${obl.id} attempts must be 1`);
		}
		...
		assert.strictEqual(deliveredAgain, false, "Subsequent drain must not redeliver delivering obligations");
		for (const obl of state.pendingNotifications!) {
			assert.strictEqual(obl.status, "delivering", `Obligation ${obl.id} must remain delivering`);
			assert.strictEqual(obl.attempts, 1, `Obligation ${obl.id} attempts must remain 1 without infinite delivery loop`);
		}
	});
	...
	await t.step("3. Fulfilling O1 leaves O2–O5 pending", () => {
		...
		// Fulfill O1
		const fulfilled = fulfillObligation(state, "obl_1", "Unit tests passed after fixing parser bug");
		assert.strictEqual(fulfilled, true, "fulfillObligation must succeed for obl_1");

		// ASSERTIONS:
		// 1. O1 is no longer in pendingNotifications
		const pendingIds = state.pendingNotifications!.map((o) => o.id);
		assert.deepStrictEqual(pendingIds, ["obl_2", "obl_3", "obl_4", "obl_5"], "O1 must be removed from pending queue, O2-O5 remain pending");
		...
		assert.strictEqual(state.obligationHistory![0].fulfilledReason, "Unit tests passed after fixing parser bug");
	...
	// -----------------------------------------------------------------------
	await t.step("4. Resolving the condition behind O2 does not affect O3–O5", () => {
		const state: StoredState = createDefaultState();
		state.active = "test-quest";
		...
		const pendingIds = state.pendingNotifications!.map((o) => o.id);
		assert.deepStrictEqual(pendingIds, ["obl_3", "obl_4", "obl_5"], "O2 must be fulfilled; O3-O5 must remain pending");
	...
	// -----------------------------------------------------------------------
	await t.step("5. Duplicate requests for O3 coalesce into O3 rather than creating O3a/O3b/O3c", () => {
		const state: StoredState = createDefaultState();
		state.active = "test-quest";
		...
		assert.strictEqual(state.pendingNotifications?.length, 1, "Duplicate requests must coalesce into single obligation");
		assert.strictEqual(state.pendingNotifications![0].id, "obl_3", "Original obligation ID must be preserved");
		assert.strictEqual(state.pendingNotifications![0].attempts, 2, "Attempts count must be incremented on coalescing");
		assert.strictEqual(state.pendingNotifications![0].correlationId, "corr_third", "Latest correlationId must be captured");
	});

	// -----------------------------------------------------------------------
	// 6. A stale obligation is superseded only when its own causal condition is no longer applicable
	// -----------------------------------------------------------------------
	await t.step("6. A stale obligation is superseded only when its own causal condition is no longer applicable", () => {
		const state: StoredState = createDefaultState();
		state.active = "test-quest";
		...
		const pendingIds = state.pendingNotifications!.map((o) => o.id);
		assert.deepStrictEqual(pendingIds, ["obl_4", "obl_5"], "O3 must be superseded; O4 and O5 must remain pending");
		...
		assert.strictEqual(isObligationCurrent(o4, state), true, "O4 must still be current");
		assert.strictEqual(isObligationCurrent(o5, state), true, "O5 must still be current");
	});

	// -----------------------------------------------------------------------
	// 7. Compaction/recovery preserves all still-actionable independent obligations
	// -----------------------------------------------------------------------
	await t.step("7. Compaction/recovery preserves all still-actionable independent obligations", () => {
		const state: StoredState = createDefaultState();
		state.active = "test-quest";
		...
		const restoredO4 = restored.pendingNotifications!.find((o) => o.id === "obl_4");
		...
		const restoredO5 = restored.pendingNotifications!.find((o) => o.id === "obl_5");
		...
		assert.strictEqual(restored.obligationHistory![0].fulfilledReason, "tests passing");
	});
	...
	await t.step("8. Delivery does not mark obligations fulfilled", () => {
		...
		assert.strictEqual(state.pendingNotifications?.length, 1, "Obligation must still be in pending queue");
		assert.strictEqual(state.pendingNotifications![0].status, "delivering", "Status must be delivering, NOT fulfilled");
		assert.notStrictEqual(state.pendingNotifications![0].status, "fulfilled", "Delivery must never mark obligation fulfilled");

		// 2. Not in fulfilled history
		const inHistory = state.obligationHistory?.find((h) => h.id === "obl_critical" && h.status === "fulfilled");
		assert.strictEqual(inHistory, undefined, "Obligation must not be in fulfilled history merely from delivery");
	});

	// -----------------------------------------------------------------------
	// 9. A fulfilled obligation remains visible in execution history
	// -----------------------------------------------------------------------
	await t.step("9. A fulfilled obligation remains visible in execution history", () => {
		const state: StoredState = createDefaultState();
		state.active = "test-quest";
		state.questId = "q_obl_9";

		const o1 = createAgentObligation(state, { id: "obl_1", kind: "error", code: "TEST_FAILED", message: "rerun tests" });
		const o2 = createAgentObligation(state, { id: "obl_2", kind: "error", code: "COMPILER_FAILED", message: "investigate compiler error" });
		...
		assert.strictEqual(history.length, 2, "Both fulfilled obligations must be in history");
	...
	// -----------------------------------------------------------------------
	await t.step("10. A late/stale message cannot resurrect an already fulfilled or superseded obligation", () => {
		const state: StoredState = createDefaultState();
		state.active = "test-quest";
		state.questId = "q_obl_10";

		const o1 = createAgentObligation(state, { id: "obl_1", kind: "error", code: "TEST_FAILED", message: "rerun tests" });
		const o2 = createAgentObligation(state, { id: "obl_2", kind: "reassessment", code: "REASSESSMENT_REQUIRED", message: "reassess plan" });
		...
		assert.strictEqual(getPendingObligations(state).length, 0, "No pending obligations");
		assert.strictEqual(state.obligationHistory?.length, 2, "2 historical obligations");

		// Stale transport retry or delayed message tries to queue O1 again with same ID
		const lateO1 = createAgentObligation(state, { id: "obl_1", kind: "error", code: "TEST_FAILED", message: "rerun tests" });
		queueAgentObligation(state, lateO1);

		// Stale message tries to queue O2 again with same ID
		const lateO2 = createAgentObligation(state, { id: "obl_2", kind: "reassessment", code: "REASSESSMENT_REQUIRED", message: "reassess plan" });
		queueAgentObligation(state, lateO2);

		// ASSERTIONS:
		// Neither O1 nor O2 should be resurrected into pending queue
		assert.strictEqual(getPendingObligations(state).length, 0, "Late messages must NOT resurrect fulfilled/superseded obligations");
		assert.strictEqual(state.pendingNotifications?.length, 0, "pendingNotifications must remain empty");

		// History remains intact
		const history = getObligationHistory(state);
		assert.strictEqual(history.length, 2);
		assert.strictEqual(history.find((h) => h.id === "obl_1")?.status, "fulfilled");
		assert.strictEqual(history.find((h) => h.id === "obl_2")?.status, "superseded");
	});

	// -----------------------------------------------------------------------
	// 11. Drain idempotency - delivering obligations must not be redelivered in a loop
	// -----------------------------------------------------------------------
	await t.step("11. Drain idempotency - delivering obligations must not be redelivered in a loop", () => {
		const api = createMockExtensionAPI();
		plugin(api.mockPi as any);
		...
		const o1 = createAgentObligation(state, { id: "obl_1", kind: "error", code: "TEST_FAILED", message: "rerun tests" });
		const o2 = createAgentObligation(state, { id: "obl_2", kind: "error", code: "COMPILER_FAILED", message: "investigate compiler" });
		const o3 = createAgentObligation(state, { id: "obl_3", kind: "error", code: "REASSESS", message: "reassess plan" });
		const o4 = createAgentObligation(state, { id: "obl_4", kind: "error", code: "REVIEW", message: "perform review" });
		const o5 = createAgentObligation(state, { id: "obl_5", kind: "error", code: "SAVE", message: "checkpoint" });
		...
		assert.strictEqual(getPendingObligations(state).length, 0, "getPendingObligations must be 0 after delivery (delivering excluded)");
		assert.strictEqual(state.pendingNotifications?.length, 5, "delivering items stay in array but are not pending");
		for (const obl of state.pendingNotifications!) {
		...
		for (let i = 0; i < 5; i++) {
			...
			assert.strictEqual(again, false, `Drain loop iteration ${i} must not redeliver`);
			assert.strictEqual(getPendingObligations(state).length, 0);
			for (const obl of state.pendingNotifications!) {
				assert.strictEqual(obl.attempts, 1, `attempts must stay 1 in loop iteration ${i}`);
			}
		}
	});
});
