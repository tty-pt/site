...
Deno.test("Parallel Critical Reviewer Suite: Complete Verification of All 15 Concurrency & Visibility Semantics", async (t) => {
  ...
  await t.step(
    ...
    async () => {
      ...
      // Trigger review
      const reviewPromise = runCriticalReview(mockPi, ctx, {
        kind: "direction",
      });
      ...
      assert.strictEqual(
        runnerCompleted,
        false,
        "Runner should still be running in background",
      );
  ...
  await t.step("2. Main state can advance while review runs", async () => {
    ...
    assert.strictEqual(
      result.superseded,
      true,
      "Review of V1 must be superseded by V2",
    );
    assert.strictEqual(
      s.planVersion,
      2,
      "Main state planVersion must remain 2",
    );
  ...
  await t.step(
    ...
    async () => {
      ...
      assert.strictEqual(
        res.superseded,
        true,
        "Result must be marked superseded",
      );
      ...
      assert.strictEqual(
        ...
        null,
        "Approval of V5 must NOT approve V6",
      );
  ...
  await t.step(
    ...
    async () => {
      ...
      assert.strictEqual(
        ...
        0,
        "Stale findings must NOT be delivered to main agent",
      );
  ...
  await t.step(
    ...
    async () => {
      ...
      assert.strictEqual(
        active.size,
        1,
        "Only ONE active review should be registered in tracker",
      );
  ...
  await t.step(
    ...
    async () => {
      ...
      assert.strictEqual(
        active.size,
        1,
        "Only ONE active review is permitted on the same quest (serialized)",
      );
      assert.strictEqual(
        callCount,
        1,
        "Second review must not immediately launch a second runner",
      );
      ...
      assert.strictEqual(
        p2Res.inProgress,
        true,
        "Second request was coalesced as inProgress / queued",
      );
  ...
  await t.step(
    ...
    async () => {
      ...
      assert.strictEqual(
        ...
        false,
        "Final completion must be blocked before PASS arrives",
      );
      ...
      assert.strictEqual(
        isCriticalReviewValidForCompletion(s),
        true,
        "Completion gate opens when PASS arrives",
      );
  ...
  await t.step("9. Reviewer progress appears in live status", async () => {
    ...
    assert.strictEqual(
      statusText,
      undefined,
      `Expected hidden critical_review slot, got: ${statusText}`,
    );

    // Turn & tool activity updates still tracked internally but not surfaced in bar
    updateReviewActivity(
      "rev_stat_1",
      { type: "turn_start", turnIndex: 6 },
      ctx,
    );
    for (let i = 0; i < 11; i++) {
      updateReviewActivity("rev_stat_1", {
        type: "tool_result",
        toolName: "read",
        path: `file_${i % 4}.ts`,
      }, ctx);
    ...
    assert.strictEqual(
      statusText,
      undefined,
      `Expected still hidden, got: ${statusText}`,
    );
    assert.strictEqual(
      (getActiveReviews().get("rev_stat_1") as any).activity.turns,
      6,
    );
    assert.strictEqual(
      (getActiveReviews().get("rev_stat_1") as any).activity.tools,
      11,
    );
  ...
  await t.step("10. Reviewer lifecycle appears in execution.log", async () => {
    ...
    assert.ok(
      log.includes("SUBAGENT_STARTED"),
      "Log must include SUBAGENT_STARTED",
    );
    assert.ok(
      log.includes("SUBAGENT_ACTIVITY"),
      "Log must include SUBAGENT_ACTIVITY",
    );
    assert.ok(
  ...
  await t.step(
    ...
    async () => {
      ...
      assert.strictEqual(
        result.review?.childSessionId,
        "child_session_xyz_789",
      );
      assert.strictEqual(
        result.review?.childTranscriptRef,
        ".pi/sessions/child_session_xyz_789.jsonl",
      );

      const log = readQuestLog(getQuestLogPath("correlation-quest"));
      assert.ok(
        log.includes("childSessionId=child_session_xyz_789"),
        "Execution log must contain correlated childSessionId",
      );
  ...
  await t.step(
    ...
    async () => {
      ...
      await runCriticalReview(mockPi, ctx, { kind: "plan_review" });
      await runCriticalReview(mockPi, ctx, { kind: "plan_review" });
      const fourth = await runCriticalReview(mockPi, ctx, {
        kind: "plan_review",
      });
  ...
  await t.step("13. Timeout records the actual timeout layer", async () => {
    assert.strictEqual(
      classifyTimeoutLayer("Subagent execution timed out on event bridge"),
      "subagent_bridge_deadline",
    );
    assert.strictEqual(
      classifyTimeoutLayer("Child process exited with code 137 (SIGKILL)"),
      "child_process_deadline",
    );
    assert.strictEqual(
      classifyTimeoutLayer("Provider model error: rate limit 429"),
      "provider_model_timeout",
    );
    assert.strictEqual(
      classifyTimeoutLayer("Inactivity timeout exceeded"),
  ...
  // -----------------------------------------------------------------------
  await t.step(
    "16. Planning updates do not spawn concurrent reviewers and coalesce into one follow-up review",
    async () => {
      const { mockPi } = createMockExtensionAPI();
      ...
      const firstReviewPromise = new Promise((resolve) => {
      ...
      const initialReviewPromise = runCriticalReview(mockPi, ctx, {
        kind: "plan_review",
        questSlug: s.active,
      });
      ...
      assert.strictEqual(
        runnerInvocations,
        1,
        "Initial review runner must start",
      );
      ...
      s.saveCount = 2;
      const update1Promise = runCriticalReview(mockPi, ctx, {
        kind: "plan_review",
        questSlug: s.active,
      });
      ...
      s.saveCount = 3;
      const update2Promise = runCriticalReview(mockPi, ctx, {
        kind: "plan_review",
        questSlug: s.active,
      });
      ...
      s.saveCount = 4;
      const update3Promise = runCriticalReview(mockPi, ctx, {
        kind: "plan_review",
        questSlug: s.active,
      });
      ...
      assert.strictEqual(
        update1Res.inProgress,
        true,
        "update1 must resolve immediately with inProgress: true",
      );
      assert.strictEqual(
        update2Res.inProgress,
        true,
        "update2 must resolve immediately with inProgress: true",
      );
      assert.strictEqual(
        update3Res.inProgress,
        true,
        "update3 must resolve immediately with inProgress: true",
      );
      ...
      assert.strictEqual(
        active.size,
        1,
        "Only one reviewer must be active in tracker across rapid planning updates",
      );
      ...
      assert.strictEqual(
        runnerInvocations,
        1,
        "Runner must be invoked only once while initial review is running (no concurrent reviewer)",
      );
      ...
      assert.strictEqual(
        initialRes.superseded,
        true,
        "Initial review result must be superseded by newer state",
      );
      ...
      assert.strictEqual(
        runnerInvocations,
        2,
        "Exactly one follow-up review must be launched for latest state (total 2 invocations: reviewer #1 -> reviewer #2)",
      );
  ...
  // -----------------------------------------------------------------------
  await t.step(
    "17. Concurrent same dedupKey queue does not create duplicates",
    async () => {
      ...
      assert.strictEqual(
        ...
        1,
        "5 concurrent same dedupKey must coalesce to 1",
      );
      assert.strictEqual(
      ...
      assert.strictEqual(
        ...
        4,
        "Attempts coalesced (0 + 4)",
      );
      assert.strictEqual(
        state.pendingNotifications![0].correlationId,
        "corr_4",
        "Last correlationId wins",
      );
      assert.strictEqual(
        getPendingObligations(state).length,
        1,
        "getPendingObligations sees 1 pending",
      );
  ...
  await t.step("18. Drain defers while reviewer lock held", async () => {
    ...
    const obl = createAgentObligation(s, {
      id: "obl_lock",
      kind: "error",
      code: "TEST_FAILED",
    ...
    assert.strictEqual(
      drainedWhileLocked,
      false,
      "Drain must defer while lock held",
    );
    assert.strictEqual(
      s.pendingNotifications![0].status,
      "pending",
      "Status must stay pending while deferred",
    );
    assert.strictEqual(
      getPendingObligations(s).length,
      1,
      "Still pending while lock held",
    );
    ...
    assert.strictEqual(
      drainedAfter,
      true,
      "Drain must succeed after lock released",
    );
    assert.strictEqual(
      s.pendingNotifications![0].status,
      "delivering",
      "Status must be delivering after drain",
    );
    assert.strictEqual(
      getPendingObligations(s).length,
      0,
      "No pending after delivering",
    );
  ...
  // -----------------------------------------------------------------------
  await t.step(
    "19. Per-quest isolation - two quests in parallel each retain obligations",
    async () => {
      const { mockPi } = createMockExtensionAPI();
      ...
      const snapA = await createReviewSnapshot(
        sA.active,
        "rev_iso_A",
        "plan_review",
        "session_iso_A",
      ...
      const snapB = await createReviewSnapshot(
        sB.active,
        "rev_iso_B",
        "plan_review",
        "session_iso_B",
      ...
      assert.strictEqual(
        sA.pendingNotifications?.length,
        1,
        "Quest A must have 1 obligation",
      );
      assert.strictEqual(
        sB.pendingNotifications?.length,
        1,
        "Quest B must have 1 obligation",
      );
      assert.strictEqual(
        sA.pendingNotifications![0].correlationId,
        "rev_iso_A",
      );
      assert.strictEqual(
        sB.pendingNotifications![0].correlationId,
        "rev_iso_B",
      );
  ...
  // -----------------------------------------------------------------------
  await t.step(
    "20. Review lock heartbeat keeps .review.lock mtime fresh during long review",
    async () => {
      const questId = "heartbeat-lock-quest";
      const lockPath = getReviewLockPath(questId);
      try {
        await mkdir(lockPath.slice(0, lockPath.lastIndexOf("/")), {
          recursive: true,
        });
        ...
        assert.ok(
          ...
          "touchReviewLockFile must advance mtime",
        );
        assert.ok(
          Date.now() - mtimeAfter < 2000,
          "Refreshed mtime must be recent",
        );
  ...
  await t.step(
    ...
    async () => {
      ...
      await mkdir(`${currentDir}/${s.questId}`, { recursive: true });
      await writeFile(
        `${currentDir}/${s.questId}/quest.md`,
        `# Quest: ${s.questId}\n\n## Goal\nCancel test\n\n## Original request\n> Cancel test\n`,
        "utf8",
      );
      let runnerStarted = false;
      let resolveRunner: (val: any) => void;
      const runnerBlocker = new Promise((r) => {
        resolveRunner = r;
      });
      const runner = async (_task: string, options?: any) => {
        runnerStarted = true;
      ...
      assert.strictEqual(
        ...
        0,
        "Active review must be deregistered after cancel",
      );
      resolveRunner!(undefined);
      const result = await reviewPromise;
      assert.ok(
        result.error === "cancelled" || result.skipped === true,
        "Cancelled review must resolve with skipped/cancelled",
      );
      clearActiveReviews();
      setCustomSubagentRunner(null);
    },
  ...
  // -----------------------------------------------------------------------
  await t.step(
    "22. Cross-process: second caller sees .review.lock held and coalesces as pending",
    async () => {
      const { mockPi } = createMockExtensionAPI();
      ...
      try {
        ...
        assert.strictEqual(
          runnerCalled,
          false,
          "Runner must not be invoked when lock is held by another process",
        );
        assert.ok(
          p2Res.inProgress === true || p2Res.skipped === true,
          "Second process must resolve with inProgress or skipped",
        );
        const pending = getPendingReviews();
        assert.ok(
          pending.size > 0,
          "A pending review must be queued for the quest",
        );
      } finally {
  ...
  // -----------------------------------------------------------------------
  await t.step(
    "23. REVIEW_CANCELLED appears in execution.log when boundary supersedes running reviewer",
    async () => {
      const { mockPi } = createMockExtensionAPI();
      ...
      const p1 = runCriticalReview(mockPi, ctx, {
        kind: "plan_review",
        boundaryKey: "bk:v1:hashA",
      });
      ...
      assert.ok(
        getActiveReviews().size === 0,
        "Active reviews cleared after cancel",
      );
      clearActiveReviews();
      setCustomSubagentRunner(null);
    },
  ...
  // -----------------------------------------------------------------------
  await t.step(
    "24. Stale .review.lock (age > STALE_MS) is recovered and next caller proceeds",
    async () => {
  ...
  // -----------------------------------------------------------------------
  await t.step(
    "25. MUTEX_ACQUIRED log event includes activeCount",
    async () => {
      const { mockPi } = createMockExtensionAPI();
      ...
      const log = readQuestLog(getQuestLogPath(s.questId));
      assert.ok(
        log.includes("activeCount="),
        "MUTEX_ACQUIRED must log activeCount=",
      );
      clearActiveReviews();
      setCustomSubagentRunner(null);
    },
