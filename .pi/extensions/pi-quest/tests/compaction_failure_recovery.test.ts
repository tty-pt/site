...
Deno.test("quest_journal_compaction_failure_recovery: compaction failures, fallback directives, error gating, and explicit reconciliation", async (t) => {
  ...
  await t.step(
    ...
    async () => {
      ...
      const ctx = createMockContext(
        10000,
        "session_compaction_failure_handling",
      );
      ...
      for (const cb of api.handlers["session_compact_failed"] || []) {
        await cb({
          error: new Error("Simulated LLM token budget compaction abort"),
        }, ctx);
      }
      ...
      assert.strictEqual(
        ...
        "resume-delivered",
        "Transaction must resolve to resume-delivered after successful fallback delivery",
      );
      assert.strictEqual(state.compactionPending, false);
      assert.strictEqual(
        state.compactCount,
        compactCountBefore,
        "compactCount must not advance on failure",
      );

      // 2. Model-visible COMPACTION_FAILURE error is reported
      const msgs = getAllMessages(api);
      assert.ok(
        msgs.some((m) => m.includes("COMPACTION_FAILURE")),
        "COMPACTION_FAILURE must be reported",
      );
  ...
  await t.step(
    ...
    async () => {
      ...
      const ctx = createMockContext(
        10000,
        "session_compaction_subquest_fallback_error",
      );
      const commands: Record<string, any> = {};
      for (const cmd of api.registeredCommands) commands[cmd.name] = cmd;
      const tools: Record<string, any> = {};
      ...
      assert.strictEqual(
        ...
        childSlug,
        "pendingSubquestResume must be preserved when delivery fails",
      );
      assert.ok(
        state.pendingResume,
        "pendingResume must be recorded for retry",
      );
      assert.strictEqual(
        state.pendingResume.reason,
      ...
      assert.ok(
        state.pendingNotifications?.some((n) =>
          n.message.includes(
      ...
      assert.ok(
        !state.pendingNotifications?.some((n) =>
          n.message.includes("Compaction completed successfully")
        ),
      ...
      assert.strictEqual(
        state.pendingResume,
        null,
        "pendingResume must be cleared after successful delivery",
      );
      const msgs = getAllMessages(api);
      assert.ok(
        msgs.some((m) => m.includes("Compaction Skipped / Fallback")),
  ...
  await t.step(
    ...
    async () => {
      ...
      const ctx = createMockContext(
        10000,
        "session_failed_tx_blocks_implementation",
      );
      const commands: Record<string, any> = {};
      for (const cmd of api.registeredCommands) commands[cmd.name] = cmd;
      const tools: Record<string, any> = {};
      ...
      assert.strictEqual(
        ...
        true,
        "Implementation must be allowed when clean",
      );
      ...
      assert.strictEqual(
        canImplement(state, ctx as any),
        false,
        "canImplement() must return false when activeTransaction is failed",
      );
      ...
      const editTool = tools["edit"] ||
        api.registeredTools.find((t: any) => t.name === "edit");
      if (editTool) {
        ...
        assert.ok(
          res.error,
          "Mutating tool must be blocked when compaction transaction failed",
        );
        assert.ok(
          res.error.includes("COMPACTION_TRANSACTION_FAILED") ||
            res.error.includes("Compaction worker timeout"),
        );
      }
      ...
      assert.ok(
        ...
        "activeTransaction must remain after quest_mark_saved",
      );
      assert.strictEqual(state.activeTransaction.phase, "failed");
      assert.strictEqual(
        canImplement(state, ctx as any),
        false,
        "Gate must remain blocked after save",
      );
      ...
      await tools["quest_update_state"].execute(
        "call_resolve_step_12",
        {
          ...
          reassessmentConclusion:
            "Investigated failure and reconciled durable state.",
        },
        {},
        () => {},
        ctx,
      ...
      assert.strictEqual(
        canImplement(state, ctx as any),
        true,
        "Gate must open after explicit reconciliation",
      );
  ...
  await t.step(
    ...
    async () => {
      ...
      const ctx = createMockContext(
        10000,
        "session_failed_tx_tool_result_persistence",
      );
      const commands: Record<string, any> = {};
      for (const cmd of api.registeredCommands) commands[cmd.name] = cmd;
      const tools: Record<string, any> = {};
      ...
      assert.ok(
        state.activeTransaction,
        "activeTransaction must not be null after successful tool_result",
      );
      assert.strictEqual(
        state.activeTransaction?.phase,
        "failed",
        "activeTransaction must remain in phase 'failed'",
      );
      assert.strictEqual(
        canImplement(state, ctx as any),
        false,
  ...
  await t.step(
    ...
    async () => {
      ...
      assert.strictEqual(
        state.activeTransaction?.phase,
        "failed",
        "Transaction must remain failed when delivery fails",
      );
      assert.strictEqual(
        canImplement(state, ctx as any),
        false,
        "Implementation must be blocked while fallback delivery has not succeeded",
      );
      assert.ok(
        state.pendingResume,
        "pendingResume must be recorded for retry",
      ...
      assert.strictEqual(
        state.activeTransaction?.phase,
        "resume-delivered",
        "activeTransaction must transition to resume-delivered",
      );
      assert.notStrictEqual(
        state.activeTransaction?.phase,
        "failed",
        "activeTransaction.phase must not be 'failed'",
      );
      assert.strictEqual(
        canImplement(state, ctx as any),
        true,
        "Implementation gate must reopen after successful fallback delivery",
      );
      ...
      assert.strictEqual(
        state.pendingResume,
        null,
        "pendingResume must be consumed on immediate delivery",
      );
      assert.strictEqual(
        state.activeTransaction?.phase,
        "resume-delivered",
        "activeTransaction must be resolved to resume-delivered",
      );
      assert.notStrictEqual(
        state.activeTransaction?.phase,
        "failed",
      ...
      assert.strictEqual(
        canImplement(state, ctx as any),
        true,
        "Implementation gate must be open immediately after successful delivery",
      );
