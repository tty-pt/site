...
import { FUTURE_DIR, QUEST_CURRENT_DIR } from "../src/constants.ts";
...
Deno.test("done_issue_coverage: tests for every done issue without coverage (plus upgrades for #04 #36)", async (t) => {
  ...
  // =====================================================================
  await t.step(
    "#01 FUTURE_QUEST_TEMPLATE prefills ## Requirements from goal",
    () => {
      const slug = "look-consumer-side-code";
      const prompt =
        "Look at the consumer side code. It has a lot of complexity. Developing the site should be easy - reduce cognitive complexity";
      const content = FUTURE_QUEST_TEMPLATE(slug, prompt);
      // Requirements must contain the prompt slice, not just "-"
      assert.ok(
        content.includes("## Requirements"),
        "must contain Requirements section",
      );
      ...
      assert.ok(
        !/^\s*-\s*$/.test(reqBody.trim()) ||
          reqBody.includes(prompt.slice(0, 20)),
        "Requirements must not be bare '-'",
      );
      assert.ok(
        reqBody.includes(prompt.slice(0, 30)) ||
          reqBody.includes("- " + prompt.slice(0, 20)),
        "Requirements must contain prompt slice",
      );
  ...
  await t.step(
    "#01 createFutureDraftFromPrompt slug generation uses full prompt",
    () => {
      const prompt =
        "Look at the consumer side code. It has a lot of complexity";
  ...
  // =====================================================================
  await t.step(
    "#03 draftLastSavedHash equals sha256 of future file content",
    async () => {
  ...
  // =====================================================================
  await t.step(
    "#09 shouldCapturePrompt returns false with SYNTHETIC_FILTERED sync",
    () => {
      // "/" prefix is synthetic
      assert.strictEqual(shouldCapturePrompt("/quest my-quest"), false);
      // Normal prompt is captured
      assert.strictEqual(
        shouldCapturePrompt("Implement feature X with research"),
        true,
      );
      ...
      for (const c of syntheticCases) {
        assert.strictEqual(
          shouldCapturePrompt(c),
          false,
          `should not capture synthetic: ${c.slice(0, 20)}`,
        );
      }
    },
  );
  ...
  // =====================================================================
  await t.step(
    "#12 research-only quest (planVersion 1, no completed) does NOT error on empty Files Modified",
    () => {
...
`;
      ...
      const hasFilesModifiedError = audit.issues.some((i) =>
        i.includes("Files Modified is empty") ||
        i.includes("Files Modified omits")
      );
      assert.strictEqual(
        hasFilesModifiedError,
        false,
        `research-only must not error on Files Modified, got: ${
      ...
      assert.strictEqual(
        audit.consistent,
        true,
        `research-only must be consistent, issues: ${
          JSON.stringify(audit.issues)
        }`,
      );
    },
  ...
  await t.step(
    "#12 non-research-only (planVersion 2 with completed) DOES error on empty Files Modified",
    () => {
...
`;
      ...
      const hasFilesError = audit.issues.some((i) =>
        i.includes("Files Modified is empty") || i.includes("i18n_dict.h")
      );
      assert.strictEqual(
        hasFilesError,
        true,
        "planVersion 2 with reassessment must still error on empty Files Modified",
      );
    },
  );

  await t.step(
    "#12b read-only Completed (research reads) with empty Files Modified is consistent",
    () => {
      const researchReadsMd = `# Quest: consumer-code-complexity

## Goal
Analyze consumer side code complexity.

## Original request
> Look at the consumer side code. Reduce cognitive complexity without splitting files.
...
### Completed
- Read all major UX files: site_forms.c, gig/ux/detail.c, index/ux/list_json.c, index/ux/list_fill.c.
- Verified concrete evidence: gig/ux/detail.c has 757 lines and 11 static globals.
- Read docs/OVERVIEW.md, ARCHITECTURE.md, DESIGN.md.

### Files Examined
- mods/common/ux/site_forms.c
- mods/gig/ux/detail.c
- mods/index/ux/list_json.c
...
Present findings to user.
`;
      const audit = auditQuestConsistency(researchReadsMd);
      const hasFilesError = audit.issues.some((i) =>
        i.includes("Files Modified is empty") ||
        i.includes("Files Modified omits")
      );
      assert.strictEqual(
        hasFilesError,
        false,
        `research-read Completed must not error on empty Files Modified, got: ${
          JSON.stringify(audit.issues)
        }`,
      );
      assert.strictEqual(
        audit.consistent,
        true,
        `research-read quest must be consistent, issues: ${
          JSON.stringify(audit.issues)
        }`,
      );
    },
  ...
  await t.step(
    "#12c live quest 1788474606 audit is consistent (read-only research)",
    async () => {
      const livePath =
        "/home/quirinpa/site/.pi/quest/current/1788474606/quest.md";
      if (!existsSync(livePath)) return;
      const md = await readFile(livePath, "utf8");
      const audit = auditQuestConsistency(md);
      assert.strictEqual(
        audit.consistent,
        true,
        `live research quest must audit consistent, got: ${
          JSON.stringify(audit.issues)
        }`,
      );
    },
  ...
  await t.step(
    "#12d write-led Completed with empty Files Modified still errors (no over-suppression)",
    () => {
      const writeLedMd = `# Quest: i18n-locale
...
Add constants
`;
      const audit = auditQuestConsistency(writeLedMd);
      const hasFilesError = audit.issues.some((i) =>
        i.includes("Files Modified is empty") || i.includes("i18n_dict.h")
      );
      assert.strictEqual(
        hasFilesError,
        true,
        "write-led reassessment must still error on empty Files Modified",
      );
    },
  );
  ...
  // =====================================================================
  await t.step(
    "#24 commands/quest.ts archive-before-unlink contains future-archive + copyFile + DRAFT_DISCARDED",
    async () => {
      const src = await readFile("src/commands/quest.ts", "utf8").catch(() =>
        readFile(".pi/extensions/pi-quest/src/commands/quest.ts", "utf8")
      );
      assert.ok(
        src.includes("future-archive"),
        "must reference future-archive",
      );
      assert.ok(src.includes("copyFile"), "must call copyFile");
      assert.ok(src.includes("DRAFT_DISCARDED"), "must log DRAFT_DISCARDED");
      assert.ok(
        src.includes("rename(futurePath, path)"),
        "must still rename after archive",
      );
      ...
      assert.ok(
        archIdx < renameIdx,
        "future-archive copy must be before rename",
      );
      // Also verify lifecycle copy-before-rename
      let lifecycleSrc = "";
      try {
        lifecycleSrc = await readFile("src/lifecycle.ts", "utf8");
      } catch {
        lifecycleSrc = await readFile(
          ".pi/extensions/pi-quest/src/lifecycle.ts",
          "utf8",
        );
      }
      assert.ok(
        lifecycleSrc.includes("future-archive") ||
          lifecycleSrc.includes("copyFile"),
        "lifecycle also archives",
      );
    },
  );
  ...
  // =====================================================================
  await t.step(
    "#26 restoreSessionState orphan fallback scans FUTURE_DIR when activeDraft null",
    async () => {
      ...
      try {
        ...
        await writeFile(
          join(tempRoot, ".pi/quest/future", "orphan-feature.md"),
          "# Proposal / Future Quest: orphan-feature\n## Requirements\n- build orphan\n",
          "utf8",
        );
        ...
        assert.strictEqual(
          (restored as any).activeDraft,
          "orphan-feature",
          "orphan future file should become activeDraft",
        );
        ...
        const hasBullet = (restored as any).draftPrompts.some((p: string) =>
          p.includes("build orphan")
        );
  ...
  await t.step(
    "#26 restoreSessionState does not override existing activeDraft",
    () => {
      const latest: any = {
      ...
      assert.deepStrictEqual((restored as any).draftPrompts, [
        "existing prompt",
      ]);
    },
  );
  ...
  // =====================================================================
  await t.step(
    "#47 read future md ENOENT with activeDraft must not be FAILURE_RECORDED",
    async () => {
      ...
      try {
        ...
        const event: any = {
          toolName: "read",
          input: {
            path: ".pi/quest/future/look-consumer-side-code-lot-complexity.md",
          },
          isError: true,
          error: {
            message:
              "ENOENT: no such file or directory, open '.pi/quest/future/look-consumer-side-code-lot-complexity.md'",
          },
        ...
        assert.strictEqual(
          classified.failure,
          null,
          "read ENOENT must not be meaningful failure",
        );
        ...
        const isFutureReadENOENT = (event.toolName === "read") &&
          ...
          Boolean(
            (state as any).pendingRootQuest || (state as any).activeDraft,
          );
        assert.strictEqual(
          isFutureReadENOENT,
          true,
          "future md ENOENT with draft pending must be whitelisted",
        );
      } finally {
  ...
  await t.step(
    "#47 read non-quest ENOENT should still be failure (not whitelisted)",
    () => {
      const s = createDefaultState();
      ...
      try {
        ...
        assert.strictEqual(
          isWhitelisted,
          false,
          "without pending draft, ENOENT not whitelisted",
        );
      } finally {
        (state as any).pendingRootQuest = prevPending;
        (state as any).activeDraft = prevDraft;
  ...
  // =====================================================================
  await t.step(
    "#48 empty bash command must not be hasFailure even with isError",
    () => {
      const tr: any = {
        toolName: "bash",
        isError: true,
      ...
      assert.strictEqual(
        res.hasFailure,
        false,
        "empty command must not be failure",
      );
    },
  );
  ...
  await t.step("#48 whitespace-only command must not be hasFailure", () => {
    ...
    assert.strictEqual(
      res.hasFailure,
      false,
      "whitespace command must not be failure",
    );
  });

  await t.step(
    "#48 empty command does not cause reassessment via analyzeTurnToolResults",
    () => {
      const results: any[] = [
        {
          toolName: "bash",
      ...
      assert.strictEqual(
        ...
        0,
        "empty command must not increment failureCount",
      );
  ...
  // =====================================================================
  await t.step(
    "#49 generateSlugFromPrompt produces deterministic slug used for future file",
    () => {
      const prompt =
        "Look at the consumer side code. It has a lot of complexity. Developing the site should be easy";
      const slug = generateSlugFromPrompt(prompt, 45);
      ...
      assert.ok(
        slug.includes("consumer") || slug.includes("complexity") ||
          slug.includes("look"),
        `slug should contain prompt words, got ${slug}`,
      );
    },
  );

  await t.step(
    "#49 FUTURE_QUEST_TEMPLATE + slug mismatch fix resolves DRAFT_SLUG_CORRECTED",
    async () => {
      ...
      const src = await readFile("src/hooks/index.ts", "utf8").catch(() =>
        readFile(".pi/extensions/pi-quest/src/hooks/index.ts", "utf8")
      );
      assert.ok(
        src.includes("DRAFT_SLUG_CORRECTED"),
        "must handle DRAFT_SLUG_CORRECTED",
      );
      assert.ok(
        src.includes("resolveFutureDraftPath"),
        "must resolve future draft path to detect mismatch",
      );
    },
  );
  ...
  // =====================================================================
  await t.step(
    "#50 snapshot.ts only emits SNAPSHOT_FALLBACK for git_diff_failed during drafting (not draft_boundary_fallback on slug mismatch)",
    async () => {
      const src = await readFile("src/critical_agent/snapshot.ts", "utf8")
        .catch(() =>
          readFile(
            ".pi/extensions/pi-quest/src/critical_agent/snapshot.ts",
            "utf8",
          )
        );
      // The legitimate fallback is git_diff_failed
      assert.ok(
        src.includes('reason: "git_diff_failed"'),
        "git_diff_failed fallback must exist",
      );
      ...
      assert.ok(
        src.includes("draft_boundary"),
        "draft_boundary_fallback path exists",
      );
      ...
      assert.ok(
        src.includes("FUTURE_DIR") || src.includes("future"),
        "snapshot should read future dir",
      );
    },
  );
  ...
  // =====================================================================
  await t.step(
    "#51 draft_not_approved coalescence gateBlocked must not be TOOL_FAILURE",
    () => {
      const tr: any = {
      ...
      assert.strictEqual(
        classified.failure,
        null,
        "gateBlocked REVIEW_COALESCENCE_PENDING must not be failure",
      );
      ...
      assert.strictEqual(
        analysis.failureCount,
        0,
        "gateBlocked must not count as failure",
      );
      assert.strictEqual(
        analysis.meaningfulFailureDetected,
        false,
        "must not trigger reassessment",
      );
    },
  );

  await t.step(
    "#51 detectBashToolFailure handles gateBlocked vs real failure distinction",
    () => {
      ...
      assert.strictEqual(
        res.hasFailure,
        true,
        "real make test failure must be hasFailure",
      );
      ...
      assert.strictEqual(
        res2.hasFailure,
        false,
        "rg exit 1 no matches must not be failure",
      );
    },
  );
  ...
  // =====================================================================
  await t.step(
    "#52 skill hint gate: reassessmentRequired true must suppress CALL quest_update_state",
    async () => {
      const src = await readFile("src/hooks/index.ts", "utf8").catch(() =>
        readFile(".pi/extensions/pi-quest/src/hooks/index.ts", "utf8")
      );
      ...
      assert.ok(
        src.includes("skillHint") || src.includes("quest_update_state"),
        "source must have skill hint logic",
      );
      ...
      const gate = (s as any).reassessmentRequired
        ? "REASSESSMENT_PENDING"
        : ((s as any).researchRequired
          ? "RESEARCH_PENDING"
          : "IMPLEMENTATION_ALLOWED");
      assert.strictEqual(
        gate,
        "REASSESSMENT_PENDING",
        "reassessmentRequired must produce REASSESSMENT_PENDING gate",
      );
  ...
  // =====================================================================
  await t.step(
    "#53 DRAFT_AUTO_REVIEW_CHECK logic: dpLen>=1 && evidence>=7 triggers review",
    async () => {
      const src = await readFile("src/hooks/index.ts", "utf8").catch(() =>
        readFile(".pi/extensions/pi-quest/src/hooks/index.ts", "utf8")
      );
      assert.ok(
        src.includes("DRAFT_AUTO_REVIEW_CHECK"),
        "must log DRAFT_AUTO_REVIEW_CHECK",
      );
      assert.ok(
        src.includes("evidence >= 7") || src.includes("evidence>=7"),
        "must check evidence>=7 threshold",
      );
      assert.ok(
        src.includes("draftPrompts") || src.includes("dpLen"),
        "must check draftPrompts length",
      );
      // The combined condition (dpLen>=2) || (dpLen>=1 && evidence>=7)
      assert.ok(
        src.includes("dpLen >= 2") || src.includes("dpLen>=2"),
        "must handle dpLen>=2 case",
      );
      assert.ok(
        src.includes("dpLen>=1 && evidence>=7") ||
          src.includes("dpLen >= 1 && evidence >= 7"),
        "must handle dpLen>=1 && evidence>=7",
      );
    },
  );

  await t.step(
    "#53 auto-review still requires hasActionablePlanDraft or canAutoReviewDespitePlaceholder",
    async () => {
      const src = await readFile("src/hooks/index.ts", "utf8").catch(() =>
        readFile(".pi/extensions/pi-quest/src/hooks/index.ts", "utf8")
      );
      assert.ok(
        src.includes("hasActionablePlanDraft"),
        "must gate on hasActionablePlanDraft",
      );
      assert.ok(
        src.includes("canAutoReviewDespitePlaceholder"),
        "must allow auto-review despite placeholder when evidence>=7",
      );
    },
  );
  ...
  // =====================================================================
  await t.step(
    "#54 DRAFT_AUTO_REVIEW_CHECK and UI_STATUS and draftNotApprovedDetails are logged",
    async () => {
      ...
      try {
        ...
        await asyncContext.run(ctx, async () => {
          logEvent(
            "DRAFT_AUTO_REVIEW_CHECK" as any,
            "check dpLen=1 evidence=7 valid=false hasPlan=false",
            {
              logPath,
              questId: qid,
              dpLen: 1,
              evidence: 7,
              isDraftReviewValid: false,
              hasActionablePlanDraft: false,
            } as any,
          );
          logEvent(
            "REVIEW_DEDUP_HIT" as any,
            "dedup hit",
            {
              logPath,
              questId: qid,
          ...
          logEvent(
            "UI_STATUS" as any,
            "ui status",
            { logPath, questId: qid, status: "drafting" } as any,
          );
        });
        const content = await readFile(logPath, "utf8");
        assert.ok(
          content.includes("DRAFT_AUTO_REVIEW_CHECK"),
          "must log DRAFT_AUTO_REVIEW_CHECK",
        );
  ...
  // =====================================================================
  await t.step(
    "#55 hasActionablePlanDraft false for empty ## Plan: 1. placeholder",
    async () => {
      const src = await readFile("src/hooks/index.ts", "utf8").catch(() =>
        readFile(".pi/extensions/pi-quest/src/hooks/index.ts", "utf8")
      );
      assert.ok(
        src.includes("PLAN_NOT_DRAFTED_YET"),
        "must log PLAN_NOT_DRAFTED_YET",
      );
      assert.ok(
        src.includes("hasActionablePlanDraft"),
        "must check hasActionablePlanDraft before triggering review",
      );
      ...
      const checkHasActionable = (planBody: string): boolean => {
        ...
        if (!body || body === "1." || body === "-" || body.length < 10) {
          return false;
        }
        return /[-*]\s+\S|^\s*\d+\.\s+\S/m.test(body);
      };
      assert.strictEqual(
        checkHasActionable("# Quest: foo\n## Plan\n1.\n\n## Goals\nx"),
        false,
        "## Plan: 1. must be not actionable",
      );
      assert.strictEqual(
        checkHasActionable("# Quest: foo\n## Plan\n-\n\n## Goals\nx"),
        false,
        "## Plan: - must be not actionable",
      );
      assert.strictEqual(
        checkHasActionable("# Quest: foo\n## Plan\n\n## Goals\nx"),
        false,
        "empty plan must be not actionable",
      );
      assert.strictEqual(
        checkHasActionable(
        ...
        true,
        "real plan with bullets must be actionable",
      );
      assert.strictEqual(
        checkHasActionable(
        ...
        true,
        "plan with dash bullets actionable",
      );
    },
  );

  await t.step(
    "#55 dpLen>=1 && evidence>=7 with empty plan logs PLAN_NOT_DRAFTED_YET without triggering review",
    async () => {
      ...
      try {
        ...
        await asyncContext.run(ctx, async () => {
          logEvent(
            "DRAFT_AUTO_REVIEW_CHECK" as any,
            "check dpLen=1 evidence=17 valid=false hasPlan=false",
            {
              logPath,
              questId: qid,
              dpLen: 1,
          ...
          logEvent(
            "PLAN_NOT_DRAFTED_YET" as any,
            "plan not drafted yet",
            { logPath, questId: qid } as any,
          );
        });
        const content = await readFile(logPath, "utf8");
        ...
        assert.ok(
          !content.includes("PLAN_REVIEW_REQUESTED"),
          "empty plan must not trigger PLAN_REVIEW_REQUESTED",
        );
      } finally {
        await rm(tempRoot, { recursive: true, force: true });
      }
  ...
  // =====================================================================
  await t.step(
    "#04 / #24 upgrade: all promote paths archive-before-unlink (source check for all 5 paths)",
    async () => {
      const readSrc = async (rel: string) => {
        try {
          return await readFile(rel, "utf8");
        } catch {
          return await readFile(".pi/extensions/pi-quest/" + rel, "utf8");
        }
      };
      ...
      for (
        ...
        if (name === "paths.ts") {
          assert.ok(
            src.includes("future-archive") || src.includes("archive"),
            `paths.ts must handle archive`,
          );
        } else {
          assert.ok(
            src.includes("future-archive"),
            `${name} must reference future-archive`,
          );
          if (promotionFiles.has(name)) {
            assert.ok(
              src.includes("DRAFT_PROMOTED"),
              `${name} must log DRAFT_PROMOTED`,
            );
          } else {
            assert.ok(
              src.includes("DRAFT_DISCARDED"),
              `${name} must log DRAFT_DISCARDED`,
            );
          }
        }
      }
      ...
      assert.ok(
        packagingSrc.includes("future-archive"),
        "packaging must copy future-archive",
      );
    },
  );
  ...
  // =====================================================================
  await t.step(
    "#36 executor must gate quest_update_state on draft approval (isDraftReviewValid)",
    async () => {
      const src = await readFile("src/tools/update/executor.ts", "utf8").catch(
        () =>
          readFile(
            ".pi/extensions/pi-quest/src/tools/update/executor.ts",
            "utf8",
          ),
      );
      assert.ok(
        src.includes("isDraftReviewValid"),
        "executor must check isDraftReviewValid",
      );
      assert.ok(
        src.includes("draft_not_approved"),
        "must handle draft_not_approved gate",
      );
      // The gate should be GATE_BLOCKED, not TOOL_FAILURE — verify it sets gateBlocked
      assert.ok(
        src.includes("gateBlocked"),
        "must set gateBlocked for draft_not_approved",
      );
      assert.ok(
  ...
  await t.step(
    "#36 draft_not_approved while REASSESSMENT_PENDING is GATE_BLOCKED (not TOOL_FAILURE) — also covers #51",
    () => {
      ...
      assert.strictEqual(
        classified.failure,
        null,
        "must be GATE_BLOCKED not TOOL_FAILURE",
      );
  ...
  // =====================================================================
  await t.step(
    "#56 activateExistingQuest must have a session-liveness coalescence guard and QUEST_REUSED_COALESCED event",
    async () => {
      const readSrc = async (rel: string) => {
        try {
          return await readFile(rel, "utf8");
      ...
      assert.ok(
        lifecycleSrc.includes("isQuestSessionActive"),
        "activateExistingQuest must probe session liveness",
      );
      assert.ok(
        lifecycleSrc.includes("QUEST_REUSED_COALESCED"),
        "guard must log QUEST_REUSED_COALESCED",
      );
      assert.ok(
        lifecycleSrc.includes("reportAgentError"),
        "guard must produce an agent-visible message",
      );
      assert.ok(
        mutexSrc.includes("SESSION_LIVENESS_FILE"),
        "mutex must own the session-liveness marker",
      );
      assert.ok(
        mutexSrc.includes("isQuestSessionActive"),
        "mutex must export isQuestSessionActive",
      );
      assert.ok(
        typesSrc.includes("QUEST_REUSED_COALESCED"),
        "QuestLogEventType must include QUEST_REUSED_COALESCED",
      );
    },
  );
  ...
  // =====================================================================
  await t.step(
    "#57 validateReassessmentPrerequisites must aggregate all missing contract fields + actionable freshness reason",
    async () => {
      const readSrc = async (rel: string) => {
        try {
          return await readFile(rel, "utf8");
      ...
      assert.ok(
        reassessmentSrc.includes("Complete ALL of the following"),
        "must aggregate ALL missing contract fields",
      );
      assert.ok(
        reassessmentSrc.includes("reassessmentConclusion"),
        "must name the missing conclusion",
      );
      assert.ok(
        reassessmentSrc.includes("read/code-search NOW"),
        "must instruct a fresh investigation",
      );
      assert.ok(
        reassessmentSrc.includes("REASSESSMENT_EVIDENCE_REQUIRED"),
        "must use the evidence-required code when freshness blocks",
      );
      assert.ok(
        researchSrc.includes(
          "records a fresh receipt automatically while reassessment is pending",
        ),
        "freshness reason must be actionable",
      );
    },
  );
  ...
  // =====================================================================
  await t.step(
    "#58 subagent cwd must be anchored to project root (resolveSubagentCwd) with SUBAGENT_CWD_REANCHORED event",
    async () => {
      const readSrc = async (rel: string) => {
        try {
          return await readFile(rel, "utf8");
      ...
      const typesSrc = await readSrc("src/logging/types.ts");
      assert.ok(
        adapterSrc.includes("resolveSubagentCwd"),
        "must define resolveSubagentCwd resolver",
      );
      assert.ok(
        adapterSrc.includes("findProjectRoot"),
        "must anchor cwd via findProjectRoot",
      );
      assert.ok(
        adapterSrc.includes("cwd: resolveSubagentCwd(ctx)"),
        "must use resolveSubagentCwd at the launch emit site",
      );
      assert.ok(
        adapterSrc.includes("SUBAGENT_CWD_REANCHORED"),
        "must emit SUBAGENT_CWD_REANCHORED on re-anchor",
      );
      assert.ok(
        typesSrc.includes("SUBAGENT_CWD_REANCHORED"),
        "SUBAGENT_CWD_REANCHORED must be a QuestLogEventType",
      );
    },
  );
  ...
  // =====================================================================
  await t.step(
    "#27 isStoredState runtime guard present + no blind as unknown as StoredState cast",
    async () => {
      const readSrc = async (rel: string) => {
        try {
          return await readFile(rel, "utf8");
      ...
      assert.ok(
        /export function isStoredState\(/.test(reconSrc),
        "isStoredState guard must exist",
      );
      assert.ok(
        reconSrc.includes("isStoredState(entry.data)"),
        "isStoredState must guard the journal snapshot cast",
      );
      assert.ok(
        !reconSrc.includes("as unknown as StoredState"),
        "no blind as unknown as StoredState cast",
      );
      assert.ok(
        typesSrc.includes("Record<string, unknown>"),
        "details must be Record<string, unknown> (not any)",
      );
    },
  );
});
