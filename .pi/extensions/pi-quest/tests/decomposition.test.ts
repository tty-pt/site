...
Deno.test("quest_journal_decomposition: structural workstream decomposition & sub-quest lifecycle", async (t) => {
	...
	await t.step("1. task with two independent subsystems creates sub-quests even with <4 phases", async () => {
		...
		await emitToolResult("read", { path: "mods/auth/callback.c" }, "auth callback code");
	...
	await t.step("2. simple multi-step task remains a single quest without artificial sub-quests", async () => {
		...
		for (const cb of handlers["tool_call"] || []) {
			await cb({ toolName: "read", input: { path: "mods/core/http.c" } }, simpleCtx);
		}
	...
	await t.step("8. parent execution resumes and child 1 is marked completed", async () => {
		...
		const status = await commands["quest-status"].handler("", mockCtx);
		assert.ok(status.includes(rootSlug.slice(0, 20)) || status.includes("repair-authentication"), "Parent quest is active again");
	...
	await t.step("10. sub-quest executes autonomously once research is complete without user confirmation", async () => {
		...
		await emitToolResult("read", { path: "mods/auth/session.h" }, "session header code");
