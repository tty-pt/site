...
Deno.test("quest_journal_streaming_compaction: safe message queueing and deferred compaction", async () => {
  ...
  assert.ok(
    archiveResult &&
      (archiveResult.content[0].text.includes("Quest archived") ||
        archiveResult.content[0].text.includes("Archived")),
    "quest_archive must succeed",
  );
  ...
  for (const cb of handlers["turn_end"] || []) {
    await cb({
      ...
      toolResults: [{ toolName: "quest_archive" }],
    }, mockCtx);
  }
  ...
  assert.strictEqual(
    compactInvocationCount,
    1,
    "Compaction must be invoked exactly once after archive",
  );
  assert.ok(
