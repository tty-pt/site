...
Deno.test("quest_journal_parser: robust markdown parsing and structured state update", async () => {
  ...
  assert.ok(
    tools["quest_update_state"],
    "quest_update_state tool should be registered",
  );
  ...
  const updateRes = await tools["quest_update_state"].execute(
    "call_up",
    {
      ...
      findings: [
        "Found bottleneck in parser",
        "AST parsing reduces regex brittleness",
      ],
  ...
  assert.ok(
    diskContent.includes("Phase 1 Complete"),
    "Disk content should have updated status",
  );
  assert.ok(
    diskContent.includes("AST parsing reduces regex brittleness"),
    "Disk content should have findings",
  );
  assert.ok(
    diskContent.includes("Default switchNow to true"),
    "Disk content should have decisions",
  );
  assert.ok(
    diskContent.includes("- [ ] Phase 2 execution"),
    "Disk content should have remaining work checklist",
  );
  assert.ok(
    diskContent.includes("Proceed with Phase 2 implementation"),
    "Disk content should have next step",
  );
  ...
  assert.ok(
    updatedComplex.includes("## Custom Benchmark Matrix"),
    "Must preserve custom table section header",
  );
  assert.ok(
    updatedComplex.includes("| Hyle | 1.2ms | 4MB |"),
    "Must preserve custom table content",
  );
  assert.ok(
    updatedComplex.includes("## Unmanaged Appendix"),
    "Must preserve appendix section header",
  );
  assert.ok(
    updatedComplex.includes("Appendix notes that must survive."),
    "Must preserve appendix body",
  );
  assert.ok(updatedComplex.includes("- [x] Done"), "Must update status");
  assert.ok(
    updatedComplex.includes("Ship to production"),
    "Must add/update next step",
  );
  ...
  assert.ok(
    statusA.includes("quest-session-a"),
    "Session A should have quest-session-a active",
  );
  assert.ok(
    statusB.includes("quest-session-b"),
    "Session B should have quest-session-b active",
  );
  ...
  assert.ok(
    finalStatusA.includes("quest-session-a"),
    "Interleaved Task A must remain on quest-session-a",
  );
  assert.ok(
    finalStatusB.includes("quest-session-b"),
    "Interleaved Task B must remain on quest-session-b",
  );
