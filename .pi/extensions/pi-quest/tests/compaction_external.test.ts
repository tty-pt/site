...
Deno.test("quest_journal_compaction_external: external compaction handling, out-of-band events, race isolation, and recovery", async (t) => {
	...
	// -----------------------------------------------------------------------
	await t.step("4. Unknown external compaction: marks transaction inconsistent, delivers RESUME_STATE_INCONSISTENT, does not emit normal resume, preserves implementation block until explicit reconciliation", async () => {
		...
		assert.ok(state.activeTransaction, "Must create activeTransaction in recovery state");
		assert.strictEqual((state.activeTransaction as any).phase, "inconsistent", "Must enter inconsistent recovery state on unknown external compaction");
		...
		assert.strictEqual(resumes.length, 0, "Normal resume must NOT be emitted for unknown compaction");

		// 4. Implementation remains blocked while the transaction is inconsistent
		assert.strictEqual(canImplement(state, ctx as any), false, "Implementation must be blocked while transaction is inconsistent");
		...
		for (const cb of api.handlers["tool_call"] || []) {
			const res = await cb({ toolName: "edit", input: { path: "some/file.c" } }, ctx);
		...
		assert.strictEqual((state.activeTransaction as any).checkpointSaveCount, undefined, "checkpointSaveCount must not be invented for unknown compaction");
		assert.strictEqual((state.activeTransaction as any).checkpointHash, undefined, "checkpointHash must not be invented for unknown compaction");
		assert.strictEqual((state.activeTransaction as any).questPath, undefined, "questPath must not be invented for unknown compaction");
		assert.strictEqual((state.activeTransaction as any).observedSaveCount, 5, "observedSaveCount can be stored for diagnostics");
		...
		await tools["quest_update_state"].execute(
			"call_reconcile_unknown",
			{
				...
				reassessmentConclusion: "Investigated external compaction and validated durable state.",
			},
			{},
			() => {},
			ctx,
		...
		assert.strictEqual(state.activeTransaction, null, "Transaction must resolve after explicit reconciliation");
		assert.strictEqual(state.reassessmentRequired, false);
		assert.strictEqual(canImplement(state, ctx as any), true, "Implementation gate must open after explicit reconciliation");

		const postReconcileMsgs = getAllMessages(api);
		assert.ok(
			postReconcileMsgs.some((m) => m.includes("REASSESSMENT RESOLVED — IMPLEMENTATION GATE OPEN")),
	...
	// -----------------------------------------------------------------------
	await t.step("25. Regression Test: Unexpected external compaction enters recovery state without fake clean success", async () => {
		const api = createMockExtensionAPI();
		plugin(api.mockPi as any);

		const ctx = createMockContext(10000, "session_external_compaction_recovery");
		const commands: Record<string, any> = {};
		for (const cmd of api.registeredCommands) commands[cmd.name] = cmd;
		const tools: Record<string, any> = {};
		for (const tool of api.registeredTools) tools[tool.name] = tool;
		...
		assert.ok(state.activeTransaction, "activeTransaction must be created in recovery state");
		assert.strictEqual((state.activeTransaction as any).phase, "inconsistent", "Transaction must be marked inconsistent");
		assert.strictEqual(state.reassessmentRequired, true, "reassessmentRequired must be set");

		// 2. Normal clean success bookkeeping was NOT performed
		assert.strictEqual(state.compactCount, 2, "compactCount must NOT be advanced to saveCount");
		assert.strictEqual(state.dirty, true, "dirty flag must NOT be reset to false");
		...
		assert.strictEqual(state.activeTransaction, null, "Transaction resolved after explicit reassessment");
		assert.strictEqual(state.reassessmentRequired, false);

		// Marking saved establishes fresh clean checkpoint
		await tools["quest_mark_saved"].execute("call_save_fresh", { name: slug }, {}, () => {}, ctx);
		assert.strictEqual(canImplement(state, ctx as any), true, "Implementation gate opens after save");
	...
	// -----------------------------------------------------------------------
	await t.step("27. Regression Test: Unexpected external compaction preserves pendingResume and blocks implementation", async () => {
		const api = createMockExtensionAPI();
		plugin(api.mockPi as any);
		...
		assert.ok(state.pendingResume, "pendingResume must be preserved across unexpected external compaction");
		...
		assert.strictEqual(state.pendingResume.checkpointHash, "hash_preserve_333");

		// 2. Transaction enters inconsistent recovery state
		assert.ok(state.activeTransaction);
		assert.strictEqual((state.activeTransaction as any).phase, "inconsistent");
		...
		const resumes = msgs.filter((m) => m.includes("Post-Compaction Autonomous Resumption Directive"));
		assert.strictEqual(resumes.length, 0, "No normal resume emitted for unmanaged compaction");
	...
	// -----------------------------------------------------------------------
	await t.step("28. Regression Test: Unexpected external compaction preserves pendingSubquestResume", async () => {
		const api = createMockExtensionAPI();
		plugin(api.mockPi as any);
		...
		assert.strictEqual(state.pendingSubquestResume, childSlug, "pendingSubquestResume must be preserved");
	...
	// -----------------------------------------------------------------------
	await t.step("29. Regression Test: Multi-compaction race isolates external transactions and preserves resume-pending obligations", async () => {
		const api = createMockExtensionAPI();
		plugin(api.mockPi as any);

		const ctx = createMockContext(10000, "session_multi_compaction_isolation");
		const commands: Record<string, any> = {};
		for (const cmd of api.registeredCommands) commands[cmd.name] = cmd;
		const tools: Record<string, any> = {};
		for (const tool of api.registeredTools) tools[tool.name] = tool;
		...
		assert.strictEqual(state.activeTransaction.id, originalTxId, "Original transaction ID must not be overwritten");
		assert.strictEqual(state.activeTransaction.phase, "resume-pending", "Phase must remain resume-pending");
		assert.strictEqual(state.activeTransaction.checkpointHash, "hash_orig_111");
		...
		const msgs = getAllMessages(api);
		assert.ok(msgs.some((m) => m.includes("RESUME_STATE_INCONSISTENT")), "Must emit RESUME_STATE_INCONSISTENT error");
