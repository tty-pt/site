...
Deno.test("quest_journal_cross_session_mount: QUEST_REUSED mount coalesces against another live session", async (t) => {
	...
	await t.step("1. session A mounts the quest (fresh session, writes liveness)", async () => {
		...
		await asyncContext.run(ctxA, async () => {
			mounted = await activateExistingQuest(mockPi, ctxA, slug, "please continue");
		});

		assert.strictEqual(mounted, true, "First live session must mount the quest");

		// A's session must be the one owning the liveness marker
		const ownActive = isQuestSessionActive(slug, "session_A_type_56");
		assert.strictEqual(ownActive, false, "Same-session liveness must NOT be seen as an active foreign session");
	});

	await t.step("2. session B (different sessionId) calling activateExistingQuest is coalesced (returns false, no TURN_START 0, no saveGeneration reset)", async () => {
		const { mockPi } = createMockExtensionAPI();
		...
		assert.strictEqual(isQuestSessionActive(slug, "session_B_type_56"), true, "B must see A's fresh liveness as active");

		let mountedB: boolean | null = null;
		await asyncContext.run(ctxB, async () => {
			mountedB = await activateExistingQuest(mockPi, ctxB, slug, "please continue");
		});

		assert.strictEqual(mountedB, false, "Second live session must be refused/coalesced");
		assert.strictEqual(stateB.saveGeneration, null, "saveGeneration must NOT be reset when coalesced");
		assert.strictEqual(stateB.questId, null, "questId must NOT be reassigned when coalesced");
		...
		assert.ok(log.includes("QUEST_REUSED_COALESCED"), "log must contain QUEST_REUSED_COALESCED for the coalesced second mount");
	});

	await t.step("3. same-session re-entry is allowed (idempotent for the owning session)", async () => {
		const { mockPi } = createMockExtensionAPI();
		const ctxA = createMockContext("session_A_type_56");
		getState(ctxA).questId = null;
		writeSessionLiveness(slug, "session_A_type_56");
		...
		assert.strictEqual(mounted, true, "Same-session re-entry must still mount");
	});

	await t.step("4. stale liveness marker lets a new session mount (crash recovery)", async () => {
		const { mockPi } = createMockExtensionAPI();
		...
		assert.strictEqual(isQuestSessionActive(slug, "session_C_type_56"), false, "Stale marker must not block a new session");

		let mounted: boolean | null = null;
		await asyncContext.run(ctxC, async () => {
			mounted = await activateExistingQuest(mockPi, ctxC, slug, "please continue");
		});
		assert.strictEqual(mounted, true, "New session must mount after stale liveness");
	});
});
