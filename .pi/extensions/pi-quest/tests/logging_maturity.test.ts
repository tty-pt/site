...
import {
  ...
  parseLogEntry,
  sanitizeLogString,
} from "../src/logging/formatters.ts";
import {
  getQuestLogPath,
  logEvent,
  ...
  try {
    await t.step("P0 draft early capture DRAFT_* hash only", async () => {
      ...
      await asyncContext.run(ctx, async () => {
        const h = createHash("sha256").update("prompt one").digest("hex").slice(
          0,
          12,
        );
        logEvent(
        ...
        logEvent(
          ...
          "ignored",
          { logPath, questId: qid, slug: "my-feature", hash: h } as any,
        );
        logEvent(
          "DRAFT_DISCARDED",
          "discarded",
      ...
      const ctxStr = formatContextFields(
        {
          draftPromptsCount: 1,
          hash: "abc",
          attemptKey: "k",
      ...
      await writeFile(
        join(futureDir, "my-feature.md"),
        "# Quest: my-feature\n## Requirements\n- prompt one\n",
        "utf8",
      );
      await mkdir(join(currentDir, qid), { recursive: true });
      await writeFile(
        join(currentDir, qid, "quest.md"),
        "# Quest: my-feature\n",
        "utf8",
      );
    ...
    await t.step(
      "P1 pending/attempt/requireConfirm observability",
      async () => {
        ...
        await asyncContext.run(ctx, async () => {
          logEvent(
            ...
            "dropped",
            { logPath: lp, questId: qid, pendingCount: 10 } as any,
          );
          logEvent(
          ...
          logEvent(
            ...
            "requireConfirm",
            { logPath: lp, questId: qid, requireConfirm: false } as any,
          );
          logEvent(
            "FIRST_PLAN_REVIEW_ALREADY_FIRED",
            "already fired",
            { logPath: lp, questId: qid } as any,
          );
          logEvent(
            "REVIEW_DEDUP_HIT",
            "dedup",
            { logPath: lp, questId: qid, shard: "draft" } as any,
          );
        });
    ...
    await t.step(
      "B2.5 provisional-stall SAVE_FAILED+ future_draft_exists",
      async () => {
        ...
        await asyncContext.run(ctx, async () => {
          logEvent(
            "SAVE_FAILED",
            "Save verification failed: Quest file not found or unreadable at `.pi/quest/current/1/quest.md`. Draft exists in `.pi/quest/future/my-feature.md` — call quest_update_state",
            {
              logPath: lp,
              questId: qid,
              path: `.pi/quest/current/${qid}/quest.md`,
              reason: "file_not_found+future_draft_exists",
              requiredAction: "quest_update_state",
            } as any,
          );
          logEvent(
            "GATE_BLOCKED",
            "gate blocked",
            {
              logPath: lp,
              questId: qid,
              gate: "PROVISIONAL_RESEARCH_PENDING",
              requiredAction: "quest_update_state",
            } as any,
          );
          logEvent(
            ...
            "reassessment",
            { logPath: lp, questId: qid, round: 2 } as any,
          );
        });
    ...
    await t.step(
      "B3 dedup+user+semantic INITIAL_PROMPT USER_PROMPT SEMANTIC_SNAPSHOT",
      async () => {
        ...
        await asyncContext.run(ctx, async () => {
          ...
          const h = createHash("sha256").update(intent).digest("hex").slice(
            0,
            12,
          );
          logEvent(
          ...
          logEvent(
            ...
            "user prompt",
            {
              ...
              slice: "my refinement",
              hash: h,
              classification: "REFINEMENT_OR_REQUIREMENT",
              intentHash: h,
            } as any,
          );
          logEvent(
      ...
      await asyncContext.run(ctx, async () => {
        logEvent(
          ...
          "wait",
          {
            logPath: lp,
            questId: qid,
            lockKey: "global:review:sess-p3b",
            waitMs: 2,
          } as any,
        );
        logEvent(
        ...
        logEvent(
          "SNAPSHOT_FALLBACK",
          "fallback",
          { logPath: lp, questId: qid, reason: "git_diff_failed" } as any,
        );
        logEvent(
    ...
    await t.step("formatters + reducers + summarize not crash", async () => {
      ...
      await writeFile(
        lp,
        "2026-09-01T00:00:00.000Z | DRAFT_APPENDED | quest=q1 draftPromptsCount=1 hash=abc | draft\n2026-09-01T00:00:01.000Z | PENDING_COALESCED_DROPPED | quest=q1 pendingCount=1 | drop\n",
        "utf8",
      );
      const summary = summarizeQuestJournalLog(lp);
      assert.ok(summary);
      const line =
        "2026-09-01T00:00:00.000Z | DRAFT_APPENDED | quest=q1 draftPromptsCount=1 hash=abc | msg";
