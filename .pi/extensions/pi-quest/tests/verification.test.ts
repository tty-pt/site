...
Deno.test("quest_journal_verification: verified save generation, fingerprinting, error-guarded writes, and surgical AST updates", async () => {
  ...
  assert.ok(rec, "Quest record should be created");
  ...
  assert.ok(
    resMissing.details?.error ||
      resMissing.content[0].text.toLowerCase().includes("error") ||
      resMissing.content[0].text.toLowerCase().includes("missing") ||
      resMissing.content[0].text.toLowerCase().includes("not found"),
    `quest_mark_saved must reject when file is missing from disk, got: ${
  ...
  assert.ok(
    !resSuccess.details?.error &&
      resSuccess.content[0].text.toLowerCase().includes("saved"),
    `quest_mark_saved must succeed when file exists, got: ${
      JSON.stringify(resSuccess)
    }`,
  );
  assert.ok(
    resSuccess.details?.hash || resSuccess.details?.generation,
    "quest_mark_saved should return hash/generation details",
  );
  ...
  const updateRes = await tools["quest_update_state"].execute(
    "call_update_ast",
    {
      ...
      findings: [
        "AST block splicing works cleanly",
        "Zero data loss for unmanaged sections",
      ],
      decisions: ["Preserve all unknown markdown headers verbatim"],
      filesTouched: [".pi/extensions/pi-quest/index.ts"],
  ...
  assert.ok(
    updateRes && !updateRes.details?.error,
    "quest_update_state should succeed",
  );
  assert.ok(
    updateRes.details?.generation > firstGeneration,
    "Save generation should increment on valid update",
  );

  const updatedDisk = await readFile(testQuestPath, "utf8");
  assert.ok(
    updatedDisk.includes("## Detailed Multi-Stage Execution Plan"),
    "Must preserve ## Detailed Multi-Stage Execution Plan",
  );
  assert.ok(
    updatedDisk.includes("### Stage 1: Initial Setup"),
    "Must preserve content in execution plan",
  );
  assert.ok(
    updatedDisk.includes("## Acceptance Criteria & Polish Checklist"),
    "Must preserve ## Acceptance Criteria & Polish Checklist",
  );
  assert.ok(
    updatedDisk.includes("- [ ] Criterion A"),
    "Must preserve checklist items",
  );
  assert.ok(
    updatedDisk.includes("## Custom Developer Notes"),
    "Must preserve custom developer notes section",
  );
  assert.ok(
    updatedDisk.includes("Important architectural invariant"),
    "Must preserve custom notes body",
  );
  assert.ok(
    updatedDisk.includes("AST block splicing works cleanly"),
    "Must include updated findings",
  );
  assert.ok(
    updatedDisk.includes("Preserve all unknown markdown headers verbatim"),
    "Must include updated decisions",
  );
  assert.ok(
    updatedDisk.includes(".pi/extensions/pi-quest/index.ts"),
    "Must include updated files touched",
  );
  ...
  assert.strictEqual(
    userMessages.length,
    0,
    "turn_end should not nag when state is clean",
  );
  ...
  const resClean = await tools["quest_mark_saved"].execute(
    "call_clean",
    {},
    null,
    null,
  ...
  assert.ok(
    !resClean.details?.error,
    "Mark saved should succeed and clear dirty flag",
  );

  // 6. Test CRB provider hook includes 'Never propose anything without doing your homework first' at the start
  const g = globalThis as any;
  assert.ok(
    g.__pi_crb_providers && g.__pi_crb_providers.length > 0,
    "CRB provider hook should be registered",
  );
  ...
  assert.ok(
    crbRules[0].toLowerCase().includes(
      "never propose anything without doing your homework first",
    ),
    `First CRB rule must be homework-first rule, got: ${crbRules[0]}`,
  );

  // Clean up
  await rm(rec.dir, { recursive: true, force: true });
});
