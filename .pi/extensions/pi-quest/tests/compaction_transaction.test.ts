...
Deno.test("quest_journal_compaction_transaction: lifecycle transactions, checkpoints, immutability, and state consistency", async (t) => {
  ...
  // -----------------------------------------------------------------------
  await t.step(
    "1. Checkpoint immutability: checkpoint fields remain immutable when state changes, mismatch is detected",
    async () => {
      ...
      assert.strictEqual(
        state.activeTransaction?.activeQuest,
        questA,
        "Transaction activeQuest must remain immutable A",
      );
      assert.strictEqual(
        state.activeTransaction?.checkpointSaveCount,
        initialSaveCount,
        "Transaction checkpointSaveCount must remain immutable",
      );
      assert.strictEqual(
        state.activeTransaction?.checkpointHash,
        initialHash,
        "Transaction checkpointHash must remain immutable",
      );
      ...
      assert.strictEqual(
        ...
        false,
        "canImplement must return false for inconsistent transaction",
      );
      ...
      assert.ok(
        ...
        "activeTransaction must NOT be cleared by quest_mark_saved",
      );
      assert.strictEqual(state.activeTransaction.phase, "inconsistent");
      assert.strictEqual(
        canImplement(state, ctx as any),
        false,
        "Implementation must stay blocked while transaction is inconsistent",
      );
      ...
      await tools["quest_update_state"].execute(
        "call_resolve_step_1",
        {
          ...
          reassessmentConclusion:
            "Investigated discrepancy and reconciled durable journal.",
        },
        {},
        () => {},
        ctx,
      ...
      assert.strictEqual(
        state.activeTransaction,
        null,
        "activeTransaction cleared after explicit reassessment",
      );
      assert.strictEqual(
        canImplement(state, ctx as any),
        true,
        "canImplement must return true once reconciled",
      );
  ...
  await t.step(
    ...
    async () => {
      ...
      assert.ok(
        recovered.activeTransaction,
        "activeTransaction must be reconstructed",
      );
      assert.strictEqual(
        recovered.activeTransaction.phase,
        "in-flight",
        "Phase must remain in-flight",
      );
      assert.strictEqual(
        recovered.activeTransaction.id,
        "cmp_in_flight_recon_123",
      );
      assert.strictEqual(
        recovered.compactionPending,
        true,
        "compactionPending must be synced to true for in-flight transaction",
      );
    },
  );
  ...
  await t.step(
    ...
    async () => {
      ...
      assert.strictEqual(
        resumes.length,
        1,
        "Exactly one successful resume directive delivered",
      );
  ...
  await t.step(
    ...
    async () => {
      ...
      const ctx = createMockContext(
        10000,
        "session_compaction_duplicate_events",
      );
      const commands: Record<string, any> = {};
      for (const cmd of api.registeredCommands) commands[cmd.name] = cmd;
      const tools: Record<string, any> = {};
      ...
      const resumes = getAllMessages(api).filter((m) =>
        m.includes("Post-Compaction Autonomous Resumption Directive")
      );
      assert.strictEqual(
        resumes.length,
        1,
        "Exactly one resume directive delivered across duplicate callbacks",
      );
  ...
  // -----------------------------------------------------------------------
  await t.step(
    "23. Regression Test F & G: Pre-compaction checkpoint verification before successful compaction bookkeeping",
    async () => {
      const api = createMockExtensionAPI();
      plugin(api.mockPi as any);
      ...
      const ctx = createMockContext(
        10000,
        "session_compaction_bookkeeping_verify",
      );
      const commands: Record<string, any> = {};
      for (const cmd of api.registeredCommands) commands[cmd.name] = cmd;
      const tools: Record<string, any> = {};
      ...
      assert.strictEqual(
        state.dirty,
        true,
        "dirty flag must NOT be cleared on mismatch",
      );
      assert.notStrictEqual(
        state.compactCount,
        99,
        "compactCount must NOT advance on mismatch",
      );
      ...
      await tools["quest_update_state"].execute(
        "call_update_reconciled",
        {
          ...
          reassessmentComplete: true,
          reassessmentConclusion:
            "Investigated save count discrepancy and reconciled durable journal.",
        },
        {},
        () => {},
        ctx,
      ...
      assert.strictEqual(
        validTx.phase,
        "resume-pending",
        "Transaction marked resume-pending on valid match with transport failure",
      );
      assert.strictEqual(
        state.dirty,
        false,
        "dirty flag cleared on valid match",
      );
      assert.strictEqual(
        state.compactCount,
        validCount,
        "compactCount advanced on valid match",
      );
      assert.ok(
        state.pendingResume,
        "Pending resume created on valid match with transport retry",
      );
      assert.strictEqual(
        state.pendingResume.checkpointSaveCount,
        validTx.checkpointSaveCount,
      );
      assert.strictEqual(
        state.pendingResume.checkpointHash,
        validTx.checkpointHash,
      );
      assert.strictEqual(
        state.pendingResume.checkpointQuestPath,
        validTx.questPath,
      );
  ...
  // -----------------------------------------------------------------------
  await t.step(
    "24. Regression Test: quest_mark_saved does not resolve failed/inconsistent transactions; gate remains blocked",
    async () => {
      const api = createMockExtensionAPI();
      plugin(api.mockPi as any);
      ...
      assert.strictEqual(
        canImplement(state, ctx as any),
        false,
        "Gate must be blocked when transaction is failed",
      );
      const blockReason = getImplementationBlockReason(state, ctx as any);
      assert.strictEqual(blockReason.blocked, true);
      ...
      const saveRes = await tools["quest_mark_saved"].execute(
        "call_save_while_failed",
        { name: slug },
        {},
        () => {},
        ctx,
      ...
      assert.ok(
        state.activeTransaction,
        "activeTransaction must still exist after quest_mark_saved",
      );
      assert.strictEqual(
        state.activeTransaction.phase,
        "failed",
        "activeTransaction must remain in failed phase",
      );
      assert.strictEqual(
        canImplement(state, ctx as any),
        false,
        "Implementation gate must stay blocked after save",
      );
      ...
      assert.strictEqual(
        canImplement(state, ctx as any),
        false,
        "Gate must be blocked when transaction is inconsistent",
      );

      await tools["quest_mark_saved"].execute(
        "call_save_while_inconsistent",
        { name: slug },
        {},
        () => {},
        ctx,
      ...
      assert.strictEqual(
        state.activeTransaction.phase,
        "inconsistent",
        "activeTransaction must remain inconsistent after save",
      );
      assert.strictEqual(
        canImplement(state, ctx as any),
        false,
      ...
      await tools["quest_update_state"].execute(
        "call_explicit_reconcile",
        {
          ...
          reassessmentComplete: true,
          reassessmentConclusion:
            "Investigated failure and explicitly reconciled state.",
        },
        {},
        () => {},
        ctx,
      ...
      assert.strictEqual(
        state.activeTransaction,
        null,
        "activeTransaction cleared after explicit reassessmentComplete",
      );
      assert.strictEqual(
        canImplement(state, ctx as any),
        true,
        "Implementation gate opens after explicit reconciliation",
      );
  ...
  // -----------------------------------------------------------------------
  await t.step(
    "9. Invalidation of prepared compaction transaction upon new save (gen 15 -> gen 16)",
    async () => {
      const api = createMockExtensionAPI();
      plugin(api.mockPi as any);
      ...
      const preparedTx = createOrGetCompactionTransaction(
        state,
        "normal-compaction",
        slug,
      );
      ...
      assert.strictEqual(
        state.activeTransaction,
        null,
        "Prepared transaction must be cleared on new save",
      );
      ...
      assert.notStrictEqual(
        freshTx.id,
        preparedTx.id,
        "Stale gen-15 transaction must NOT be reused",
      );
      assert.strictEqual(
        freshTx.checkpointSaveCount,
        16,
        "Fresh transaction must point to gen 16",
      );
      assert.strictEqual(
        freshTx.checkpointHash,
        gen16Hash,
        "Fresh transaction must point to gen 16 hash",
      );
  ...
  // -----------------------------------------------------------------------
  await t.step(
    "10. In-flight compaction transaction is not cleared by save",
    async () => {
      const api = createMockExtensionAPI();
      plugin(api.mockPi as any);
      ...
      assert.ok(
        state.activeTransaction,
        "in-flight transaction must survive save",
      );
