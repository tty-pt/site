...
Deno.test("quest_journal_compaction_resume: resume delivery, retries, deduplication, obligations, and subquest continuations", async (t) => {
	...
	// -----------------------------------------------------------------------
	await t.step("2. Pending resume cannot resurrect a quest: archived child is not resurrected by pending resume", async () => {
		...
		await tools["quest_archive"].execute("call_arch", { questName: childSlug, compact: false }, {}, () => {}, ctx);

		assert.strictEqual(state.active, parentSlug, "Parent must be active after child archiving");
		...
		assert.strictEqual(state.active, parentSlug, "Parent MUST remain active; child must NOT be resurrected");
		assert.strictEqual(state.pendingSubquestResume, null, "Obsolete pendingSubquestResume must be cleared");
		...
		assert.ok(resumes[0].includes(parentPath), "Resume must target parent quest");
	...
	// -----------------------------------------------------------------------
	await t.step("3. Stale pending target: pending resume for A does not resume A when active quest is B", async () => {
		const api = createMockExtensionAPI();
		plugin(api.mockPi as any);
		...
		const msgs = getAllMessages(api);
		const resumes = msgs.filter((m) => m.includes("Post-Compaction Autonomous Resumption Directive"));
		assert.strictEqual(resumes.length, 1, "Exactly one resume delivered on retry");
		assert.ok(resumes[0].includes(pathB), "Resume must target current active Quest B");
		assert.ok(!resumes[0].includes(pathA), "Resume must NOT target stale Quest A");
		assert.strictEqual(state.pendingResume, null, "pendingResume must be consumed");
		assert.strictEqual(state.active, questB, "state.active must remain Quest B");
	...
	// -----------------------------------------------------------------------
	await t.step("10. Transport outage & recovery: pendingResume survives and is drained on turn_end", async () => {
		const api = createMockExtensionAPI();
		plugin(api.mockPi as any);

		const ctx = createMockContext(10000, "session_compaction_transport_recovery");
		const commands: Record<string, any> = {};
		for (const cmd of api.registeredCommands) commands[cmd.name] = cmd;
		const tools: Record<string, any> = {};
		for (const tool of api.registeredTools) tools[tool.name] = tool;
		...
		const msgs = getAllMessages(api);
		assert.ok(msgs.some((m) => m.includes("Post-Compaction Autonomous Resumption Directive")));
	...
	// -----------------------------------------------------------------------
	await t.step("14. Subquest launch compaction preserves pendingSubquestResume on transport error", async () => {
		const api = createMockExtensionAPI();
		plugin(api.mockPi as any);

		const ctx = createMockContext(10000, "session_external_compaction_pending_subquest");
		const commands: Record<string, any> = {};
		for (const cmd of api.registeredCommands) commands[cmd.name] = cmd;
		const tools: Record<string, any> = {};
		for (const tool of api.registeredTools) tools[tool.name] = tool;
		...
		assert.strictEqual(state.pendingSubquestResume, childSlug, "pendingSubquestResume must NOT be silently deleted on subquest launch compaction");
		assert.ok(state.pendingResume, "pendingResume must be recorded for delivery retry");
		...
		assert.strictEqual(state.pendingResume, null, "pendingResume must be consumed after delivery");

		const msgs = getAllMessages(api);
		assert.ok(
			msgs.some((m) => m.includes(`Compaction finished after launching sub-quest \`${childSlug}\``)),
	...
	// -----------------------------------------------------------------------
	await t.step("16. Pending resume does not resurrect stale target when authoritative active quest is null", async () => {
		const api = createMockExtensionAPI();
		plugin(api.mockPi as any);
		...
		const pathA = questPath(getState(ctx as any).questId);
		await tools["quest_mark_saved"].execute("call_save", { name: slugA }, {}, () => {}, ctx);
		...
		assert.strictEqual(state.active, null, "state.active must remain null; pendingResume must not resurrect quest A");
	...
	// -----------------------------------------------------------------------
	await t.step("17. Pending resume obligation is preserved across subsequent compaction attempts until delivered and consumed", async () => {
		const api = createMockExtensionAPI();
		plugin(api.mockPi as any);

		const ctx = createMockContext(10000, "session_preserve_pending_resume_across_compactions");
		const commands: Record<string, any> = {};
		for (const cmd of api.registeredCommands) commands[cmd.name] = cmd;
		const tools: Record<string, any> = {};
		for (const tool of api.registeredTools) tools[tool.name] = tool;
		...
		assert.ok(state.pendingResume, "pendingResume must exist for transaction A");
		assert.strictEqual(state.pendingResume.compactionId, idA, "pendingResume must point to transaction A");
		assert.strictEqual(state.activeTransaction?.id, idA, "activeTransaction must be transaction A");
		assert.strictEqual(state.activeTransaction?.phase, "resume-pending", "activeTransaction must be in resume-pending phase");
		...
		assert.strictEqual(state.pendingResume.compactionId, idA, "pendingResume must still point to transaction A");
		assert.strictEqual(state.activeTransaction?.id, idA, "activeTransaction must still be transaction A");
		...
		assert.strictEqual(state.pendingResume, null, "pendingResume A must be consumed after successful delivery");
		assert.ok(
			!state.activeTransaction || (state.activeTransaction as any).phase === "resume-delivered",
		...
		await tools["quest_mark_saved"].execute("call_save_b", { name: slug }, {}, () => {}, ctx);
		...
		assert.strictEqual(txB.phase, "in-flight", "Transaction B must be in-flight");

		// Complete compaction B
		for (const cb of api.handlers["session_compact"] || []) {
			await cb({}, ctx);
		}
		assert.strictEqual(state.pendingResume, null, "Transaction B delivered immediately");
	...
	// -----------------------------------------------------------------------
	await t.step("18. Pending agent notifications and resume obligations are not starved by compactionPending", async () => {
		const api = createMockExtensionAPI();
		plugin(api.mockPi as any);
	...
	// -----------------------------------------------------------------------
	await t.step("19. Regression Test A & H: Pending resume reconstruction preserves original checkpoint hash/count/path without borrowing live state", async () => {
		const api = createMockExtensionAPI();
		plugin(api.mockPi as any);

		const ctx = createMockContext(10000, "session_pending_recon_immutability");
		const state = getState(ctx as any);
		const slug = "test-pending-recon-quest";
		state.active = slug;
		state.saveCount = 42;
		state.saveGeneration = { count: 42, hash: "current_live_hash_999", path: `.pi/quest/current/${slug}.md`, savedAt: Date.now() };
		...
		state.saveCount = 50;
		state.saveGeneration = { count: 50, hash: "new_mutated_live_hash", path: `.pi/quest/current/${slug}.md`, savedAt: Date.now() };

		const tx = createOrGetCompactionTransaction(state);
		assert.strictEqual(tx.checkpointSaveCount, 7, "Must preserve historical checkpointSaveCount (7), NOT live count (50)");
		assert.strictEqual(tx.checkpointHash, "historical_checkpoint_hash_111", "Must preserve historical checkpointHash, NOT live hash");
	...
	// -----------------------------------------------------------------------
	await t.step("20. Regression Test B: Current save/hash changing later does not rewrite pending resume checkpoint identity", async () => {
		const api = createMockExtensionAPI();
		plugin(api.mockPi as any);
		...
		state.saveGeneration = { count: 20, hash: "hash_mutated_20", path: `.pi/quest/current/${slug}.md`, savedAt: Date.now() };

		const snapshot = snapshotState(ctx as any);
		assert.strictEqual(snapshot.pendingResume?.checkpointSaveCount, 5, "Snapshot must preserve immutable checkpointSaveCount");
		assert.strictEqual(snapshot.pendingResume?.checkpointHash, "hash_b_555", "Snapshot must preserve immutable checkpointHash");
	});

	// -----------------------------------------------------------------------
	// 21. Pending child A + parent B active does NOT clear A; emits PENDING_RESUME_INCONSISTENT and blocks implementation
	// -----------------------------------------------------------------------
	await t.step("21. Regression Test C & E: Pending child A + parent B active does NOT clear A; emits PENDING_RESUME_INCONSISTENT and blocks implementation", async () => {
		const api = createMockExtensionAPI();
		plugin(api.mockPi as any);

		const ctx = createMockContext(10000, "session_pending_child_inconsistent");
		const commands: Record<string, any> = {};
		for (const cmd of api.registeredCommands) commands[cmd.name] = cmd;
		const tools: Record<string, any> = {};
		for (const tool of api.registeredTools) tools[tool.name] = tool;
		...
		assert.strictEqual(state.pendingSubquestResume, childSlug, "Must NOT silently delete pendingSubquestResume when child is not archived");
		...
		assert.strictEqual(canImplement(state, ctx as any), false, "Implementation must be blocked on inconsistent pending subquest resume");
		const blockReason = getImplementationBlockReason(state, ctx as any);
		assert.strictEqual(blockReason.code, QuestErrorCode.PENDING_RESUME_INCONSISTENT);
	...
	// -----------------------------------------------------------------------
	await t.step("22. Regression Test D: Pending child A + A demonstrably archived -> explicit obsolete resolution -> persisted resolution -> no resurrection", async () => {
		const api = createMockExtensionAPI();
		plugin(api.mockPi as any);
		...
		await commands["quest"].handler(parentSlug, ctx);
		const parentPath = questPath(getState(ctx as any).questId);
		await tools["quest_subquest"].execute("call_sub_d", { name: childSlug, switchNow: true }, {}, () => {}, ctx);

		const state = getState(ctx as any);
		state.pendingSubquestResume = childSlug;

		// Archive child subquest
		await tools["quest_archive"].execute("call_arch_d", { questName: childSlug, compact: false }, {}, () => {}, ctx);
		...
		assert.strictEqual(state.pendingSubquestResume, null, "pendingSubquestResume cleared after obsolete resolution");
		assert.ok(state.pendingSubquestResumeResolution, "Resolution must be persisted");
		assert.strictEqual(state.pendingSubquestResumeResolution.child, childSlug);
		assert.strictEqual(state.pendingSubquestResumeResolution.resolution, "obsolete-after-archive");
		assert.strictEqual(state.pendingSubquestResumeResolution.parent, parentSlug);

		// 2. Child is NOT resurrected; parent remains active
		assert.strictEqual(state.active, parentSlug, "Child must NOT be resurrected");
	...
	// -----------------------------------------------------------------------
	await t.step("26. Regression Test: pendingSubquestResume not consumed on missing child file without positive archive evidence", async () => {
		const api = createMockExtensionAPI();
		plugin(api.mockPi as any);
		...
		assert.strictEqual(state.pendingSubquestResume, childSlug, "Obligation must remain pending without archive evidence");
		assert.strictEqual(state.pendingSubquestResumeResolution, null, "Must NOT record obsolete-after-archive without evidence");

		// 2. Implementation gate remains blocked with PENDING_RESUME_INCONSISTENT
		assert.strictEqual(canImplement(state, ctx as any), false, "Implementation gate must be blocked");
		const reason = getImplementationBlockReason(state, ctx as any);
		assert.strictEqual(reason.blocked, true);
		assert.strictEqual(reason.code, QuestErrorCode.PENDING_RESUME_INCONSISTENT);
		...
		assert.strictEqual(state.pendingSubquestResume, null, "pendingSubquestResume cleared with positive archive evidence");
		assert.ok(state.pendingSubquestResumeResolution, "Resolution must be persisted");
		assert.strictEqual((state.pendingSubquestResumeResolution as any).child, childSlug);
		assert.strictEqual((state.pendingSubquestResumeResolution as any).resolution, "obsolete-after-archive");
