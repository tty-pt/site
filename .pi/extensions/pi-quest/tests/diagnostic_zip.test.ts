...
Deno.test("diagnostic_zip: project root and extension dir resolution", async (t) => {
  await t.step(
    "findProjectRoot resolves host project root from cwd and subdirectories",
    () => {
      ...
      const fromDeep = findProjectRoot(
        join(root, ".pi/extensions/pi-quest/src/nested"),
      );
...
Deno.test("diagnostic_zip: 12-point run-archive & diagnostic simplification requirements", async (t) => {
  ...
  try {
    ...
    await writeFile(
      join(archiveDir, "auth-crypto-helper-12345.md"),
      subContent,
      "utf8",
    );
    ...
    await writeFile(
      join(archiveDir, "unrelated-old-quest-999.md"),
      "# Unrelated\n",
      "utf8",
    );
    ...
    await writeFile(
      logPath,
      "2026-08-31T01:00:00.000Z | QUEST_CREATED | questId=" + questId +
        " root=user-auth-flow quest=user-auth-flow | user prompt\n" +
        "2026-08-31T01:01:00.000Z | GATE_BLOCKED | questId=" + questId +
        " root=user-auth-flow quest=user-auth-flow gate=RESEARCH_PENDING | blocked by gate\n" +
        "2026-08-31T01:02:00.000Z | RESEARCH_COMPLETED | questId=" + questId +
        " root=user-auth-flow quest=user-auth-flow | research done\n" +
        "2026-08-31T01:03:00.000Z | SUBQUEST_START | questId=" + questId +
        " root=user-auth-flow quest=auth-crypto-helper parent=user-auth-flow | sub start\n" +
        "2026-08-31T01:04:00.000Z | ARCHIVE | questId=" + questId +
        " root=user-auth-flow quest=auth-crypto-helper parent=user-auth-flow dest=.pi/quest/archive/auth-crypto-helper-12345.md | sub archived\n" +
        "2026-08-31T01:05:00.000Z | COMPACTION_COMPLETED | questId=" + questId +
        " root=user-auth-flow quest=user-auth-flow compactionId=cmp_1 | compacted\n" +
        "2026-08-31T01:06:00.000Z | RESUME_DELIVERED | questId=" + questId +
        " root=user-auth-flow quest=user-auth-flow compactionId=cmp_1 | resume delivered\n" +
        "2026-08-31T01:07:00.000Z | TEST_PASSED | questId=" + questId +
        " root=user-auth-flow quest=user-auth-flow | tests passed\n" +
        "2026-08-31T01:08:00.000Z | ARCHIVE | questId=" + questId +
        " root=user-auth-flow quest=user-auth-flow dest=.pi/quest/archive/user-auth-flow-99999.md | root archived\n",
      "utf8",
    );

    // Requirement 1 & 2: Completed root quest produces one run summary synthesized from quest state + log
    await t.step(
      "Req 1 & 2: completed root quest produces summary.md generated from final quest state + execution log",
      async () => {
        ...
        assert.ok(
          summary.includes("Implement user authentication flow with passkeys."),
        );
        ...
        assert.ok(
          summary.includes("Keep all UX isomorphic in mods/auth/ux/auth_ux.c"),
        );
        assert.ok(
          summary.includes("RESEARCH_PENDING"),
          "Summary must mention diagnostic gate events",
        );
        assert.ok(
          summary.includes("Compactions: 1"),
          "Summary must reflect compaction outcome",
        );
      },
    );
    ...
    await t.step(
      "Req 3, 4, 5, 6, 8: run archive includes initial-prompt + summary + manifest + execution.log, excludes source code",
      async () => {
        ...
        assert.ok(
          entries.includes("initial-prompt.txt") ||
            entries.includes("run/initial-prompt.txt"),
        );
        assert.ok(
          entries.includes("summary.md") || entries.includes("run/summary.md"),
        );
        assert.ok(
          entries.includes("execution.log") ||
            entries.includes("run/execution.log"),
        );

        // Source code and node_modules excluded from run archive
        assert.ok(
          !entries.some((e) =>
            e.startsWith("run/src/") || e.endsWith(".c") || e.endsWith(".ts")
          ),
        );
    ...
    await t.step(
      "Req 7: concurrent runs create isolated run directories and logs with short 7-10 char hashes",
      async () => {
        ...
        await writeFile(
          join(qDir1, "execution.log"),
          `2026-08-31T01:00:00.000Z | QUEST_CREATED | questId=${qid1} quest=quest1 | run 1\n`,
          "utf8",
        );
        await writeFile(
          join(qDir2, "execution.log"),
          `2026-08-31T01:00:00.000Z | QUEST_CREATED | questId=${qid2} quest=quest2 | run 2\n`,
          "utf8",
        );
    ...
    await t.step(
      "Req 10 & 11: one concise CHANGELOG line added for terminal root quest, none for subquest alone, truthful status prefix",
      async () => {
        ...
        assert.strictEqual(
          ...
          false,
          "Subquest alone must not create CHANGELOG.md",
        );
        ...
        assert.strictEqual(
          lines.length,
          1,
          "CHANGELOG must contain exactly one concise line",
        );
        assert.ok(lines[0].startsWith("- 2026-"));
        assert.ok(
          lines[0].includes(
            `Completed \`user-auth-flow\` [${questId}]: implemented passkey`,
          ),
        );
        ...
        assert.ok(
          lines[1].includes(
            "Terminal failure for `failed-auth-quest` [f1e2d3c400]: unresolved compilation errors",
          ),
        );
      },
    );
    ...
    await t.step(
      "Req 12: createUnifiedBundleZip produces one combined code + run-evidence ZIP",
      async () => {
...
Deno.test("diagnostic_zip: real project bundle packaging verification", async (t) => {
  ...
  await t.step(
    "CLI zip_bundle.ts runs cleanly and outputs verified status",
    async () => {
      ...
      assert.strictEqual(
        res.exitCode,
        0,
        `CLI run failed: ${res.stderr || res.stdout}`,
      );
...
Deno.test("diagnostic_zip: authoritative terminal status calculation and consistency", async (t) => {
  ...
  try {
    ...
    await t.step(
      "execution log with unresolved error results in FAILED in both manifest.txt and summary.md",
      async () => {
        ...
        assert.ok(
          summaryContent.includes("- **Final Status**: FAILED"),
          "summary.md must report FAILED",
        );
        assert.ok(
          manifestContent.includes("status: FAILED"),
          "manifest.txt must report FAILED",
        );
        assert.ok(
          !manifestContent.includes("status: COMPLETED"),
          "manifest.txt must NOT contradict summary",
        );
      },
    );
    ...
    await t.step(
      "clean execution log results in COMPLETED in both manifest.txt and summary.md",
      async () => {
        ...
        assert.ok(
          summaryContent.includes("- **Final Status**: COMPLETED"),
          "summary.md must report COMPLETED",
        );
        assert.ok(
          manifestContent.includes("status: COMPLETED"),
          "manifest.txt must report COMPLETED",
        );
      },
    );
  } finally {
...
Deno.test("diagnostic_zip: stable questId identity model (14 criteria)", async (t) => {
  ...
  try {
    ...
    await t.step(
      "1. New root quest gets one stable 7-10 character ID",
      async () => {
        const qPath = join(currentDir, "my-feature.md");
        const { QUEST_TEMPLATE, parseQuestId } = await import(
          "../src/markdown.ts"
        );
    ...
    await t.step("2. Reloading the quest preserves the same ID", async () => {
      const { loadExistingQuestEpistemicState } = await import(
        "../src/reconstruction.ts"
      );
      const loaded = await loadExistingQuestEpistemicState(
        "my-feature",
        currentDir,
      );
    ...
    await t.step("3. Context compaction preserves the same ID", async () => {
      const { createOrGetCompactionTransaction } = await import(
        "../src/compaction/transaction.ts"
      );
    ...
    await t.step(
      ...
      async () => {
        ...
        const freshLoaded = await loadExistingQuestEpistemicState(
          "my-feature",
          currentDir,
        );
    ...
    await t.step("5. Subquests use the parent's quest ID", async () => {
      ...
      const subTemplate = QUEST_TEMPLATE(
        "sub-feature-a",
        "Sub feature",
        "my-feature",
        "",
    ...
    await t.step(
      ...
      async () => {
        ...
        logEvent("QUEST_CREATED", "root A created", {
          questId: rootQuestId,
          root: "my-feature",
          logPath: logA,
        });
        logEvent("QUEST_CREATED", "root B created", {
        ...
        assert.ok(
          contentA.includes("root A created") &&
            !contentA.includes("root B created"),
        );
        assert.ok(
          contentB.includes("root B created") &&
            !contentB.includes("root A created"),
        );
      },
    );
    ...
    await t.step(
      "8. Archive filename/path contains the correct quest ID",
      async () => {
        archiveZipPath = join(
          tempRoot,
          ".pi/quest/archive",
    ...
    await t.step("12. Archived/completed quests retain their ID", async () => {
      ...
      await writeFile(
        dest,
        await readFile(join(currentDir, "my-feature.md"), "utf8"),
        "utf8",
      );
    ...
    await t.step(
      ...
      async () => {
        ...
        assert.ok(
          lastStatus.includes(rootQuestId),
          `UI status must show quest ID, got: ${lastStatus}`,
        );
      },
    );
    ...
    await t.step(
      ...
      async () => {
        ...
        assert.ok(
          lines[0].includes(
            `Completed \`my-feature\` [${rootQuestId}]: built my feature`,
          ),
        );
      },
    );
