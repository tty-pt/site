...
Deno.test("quest_journal_subquest_launch_compaction_resume: subquest launch, compaction, and child resumption lifecycle", async (t) => {
  ...
  // -----------------------------------------------------------------------
  await t.step(
    "1. Subquest launch with launch compaction consumes pendingSubquestResume and resumes child",
    async () => {
      const {
        mockPi,
        handlers,
      ...
      assert.strictEqual(
        preState.active,
        parentSlug,
        "Parent quest must be active initially",
      );
      ...
      assert.strictEqual(
        stateAfterLaunch.active,
        childSlug,
        "Child must be active immediately after launch",
      );
      // 5. Assert state.pendingSubquestResume === "child"
      assert.strictEqual(
        stateAfterLaunch.pendingSubquestResume,
        childSlug,
        "pendingSubquestResume must record the child slug",
      );
      // 6. Assert state.subquestLaunchCompactionPending === true
      assert.strictEqual(
        stateAfterLaunch.subquestLaunchCompactionPending,
        true,
        "subquestLaunchCompactionPending must be set",
      );
      ...
      assert.strictEqual(
        mockCtx.getCompactInvocationCount(),
        1,
        "Compaction must be scheduled via ctx.compact",
      );
      const compactOpts = mockCtx.getLastCompactOptions();
      assert.ok(
        compactOpts?.customInstructions?.includes(childSlug),
        "Compaction instructions must target the child",
      );
      ...
      assert.strictEqual(
        stateAfterCompact.pendingSubquestResume,
        null,
        "pendingSubquestResume must be consumed (null)",
      );
      assert.strictEqual(
        stateAfterCompact.subquestLaunchCompactionPending,
        false,
        "subquestLaunchCompactionPending must be cleared",
      );
      ...
      assert.strictEqual(
        userMessages.length,
        1,
        "Exactly one resume directive must be emitted upon compaction completion",
      );
      const resumeEntry = userMessages[0];
      const resumeText = typeof resumeEntry.msg === "string"
        ? resumeEntry.msg
        : (resumeEntry.msg?.text || "");

      // 11. Assert its target is "child" and contains required invariants
      assert.ok(
        resumeText.includes(`sub-quest \`${childSlug}\``) ||
          resumeText.includes(`sub-quest **${childSlug}**`),
        "Continuation must target child slug",
      );
      assert.ok(
        resumeText.includes(childPath),
        "Continuation must reference child quest file",
      );
      assert.ok(
        resumeText.includes("Independently verify inherited context"),
        "Must include inherited context verification invariant",
      );
      assert.ok(
        resumeText.includes("Perform the child's required research"),
        "Must include research requirement invariant",
      );
      assert.ok(
        resumeText.includes(
          "Once the child research gate is satisfied, continue autonomously",
        ),
        "Must include autonomous continuation invariant",
      );
      assert.ok(
        resumeText.includes(
          "Do not return to the parent until this sub-quest is actually complete",
        ),
        "Must include parent return invariant",
      );

      // 12. Assert deliverAs === "followUp"
      assert.strictEqual(
        resumeEntry.options?.deliverAs,
        "followUp",
        "Resumption directive must use deliverAs: 'followUp'",
      );

      // 13. Assert parent is NOT resumed at this point
      assert.ok(
        !resumeText.includes(
          `You are working on active quest **${parentSlug}**`,
        ),
        "Parent quest must not be resumed",
      );
  ...
  // -----------------------------------------------------------------------
  await t.step(
    "2. Subquest launch below compaction threshold sends immediate entry directive without pending state",
    async () => {
      const { mockPi, registeredTools, registeredCommands, userMessages } =
        createMockExtensionAPI();
      plugin(mockPi as any);
      ...
      assert.strictEqual(
        ...
        null,
        "pendingSubquestResume must remain null when no compaction is needed",
      );
      assert.strictEqual(
        state.subquestLaunchCompactionPending,
        false,
        "subquestLaunchCompactionPending must be false",
      );
      ...
      assert.strictEqual(
        userMessages.length,
        1,
        "Child entry directive must be sent immediately",
      );
      const entry = userMessages[0];
      const text = typeof entry.msg === "string"
        ? entry.msg
        : (entry.msg?.text || "");
      assert.ok(
        text.includes(`sub-quest **${childSlug}**`),
        "Entry directive must target child",
      );
      assert.strictEqual(
        entry.options?.deliverAs,
        "followUp",
        "Entry directive must use deliverAs: 'followUp'",
      );
  ...
  // -----------------------------------------------------------------------
  await t.step(
    "3. Compaction failure fallback immediately resumes the pending child",
    async () => {
      const {
        mockPi,
        handlers,
      ...
      assert.strictEqual(
        getState(mockCtx as any).pendingSubquestResume,
        childSlug,
      );
      ...
      assert.strictEqual(
        stateAfterFailed.pendingSubquestResume,
        null,
        "pendingSubquestResume must be consumed by fallback handler",
      );
      assert.ok(
        userMessages.length >= 1,
        "Fallback handler must dispatch messages",
      );
      const allTexts = userMessages.map((m) =>
        typeof m.msg === "string" ? m.msg : (m.msg?.text || "")
      ).join("\n\n");
      assert.ok(
        allTexts.includes("COMPACTION_FAILURE") ||
          allTexts.includes("Session context compaction failed"),
        "Agent must receive COMPACTION_FAILURE",
      );
      assert.ok(
        allTexts.includes(`sub-quest \`${childSlug}\``) ||
          allTexts.includes(`sub-quest **${childSlug}**`) ||
          allTexts.includes(childSlug),
        "Fallback continuation must target child",
      );
  ...
  // -----------------------------------------------------------------------
  await t.step(
    "4. Child remains gated by research after compaction until researchComplete is marked",
    async () => {
      const {
        mockPi,
        handlers,
      ...
      assert.strictEqual(
        canImplement(state),
        false,
        "Child implementation must remain blocked by research gate post-compaction",
      );
      assert.strictEqual(
        state.researchComplete,
        false,
        "researchComplete must be false initially",
      );
      ...
      for (const cb of handlers["tool_call"] || []) {
        await cb(
          { toolName: "read", input: { path: "mods/child/module.c" } },
          mockCtx,
        );
      }
      for (const cb of handlers["tool_result"] || []) {
        await cb({
          toolName: "read",
          input: { path: "mods/child/module.c" },
          output: "module code",
          isError: false,
        }, mockCtx);
      }
      ...
      assert.strictEqual(
        updatedState.researchComplete,
        true,
        "researchComplete must now be true",
      );
      // Child sub-quest is autonomous (no user confirmation required)
      assert.strictEqual(
        canImplement(updatedState),
        true,
        "Child implementation gate must now be open autonomously",
      );
