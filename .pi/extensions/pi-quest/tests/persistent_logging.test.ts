...
Deno.test("persistent_logging: formatting, parsing, safety, and run summarizer", async (t) => {
	...
	await t.step("formatLogEntry and parseLogEntry round-trip correctly", () => {
		...
		const line = formatLogEntry(
			...
			{ quest: "test-quest", round: 1, reads: 3, searches: 2, kind: "code-search" },
			ts,
		);
	...
	await t.step("lifecycle helpers write structured log entries across all 18 categories", () => {
		clearQuestLog(tempLogPath);

		logQuestTransition("QUEST_DETECTED", "substantive root prompt detected", { quest: "", reason: "new_prompt" });
		logQuestTransition("QUEST_CREATED", "provisional root quest created", { quest: "auth-flow" });
		logTurnBoundary("TURN_START", "turn started", { quest: "auth-flow" });
		logGateTransition("GATE_BLOCKED", "research pending", { quest: "auth-flow", gate: "RESEARCH_PENDING", reason: "research required" });
		logResearchTransition("RESEARCH_REQUIRED", "research required", { quest: "auth-flow", round: 1 });
		logResearchTransition("RESEARCH_EVIDENCE", "evidence observed", { quest: "auth-flow", kind: "code-search", reads: 2 });
		logResearchTransition("RESEARCH_COMPLETED", "research completed", { quest: "auth-flow", round: 1 });
		logGateTransition("GATE_OPENED", "gate opened", { quest: "auth-flow", to: "CONFIRMATION_PENDING" });
		logUserInteraction("CONFIRMATION_REQUESTED", "awaiting confirmation", { quest: "auth-flow" });
		logUserInteraction("CONFIRMATION_RECEIVED", "confirmation received", { quest: "auth-flow" });
		logGateTransition("GATE_OPENED", "implementation gate opened", { quest: "auth-flow", to: "IMPLEMENTATION_ALLOWED" });
		logImplementationOutcome("IMPLEMENTATION_ATTEMPT", "attempted edit src/auth.ts", { quest: "auth-flow", tool: "edit", path: "src/auth.ts" });
		logImplementationOutcome("IMPLEMENTATION_COMPLETED", "completed edit src/auth.ts", { quest: "auth-flow", tool: "edit", path: "src/auth.ts" });
		logVerificationTransition("TEST_STARTED", "running tests", { quest: "auth-flow", command: "make test" });
		logVerificationTransition("TEST_FAILED", "3 failed tests", { quest: "auth-flow", command: "make test", reason: "3 failed tests" });
		logReassessmentTransition("REASSESSMENT_REQUIRED", "test failure", { quest: "auth-flow", reason: "test failure", version: 1 });
		logReassessmentTransition("REASSESSMENT_EVIDENCE", "contradiction inspected", { quest: "auth-flow", kind: "file-read" });
		logReassessmentTransition("REASSESSMENT_COMPLETED", "reassessment resolved", { quest: "auth-flow", version: 1 });
		logStateUpdateTransition("STATE_UPDATE_ACCEPTED", "state update accepted", { quest: "auth-flow", planVersion: 2 });
		logPersistenceTransition("SAVE_STARTED", "save started", { quest: "auth-flow" });
		logPersistenceTransition("SAVE_VERIFIED", "save verified", { quest: "auth-flow", gen: 1, hash: "abc12345" });
		logCompactionTransition("COMPACTION_PREPARED", "compaction prepared", { quest: "auth-flow", compactionId: "cmp_1" });
		logCompactionTransition("COMPACTION_STARTED", "compaction started", { quest: "auth-flow", compactionId: "cmp_1" });
		logCompactionTransition("COMPACTION_COMPLETED", "compaction completed", { quest: "auth-flow", compactionId: "cmp_1" });
		logResumeTransition("RESUME_OBLIGATION_CREATED", "resume obligation created", { quest: "auth-flow", compactionId: "cmp_1" });
		logResumeTransition("RESUME_ATTEMPTED", "resume attempted", { quest: "auth-flow", compactionId: "cmp_1" });
		logResumeTransition("RESUME_DELIVERED", "resume delivered", { quest: "auth-flow", compactionId: "cmp_1" });
		logToolAnomaly("UNKNOWN_TOOL", "unknown tool invoked", { quest: "auth-flow", tool: "magic_tool" });
		logToolFailure("TOOL_TIMEOUT", "tool timed out", { quest: "auth-flow", tool: "bash", command: "sleep 100" });
		logContinuationAnomaly("REPEATED_BLOCK", "gate repeatedly blocked", { quest: "auth-flow", gate: "RESEARCH_PENDING", count: 3 });
		logSubquestTransition("SUBQUEST_START", "subquest started", { quest: "auth-flow", subquest: "auth-token", parent: "auth-flow" });
		logRecoveryTransition("STATE_INCONSISTENT", "state inconsistent detected", { quest: "auth-flow", code: "RESUME_STATE_INCONSISTENT" });
		logTurnBoundary("TURN_END", "turn ended", { quest: "auth-flow", substantive: true, toolsUsed: 4, mutations: 1, failures: 1 });
	...
	await t.step("logEvent does not throw on invalid paths or write errors and sets degraded flag", () => {
		resetQuestLoggingDegraded();
		assert.strictEqual(isQuestLoggingDegraded(), false);

		assert.doesNotThrow(() => {
			logEvent("ERROR", "test error without throwing", { code: "TEST_ERR", logPath: "/invalid-nonexistent-root-dir/protected/test.log" });
		});

		assert.strictEqual(isQuestLoggingDegraded(), true, "Logging failure must be detectable via isQuestLoggingDegraded()");
		resetQuestLoggingDegraded();
		assert.strictEqual(isQuestLoggingDegraded(), false);
	});

	await t.step("summarizeQuestJournalLog generates concise and accurate run metrics with canonical major phases", () => {
		const sampleLog = `
...
`.trim();
		...
		const expectedPhases = ["INITIALIZATION", "RESEARCH", "IMPLEMENTATION", "CONFIRMATION", "COMPACTION", "RESUME", "VERIFICATION", "REASSESSMENT", "RECOVERY", "COMPLETION"];
		for (const ep of expectedPhases) {
			assert.ok(summary.majorPhases.includes(ep), `majorPhases must include ${ep}`);
		}
		assert.ok(!summary.majorPhases.includes("RESEARCH_EVIDENCE"), "majorPhases must not contain raw event names");
		assert.ok(!summary.majorPhases.includes("TEST_FAILED"), "majorPhases must not contain raw event names");
		assert.strictEqual(summary.hasUnresolvedError, false);

		assert.ok(summary.formattedSummary.includes("Quests Tracked (1): persistent-agent"));
		assert.ok(summary.formattedSummary.includes("Research Rounds: 1"));
		assert.ok(summary.formattedSummary.includes("Implementation Attempts: 1 (allowed: 0, blocked: 1)"));
		assert.ok(summary.formattedSummary.includes("Compactions (1): Total 1 (successful: 1, failed: 0, inconsistent/external: 0)"));
		assert.ok(summary.formattedSummary.includes("Resumes (1): Total 1 (successful: 1, failed: 0, pending: 0)"));
		assert.ok(summary.formattedSummary.includes("Flow Warnings / Deadlocks (1)"));
	});

	await t.step("logical compaction and resume deduplication in summarizer", () => {
...
Deno.test("persistent_logging: end-to-end quest journal lifecycle emission verification", async (t) => {
	...
	await t.step("1. Agent start creates root quest and emits QUEST_DETECTED / QUEST_CREATED", async () => {
		for (const cb of handlers["before_agent_start"] || []) {
			await cb({ prompt: "Please implement the new caching layer for database records" }, mockCtx);
		}
	...
	await t.step("2. Implementation tool call blocked during research emits IMPLEMENTATION_BLOCKED", async () => {
		for (const cb of handlers["tool_call"] || []) {
			await cb({ toolName: "edit", input: { path: "src/cache.ts" } }, mockCtx);
		}
	...
	await t.step("3. Research update emits STATE_UPDATE_ACCEPTED and RESEARCH_COMPLETED", async () => {
		// Provide observed research
		for (const cb of handlers["tool_result"] || []) {
			await cb({ toolName: "read", input: { path: "src/cache.ts" }, content: "caching code" }, mockCtx);
		}
	...
	await t.step("5. Multi-failure preservation in analyzeTurnToolResults without full output dumps", () => {
		...
		assert.ok(analysis.failureEvidence.length < 2000, "Evidence must be bounded and not log full 10k output");
	});

	await t.step("6. Real error codes in logError and reportAgentError", () => {
		clearQuestLog(tempLogPath);

		logError("Explicit gate failure", undefined, mockCtx, QuestErrorCode.IMPLEMENTATION_BLOCKED, "corr_123");
		const log = readQuestLog(tempLogPath);
	...
	await t.step("7. Acceptance test: Simulated failed run tells complete story without transcript bloat", () => {
		clearQuestLog(tempLogPath);
		const corrId = "chain_fail_1";

		// 1. What was the agent trying to do?
		logImplementationOutcome("IMPLEMENTATION_ATTEMPT", "attempted edit src/critical.c", {
		...
		logGateTransition("GATE_BLOCKED", "gate blocked: REASSESSMENT_PENDING", {
			...
			requiredAction: "Investigate contradiction in quest file before editing code",
			correlationId: corrId,
		});

		// 3. What message did it send the agent?
		logAgentMessageTransition("AGENT_MESSAGE_ATTEMPTED", "agent message attempted (steer)", {
			quest: "failed-run-quest",
			deliverAs: "steer",
		...
		logAgentMessageTransition("AGENT_MESSAGE_DELIVERED", "agent message delivered (steer)", {
			quest: "failed-run-quest",
			deliverAs: "steer",
			type: "reassessment_required",
			correlationId: corrId,
		...
		logReassessmentTransition("REASSESSMENT_EVIDENCE", "investigating contradiction", {
			quest: "failed-run-quest",
			kind: "file-read",
			path: "src/critical.c",
			correlationId: corrId,
		...
		logReassessmentTransition("REASSESSMENT_COMPLETED", "reassessment resolved", {
			quest: "failed-run-quest",
			version: 2,
			correlationId: corrId,
		});
		...
		logStateUpdateTransition("STATE_UPDATE_ACCEPTED", "state update accepted", {
			quest: "failed-run-quest",
			planVersion: 3,
			correlationId: corrId,
		});
		...
		assert.strictEqual(lines.length, 8, "Raw log must be sparse and contain exactly the 8 lifecycle events");

		for (const line of lines) {
			assert.ok(line.includes(`correlationId=${corrId}`), `Line must correlate to ${corrId}: ${line}`);
		}

		const summary = summarizeQuestJournalLog(rawLog);
		assert.strictEqual(summary.implementationAttempts, 1);
		assert.strictEqual(summary.reassessmentCycles, 2);
		assert.deepStrictEqual(summary.majorPhases.sort(), ["IMPLEMENTATION", "PLANNING", "REASSESSMENT"].sort());
	});
