...
import {
  scheduleArchiveCompaction,
  scheduleEconomyCompaction,
  scheduleSubquestLaunchCompaction,
} from "../src/compaction/execution.ts";
...
Deno.test("lifecycle_consistency: authoritative completion, diagnostic archive ordering, and compaction transactional consistency", async (t) => {
  ...
  // -----------------------------------------------------------------------
  await t.step(
    "1. Completion: final state committed -> archive generated afterward -> changelog records authoritative outcome",
    async () => {
      const { api, ctx, commands, tools } = setupCompactionTestHarness(
        10000,
        "session_completion_ordering",
      );
...
`;
      ...
      assert.strictEqual(
        archiveRes.details?.error,
        undefined,
        `Archive must succeed: ${JSON.stringify(archiveRes)}`,
      );
      assert.strictEqual(
        state.active,
        null,
        "Active quest must be cleared after root completion",
      );
      ...
      assert.ok(
        existsSync(zipPath),
        `Run archive zip must exist at ${zipPath}`,
      );

      // Active directory should be removed
      const qDirResolved = resolve(projectRoot, QUEST_CURRENT_DIR, qid);
      assert.strictEqual(
        existsSync(qDirResolved),
        false,
        `Active quest directory ${qDirResolved} must be removed after archive packaging`,
      );
      ...
      assert.ok(
        zipEntries.includes("manifest.txt"),
        "Zip must contain manifest.txt",
      );
      assert.ok(
        zipEntries.includes("initial-prompt.txt"),
        "Zip must contain initial-prompt.txt",
      );
      assert.ok(
        zipEntries.includes("summary.md"),
        "Zip must contain summary.md",
      );
      assert.ok(
        zipEntries.includes("execution.log"),
        "Zip must contain execution.log",
      );
  ...
  // -----------------------------------------------------------------------
  await t.step(
    "2. Pre-compaction checkpoint -> compaction -> resume -> state reconstruction matches checkpoint",
    async () => {
      const { api, ctx, commands, tools } = setupCompactionTestHarness(
        10000,
        "session_compaction_exact_match",
      );
      ...
      assert.strictEqual(
        state.activeTransaction.checkpointSaveCount,
        initialSaveCount,
      );
  ...
  // -----------------------------------------------------------------------
  await t.step(
    "3. Completion/archive racing with pending compaction: no resurrection, no stale resume",
    async () => {
      const { api, ctx, commands, tools } = setupCompactionTestHarness(
        10000,
        "session_racing_compaction",
      );
...
`;
      ...
      assert.ok(
        ...
        "Transaction must be in-flight prior to race",
      );
      assert.strictEqual(state.activeTransaction.phase, "in-flight");
      ...
      assert.strictEqual(
        state.active,
        null,
        "state.active must be null after archive",
      );
      assert.strictEqual(
        state.activeTransaction,
        null,
        "activeTransaction must be null after archive",
      );
      assert.strictEqual(
        state.pendingResume,
        null,
        "pendingResume must be null after archive",
      );
      ...
      assert.strictEqual(
        state.active,
        null,
        "state.active MUST remain null after compaction callback",
      );
      assert.strictEqual(
        state.pendingResume,
        null,
        "No pending resume obligation may be created for completed quest",
      );
      ...
      assert.strictEqual(
        msgs.filter((m) =>
          m.includes("RESUME_STATE_INCONSISTENT") ||
          m.includes("RESUME_DELIVERY_FAILURE")
        ).length,
        0,
      ...
      assert.strictEqual(
        state.active,
        null,
        "state.active must still remain null after turn_end",
      );
  ...
  // -----------------------------------------------------------------------
  await t.step(
    "4. Terminal ordering: commit/verify state -> remove active quest -> diagnostic zip post-completion -> changelog -> completion",
    async () => {
      const tempArchiveBase = await mkdtemp(
        join(tmpdir(), "pi-quest-archive-"),
      );
      try {
        const { api, ctx, commands, tools } = setupCompactionTestHarness(
          10000,
          "session_terminal_ordering_proof",
        );
        ...
        try {
          await rm(questArchivePath(qid, projectRoot), { force: true });
        } catch {}
        try {
          await rm(questArchivePath(qid), { force: true });
        } catch {}
...
`;
        ...
        let archiveRes: any;
        try {
          archiveRes = await tools["quest_archive"].execute(
            "call_archive",
            { questName, compact: false },
            {},
            () => {},
        ...
        assert.strictEqual(
          state.active,
          null,
          "state.active must be null post-archival",
        );
        ...
        assert.strictEqual(
          stage1.details.activeDirExists,
          true,
          "Active quest dir must exist during terminal state commit",
        );
        assert.strictEqual(
          stage1.details.zipExists,
          false,
          "Diagnostic zip must not exist prior to removal",
        );
        assert.strictEqual(
          stage1.details.authoritativeTerminalStatus,
          "COMPLETED",
          "Authoritative status must be COMPLETED",
        );

        // 3. At stage 2 (active_removal): active quest dir MUST BE GONE, zip MUST NOT exist yet
        const stage2 = observedStages[1];
        assert.strictEqual(
          stage2.details.activeDirExists,
          false,
          "Active quest dir must be removed at active_removal stage",
        );
        assert.strictEqual(
          stage2.details.zipExists,
          false,
          "Diagnostic zip must not exist at active_removal stage",
        );

        // 4. At stage 3 (zip_creation): active quest dir MUST ALREADY BE ABSENT, zip MUST now exist
        const stage3 = observedStages[2];
        assert.strictEqual(
          stage3.details.activeDirExists,
          false,
          "Active quest dir must already be absent when zip is created",
        );
        assert.strictEqual(
          stage3.details.zipExists,
          true,
          "Diagnostic zip must exist after zip_creation stage",
        );

        // 5. At stage 4 (changelog_appended): zip MUST already exist, changelog recorded
        const stage4 = observedStages[3];
        assert.strictEqual(
          stage4.details.zipExists,
          true,
          "Diagnostic zip must exist when changelog is appended",
        );
        assert.strictEqual(
          stage4.details.activeDirExists,
          false,
          "Active quest dir must remain absent",
        );
        ...
        assert.strictEqual(
          existsSync(qDirResolved),
          false,
          `Active quest directory ${qDirResolved} must be removed`,
        );
        ...
        try {
          await rm(tempArchiveBase, { recursive: true, force: true });
        } catch {}
      }
    },
  );
  ...
  // -----------------------------------------------------------------------
  await t.step(
    "5. Timing-sensitive race: scheduled callback for Tx A cannot mutate Transaction B",
    async () => {
      const { api, ctx, commands, tools } = setupCompactionTestHarness(
        10000,
        "session_async_timing_race",
      );
      ...
      const txA = createOrGetCompactionTransaction(
        stateA,
        "normal-compaction",
        questA,
      );
      ...
      const txB = createOrGetCompactionTransaction(
        stateB,
        "normal-compaction",
        questB,
      );
      ...
      assert.notStrictEqual(
        idA,
        idB,
        "Transaction B must have distinct ID from A",
      );
      ...
      if (compactErrorCallbackA) {
        ...
        compactErrorCallbackA(
          new Error("Compaction failed for A in background"),
        );
      }
      ...
      assert.strictEqual(
        stateB.activeTransaction?.id,
        idB,
        "Active transaction must remain B's ID",
      );
      assert.strictEqual(
        stateB.activeTransaction?.phase,
        "in-flight",
        "Transaction B must remain in-flight (not failed by A)",
      );
      assert.strictEqual(
        stateB.compactionPending,
        true,
        "compactionPending for B must not be cleared by A",
      );
      assert.strictEqual(
        stateB.activeTransaction?.error,
        undefined,
        "Transaction B must not have error from A",
      );
  ...
  // -----------------------------------------------------------------------
  await t.step(
    "6. Timing-sensitive race: scheduled callback for Tx A with cleared activeTransaction performs no state mutation",
    async () => {
      const { api, ctx, commands, tools } = setupCompactionTestHarness(
        10000,
        "session_async_cleared_tx",
      );
      ...
      assert.strictEqual(
        stateA.activeTransaction,
        null,
        "activeTransaction must remain null",
      );
      assert.strictEqual(
        stateA.compactionPending,
        false,
        "compactionPending must remain false",
      );
      assert.strictEqual(
        stateA.activeCompactionId,
        null,
        "activeCompactionId must remain null",
      );
  ...
  // -----------------------------------------------------------------------
  await t.step(
    "7. Late log events after active removal go to finalized log, not a recreated current/<qid>/execution.log",
    async () => {
      const { api, ctx, commands, tools } = setupCompactionTestHarness(
        10000,
        "session_late_logging",
      );
...
`;
      ...
      assert.strictEqual(
        existsSync(currentDirPath),
        false,
        "Active quest directory must be removed after archival",
      );
      ...
      logEvent(
        "TOOL_ACTIVITY",
        "Late callback execution event after active removal",
        {
          questId: qid,
          quest: questName,
          operation: "success",
      ...
      assert.strictEqual(
        existsSync(currentDirPath),
        false,
        "Active quest directory must NOT be resurrected by late log events",
      );
      assert.strictEqual(
        existsSync(`${currentDirPath}/execution.log`),
        false,
        "current/<qid>/execution.log must NOT exist",
      );

      // Pinned log must exist in archive and contain the late events (3-dir invariant: no finalized_logs)
      const projectRoot = findProjectRoot(ctx?.cwd);
      const finalizedLogPath = resolve(
        projectRoot,
        ".pi/quest/archive",
        `${qid}.log`,
      );
      assert.ok(
        existsSync(finalizedLogPath),
        `Pinned log must exist at ${finalizedLogPath}`,
      );

      const finalizedContent = await readFile(finalizedLogPath, "utf8");
      assert.ok(
        finalizedContent.includes(
          "Late callback execution event after active removal",
        ),
        "Pinned log must contain late events",
      );
      assert.ok(
        finalizedContent.includes(
          "Late turn boundary event after active removal",
        ),
        "Pinned log must contain late turn events",
      );
