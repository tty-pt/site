...
function createMockContext(cwd: string, sessionId = `session_${Date.now().toString(36)}`): ExtensionContext {
	const branch: any[] = [];
	return {
		cwd,
		mode: "agent",
...
Deno.test("Subagent Working Dir Suite: subagent cwd is anchored to project root (#58)", async (t) => {
	...
	await t.step("1. resolveSubagentCwd returns project root for a stray extension-dir cwd", () => {
		const stray = extensionDirInside(projectRoot);
		const cwd = resolveSubagentCwd(createMockContext(stray) as any);
		assert.strictEqual(cwd, projectRoot, `expected ${projectRoot}, got ${cwd}`);
		assert.ok(!cwd.includes("/.pi/extensions/"), "must not resolve into the extension dir");
	});

	await t.step("2. resolveSubagentCwd returns project root for a normal repo cwd", () => {
		const ctx = createMockContext(projectRoot);
		const cwd = resolveSubagentCwd(ctx as any);
		assert.strictEqual(cwd, projectRoot);
	});

	await t.step("3. subagent request emits cwd anchored to project root even from stray cwd", async () => {
		const { mockPi, bus } = createMockExtensionAPI(["subagent"]);
		const stray = extensionDirInside(projectRoot);
		const executor = resolveSubagentExecutor(mockPi as any, createMockContext(stray) as any);
		assert.ok(executor, "subagent executor must be resolved when subagent tool registered");
		...
		assert.ok(!emittedCwd.includes("/.pi/extensions/"), "must not emit extension dir as cwd");
	});

	await t.step("4. SUBAGENT_CWD_REANCHORED is logged when cwd was misdirected, not when normal", async () => {
		...
		assert.ok(strayLog.includes("SUBAGENT_CWD_REANCHORED"), "re-anchor diagnostic must be logged for stray cwd");
		...
		assert.ok(!normalLog.includes("SUBAGENT_CWD_REANCHORED"), "no re-anchor diagnostic for normal cwd");
	});
