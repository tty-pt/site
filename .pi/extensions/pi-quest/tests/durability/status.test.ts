...
Deno.test("steady status renders dim with a permanent hint", () => {
  ...
  try {
    refreshStatus(ctx);
    check(calls.length === 1 && calls[0] === dimFrame, "dim frame with permanent [F2]");
...
Deno.test("draft update flashes bright then relaxes to dim", async () => {
  ...
  try {
    ...
    check(calls.every((text) => (text ?? "").includes("[F2]")), "hint never disappears");
  } finally {
    replaceState(IDLE_STATE);
  }
});
