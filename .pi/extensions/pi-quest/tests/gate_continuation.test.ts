...
`,
		...
		for (const cb of handlers["tool_call"] || []) {
			await cb({ toolName: "read", input: { path: "mods/core/socket.c" } }, mockCtx);
		}
		for (const cb of handlers["tool_result"] || []) {
			await cb({ toolName: "read", input: { path: "mods/core/socket.c" }, output: "socket code", isError: false }, mockCtx);
		}
