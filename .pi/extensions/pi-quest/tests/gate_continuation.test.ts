...
import plugin, { canImplement, type StoredState } from "../index.ts";
...
Deno.test("quest_journal_gate_continuation: gate opening continuation steering lifecycle", async (t) => {
  const qid = "reassessment-gate-qid";
  const questDirPath = `.pi/quest/current/${qid}`;
  await mkdir(questDirPath, { recursive: true });

  // -----------------------------------------------------------------------
  // 1. Reassessment resolution triggers continuation steer
  // -----------------------------------------------------------------------
  await t.step(
    "1. Reassessment resolution triggers continuation steer to resume execution",
    async () => {
      const {
        mockPi,
        handlers,
      ...
      const questFilePath = `${questDirPath}/quest.md`;
...
`,
      ...
      for (const cb of handlers["tool_call"] || []) {
        toolCallResult = await cb({
          toolName: "edit",
          input: { path: "mods/core/socket.c" },
        }, mockCtx);
      }

      assert.ok(toolCallResult, "Tool call must be intercepted by gate");
      assert.strictEqual(
        toolCallResult.block,
        true,
        "Mutation tool must be blocked while reassessment pending",
      );
      assert.ok(
        toolCallResult.reason.includes("REASSESSMENT_PENDING"),
        "Block reason must cite REASSESSMENT_PENDING",
      );
      assert.ok(
        toolCallResult.reason.includes("quest_update_state"),
        "Block reason must instruct resolving via quest_update_state",
      );

      userMessages.length = 0;

      // Perform investigation tool call & result
      for (const cb of handlers["tool_call"] || []) {
        await cb(
          { toolName: "read", input: { path: "mods/core/socket.c" } },
          mockCtx,
        );
      }
      ...
      const res: any = await updateTool.execute(
        "call_resolve_1",
        {
          ...
          reassessmentConclusion:
            "Investigation showed O_NONBLOCK requires explicit epoll ET edge handling. Plan revised to add retry loop.",
          understanding:
            "O_NONBLOCK requires ET edge-triggered polling with retry loop on EAGAIN.",
          assumptions: [
            "ET edge-triggered polling handles EAGAIN without busy waiting.",
          ],
          openQuestions: ["No further socket blockers."],
          findings: [
            "Socket socket_write verified to return EAGAIN when buffer fills.",
          ],
          plan: [
            "1. Implement epoll ET loop in socket.c",
            "2. Run socket unit test suite",
          ],
          planConfidence: "high",
          planConfidenceReason:
            "Verified against Linux epoll manpage and socket unit test.",
          ...
          remaining: [
            "Implement epoll ET loop in socket.c",
            "Run socket unit test suite",
          ],
          exactNextAction:
            "Implement epoll ET loop in socket.c and rerun socket unit tests",
        },
        undefined,
        undefined,
        mockCtx,
      ...
      assert.strictEqual(
        res.details.reassessmentRequired,
        false,
        "Reassessment must be marked resolved",
      );

      // Verify that exactly ONE steer message was emitted informing the model that the gate is open
      const steerMessages = userMessages.filter((m) =>
        m.options?.deliverAs === "steer"
      );
      assert.strictEqual(
        steerMessages.length,
        1,
        "Exactly one steer continuation must be emitted on reassessment resolution",
      );
      const steerMsg = steerMessages[0].msg;
      assert.ok(
        steerMsg.includes("REASSESSMENT RESOLVED — IMPLEMENTATION GATE OPEN"),
        "Must identify reassessment resolved header",
      );
      assert.ok(
        steerMsg.includes("Your implementation gate is now OPEN"),
        "Must state implementation gate is open",
      );
      assert.ok(
        steerMsg.includes("EXACT NEXT ACTION"),
        "Must instruct model to follow current Exact Next Action",
      );

      // Now, subsequent edit tool call must be ALLOWED (not blocked)
      let nextToolCallResult: any;
      for (const cb of handlers["tool_call"] || []) {
        nextToolCallResult = await cb({
          toolName: "edit",
          input: { path: "mods/core/socket.c" },
        }, mockCtx);
      }
      assert.strictEqual(
        nextToolCallResult,
        undefined,
        "Edit must be permitted after gate opens",
      );
    },
  );
  ...
  // -----------------------------------------------------------------------
  await t.step(
    "2. Deduplication: repeated updates when gate is already open do not emit duplicate steer",
    async () => {
      const { mockPi, registeredTools, userMessages } =
        createMockExtensionAPI();
      plugin(mockPi as any);
      const mockCtx = createMockContext();
      ...
      const duplicateSteers = userMessages.filter((m) =>
        m.options?.deliverAs === "steer" && m.msg.includes("GATE OPEN")
      );
      assert.strictEqual(
        duplicateSteers.length,
        0,
        "No duplicate continuation steer when gate was already open",
      );
    },
  );
  ...
  // -----------------------------------------------------------------------
  await t.step(
    "3. Root confirmation accepted triggers confirmation continuation steer",
    async () => {
      const {
        mockPi,
        handlers,
      ...
      const confirmSteers = userMessages.filter((m) =>
        m.options?.deliverAs === "steer" &&
        m.msg.includes("CONFIRMATION ACCEPTED")
      );
      assert.strictEqual(
        confirmSteers.length,
        1,
        "Confirmation acceptance must emit confirmation continuation steer",
      );
      assert.ok(
        confirmSteers[0].msg.includes("Your implementation gate is now OPEN"),
      );
    },
  );

  // Clean up
  await rm(`.pi/quest/current/${qid}`, { recursive: true, force: true });
});
