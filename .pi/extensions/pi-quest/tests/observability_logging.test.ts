...
Deno.test("observability_logging: path normalization, quoted context parsing, and phase classification", async (t) => {
  await t.step(
    "1. normalizeLogPath cleans paths consistently without altering project semantics",
    () => {
      const cwd = "/home/user/project";
      assert.strictEqual(
        normalizeLogPath("./src/router.ts", cwd),
        "src/router.ts",
      );
      assert.strictEqual(
        normalizeLogPath("/home/user/project/src/router.ts", cwd),
        "src/router.ts",
      );
      assert.strictEqual(
        normalizeLogPath(
          "/home/user/project/.pi/quest/current/qid123/quest.md",
          cwd,
        ),
        ".pi/quest/current/qid123/quest.md",
      );
      assert.strictEqual(
        normalizeLogPath("tests/unit/test.ts", cwd),
        "tests/unit/test.ts",
      );
      assert.strictEqual(
        normalizeLogPath('"src/state.ts"', cwd),
        "src/state.ts",
      );
  ...
  await t.step(
    "2. formatContextFields quotes values with spaces and parseLogEntry round-trips them",
    () => {
      ...
      assert.ok(
        formatted.includes('reason="assertion failed: expected 2 got 1"'),
      );
  ...
  await t.step(
    "3. classifyActivityPhase correctly categorizes actions into major lifecycle phases",
    () => {
      // Research phase during active research
      const researchState: any = {
        researchRequired: true,
        researchComplete: false,
      };
      assert.strictEqual(
        classifyActivityPhase("read", { path: "src/foo.ts" }, researchState),
        "research",
      );
      assert.strictEqual(
      ...
      assert.strictEqual(
        classifyActivityPhase("web_search", { query: "docs" }, researchState),
        "research",
      );
      assert.strictEqual(
        classifyActivityPhase("quest_update_state", {}, researchState),
        "planning",
      );
      assert.strictEqual(
        classifyActivityPhase("quest_mark_saved", {}, researchState),
        "checkpoint",
      );

      // Verification phase for test/build commands
      assert.strictEqual(
        classifyActivityPhase("bash", { command: "make test" }, researchState),
        "verification",
      );
      assert.strictEqual(
        classifyActivityPhase("bash", {
          command: "deno test tests/bar.test.ts",
        }, researchState),
        "verification",
      );
      assert.strictEqual(
      ...
      assert.strictEqual(
        classifyActivityPhase("edit", { path: "src/foo.ts" }, implState),
        "implementation",
      );
      assert.strictEqual(
        classifyActivityPhase("write", { path: "src/new.ts" }, implState),
        "implementation",
      );
      assert.strictEqual(
        classifyActivityPhase("subagent", { task: "fix bug" }, implState),
        "implementation",
      );
      // Reads during implementation mode do not guess 'implementation'; phase is omitted (undefined)
      assert.strictEqual(
        classifyActivityPhase("read", { path: "src/foo.ts" }, implState),
        undefined,
      );
      ...
      assert.strictEqual(
        classifyActivityPhase("read", { path: "src/foo.ts" }, reassessState),
        "reassessment",
      );
      assert.strictEqual(
        classifyActivityPhase(
      ...
      assert.strictEqual(
        classifyActivityPhase("bash", { command: "make test" }, reassessState),
        "verification",
      );
    },
  );
...
Deno.test("observability_logging: tool lifecycle events and causal chain verification", async (t) => {
  ...
  await t.step(
    "1. read result logs tool, normalized path, and phase",
    async () => {
      ...
      assert.ok(
        !log.includes("router code content"),
        "Tool output content must NOT be dumped in the log",
      );
    },
  );

  await t.step(
    "2. search_graph logs tool, search target/query, and phase",
    async () => {
      for (const cb of handlers["tool_result"] || []) {
        await cb(
          {
      ...
      assert.ok(log.includes("phase=research"));
      assert.ok(
        log.includes("query=.*OrderHandler.*") ||
          log.includes('query=".*OrderHandler.*"'),
      );
    },
  );
  ...
  await t.step(
    "3. Blocked implementation attempt logs GATE_BLOCKED, IMPLEMENTATION_BLOCKED, and blocked TOOL_ACTIVITY",
    async () => {
  ...
  await t.step(
    "4. After research completion and confirmation, edit/write logs successful implementation activity",
    async () => {
  ...
  await t.step(
    "5. Failed verification command produces full causal sequence (TOOL_ACTIVITY -> TEST_FAILED -> REASSESSMENT_REQUIRED)",
    async () => {
      ...
      const toolActivityIdx = lines.findIndex((l) =>
        l.includes("TOOL_ACTIVITY") && l.includes('command="make test"') &&
        l.includes("operation=failure")
      );
      const testFailedIdx = lines.findIndex((l) => l.includes("TEST_FAILED"));
      const reassessIdx = lines.findIndex((l) =>
        l.includes("REASSESSMENT_REQUIRED")
      );

      assert.ok(
        toolActivityIdx >= 0,
        "TOOL_ACTIVITY for failed bash command must exist",
      );
      assert.ok(
        testFailedIdx > toolActivityIdx,
        "TEST_FAILED must follow failed TOOL_ACTIVITY",
      );
      assert.ok(
        reassessIdx > testFailedIdx,
        "REASSESSMENT_REQUIRED must follow TEST_FAILED",
      );
    },
  );

  await t.step(
    "6. TURN_END logs accurate breakdown counts (reads, searches, writes, commands, mutations, failures)",
    async () => {
      const toolResults = [
      ...
      assert.ok(
        lastTurnEnd.includes("reads=2"),
        `TURN_END must include reads=2: ${lastTurnEnd}`,
      );
      assert.ok(
        lastTurnEnd.includes("searches=1"),
        `TURN_END must include searches=1: ${lastTurnEnd}`,
      );
      assert.ok(
        lastTurnEnd.includes("writes=2"),
        `TURN_END must include writes=2: ${lastTurnEnd}`,
      );
      assert.ok(
        lastTurnEnd.includes("commands=1"),
        `TURN_END must include commands=1: ${lastTurnEnd}`,
      );
      assert.ok(
        lastTurnEnd.includes("failures=1"),
        `TURN_END must include failures=1: ${lastTurnEnd}`,
      );
    },
  );

  await t.step(
    "7. Integration scenario: read -> read -> edit -> test failure is completely reconstructable",
    async () => {
      clearQuestLog(tempLogPath);
      ...
      for (const cb of handlers["turn_end"] || []) {
        await cb({
          toolResults: [{ toolName: "read" }, { toolName: "read" }, {
            toolName: "edit",
          }, testFail],
        }, mockCtx);
      }
      ...
      assert.ok(
        ...
        `failureCount should be at least 1, got ${summary.failureCount}`,
      );
      ...
      assert.strictEqual(
        ...
        4,
        "Must have exactly 4 TOOL_ACTIVITY entries in order",
      );
      ...
      const reassessEvents = entries.filter((e) =>
        e.type === "REASSESSMENT_REQUIRED"
      );
      assert.strictEqual(
        reassessEvents.length,
        1,
        "Must have exactly 1 REASSESSMENT_REQUIRED entry (no duplicates)",
      );
    },
  );

  await t.step(
    "8. Full end-to-end causal trace: investigation -> changes -> verification -> failure -> reassessment -> renewed investigation -> subsequent fix -> passing test",
    async () => {
      clearQuestLog(tempLogPath);
      ...
      for (const cb of handlers["turn_end"] || []) {
        await cb({
          toolResults: [{ toolName: "read" }, { toolName: "search_graph" }],
        }, mockCtx);
      }
      ...
      assert.ok(
        types.includes("REASSESSMENT_REQUIRED"),
        "Must include REASSESSMENT_REQUIRED",
      );
      ...
      assert.strictEqual(
        activities.length,
        7,
        "Must have exactly 7 tool activities across 4 turns",
      );

      // Check turn correlation and phase labels
      const t1Read = activities.find((a) =>
        a.context.path === "src/order.ts" && a.context.turn === "1"
      );
      assert.ok(t1Read, "Turn 1 read must exist");
      assert.strictEqual(t1Read.context.phase, "research");

      const t2Edit = activities.find((a) =>
        a.context.tool === "edit" && a.context.turn === "2"
      );
      assert.ok(t2Edit, "Turn 2 edit must exist");
      assert.strictEqual(t2Edit.context.phase, "implementation");

      const t2Fail = activities.find((a) =>
        a.context.tool === "bash" && a.context.turn === "2"
      );
      ...
      const t3Read = activities.find((a) =>
        a.context.path === "tests/order_test.c" && a.context.turn === "3"
      );
      assert.ok(t3Read, "Turn 3 read must exist");
      assert.strictEqual(t3Read.context.phase, "reassessment");

      const t4Edit = activities.find((a) =>
        a.context.tool === "edit" && a.context.turn === "4"
      );
      assert.ok(t4Edit, "Turn 4 edit must exist");
      assert.strictEqual(t4Edit.context.phase, "implementation");

      const t4Pass = activities.find((a) =>
        a.context.tool === "bash" && a.context.turn === "4"
      );
