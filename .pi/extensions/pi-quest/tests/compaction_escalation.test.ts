...
Deno.test("quest_journal_compaction_escalation: periodic checkpoint lifecycle", async (t) => {
	...
	const mockCtx: any = {
		...
		compact: (opts?: any) => { compactCallCount++; if (typeof opts?.onComplete === "function") opts.onComplete(); },
		ui: { notify: () => {}, setStatus: () => {} },
		hasUI: true, mode: "headless",
	};
	...
	const emitTurnEnd = async (toolResults: any[] = [{ toolName: "edit", input: { path: "mods/song/song.c" } }]) => {
		...
		for (const cb of handlers["turn_end"] || []) await cb({ toolResults }, mockCtx);
	};
	...
	await t.step("1. Below 6 turns: no periodic steering", async () => {
		...
		for (let i = 0; i < 5; i++) {
			...
			await emitTurnEnd([{ toolName: "edit", args: { path: `mods/song/${i}.c` } }]);
		}
	...
	await t.step("3. After save, need another 6 turns", async () => {
		...
		userMessages.length = 0;
		for (let i = 0; i < 5; i++) {
			for (const cb of handlers["tool_result"] || []) await cb({ toolName: "edit", input: { path: `mods/song/again${i}.c` } }, mockCtx);
			await emitTurnEnd([{ toolName: "edit", args: { path: `mods/song/again${i}.c` } }]);
		}
		assert.strictEqual(userMessages.filter((m) => m.msg?.includes("Periodic")).length, 0);
		for (const cb of handlers["tool_result"] || []) await cb({ toolName: "edit", input: { path: "mods/song/again5.c" } }, mockCtx);
		await emitTurnEnd([{ toolName: "edit", args: { path: "mods/song/again5.c" } }]);
		assert.ok(userMessages.some((m) => m.msg?.includes("Periodic Durable Checkpoint")));
	});
	...
	await t.step("4. Dirty blocks compaction", async () => {
		...
		userMessages.length = 0;
		let res: any;
		for (const cb of handlers["session_before_compact"] || []) res = await cb({}, mockCtx);
		assert.strictEqual(res?.cancel, true);
		assert.ok(userMessages[0].msg.includes("Compaction Blocked"));
	});

	await t.step("5. After save, compaction allowed and post-compaction resumes", async () => {
		...
		assert.ok(userMessages.some((m) => (typeof m.msg === "string" ? m.msg : "").includes("Post-Compaction")));
	});
