...
import {
  ...
  getGlobalReviewLockKey,
  getGlobalReviewLockKeyForQuest,
  getQuestLockKey,
  getReviewLockKey,
  getReviewLockPath,
  isReviewActive,
...
export function isDraftReviewValid(targetState?: StoredState): boolean {
  ...
  try {
    ...
    if (approval.boundaryKey) {
      return (
        approval.boundaryKey === expectedKey ||
...
export function isCriticalReviewValidForCompletion(
  ...
  if (
    currentHash &&
    rev.reviewedStateVersion.saveHash &&
    rev.reviewedStateVersion.saveHash !== currentHash
  ) {
    return false;
  }
...
export async function runCriticalReview(
  ...
  if (!reviewer.isAvailable()) {
    logCriticalReviewTransition(
      ...
      "critical review unavailable: subagent tool not executable",
      {
        quest: slug,
        questId,
        sessionId,
  ...
  if (!options.force && reviewPromiseByKey.has(singleKey)) {
    ...
    logCriticalReviewTransition(
      ...
      }, cap=${GLOBAL_REVIEW_CAP})`,
      {
        ...
        activeCount: [...getActiveReviews().values()].filter(
    ...
    return {
      ...
      inProgress: true,
      skipped: true,
      reviewPromise: existingPromise,
      error: "global_review_cap_single_flight",
    };
  }
  ...
  if (!options.force && isReviewActive(questId)) {
    ...
    logCriticalReviewTransition(
      ...
      `review coalesced: pending ${slug}:${options.kind} (active file)`,
      {
        quest: slug,
        questId,
        sessionId,
    ...
    return {
      ...
      inProgress: true,
      skipped: true,
      error: "global_review_cap_active_file",
    };
  }
  ...
  const lockResult: { blocked?: boolean; limited?: boolean; response?: any } =
    {};
      ...
      await withQuestLock(
        ...
        async () => {
          ...
          if (!fsLock.acquired) {
            ...
            logCriticalReviewTransition(
              "CRITICAL_REVIEW_COALESCED",
              `review coalesced: pending ${slug}:${options.kind} (filesystem lock)`,
              {
                quest: slug,
                questId,
                sessionId,
            ...
            Object.assign(lockResult, {
              ...
              response: {
                ...
                inProgress: true,
                skipped: true,
                error: "global_review_cap_filesystem",
              },
            });
            return;
          }
          ...
          try {
            ...
            if (isReviewActive(questId)) {
              ...
              logCriticalReviewTransition(
                "CRITICAL_REVIEW_COALESCED",
                `review coalesced: pending ${slug}:${options.kind} (active file inside lock)`,
                {
                  quest: slug,
                  questId,
                  sessionId,
              ...
              Object.assign(lockResult, {
                ...
                response: {
                  ...
                  inProgress: true,
                  skipped: true,
                  error: "global_review_cap_active_file_inside",
                },
              });
              return;
            }
            // Global burst cap check inside hierarchical lock (global → per-quest)
            const globalActive = [...getActiveReviews().values()].filter(
              (r) => r.status === "starting" || r.status === "running",
            ).length;
            if (globalActive >= GLOBAL_REVIEW_CAP) {
              ...
              logCriticalReviewTransition(
                "GLOBAL_REVIEW_CAP_HIT",
                `global review cap hit (active=${globalActive}, cap=${GLOBAL_REVIEW_CAP})`,
                {
                  quest: slug,
                  questId,
                  sessionId,
              ...
              Object.assign(lockResult, {
                ...
                response: {
                  ...
                  inProgress: true,
                  skipped: true,
                  error: "global_review_cap",
                },
              });
              return;
            }

            const launchGuard = checkLaunchGuard(
              slug,
              options.kind,
              targetState,
              options,
            ...
            if (launchGuard.blocked) {
              ...
              logCriticalReviewTransition(
                "CRITICAL_REVIEW_COALESCED",
                `review coalesced: pending ${slug}:${options.kind}`,
                {
                  quest: slug,
                  questId,
                  sessionId,
              ...
              Object.assign(lockResult, {
                blocked: true,
                response: launchGuard.response,
              });
              return;
            }
            const attemptCheck = checkAttemptLimit(
              slug,
              options.kind,
              targetState,
              options,
            ...
            if (attemptCheck.limited) {
              Object.assign(lockResult, {
                limited: true,
                response: attemptCheck.response,
              });
              return;
            }
            if (options.force) {
              logCriticalReviewTransition(
                ...
                `critical review forced (correlationId=${correlationId})`,
                {
                  quest: slug,
                  questId,
                  sessionId,
            ...
            provisionalSnapshot = {
              ...
              stateHash: currentHash,
              originalUserRequest:
                targetState.prompts && targetState.prompts.length > 0
                  ? targetState.prompts[0]
                  : "",
            ...
            executionPromise = new Promise<CriticalReviewExecutionResult>(
              (res) => {
            ...
            executionPromise.finally(() => {
              try {
                reviewPromiseByKey.delete(singleKey);
              } catch {}
              try {
                removeReviewActiveFile(questId);
              } catch {}
            });
            ...
            try {
              ...
              try {
                createReviewActiveFile(questId, correlationId);
              } catch {}
            } catch (e: any) {
              logCriticalReviewTransition(
            ...
            if (
              ...
              options.kind === "final_acceptance"
            ) {
              targetState.awaitingReview = {
                kind: options.kind,
                reviewId: correlationId,
                triggerReason: options.triggerReason,
                since: Date.now(),
            ...
            const requestEventType = isPlanReviewKind
              ? "PLAN_REVIEW_REQUESTED"
              : "CRITICAL_REVIEW_REQUESTED";
            const startEventType = isPlanReviewKind
              ? "PLAN_REVIEW_STARTED"
              : "CRITICAL_REVIEW_STARTED";

            logCriticalReviewTransition(
              requestEventType,
              `critical ${options.kind} review requested`,
              {
                quest: slug,
                questId,
                sessionId,
            ...
            logCriticalReviewTransition(
              startEventType,
              `critical ${options.kind} review started`,
              {
                quest: slug,
                questId,
                sessionId,
            ...
            if (fsHeld && reviewFileLockPath) {
              try {
                releaseReviewFileLock(reviewFileLockPath, reviewFileLockOwned);
              } catch {}
  ...
  if (lockResult.blocked || lockResult.limited) {
    ...
    if (reviewFileLockOwned && reviewFileLockPath) {
      try {
        releaseReviewFileLock(reviewFileLockPath, true);
      } catch {}
      reviewFileLockOwned = false;
    }
        ...
        await withQuestLock(
          ...
          async () =>
            await withQuestLock(questKey, async () => {
              const pending = dequeuePendingIfNeeded(
                slug,
                currentPlanVersion,
                currentHash,
                snapshot,
...
export async function requestPlanReview(
  ...
  return runCriticalReview(pi, ctx, {
    kind: "plan_review",
    questSlug,
    ...options,
  });
...
export async function checkAndTriggerPlanReview(
  ...
  if (s.activeDraft) {
    if (isDraftReviewValid(s)) {
      logEvent(
        ...
        `first plan review already fired (draft valid)`,
        {
          quest: s.activeDraft,
          shard: "draft",
    ...
    if (s.lastDraftReviewRequestKey === key) {
      logEvent(
        "REVIEW_DEDUP_HIT",
        `review dedup hit (draft shard key=${key})`,
        {
          quest: draftSlug,
          shard: "draft",
          key,
    ...
    const result = await runCriticalReview(pi, ctx, {
      kind: "plan_review",
      questSlug: draftSlug,
  ...
  if (isPlanReviewValidForState(s) && s.lastPlanReviewRequestKey === key) {
    logEvent(
      "FIRST_PLAN_REVIEW_ALREADY_FIRED",
      `first plan review already fired (key=${key})`,
      {
        quest: s.active,
        shard: "root",
        key,
    ...
    logCriticalReviewTransition(
      "CRITICAL_REVIEW_SUPPRESSED_DUPLICATE",
      `review suppressed duplicate: ${key}`,
      {
        quest: s.active!,
        questId: s.questId || s.active!,
        sessionId: getSessionId(c),
...
export async function checkAndTriggerDirectionReview(
  ...
  if (triggerReason === "no_progress") {
    ...
    if (hasActivePlan || hasPendingPlan) {
      ...
      logEvent(
        ...
        `direction review throttled (plan_review active/pending)`,
        {
          quest: s.active,
          triggerReason,
    ...
    if (
      s.lastDirectionReviewAt &&
      Date.now() - s.lastDirectionReviewAt < DIRECTION_REVIEW_COOLDOWN_MS
    ) {
      const { logEvent } = await import("../logging.ts");
      logEvent(
        "DIRECTION_REVIEW_THROTTLED",
        `direction review throttled (cooldown ${DIRECTION_REVIEW_COOLDOWN_MS}ms)`,
        {
          quest: s.active,
          triggerReason,
          cooldownMs: DIRECTION_REVIEW_COOLDOWN_MS,
          lastAt: s.lastDirectionReviewAt,
        },
      );
      s.substantiveTurnsSinceCheckpoint = 0;
      return null;
  ...
  if (
    ...
    s.lastDirectionReviewKey === key
  ) {
    logEvent(
      "REVIEW_DEDUP_HIT",
      `review dedup hit (direction shard key=${key})`,
      {
        quest: s.active!,
        shard: "direction",
        key,
  ...
  if (result?.review?.verdict) {
    s.lastDirectionReviewKey = key;
    s.__lastDirectionReviewKey = key;
    s.lastDirectionReviewAt = Date.now();
  }
