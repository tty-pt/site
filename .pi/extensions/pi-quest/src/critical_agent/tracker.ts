...
export function getPendingReviews(): Map<string, PendingReviewRequest> {
  const copy: Map<string, PendingReviewRequest> & {
    get: (k: string) => PendingReviewRequest | undefined;
  } = new Map(pendingReviews);
...
export function getAllPendingForSlug(
  questSlug: string,
): PendingReviewRequest[] {
...
export function canLaunchReview(
  ...
  if (
    targetState &&
    (targetState.active === questSlug || targetState.questId === questSlug) &&
    targetState.inCriticalReview
  ) {
  ...
  if (kind === "final_acceptance") {
    for (const rev of activeReviews.values()) {
      if (
        rev.kind === "final_acceptance" &&
        (rev.status === "starting" || rev.status === "running")
      ) {
        return {
          allowed: false,
...
export function registerActiveReview(
  ...
  if (existing) {
    throw new Error(
      `Invariant violated: active review ${existing.reviewId} already running for quest ${questSlug} (attempt to register ${reviewId} kind=${kind})`,
    );
  }
...
export function updateReviewActivity(
  ...
  if (
    eventName.includes("turn") ||
    eventName.includes("turn_start") ||
    eventName.includes("turn_end")
  ) {
...
export function updateReviewerUIStatus(
  ctx?: ExtensionContext,
