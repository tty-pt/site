...
import { CriticalReviewKind, ExtensionAPI, ExtensionContext, ReviewActivityStats, ReviewSnapshot, StoredState } from "../../types.ts";
import { PiSubagentReviewer } from "../pi_adapter.ts";
import { createReviewSnapshot } from "../snapshot.ts";
import { completeActiveReview, updateReviewActivity, updateReviewerUIStatus } from "../tracker.ts";
...
export async function executeReviewBackground(
	...
	const onActivity = (activityEvent: any) => {
		if (activityEvent?.childSessionId && !childSessionIdRecorded) {
			...
			logEvent("SUBAGENT_STARTED", `child reviewer session started (${childSessionIdRecorded})`, {
				quest: slug,
				questId,
				sessionId,
		...
		if (updated) {
			...
			logEvent("SUBAGENT_ACTIVITY", `subagent activity: turns=${activityStats.turns} tools=${activityStats.tools} files=${activityStats.files}`, {
				quest: slug,
				questId,
				sessionId,
				reviewId: correlationId,
	...
	try {
		...
		if (parsedResult.selfCritique) {
			...
			if (parsedResult.selfCritique.initialJudgment !== parsedResult.selfCritique.revisedJudgment) {
				logCriticalReviewTransition("SELF_CRITIQUE_REVISED", `reviewer self-critique revised verdict from ${parsedResult.selfCritique.initialJudgment} to ${parsedResult.selfCritique.revisedJudgment}`, {
					quest: slug,
					questId,
					sessionId,
					reviewId: correlationId,
		...
		logCriticalReviewTransition("CRITICAL_REVIEW_ERROR", `critical review execution error: ${err?.message || "subagent failure"} (${timeoutLayer})`, {
			quest: slug,
			questId,
			sessionId,
			reviewId: correlationId,
