...
export async function reconcileReviewResult(
  ...
  return withQuestLock(lockKey, async () => {
    ...
    const isApproved = parsedResult.verdict === "PASS" ||
      parsedResult.verdict === "APPROVE";
    const isRejected = parsedResult.verdict === "FAIL" ||
    ...
    if (!currentness.current) {
      ...
      logCriticalReviewTransition(
        ...
        `review result superseded by newer state (${currentness.reason})`,
        {
          quest: slug,
          questId,
          sessionId,
    ...
    if (isRejected) {
      return await handleRejectedVerdict(
        snapshot,
        parsedResult,
        targetState,
        correlationId,
