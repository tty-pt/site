...
export function handleApprovedVerdict(
  ...
  const passEventType = isPlanReviewKind
    ? "PLAN_REVIEW_APPROVED"
    : "CRITICAL_REVIEW_PASSED";
  logCriticalReviewTransition(
    passEventType,
    `critical review passed (${snapshot.reviewKind})`,
    {
      quest: slug,
      questId,
      sessionId,
  ...
  if (isPlanReviewKind) {
    logEvent(
      ...
      `requireConfirm decision: plan_review approval`,
      {
        quest: slug,
        questId,
        sessionId,
    ...
    targetState.lastPlanReviewApproval = {
      ...
      saveCount: targetState.saveCount,
      timestamp: Date.now(),
    };
    if (snapshot.boundaryKey) {
  ...
  reviewState.reviewedStateVersion.saveHash = targetState.lastSavedHash ||
    snapshot.stateHash;
