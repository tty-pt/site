...
import { createAgentObligation, queueAgentObligation } from "../../obligations.ts";
...
export async function handleRejectedVerdict(
	...
	const failEventType = isPlanReviewKind ? "PLAN_REVIEW_FAILED" : "CRITICAL_REVIEW_FAILED";
	logCriticalReviewTransition(failEventType, `critical review failed: ${parsedResult.findings.map((f: any) => f.issue).join("; ") || "issues found"}`, {
	...
	logCriticalReviewTransition("REMEDIATION_REQUIRED", `remediation required: ${parsedResult.requiredActions.join("; ") || "fix findings"}`, {
		quest: slug, questId, sessionId, reviewId: correlationId,
		childSessionId: parsedResult.childSessionId, parentSessionId: sessionId,
		reviewKind: snapshot.reviewKind, severity: parsedResult.severity,
		requiredAction: parsedResult.requiredActions.join("; "), reviewedVersion: snapshot.planVersion,
	});

	if (isPlanReviewKind) targetState.lastPlanReviewApproval = null;

	const findingsSummary = parsedResult.findings.map((f: any) => `- ${f.issue}${f.evidence ? `\n  Evidence: ${f.evidence}` : ""}`).join("\n");
	const actionsSummary = parsedResult.requiredActions.map((a: any) => `- ${a}`).join("\n");
	const complianceSummary = parsedResult.originalRequestCheck?.items?.length
		? parsedResult.originalRequestCheck.items.map((i: any) => `- ${i.requirement} -> Status: ${i.status}${i.planHandling ? ` (Plan: ${i.planHandling})` : ""}`).join("\n")
		: "";

	const errorMsg = isPlanReviewKind
		? `[Quest Journal] ADVERSARIAL PLAN REVIEW REJECTED (VERDICT: REVISE)\n\nSeverity: ${parsedResult.severity}\n\nFindings:\n${findingsSummary || "(Unspecified plan finding)"}\n\nRequired Revisions:\n${actionsSummary || "Address reviewer findings, revise the execution plan, and re-submit for review."}${complianceSummary ? `\n\nPrompt Compliance Check:\n${complianceSummary}` : ""}\n\nThe plan draft is NOT approved. You must revise your execution plan to address these findings, update the quest state with your revised plan and plan revisions, and submit for re-review before implementation can proceed.`
	...
	if (!isReviewSnapshotCurrent(snapshot, targetState).current) {
		...
		const obl = createAgentObligation(targetState, {
			...
			code: isPlanReviewKind ? QuestErrorCode.PLAN_REVIEW_REQUIRED : QuestErrorCode.CRITICAL_REVIEW_FAILED,
			message: errorMsg, deliverAs: "steer",
			requiredNextAction: actionsSummary || "Revise plan/implementation to address critical review findings.",
			correlationId,
			isCurrent: (s) => isReviewSnapshotCurrent(snapshot, s).current,
		});
		queueAgentObligation(targetState, obl);
		sendInternalAgentMessage(pi, errorMsg, "steer", isPlanReviewKind ? "plan_review_failed" : "critical_review_failed", correlationId);
	}

	if (!isPlanReviewKind && (parsedResult.severity === "CRITICAL" || parsedResult.severity === "MAJOR")) {
		triggerReassessment(targetState, `Critical Review Failed: ${parsedResult.findings.map((f: any) => f.issue).join("; ")}`, findingsSummary);
	}
		...
		try {
			...
			if (await fileExists(qPath)) {
				...
				const newItems = parsedResult.requiredActions
					...
					.join("\n");
				if (newItems) {
					const combinedRemaining = existingRemaining && existingRemaining !== "-" ? `${existingRemaining}\n${newItems}` : newItems;
