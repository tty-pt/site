...
export function questDirPath(qid?: string | null, baseDir?: string): string {
  ...
  if (!targetQid) return "";
  return baseDir
    ? join(baseDir, targetQid)
    : `${QUEST_CURRENT_DIR}/${targetQid}`;
}
...
export function questWorkingFilePath(
  qid?: string | null,
  baseDir?: string,
): string {
...
export function questResearchPath(
  qid?: string | null,
  baseDir?: string,
): string {
  const dir = questDirPath(qid, baseDir);
...
export function questExecutionPath(
  qid?: string | null,
  baseDir?: string,
): string {
  const dir = questDirPath(qid, baseDir);
...
export function listShardPaths(
  qid?: string | null,
  baseDir?: string,
): string[] {
  return [
...
export function questArchivePath(qid: string, projectRoot?: string): string {
  ...
  return root
    ? join(root, QUEST_ARCHIVE_DIR, `${qid}.zip`)
    : `${QUEST_ARCHIVE_DIR}/${qid}.zip`;
}
...
export function generateSlugFromPrompt(prompt: string, maxLen = 45): string {
  ...
  text = text.replace(
    /^(please\s+|can\s+you\s+|could\s+you\s+|help\s+me\s+|i\s+want\s+to\s+|i\s+need\s+to\s+|we\s+need\s+to\s+|let'?s\s+)+/i,
    "",
  );
...
export function shouldStartPersistentQuest(prompt: string): boolean {
  ...
  const conversationalOnly =
    /^(hi|hello|hey|greetings|thanks|thank you|thx|ok|okay|k|yes|no|yep|nope|sure|sounds good|looks good|lgtm|continue|go ahead|proceed|approved|got it|cool|nice|great|good|fine|done|quit|exit)[.!?\s]*$/i;
  if (conversationalOnly.test(lower)) return false;

  // Simple non-action factual / informational queries
  const isInformationalQuery =
    /^(what is|what are|what does|where is|where are|who is|how do i|how does|how can i|can you explain|explain to me|tell me about|is there a|are there any|why is|why does|why do)\b/i
      .test(lower);

  if (isInformationalQuery && trimmed.length < 150) {
    const hasActionKeywords =
      /\b(implement|build|refactor|fix|debug|rewrite|migrate|redesign|create feature|add feature)\b/i
        .test(lower);
  ...
  if (
    /^(is this valid|is this correct|check this syntax|how to write|how do i write)\b/i
      .test(lower) && trimmed.length < 120
  ) {
    return false;
  }
...
export async function listActiveQuestRecords(
  baseDir: string = QUEST_CURRENT_DIR,
): Promise<QuestRecord[]> {
      ...
      if (entry.isDirectory()) {
        ...
        try {
          ...
          for (const qf of qFiles) {
            if (
              qf.isFile() && qf.name.endsWith(".md") && qf.name !== "summary.md"
            ) {
...
export async function resolveQuestRecordBySlug(
  slug: string,
  baseDir: string = QUEST_CURRENT_DIR,
  ...
  for (const r of records) {
    if (r.path.endsWith("quest.md")) {
      try {
        ...
        const subQuestsMatch = content.match(
          /##\s+Sub-Quests\s*([\s\S]*?)(?=\n##\s+|$)/i,
        );
        if (
          subQuestsMatch &&
          subQuestsMatch[1].toLowerCase().includes(`[[${target}]]`)
        ) {
...
export async function resolveFutureDraftPath(slug: string): Promise<string> {
  ...
  try {
    ...
    const hit = mds.find((f) =>
      f.includes(slug) || slug.includes(f.replace(/\.md$/, ""))
    );
...
export async function appendToFutureDraft(
  slug: string,
  promptText: string,
): Promise<boolean> {
  try {
...
export async function createFutureDraftFromPrompt(
  slug: string,
  promptText: string,
): Promise<string> {
    ...
    if (await fileExists(futurePath)) {
      try {
        ...
        if (qid) {
          ...
          const dest = join(archDir, basename(futurePath));
          try {
            await copyFile(futurePath, dest);
          } catch {
            // Target dir may not exist yet; create then retry. Best-effort.
            try {
              ...
              try {
                copyFileSync(futurePath, dest);
              } catch {}
            }
          }
          const { tryLog } = await import("./logging.ts");
          const h = createHash("sha256").update(content || slug).digest("hex")
            .slice(0, 12);
          tryLog("DRAFT_DISCARDED", "draft discarded", {
            quest: slug,
            slug,
            hash: h,
            dest,
            reason: "cleanDraftIfExists",
          }, ctx);
        }
      } catch {}
