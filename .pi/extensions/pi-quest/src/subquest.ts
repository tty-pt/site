...
import { parseMarkdownSections, QUEST_TEMPLATE } from "./markdown.ts";
import {
  logError,
  reportAgentError,
...
async function hasPositiveSubquestCompletionEvidence(
  ...
  if (parentSlug) {
    ...
    const parentPath = parentRecord
      ? parentRecord.path
      : questPath(qid || state.questId || parentSlug);
    try {
...
export async function reconcilePendingSubquestResume(
  ...
  const hasEvidence = await hasPositiveSubquestCompletionEvidence(
    pendingChild,
    targetState.active,
    targetState.questId,
  );
  ...
  if (hasEvidence) {
    ...
    logResumeTransition(
      "RESUME_OBSOLETED",
      `pending subquest resume for '${pendingChild}' obsoleted after archive`,
      {
...
export async function linkSubQuestInParent(
  parentSlug: string,
  childSlug: string,
  description = "",
  ctx?: ExtensionContext,
  ...
  try {
    ...
    const linkEntry = description
      ? `- [ ] [[${childSlug}]] - ${description}`
      : `- [ ] [[${childSlug}]]`;

    const sections = parseMarkdownSections(content);
    const subSec = sections.get("sub-quests") || sections.get("subquests") ||
      sections.get("sub quests");
    ...
    if (subSec) {
      const cleanedBody = subSec.body.replace(/- \[\s*\]\s*(\n|$)/g, "")
        .replace(/^-\s*(\n|$)/gm, "").trimEnd();
      const newSectionBody = cleanedBody
        ? `${cleanedBody}\n${linkEntry}`
    ...
    logError(
      `Failed to link sub-quest in ${targetPath}`,
      err,
      ctx,
      QuestErrorCode.SUBQUEST_FAILURE,
    );
    return false;
  }
}

export async function markSubQuestCompletedInParent(
  parentSlug: string,
  childSlug: string,
  ctx?: ExtensionContext,
): Promise<boolean> {
  ...
  const parentPath = parentRecord
    ? parentRecord.path
    : questPath(state.questId || parentSlug);
  try {
  ...
  try {
    ...
    logError(
      `Failed to mark subquest ${childSlug} completed in ${parentPath}`,
      err,
      ctx,
      QuestErrorCode.SUBQUEST_FAILURE,
    );
...
export function sendChildReturnParentPrompt(
  ...
  const directiveText =
    `⚡ **Sub-Quest '${childName}' Completed — Parent Evaluation Directive**:
...
5. Proceed autonomously with parent quest execution.`;
  ...
  logSubquestTransition(
    "SUBQUEST_COMPLETE",
    `subquest ${childName} completed, returned to ${parentName}`,
    {
      quest: parentName,
      subquest: childName,
      child: childName,
...
export function pushSubquestToStack(
  targetState: StoredState,
  parentName: string,
  name: string,
): void {
...
export function applyLoadedSubquestState(
  ...
  if (subLoaded.exists && isExisting) {
    targetState.prompts = subLoaded.originalRequest
      ? [subLoaded.originalRequest]
      : [goal];
    ...
    targetState.resolvedReassessmentVersion =
      subLoaded.resolvedReassessmentVersion;
...
export function buildSubquestProtocolInstructions(
  ...
  const goalText = goal ? `\n\n**Stated Goal**: ${goal}` : "";
  return `Now working on sub-quest **${name}**${
    parentName ? ` (parent: **${parentName}**)` : ""
  }. Sub-quest file: \`${path}\`.${goalText}
