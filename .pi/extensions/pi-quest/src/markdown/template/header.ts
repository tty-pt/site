...
export function extractParentFromQuest(content: string): string | null {
  ...
  const parentSec = sections.get("parent quest") || sections.get("parent") ||
    sections.get("parentquest");
  ...
  if (cleanLines.length > 0) {
    const token = cleanLines[0].replace(/^-\s*\[[ x]\]\s*/, "").replace(
      /^-\s*/,
      "",
    ).trim();
...
export function parseQuestId(content: string): string | null {
  ...
  if (frontmatterMatch) {
    const idMatch = frontmatterMatch[1].match(
      /^(?:questId|quest_id):\s*([a-zA-Z0-9_-]+)/mi,
    );
    if (idMatch && idMatch[1]) return idMatch[1].trim();
  }
  const commentMatch = content.match(
    /<!--\s*(?:questId|quest_id):\s*([a-zA-Z0-9_-]+)\s*-->/i,
  );
...
export function ensureQuestIdInContent(
  content: string,
  questId: string,
): string {
