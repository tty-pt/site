...
export function extractFormattedResumeSection(
	...
	for (const alias of aliases) {
		...
		if (found && !usedRawSections.has(found)) { sec = found; break; }
	}
	if (sec && sec.body && sec.body.trim() && sec.body.trim() !== "-" && !sec.body.trim().startsWith("> Paste the verbatim user prompt here") && !sec.body.trim().startsWith("> What we are trying to accomplish.")) {
		...
		if (target.maxChars && body.length > target.maxChars) body = body.slice(0, target.maxChars).trim() + "… [see quest file for full section]";
...
export async function loadActiveQuestResumeContext(opts?: { includeShards?: boolean }): Promise<string> {
	...
	const loadFromPath = async (p: string): Promise<string> => {
		try {
			...
			let merged = content;
			if (includeShards) {
				const dir = p.endsWith("quest.md") ? p.slice(0, -"/quest.md".length) : p.slice(0, p.lastIndexOf("/"));
			...
			logError(`Failed to load resume context from ${p}`, err, undefined, QuestErrorCode.RESUME_STATE_INCONSISTENT);
			return "";
		}
	};
	if (path) {
