...
export function parseReassessmentState(
	...
	if (evidenceSec && !isPlaceholderOrEmpty(evidenceSec.body)) reassessmentEvidence = evidenceSec.body.trim();
...
export function parseOriginalRequest(sections: Map<string, any>): string {
	...
	if (rawText && !rawText.startsWith("Paste the verbatim user prompt") && !rawText.startsWith("Goal:")) return rawText;
	return "";
}
...
export function parseRefinements(sections: Map<string, any>): string[] {
	...
	if (!refSec) return [];
	return refSec.body.split(/\r?\n/).map((l: string) => l.replace(/^[-*]\s*/, "").replace(/^\d+\.\s*/, "").trim()).filter((l: string) => l && l !== "-" && !l.startsWith(">"));
}
...
export function parseAwaitingUserConfirmation(sections: Map<string, any>): boolean {
	...
	if (currentStatusSec && !isPlaceholderOrEmpty(currentStatusSec.body)) {
		...
		if (bodyLower.includes("plan provisional") || bodyLower.includes("research pending") || bodyLower.includes("confirmation pending") || bodyLower.includes("research complete") || bodyLower.includes("provisional")) return true;
		if (bodyLower.includes("plan confirmed") || bodyLower.includes("in progress") || bodyLower.includes("done")) return false;
	}
	return false;
}
...
export async function loadExistingQuestEpistemicState(slugOrQid: string, basePath?: string): Promise<LoadedQuestState> {
	...
	try {
		...
		const mdPlanVersion = parseSectionInteger(sections.get("plan version") || sections.get("version"), 1, 1);
		const mdResearchRound = parseSectionInteger(sections.get("research round") || sections.get("research cycle"), 1, 1);
		const mdLastResearchAt = parseSectionTimestamp(sections.get("last research at") || sections.get("last research timestamp") || sections.get("last research"));
		const mdLastPlanRevisionAt = parseSectionTimestamp(sections.get("last plan revision at") || sections.get("last plan revision timestamp") || sections.get("last plan revision"));
		const mdReassessmentVersion = parseSectionInteger(sections.get("reassessment version"), 0, 0);
		const mdResolvedReassessmentVersion = parseSectionInteger(sections.get("resolved reassessment version"), 0, 0);
		const planConfidence = meta?.planConfidence ? parseSectionConfidence({ body: String(meta.planConfidence) } as any) : parseSectionConfidence(sections.get("plan confidence") || sections.get("confidence"));
		const planVersion = meta?.planVersion !== undefined ? Math.max(mdPlanVersion, Math.max(1, Number(meta.planVersion) || 1)) : mdPlanVersion;
		const researchRound = meta?.researchRound !== undefined ? Math.max(mdResearchRound, Math.max(1, Number(meta.researchRound) || 1)) : mdResearchRound;
		const lastResearchAt = meta?.lastResearchAt !== undefined && meta.lastResearchAt ? Math.max(mdLastResearchAt || 0, Number(meta.lastResearchAt) || 0) || mdLastResearchAt : mdLastResearchAt;
		const lastPlanRevisionAt = meta?.lastPlanRevisionAt !== undefined && meta.lastPlanRevisionAt ? Math.max(mdLastPlanRevisionAt || 0, Number(meta.lastPlanRevisionAt) || 0) || mdLastPlanRevisionAt : mdLastPlanRevisionAt;
		const reassessmentVersion = meta?.reassessmentVersion !== undefined ? Math.max(mdReassessmentVersion, Math.max(0, Number(meta.reassessmentVersion) || 0)) : mdReassessmentVersion;
		const resolvedReassessmentVersion = meta?.resolvedReassessmentVersion !== undefined ? Math.max(mdResolvedReassessmentVersion, Math.max(0, Number(meta.resolvedReassessmentVersion) || 0)) : mdResolvedReassessmentVersion;
		const { reassessmentRequired, reassessmentReason, reassessmentEvidence } = parseReassessmentState(sections, reassessmentVersion, resolvedReassessmentVersion);
