...
type ObligationTransitionConfig = {
	...
	reasonField: "fulfilledReason" | "supersededReason" | "cancelledReason" | "failedReason";
...
function requirePendingNotifications(state: StoredState): boolean {
	return Array.isArray(state.pendingNotifications) && state.pendingNotifications.length > 0;
}
...
export function fulfillObligation(
	...
	return transitionObligations(state, resolveObligationPredicate(obligationIdOrPredicate), {
		status: "fulfilled",
		timestampField: "fulfilledAt",
		reasonField: "fulfilledReason",
		defaultReason: reason,
		logEvent: "AGENT_MESSAGE_DELIVERED",
		logMessage: (obl) => `agent obligation fulfilled: [${obl.code || obl.kind}]`,
	}, reason);
}
...
export function supersedeObligation(
	...
	return transitionObligations(state, resolveObligationPredicate(obligationIdOrPredicate), {
		...
		defaultReason: reason,
		extra: (obl) => { obl.superseded = true; },
		logEvent: "AGENT_MESSAGE_SUPERSEDED",
		logMessage: (obl) => `agent obligation superseded: [${obl.code || obl.kind}]`,
	}, reason);
}
...
export function reconcileObligations(
	...
	if (!Array.isArray(state.pendingNotifications) || state.pendingNotifications.length === 0) {
		return;
	}

	const queue = [...state.pendingNotifications];
	for (const obligation of queue) {
		if (typeof obligation.isFulfilled === "function" && obligation.isFulfilled(state)) {
			fulfillObligation(state, obligation.id, "Obligation fulfillment verified by predicate");
		} else if (!isObligationCurrent(obligation, state)) {
			supersedeObligation(state, obligation.id, "Obligation condition no longer applicable in authoritative state");
		}
	}
}
