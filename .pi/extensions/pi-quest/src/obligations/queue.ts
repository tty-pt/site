...
import { AgentObligation, ObligationKind, ObligationStatus, QuestErrorCode, StoredState } from "../types.ts";

const MAX_HISTORY_SIZE = 50;

export function pushToObligationHistory(state: StoredState, obligation: AgentObligation): void {
	if (!Array.isArray(state.obligationHistory)) {
		state.obligationHistory = [];
	}
	const existingIdx = state.obligationHistory.findIndex((o) => o.id === obligation.id);
	if (existingIdx >= 0) {
		...
		if (state.obligationHistory.length > MAX_HISTORY_SIZE) {
			state.obligationHistory = state.obligationHistory.slice(-MAX_HISTORY_SIZE);
		}
	}
}
...
export function createAgentObligation(
	...
	return {
		id: opts.id || `obl_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`,
...
export function queueAgentObligation(
	...
	if (Array.isArray(state.obligationHistory) && obligation.id) {
		const historical = state.obligationHistory.find((o) => o.id === obligation.id);
		if (historical && (historical.status === "fulfilled" || historical.status === "superseded" || historical.status === "cancelled")) {
			return historical;
		}
	}

	const dedupKey = obligation.dedupKey || (obligation.code && obligation.message ? `${obligation.code}:${obligation.message}` : (obligation.id || ""));

	const existing = state.pendingNotifications.find(
		(n) =>
			(n.id && n.id === obligation.id) ||
			(dedupKey && n.dedupKey && n.dedupKey === dedupKey) ||
			(n.code && n.code === obligation.code && n.message === obligation.message),
	);
	...
	if (existing) {
		...
		logAgentMessageTransition("AGENT_MESSAGE_QUEUED", `agent obligation queued: [${obligation.code || obligation.kind}]`, {
			code: typeof obligation.code === "string" ? obligation.code : String(obligation.code || obligation.kind),
...
export function getObligationHistory(state: StoredState): AgentObligation[] {
	return Array.isArray(state.obligationHistory) ? [...state.obligationHistory] : [];
}
