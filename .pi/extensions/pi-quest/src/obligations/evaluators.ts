...
export function getObligationEvaluator(kind: string): ObligationCurrentnessEvaluator | undefined {
	return obligationEvaluators.get(kind.toLowerCase());
}

const EVALUATOR_TABLE: Array<[string, ObligationCurrentnessEvaluator]> = [
	["research", (_obl, s) => !s.researchComplete || Boolean(s.reassessmentRequired)],
	["reassessment", (obl, s) => {
		if (!s.reassessmentRequired) return false;
		if (typeof obl.reassessmentVersion === "number" && (s.resolvedReassessmentVersion || 0) >= obl.reassessmentVersion) return false;
		return true;
	}],
	["confirmation", (_obl, s) => Boolean(s.awaitingUserConfirmation)],
	["checkpoint", (obl, s) => !(typeof obl.stateGeneration === "number" && s.saveCount > obl.stateGeneration && !s.dirty)],
	["error", () => true],
	["research_required", (_obl, s) => !s.researchComplete || Boolean(s.reassessmentRequired)],
	["reassessment_required", (obl, s) => Boolean(s.reassessmentRequired) && ((obl.reassessmentVersion === undefined) || (s.resolvedReassessmentVersion || 0) < obl.reassessmentVersion)],
	["confirmation_required", (_obl, s) => Boolean(s.awaitingUserConfirmation)],
	["checkpoint_required", (obl, s) => !(typeof obl.stateGeneration === "number" && s.saveCount > obl.stateGeneration && !s.dirty)],
];
...
export function isObligationCurrent(
	...
	if (obligation.status === "fulfilled" || obligation.status === "superseded" || obligation.status === "cancelled" || obligation.status === "failed" || obligation.superseded) {
		return false;
	}
	...
	if (typeof obligation.isFulfilled === "function" && obligation.isFulfilled(currentState)) {
		return false;
	}
