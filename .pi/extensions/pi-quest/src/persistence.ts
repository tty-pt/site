...
import { type FileFingerprint, memoFileFingerprint } from "./utils/cache.ts";
...
export function persist(pi: ExtensionAPI, ctx?: ExtensionContext): boolean {
  try {
    ...
    logPersistenceTransition(
      ...
      `persistence append failed: ${err?.message || String(err)}`,
      {
        quest: state.active || "",
    ...
    reportAgentError(
      pi,
      ctx,
      `The quest state could not be durably persisted: ${
      ...
      }.\n\nDo not assume the current state will survive compaction or session reconstruction.`,
      {
        ...
        deliverAs: "followUp",
        requiredNextAction:
          "Restore persistence, then perform and verify a fresh durable quest save before crossing a durability boundary.",
        details: {
...
> {
  ...
  logPersistenceTransition(
    "SAVE_STARTED",
    `verifying save for '${targetSlug || "(none)"}'`,
    {
      quest: targetSlug || "",
    },
  );
  if (!targetSlug && !targetQid) {
    ...
    logPersistenceTransition("SAVE_REJECTED", errMsg, {
      quest: "",
      reason: "no_active_quest",
    });
    reportAgentError(
      pi,
      ctx,
      errMsg,
      {
        ...
        requiredNextAction:
          "Set an active quest with /quest <name> before attempting to save.",
      },
    );
    return { success: false, count: s.saveCount, error: errMsg };
  }
  const doVerify = async (): Promise<
    {
      success: boolean;
      hash?: string;
      count: number;
  ...
  > => {
    ...
    if (!fp) {
      ...
      const futureDraftExists = (() => {
        try {
          ...
          return slug ? existsSync(join(FUTURE_DIR, `${slug}.md`)) : false;
        } catch {
          return false;
        }
      })();
      const reason = futureDraftExists
        ? "file_not_found+future_draft_exists"
        : "file_not_found";
      const requiredAction = futureDraftExists
        ? "quest_update_state"
        : undefined;
      const draftHint = futureDraftExists
        ? ` Draft exists in \`${
          join(FUTURE_DIR, `${targetSlug || targetQid}.md`)
        }\` — call quest_update_state (not quest_mark_saved or bash mkdir) with researchComplete:true`
        : "";
      const errMsg =
        `Save verification failed: Quest file not found or unreadable at \`${p}\`.${draftHint}`;
      logPersistenceTransition("SAVE_FAILED", errMsg, {
      ...
      reportAgentError(
        ...
        {
          code: QuestErrorCode.SAVE_VERIFICATION_FAILURE,
          requiredNextAction: futureDraftExists
            ? `Draft exists in ${
              join(FUTURE_DIR, `${targetSlug || targetQid}.md`)
            } — call quest_update_state (not quest_mark_saved or bash mkdir) with researchComplete:true to create the durable quest file.`
            : `Ensure the file is written to disk at \`${p}\` before calling quest_mark_saved.`,
          details: {
      ...
      return {
        ...
        count: s.saveCount,
        error:
          `Quest file not found or unreadable at \`${p}\`.${draftHint} Ensure the file is written to disk before marking as saved.`,
      };
    }
    ...
    if (fp.hash !== s.lastSavedHash) {
      try {
        ...
        audit = auditQuestConsistency(content, {
          recentModifiedFiles: s.sessionModifiedFiles ||
            state.sessionModifiedFiles,
        });
        if (!audit.consistent) {
          logError(
            `Consistency audit issues in ${p}: ${audit.issues.join("; ")}`,
            undefined,
            ctx,
            QuestErrorCode.SAVE_VERIFICATION_FAILURE,
          );
        }
      } catch {}
    }

    const isSameAsLastSave = s.saveGeneration &&
    ...
    if (isSameAsLastSave && !s.dirty) {
      if (
        s.activeTransaction && s.activeTransaction.phase === "resume-delivered"
      ) {
    ...
    logPersistenceTransition(
      "SAVE_VERIFIED",
      `save verified for '${targetSlug || targetQid}' (gen ${s.saveCount})`,
      {
        quest: targetSlug || targetQid || "",
        gen: s.saveCount,
