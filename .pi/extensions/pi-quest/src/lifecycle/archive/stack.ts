...
export async function hydrateNextActive(nextActive: string | null, stack: string[], s: any, archivedName: string, parentSlug: string | null, pi: ExtensionAPI, ctx: ExtensionContext): Promise<void> {
	if (parentSlug) await markSubQuestCompletedInParent(parentSlug, archivedName, ctx);
	...
	if (nextActive) {
		...
		persist(pi, ctx);
		logSubquestTransition("SUBQUEST_RETURN", `returned to quest '${nextActive}' (LIFO stack)`, { quest: nextActive, parent: nextActive, child: archivedName });
	} else {
		persist(pi, ctx);
	}
}
