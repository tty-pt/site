...
export function applyLoadedEpistemicState(targetState: any, loaded: any, fallbackPrompt?: string): void {
	if (loaded && loaded.exists) {
		if (loaded.questId) targetState.questId = loaded.questId;
		targetState.prompts = loaded.originalRequest ? [loaded.originalRequest] : fallbackPrompt ? [fallbackPrompt] : [];
		...
		targetState.resolvedReassessmentVersion = loaded.resolvedReassessmentVersion;
		...
		if (loaded.researchComplete) {
			...
			startResearchEpoch(targetState, targetState.reassessmentRequired ? "reassessment" : "research");
		}
	} else {
