...
import { FUTURE_DIR, NOTES_FILE, QUEST_ARCHIVE_DIR, QUEST_CURRENT_DIR, QUEST_ROOT } from "../constants.ts";
import { questPath } from "../paths.ts";
import { state } from "../state.ts";
import { ToolPermission } from "../types.ts";
import { cleanCommandPreamble, hasFileRedirection, isHelpOrVersionInvocation, splitBashCommandChain, splitBashTokens } from "./shell_parser.ts";
...
export function classifySpecialGitCommand(gitSub: string, tokens: string[]): ToolPermission | null {
	if (gitSub === "branch") {
		return tokens.some((t) => t === "-d" || t === "-D" || t === "-m" || t === "-M") ? "implementation" : "read";
	}
	if (gitSub === "tag") {
		return tokens.some((t) => t === "-d" || t === "--delete") ? "implementation" : "read";
	}
	if (gitSub === "remote") {
		return tokens.some((t) => t === "add" || t === "remove" || t === "rm" || t === "rename" || t === "set-url") ? "implementation" : "read";
	}
	if (gitSub === "config") {
		return tokens.some((t) => t === "--get" || t === "--list" || t === "-l" || t.startsWith("--get-")) ? "read" : "unknown";
	}
	return null;
}
...
export function classifyInterpreterCommand(bin: string, tokens: string[]): ToolPermission {
...
export function classifySedPerlCommand(bin: string, tokens: string[]): ToolPermission {
	if (bin === "sed") {
...
export function classifyAwkCommand(bin: string, tokens: string[]): ToolPermission {
...
export function classifyByBinaryCategory(bin: string, tokens: string[]): ToolPermission {
