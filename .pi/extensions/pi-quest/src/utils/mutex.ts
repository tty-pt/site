...
export function startReviewLockHeartbeat(
  ...
  logEvent(
    "REVIEW_LOCK_HEARTBEAT_STARTED",
    `review lock heartbeat started lockPath=${lockPath}`,
    {
      quest: questId || "",
      lockPath,
  ...
  return () => {
    clearInterval(iv);
    logEvent(
      "REVIEW_LOCK_HEARTBEAT_STOPPED",
      `review lock heartbeat stopped lockPath=${lockPath}`,
      {
        quest: questId || "",
        lockPath,
      },
...
export async function withQuestLock<T>(
  key: string,
  fn: () => T | Promise<T>,
  ...
  try {
    ...
    let activeCount = 0;
    try {
      if (getActiveCount) activeCount = getActiveCount();
    } catch (_e) {}
    let questForLog = "";
    try {
      if (key.startsWith("review:")) {
        questForLog = key.slice("review:".length).split(":")[0];
...
export function getQuestLockKey(
  questId: string | null | undefined,
  sessionId?: string,
): string {
...
export function getGlobalReviewLockKeyForQuest(
...
export function getReviewLockPath(questId: string | null | undefined): string {
  ...
  return dir
    ? join(dir, REVIEW_LOCK_FILE)
    : `${QUEST_CURRENT_DIR}/${qid}/${REVIEW_LOCK_FILE}`;
}

export function getReviewActivePath(
  questId: string | null | undefined,
): string {
  const qid = questId || "quest";
  const dir = questDirPath(qid);
  return dir
    ? join(dir, REVIEW_ACTIVE_FILE)
    : `${QUEST_CURRENT_DIR}/${qid}/${REVIEW_ACTIVE_FILE}`;
}
...
export function createReviewActiveFile(
  questId: string | null | undefined,
  reviewId: string,
): void {
  const p = getReviewActivePath(questId);
...
export function removeReviewActiveFile(
  questId: string | null | undefined,
): void {
  const p = getReviewActivePath(questId);
  try {
...
export function acquireReviewFileLock(
  questId: string | null | undefined,
): {
  acquired: boolean;
  path: string;
    ...
    if (existsSync(lockPath)) {
      try {
        ...
        if (age > staleMs) {
          ...
          logEvent(
            "REVIEW_LOCK_STALE_RECOVERED",
            `review lock stale recovered age=${Math.round(age)}`,
            { quest: questId || "", lockPath, ageMs: age, staleMs },
          );
      ...
      try {
        ...
        if (age > staleMs) {
          try {
            ...
            logEvent(
              "REVIEW_LOCK_STALE_RECOVERED",
              `review lock stale recovered (race) age=${Math.round(age)}`,
              { quest: questId || "", lockPath, ageMs: age, staleMs },
            );
          } catch (_e) {}
  ...
  return await withQuestLock(qKey, async () => {
    ...
    while (attempts <= maxRetries) {
      ...
      if (res.acquired) {
        ...
        if (attempts > 0) {
          ...
          try {
            logEvent(
              "MUTEX_WAIT",
              `mutex wait file ${res.path} retries=${attempts}`,
              { lockKey: qKey, waitMs, retries: attempts, lockPath: res.path },
            );
          } catch (_e) {}
        }
        break;
      ...
      if (attempts === maxRetries) {
        try {
          logEvent("MUTEX_WAIT", `mutex wait file timeout ${res.path}`, {
            lockKey: qKey,
    ...
    if (!owned || !acquiredPath) {
      throw new Error(`Failed to acquire review file lock for ${questId}`);
    }
    try {
      ...
      try {
        releaseReviewFileLock(acquiredPath!, owned);
      } catch (_e) {}
      try {
        heldFileLocks.delete(acquiredPath!);
      } catch (_e) {}
    }
  });
}
...
export function writeSessionLiveness(
  questId?: string | null,
...
export function startSessionLivenessHeartbeat(
  questId?: string | null,
): () => void {
  const iv = setInterval(
    () => touchSessionLiveness(questId),
    REVIEW_LOCK_HEARTBEAT_MS,
  );
  return () => {
    clearInterval(iv);
 ...
 */
export function readSessionLiveness(
  questId?: string | null,
): SessionLivenessInfo | null {
  const path = getSessionLivenessPath(questId);
 ...
 */
export function isQuestSessionActive(
  questId?: string | null,
