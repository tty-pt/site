...
function classifyGitInvestigation(
  tokens: string[],
  trimmed: string,
  ...
  if (
    ["diff", "log", "show", "blame", "ls-files", "ls-tree"].includes(gitSub)
  ) {
...
function classifySedInvestigation(
  tokens: string[],
  trimmed: string,
): { kind: InvestigationKind; target?: string; command?: string } {
...
export function classifySingleBashInvestigationKind(
  cmdSegment: string,
): { kind: InvestigationKind; target?: string; command?: string } {
  ...
  if (bin === "xargs") {
    ...
    return nonFlagTokens.length > 0
      ? classifySingleBashInvestigationKind(nonFlagTokens.join(" "))
      : { kind: "none" };
  }
...
export function classifyBashInvestigationKind(
  cmdStr: string,
): { kind: InvestigationKind; target?: string; command?: string } {
...
function classifyReadInvestigation(
  input: any,
...
function classifyGraphInvestigation(
  norm: string,
  input: any,
): { kind: InvestigationKind; target?: string } {
  ...
  return {
    ...
    target: `${norm}:${String(target).slice(0, 80)}`,
  };
}
...
export function classifyInvestigationKind(
  ...
  if (norm === "memory_search") {
    ...
    return {
      kind: "architecture-research",
      target: `memory_search:${String(target).slice(0, 80)}`,
    };
  }
  if (norm === "search_code") {
    ...
    return {
      kind: "code-search",
      target: `search_code:${String(target).slice(0, 80)}`,
    };
  }
  if (
    norm === "search_graph" ||
  ...
  if (
    ...
    const target = input?.query || input?.claim || input?.url || input?.path ||
      "external";
    return {
  ...
  if (
    ...
    const target = input?.name || input?.objective || input?.prompt ||
      "investigate";
    return {
      kind: "architecture-research",
      target: `${norm}:${String(target).slice(0, 80)}`,
