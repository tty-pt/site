...
let settingsCache: { mtime: number; rawHash: string; parsed: any | null } = {
  mtime: 0,
  rawHash: "",
  parsed: null,
};
let lastWorkflowCache:
  | { saveHash: string; pressure: string; prompt: string }
  | null = null;
let resumeCache: { saveHash: string; value: string } = {
  saveHash: "",
  value: "",
};
...
export async function memoFileFingerprint(
  p: string,
...
export function memoParseMarkdown(
  content: string,
  hash: string,
  parser: (c: string) => Map<string, MarkdownSection>,
...
export function memoSettings(
  ...
  if (
    settingsCache.mtime === mtimeMs && settingsCache.rawHash === h &&
    settingsCache.parsed !== null
  ) return settingsCache.parsed;
  const parsed = parser(raw);
  settingsCache = { mtime: mtimeMs, rawHash: h, parsed };
  return parsed;
}

export function memoResumeContext(
  saveHash: string,
  extracted: string[],
): string {
  if (resumeCache.saveHash === saveHash && resumeCache.value) {
    return resumeCache.value;
  }
...
export function getCachedWorkflow(
  ...
  if (
    lastWorkflowCache && lastWorkflowCache.saveHash === saveHash &&
    lastWorkflowCache.pressure === pressureKey
  ) return lastWorkflowCache.prompt;
  return null;
}
...
let settingsJsonCache: {
  path: string;
  mtime: number;
  hash: string;
  parsed: any | null;
} = { path: "", mtime: 0, hash: "", parsed: null };
...
export function getCachedSettingsJson(path: string): any | null {
  try {
    ...
    if (
      settingsJsonCache.path === path && settingsJsonCache.mtime === mtime &&
      settingsJsonCache.parsed !== null
    ) return settingsJsonCache.parsed;
    const raw = readFileSync(path, "utf8");
    const h = hashContent(raw).slice(0, 16);
    if (
      settingsJsonCache.path === path && settingsJsonCache.hash === h &&
      settingsJsonCache.parsed !== null
    ) {
