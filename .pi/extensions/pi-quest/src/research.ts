...
import { logEvent, logReassessmentTransition, logResearchTransition } from "./logging.ts";
...
export function recordObservedInvestigation(
	...
	if (!targetState.currentReceipt || targetState.currentReceipt.epoch !== (targetState.investigationEpoch || 1)) {
	...
	if (investigation.kind === "file-read") {
		const target = investigation.target || (typeof input === "string" ? input : input?.path || input?.file || "");
		if (target) {
		...
		logResearchTransition("RESEARCH_EVIDENCE", `investigation evidence recorded (${investigation.kind})`, {
			quest: targetState.active || "",
...
export function hasSufficientInvestigation(
	...
	if (!r) {
		return {
			sufficient: false,
			reason: `No observed investigation receipt exists for epoch ${requiredEpoch}.`,
		};
	}
	...
	if (r.isHistorical) {
		return {
			sufficient: false,
			reason: `Historical receipt cannot satisfy new investigation gate in active session.`,
			receipt: r,
		};
	}
	...
	if (r.epoch !== requiredEpoch) {
		return {
			sufficient: false,
			reason: `Investigation receipt is from epoch ${r.epoch}, but current required epoch is ${requiredEpoch}.`,
			receipt: r,
		};
	}
	...
	const hasSearch = Array.isArray(r.searchTargets) && r.searchTargets.length > 0;
	...
	if (totalEvidence === 0 || (!hasRead && !hasSearch && !hasCmd)) {
		const phaseLabel = expectedType === "reassessment" ? `Reassessment v${targetState.reassessmentVersion || 1}` : `Research Round ${targetState.researchRound || 1}`;
		return {
			sufficient: false,
			reason: `Zero investigation tool calls or commands observed since ${phaseLabel} began.`,
			receipt: r,
		};
	}
...
export function formatInvestigationEvidenceSummary(receipt: InvestigationReceipt): string {
	if (receipt.isHistorical) {
		return `Historical verified research`;
	}
	const items: string[] = [];
	if (receipt.readTargets && receipt.readTargets.length > 0) {
		items.push(`read [${receipt.readTargets.slice(0, 3).join(", ")}${receipt.readTargets.length > 3 ? "..." : ""}]`);
	}
	if (receipt.searchTargets && receipt.searchTargets.length > 0) {
		items.push(`search [${receipt.searchTargets.slice(0, 3).join(", ")}${receipt.searchTargets.length > 3 ? "..." : ""}]`);
	}
	if (receipt.commands && receipt.commands.length > 0) {
		items.push(`commands [${receipt.commands.slice(0, 3).join(", ")}${receipt.commands.length > 3 ? "..." : ""}]`);
	}
	const detailsStr = items.length > 0 ? items.join(", ") : "verified inspection";
	const dateStr = receipt.completedAt ? new Date(receipt.completedAt).toISOString() : new Date().toISOString();
...
export function triggerReassessment(
	...
	startResearchEpoch(targetState, "reassessment");
	syncImplementationPermission(targetState);

	logReassessmentTransition("REASSESSMENT_REQUIRED", `reassessment triggered: ${reason}`, {
		quest: targetState.active || "",
		reason,
...
export function buildResearchCheckpointPrompt(activeQuest: string, planVersion?: number, planConfidence?: string): string {
...
**Action Required**:
Update \`${questPath(activeQuest)}\` (using \`quest_update_state\` or edit + \`quest_mark_saved\`), then proceed autonomously.`;
}
