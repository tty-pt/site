...
export function generateQuestId(now: number = Date.now()): string {
  ...
  try {
    while (
      existsSync(`${QUEST_CURRENT_DIR}/${String(sec)}`) ||
      existsSync(`${QUEST_ARCHIVE_DIR}/${String(sec)}.zip`)
    ) {
      sec += 1;
...
export function snapshotState(ctx?: ExtensionContext): StoredState {
  ...
  return {
    ...
    pendingSubquestResumeResolution: s.pendingSubquestResumeResolution
      ? { ...s.pendingSubquestResumeResolution }
      : null,
    ...
    lastPeriodicCheckpointAt: s.lastPeriodicCheckpointAt || 0,
    lastPeriodicCheckpointTurn: s.lastPeriodicCheckpointTurn || 0,
    lastPeriodicSteerTurn: s.lastPeriodicSteerTurn ?? -1,
    lastPeriodicSteerAt: s.lastPeriodicSteerAt || 0,
    ...
    pendingNotifications: Array.isArray(s.pendingNotifications)
      ...
      : [],
    obligationHistory: Array.isArray(s.obligationHistory)
      ? s.obligationHistory.map((n) => ({ ...n }))
      : [],
    ...
    confirmedQuests: Array.isArray(s.confirmedQuests)
      ? [...s.confirmedQuests]
      : [],
    ...
    lastCriticalReview: s.lastCriticalReview
      ...
      : null,
    criticalReviews: Array.isArray(s.criticalReviews)
      ? s.criticalReviews.map((r) => ({ ...r }))
      : [],
    criticalReviewAttempts: s.criticalReviewAttempts
      ? { ...s.criticalReviewAttempts }
      : {},
    ...
    semanticSummaryEnabled: s.semanticSummaryEnabled,
    thoughtLoggingEnabled: s.thoughtLoggingEnabled,
    autonomousSubquestDuringDrafting: s.autonomousSubquestDuringDrafting,
    initialPromptLogged: !!s.initialPromptLogged,
    lastPlanReviewApproval: s.lastPlanReviewApproval
    ...
    lastPlanReviewRequestKey: s.lastPlanReviewRequestKey || null,
    lastDraftReviewRequestKey: s.lastDraftReviewRequestKey || null,
