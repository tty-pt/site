...
export async function handleSubquestCommand(
  args: string,
  ...
  if (!raw && ctx.mode === "tui") {
    raw = ((await ctx.ui.input(
      "Describe the sub-quest (e.g. handle auth edge cases):",
    )) ?? "").trim();
  }
  if (!raw) {
  ...
  let switchNow = true;
  if (
    raw.startsWith("--plan ") || raw.startsWith("-p ") ||
    raw.startsWith("--no-switch ")
  ) {
    switchNow = false;
  ...
  if (!switchNow) {
    const msg = `Planned sub-quest **${name}** at \`${path}\`${
      parentName ? ` linked in parent **${parentName}**` : ""
    }. Kept active quest **${state.active}**.`;
  ...
  persist(pi, ctx);
  updateUIStatus(ctx);
  const subquestMsg = buildSubquestProtocolInstructions(
    name,
    goal,
    parentName,
    path,
