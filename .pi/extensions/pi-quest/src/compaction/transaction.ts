...
function createPendingResumeTransaction(targetState: StoredState): CompactionTransaction {
	...
	const pendingTx: CompactionTransaction = {
		...
		checkpointSaveCount: hasCheckpointIdentity ? pending.checkpointSaveCount : undefined,
		checkpointHash: hasCheckpointIdentity ? pending.checkpointHash : undefined,
		stack: Array.isArray(targetState.stack) ? [...targetState.stack] : (pending.activeQuest ? [pending.activeQuest] : []),
...
function createNewCompactionTransaction(
	...
	const tx: CompactionTransaction = {
		...
		checkpointHash: targetState.saveGeneration?.hash || targetState.lastSavedHash || "",
		stack: Array.isArray(targetState.stack) ? [...targetState.stack] : (active ? [active] : []),
		researchRound: targetState.researchRound || 1,
		reassessmentVersion: targetState.reassessmentVersion || 0,
		planVersion: targetState.planVersion || 1,
		createdAt: Date.now(),
	...
	logCompactionTransition("COMPACTION_PREPARED", `compaction prepared for ${active}`, {
		quest: active,
		compactionId: id,
		phase: "prepared",
...
export function invalidatePreparedCompactionTransaction(
	...
	if (targetState.activeTransaction && targetState.activeTransaction.phase === "prepared") {
		const oldTx = targetState.activeTransaction;
		oldTx.phase = "invalidated_by_new_save";
		logCompactionTransition("COMPACTION_INVALIDATED", `prepared compaction checkpoint invalidated (${reason})`, {
...
export function createOrGetCompactionTransaction(
	...
	const currentHash = targetState.saveGeneration?.hash || targetState.lastSavedHash || "";
	...
	if (
		...
		if (
			...
			(currentQuest && targetState.activeTransaction.activeQuest !== currentQuest) ||
			(currentQuestPath && targetState.activeTransaction.questPath !== currentQuestPath)
		) {
			invalidatePreparedCompactionTransaction(targetState, "stale_checkpoint_detected");
		} else {
	...
	if (targetState.pendingResume) {
		if (
			targetState.activeTransaction &&
			targetState.activeTransaction.id === targetState.pendingResume.compactionId &&
			(!currentQuest || targetState.activeTransaction.activeQuest === currentQuest)
		) {
			return targetState.activeTransaction;
		}
		if (!currentQuest || targetState.pendingResume.activeQuest === currentQuest) {
