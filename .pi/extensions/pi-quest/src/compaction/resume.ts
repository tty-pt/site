...
import { INTERNAL_MESSAGE_PREFIX, QUEST_CURRENT_DIR } from "../constants.ts";
...
import {
  drainPendingAgentNotifications,
  logDebug,
  logError,
  reportAgentError,
  safeSendUserMessage,
...
function resolveChildStateAction(
  targetState: StoredState,
  childName: string,
): string {
  const qp = questPath(targetState.questId);
  if (targetState.reassessmentRequired) {
    return `⚡ **State: REASSESSMENT_PENDING** (Reason: ${
      targetState.reassessmentReason || "Unresolved contradiction"
    }).
...
4. Proceed with the revised Exact Next Action.`;
  ...
  if (targetState.researchRequired || !targetState.researchComplete) {
    return `⚡ **State: RESEARCH_PENDING** (Research Round: ${
...
3. Await user confirmation before modifying project code.`;
  ...
  return `⚡ **State: PLAN_ESTABLISHED** (Plan: v${
    targetState.planVersion || 1
  }, Confidence: ${targetState.planConfidence || "high"}).
1. Read \`${qp}\` using \`read\`.
...
export function buildSubquestLaunchPostCompactionDirective(
  ...
  const isSubQuest = Array.isArray(targetState.stack) &&
    targetState.stack.length > 1;
  const parentQuest = isSubQuest
    ? targetState.stack[targetState.stack.length - 2]
    : null;
...
function resolveFallbackStateAction(
  targetState: StoredState,
  activeQuest: string,
): string {
  const qp = questPath(targetState.questId);
...
function resolvePostCompactionStateAction(
  targetState: StoredState,
  activeQuest: string,
): string {
  const qp = questPath(targetState.questId);
  const logTail = targetState.reassessmentRequired
    ? getRecentLogTail(targetState)
    : "";
  if (targetState.reassessmentRequired) {
    return `⚡ **State: REASSESSMENT_PENDING** (Reason: ${
      targetState.reassessmentReason || "Unresolved contradiction"
...
This sub-quest is temporary work in service of the parent/root objective. Completing this sub-quest does not mean the overall objective is complete. After finishing it, return to the parent quest and continue its remaining work.`
  ...
  const specificAction = resolvePostCompactionStateAction(
    targetState,
    activeQuest,
  );
  const qp = questPath(targetState.questId);
...
export function createCompactionResumeObligation(
  ...
  return {
    ...
    checkpointQuestPath: tx.questPath ??
      (activeQuest ? questPath(activeQuest) : ""),
    attempts: sessionState.pendingResume?.compactionId === tx.id
      ? (sessionState.pendingResume.attempts || 0)
      : 0,
    createdAt: sessionState.pendingResume?.compactionId === tx.id
      ? sessionState.pendingResume.createdAt
      : Date.now(),
  };
}
...
export function recordResumeDeliverySuccess(
  ...
  if (
    ...
    targetState.activeTransaction.phase !== "inconsistent"
  ) {
    if (
      reason === "compaction-failure-fallback" ||
      targetState.activeTransaction.phase !== "failed"
    ) {
  ...
  logResumeTransition(
    "RESUME_DELIVERED",
    `resume delivered for ${activeQuest}`,
    {
      quest: activeQuest,
      compactionId,
...
export function recordResumeDeliveryFailure(
  ...
  if (!targetState.pendingResume) {
    targetState.pendingResume = {
      ...
      activeQuest,
      reason,
      checkpointSaveCount: targetState.activeTransaction?.checkpointSaveCount ??
        -1,
      checkpointHash: targetState.activeTransaction?.checkpointHash ?? "",
      checkpointQuestPath: targetState.activeTransaction?.questPath ??
        questPath(targetState.questId),
      attempts: 1,
      createdAt: Date.now(),
      lastAttemptAt: Date.now(),
    };
  } else {
    targetState.pendingResume.attempts =
      (targetState.pendingResume.attempts || 0) + 1;
  ...
  logResumeTransition(
    "RESUME_FAILED",
    `resume delivery failed for ${activeQuest}`,
    {
      quest: activeQuest,
      compactionId,
  ...
  reportAgentError(
    ...
    failureMessage,
    {
      ...
      requiredNextAction: `Read ${
        questPath(targetState.questId)
      }, reconcile the recovered state if necessary, and continue from the recorded Exact Next Action.`,
      details: {
...
export function dispatchCompactionResume(
  ...
  if (!activeQuest) {
    logDebug(
      "Quest Journal: dispatchCompactionResume skipped; no authoritative active quest.",
    );
    return false;
  }

  const compactionId = options.compactionId ||
    targetState.pendingResume?.compactionId ||
    targetState.activeTransaction?.id || targetState.activeCompactionId ||
    `cmp_${targetState.compactCount}`;
  ...
  const directiveText = resolveResumeDirectiveText(
    options.reason,
    activeQuest,
    targetState,
  );
  try {
    const hash = createHash("sha256").update(directiveText, "utf8").digest(
      "hex",
    ).slice(0, 12);
    ...
    try {
      ...
      writeFileSync(runCompPath, directiveText, "utf8");
    } catch (_e) {}
    try {
      logEvent("RESUME_DIRECTIVE_SENT", `resume directive sent`, {
        quest: activeQuest,
        hash,
        compactionId,
  ...
  logResumeTransition(
    "RESUME_ATTEMPTED",
    `attempting resume delivery for '${activeQuest}'`,
    {
      quest: activeQuest,
      compactionId,
      reason: options.reason,
...
export function retryPendingResume(
  pi: ExtensionAPI,
  ctx?: ExtensionContext,
): boolean {
  ...
  logResumeTransition(
    "RESUME_RETRIED",
    `retrying pending resume for '${
      activeQuest || targetState.pendingResume.activeQuest
    }'`,
    {
  ...
  if (!activeQuest) {
    logDebug(
      `Quest Journal: Pending resume for '${targetState.pendingResume.activeQuest}' cannot be retried because there is no authoritative active quest.`,
    );
    reportAgentError(
      pi,
      c,
      `[Quest Journal] A pending resume obligation exists for '${targetState.pendingResume.activeQuest}', but no authoritative active quest is currently set.\n\nUse /quest <name> to activate a quest or reconcile the session state.`,
      {
        ...
        requiredNextAction:
          "Set an active quest with /quest <name> to resume execution.",
        details: {
