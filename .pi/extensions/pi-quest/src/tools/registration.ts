...
import { QUEST_MARK_SAVED_SCHEMA, QUEST_UPDATE_STATE_SCHEMA } from "./validation.ts";

export function installArchiveTool(pi: ExtensionAPI) {
	const archiveHandler = withContext((_toolCallId: string, params: any, _signal: any, _onUpdate: any, ctx: ExtensionContext) =>
		executeArchiveTool(params, pi, ctx));

	pi.registerTool({
		name: "quest_archive",
		label: "Archive Quest",
		description: `Archive the active (or specified) quest into ${QUEST_ARCHIVE_DIR}/<qid>.zip and optionally trigger session context compaction.`,
		parameters: {
			type: "object",
			properties: {
				questName: {
...
export function installSubQuestTool(pi: ExtensionAPI) {
	const subquestHandler = withContext((_toolCallId: string, params: any, _signal: any, _onUpdate: any, ctx: ExtensionContext) =>
...
export function installMarkTool(pi: ExtensionAPI) {
	const markHandler = withContext((_toolCallId: string, params: any, _signal: any, _onUpdate: any, ctx: ExtensionContext) =>
	...
	const updateStateHandler = withContext((_toolCallId: string, params: any, _signal: any, _onUpdate: any, ctx: ExtensionContext) =>
