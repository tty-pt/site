...
export function handleResearchCompletion(
  ...
  const validation = validateResearchPrerequisites(
    provisionalMarkdown,
    params.planConfidence || state.planConfidence,
  ...
  if (!validation.valid) {
    ...
    logResearchTransition(
      "RESEARCH_REJECTED",
      "research completion rejected: missing or placeholder sections",
      {
        quest: targetName,
    ...
    reportAgentError(
      pi,
      ctx,
      `Research completion refused: Missing or placeholder sections: [${
        validation.missingSections.join(", ")
      }]${validation.confidenceIssue ? `; ${validation.confidenceIssue}` : ""}`,
      {
        code: QuestErrorCode.RESEARCH_REQUIRED,
        requiredNextAction:
          "Update the missing sections with verified discoveries and execution plan, then retry researchComplete.",
        details: {
          Quest: targetName,
    ...
    return ` (Note: researchComplete refused -- missing or placeholder: [${
      validation.missingSections.join(", ")
    }]${validation.confidenceIssue ? `; ${validation.confidenceIssue}` : ""})`;
  }
  ...
  if (!evidenceCheck.sufficient) {
    ...
    logResearchTransition(
      "RESEARCH_REJECTED",
      `research completion rejected: ${evidenceCheck.reason}`,
      {
        quest: targetName,
    ...
    reportAgentError(
      pi,
      ctx,
      `The research record is not sufficient to complete this research round. The extension has not observed enough fresh investigation since Research Round ${
        state.researchRound || 1
      } began. (${evidenceCheck.reason})`,
      {
        code: QuestErrorCode.RESEARCH_EVIDENCE_REQUIRED,
        requiredNextAction:
          "Perform targeted investigation of the relevant architecture/code paths using read/search tools, then update the quest state with the conclusions and retry researchComplete.",
  ...
  if (
    params.researchComplete === true && validation.valid &&
  ...
  const isAlreadyConfirmed = Array.isArray(state.confirmedQuests) &&
    state.confirmedQuests.includes(targetName);
...
export function applyEpistemicMetadataToUpdates(
  updates: Map<string, string>,
  targetState?: any,
): void {
  ...
  updates.set(
    "resolved reassessment version",
    String(s.resolvedReassessmentVersion || 0),
  );
  if (s.lastResearchAt) {
    updates.set("last research at", String(s.lastResearchAt));
  }
  if (s.lastPlanRevisionAt) {
    updates.set("last plan revision at", String(s.lastPlanRevisionAt));
  }
  if (s.reassessmentRequired) {
    updates.set(
      "reassessment status",
      `REQUIRED (v${s.reassessmentVersion || 1}) - ${
    ...
    updates.set(
      "reassessment status",
      `RESOLVED (v${s.resolvedReassessmentVersion || 0})`,
    );
  }
  if (s.reassessmentEvidence) {
    updates.set("reassessment evidence", s.reassessmentEvidence);
  }
}
