...
import { computePlanReviewBoundaryKey, isPlanReviewValidForState } from "../../critical_agent.ts";
...
export function capturePreUpdatePlanningSnapshot(
	...
	const planRaw = sections.get("plan")?.body || sections.get("detailed multi-stage execution plan")?.body || "";
	const revisionsRaw = sections.get("plan revisions")?.body || sections.get("plan revision history")?.body || targetState.lastPlanRevisionsText || "";
	...
	const boundaryKey = computePlanReviewBoundaryKey(targetName, planVersion, normalizedPlan, normalizedRevisions);
