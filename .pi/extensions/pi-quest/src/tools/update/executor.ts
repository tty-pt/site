...
function syncQuestIdentity(targetName: string, pi: ExtensionAPI, ctx: ExtensionContext): void {
	...
	const isNewIdentity = !state.active || state.pendingRootQuest || targetName !== state.active;
	if (isNewIdentity) {
		state.active = targetName;
...
export async function ensureQuestFileExists(targetName: string, goal = ""): Promise<string> {
	...
	if (!isRootQuest(state)) {
		...
		if (await fileExists(childPath) || state.active === targetName || (Array.isArray(state.stack) && state.stack.includes(targetName) && state.stack[0] !== targetName)) {
			path = childPath;
		}
	}
	const originalReq = state.pendingRootRequest || (state.prompts && state.prompts.length > 0 ? state.prompts[0] : "");
...
async function maybeTriggerPlanReview(
	...
	const hasReviewCapability = isSubagentToolRegistered(pi, ctx) || Boolean(getCustomSubagentRunner());
	if (!isRootQuest(targetState) || !hasReviewCapability || !postPlanning.hasActionablePlan) return "";

	const isMaterialPlanChange = prePlanning.boundaryKey !== postPlanning.boundaryKey;
	const isInitialResearchCompletion = (params?.researchComplete === true || (!prePlanning.researchComplete && targetState.researchComplete)) && (!targetState.lastPlanReviewApproval && !targetState.lastPlanReviewBoundaryKey);
	const isFirstPlanDraft = !prePlanning.hasActionablePlan && postPlanning.hasActionablePlan && (!targetState.lastPlanReviewApproval && !targetState.lastPlanReviewBoundaryKey);

	const priorReviewRejected = targetState.lastCriticalReview &&
		(targetState.lastCriticalReview.verdict === "REVISE" || targetState.lastCriticalReview.verdict === "FAIL" || targetState.lastCriticalReview.verdict === "UNCERTAIN");
	...
	const planRevRes = await requestPlanReview(pi, ctx, targetState.active || "", { boundaryKey: postPlanning.boundaryKey });
	let planReviewNote = "";
	if (planRevRes?.review?.verdict === "APPROVE" || planRevRes?.review?.verdict === "PASS") {
		targetState.researchComplete = true;
		targetState.researchRequired = false;
		const isAlreadyConfirmed = Array.isArray(targetState.confirmedQuests) && targetState.confirmedQuests.includes(targetState.active || "");
		targetState.awaitingUserConfirmation = isRootQuest(targetState) && !isAlreadyConfirmed;
		...
		planReviewNote = ` [Adversarial Plan Review: ${planRevRes.review.verdict} - Findings delivered to main agent]`;
	} else if (planRevRes?.error && !planRevRes.inProgress && !planRevRes.skipped) {
		targetState.researchComplete = false;
		targetState.researchRequired = true;
		targetState.awaitingUserConfirmation = false;
...
function logStateUpdate(
	...
	const completedCount = Array.isArray(params.completed) ? params.completed.length : (params.completed ? 1 : undefined);
	const remainingCount = Array.isArray(params.remaining) ? params.remaining.length : (params.remaining ? 1 : undefined);
	const filesMod = Array.isArray(params.filesModified || params.filesTouched)
		? (params.filesModified || params.filesTouched).join(",")
		: (typeof (params.filesModified || params.filesTouched) === "string" ? (params.filesModified || params.filesTouched) : undefined);
	...
	logStateUpdateTransition("STATE_UPDATE_ACCEPTED", `state updated for ${targetName}`, {
		...
		testStatus: typeof params.testStatus === "string" ? params.testStatus : undefined,
		activeGate: targetState.reassessmentRequired ? "REASSESSMENT_PENDING" : (targetState.researchRequired || !targetState.researchComplete ? "RESEARCH_PENDING" : (targetState.awaitingUserConfirmation ? "CONFIRMATION_PENDING" : "IMPLEMENTATION_ALLOWED")),
	});
}
...
export async function executeUpdateStateTool(params: any, pi: ExtensionAPI, ctx: ExtensionContext) {
			...
			if (allowsInitialDraftWrite) {
				...
				if ((state as any).reassessmentRequired) {
					...
					return {
						...
						details: { error: "draft_not_approved", success: false, shard: "draft", boundaryKey, hash, quest: draftSlug, gateBlocked: true, code: "REVIEW_COALESCENCE_PENDING", dpLen, evidence },
					};
				}
				...
				return {
					content: [{ type: "text", text: msg }],
					details: { error: "draft_not_approved", success: false, shard: "draft", boundaryKey, hash, quest: draftSlug, dpLen, evidence },
				};
			}
		}
	}
	...
	try {
		...
		const targetState = getState(ctx);
		const prePlanning = capturePreUpdatePlanningSnapshot(targetName, content, targetState);
