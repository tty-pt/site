...
export function validateSubquestParams(
  ...
  | { name: string; goal: string; parentName: string; switchNow: boolean }
  | null {
  ...
  if (!name) {
    reportAgentError(
      ...
      "Sub-quest creation failed: Sub-quest name or goal description is required.",
      {
        ...
        requiredNextAction:
          "Provide a descriptive sub-quest name or goal when calling quest_subquest.",
      },
    );
    return null;
  }
  if (!goal) {
    reportAgentError(
      ...
      "Sub-quest creation failed: Sub-quest goal description is required.",
      {
        code: QuestErrorCode.SUBQUEST_FAILURE,
        requiredNextAction:
          "Provide a non-empty goal description when calling quest_subquest.",
      },
    );
    return null;
  }
...
export const QUEST_MARK_SAVED_SCHEMA = {
  ...
  properties: {
    name: {
      type: "string",
      description:
        "Optional quest name/slug if setting or marking for the first time.",
    },
  },
...
export const QUEST_UPDATE_STATE_SCHEMA = {
  ...
  properties: {
    ...
    status: {
      type: "string",
      description:
        "Current status description (e.g. 'Phase 1 Complete - tests passing').",
    },
    findings: {
      type: "array",
      items: { type: "string" },
    ...
    planConfidenceReason: {
      type: "string",
      description:
        "Justification for the confidence level (verified execution paths, validated assumptions, remaining uncertainties).",
    },
    reassessmentConclusion: {
      type: "string",
      description:
        "What the fresh investigation established about the triggering contradiction, whether the previous assumption was validated/invalidated/reformulated, and whether the current plan survived or changed.",
    },
    planRevisions: {
      type: "array",
      items: { type: "string" },
    ...
    executionSnapshot: {
      type: "string",
      description:
        "Comprehensive execution snapshot containing objective, completed, in progress, discoveries, decisions, files, test status, remaining work, and exact next action.",
    },
    exactNextAction: {
      type: "string",
      description:
        "Concrete next action to be performed immediately by a fresh agent.",
    },
    nextAction: {
      type: "string",
    ...
    resumeContext: {
      type: "string",
      description:
        "Concise briefing giving the next agent iteration complete context.",
    },
    researchComplete: {
      type: "boolean",
    ...
    reassessmentComplete: {
      type: "boolean",
      description:
        "Whether pending reassessment has been completed and resolved.",
    },
    allowLowConfidence: {
      type: "boolean",
      description:
        "Allow completing research even if plan confidence is low (requires justification).",
    },
    planVersion: {
      type: "number",
