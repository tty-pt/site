...
export function formatMarkSavedResponse(
	targetName: string,
	res: { count: number; hash?: string; consistency?: { consistent: boolean; issues: string[] } },
...
export function formatUpdateStateResponse(
	...
	path: string,
	params: any,
	saveRes: { count: number; hash?: string; consistency?: { consistent: boolean; issues: string[] } },
	planVersion: number,
	researchNote: string,
	...
	return {
		content: [
			{
				type: "text",
				text: `Successfully updated quest state for **${targetName}** at \`${path}\` (gen #${saveRes.count}, hash: ${saveRes.hash || ""}, plan v${planVersion || 1}).${researchNote}${reassessmentNote}${consistencyNote}`,
			},
		],
		details: {
			quest: targetName,
