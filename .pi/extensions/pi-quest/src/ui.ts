...
export function updateUIStatus(ctx?: ExtensionContext) {
  ...
  if (c?.hasUI) {
    ...
    if (
      (state.active || state.activeDraft || state.pendingRootQuest) &&
      !state.questId
    ) {
      try {
        state.questId = generateQuestId();
      } catch (_e) {}
    }
...
export function buildSessionAwarenessBlock(ctx: ExtensionContext): string {
  ...
  const awarenessKey = `${ctx.cwd}|${branch || ""}|${
    state.questId || ""
  }|${activeKey}|${stackKey}|${fresh ? "fresh" : "dirty"}|${guidelineFp}`;
  const isSteadyAwareness = !state.pendingRootQuest &&
    !state.researchRequired && !state.reassessmentRequired && (() => {
      try {
  ...
  if (
    isSteadyAwareness && lastAwarenessKey === awarenessKey && lastAwarenessValue
  ) {
  ...
  if (state.activeDraft) {
    lines.push(
      `- Draft: \`.pi/quest/future/${state.activeDraft}.md\` (id: ${
        state.questId || "provisional"
      })`,
    );
  }
  ...
  if (state.active) {
    ...
    const stackInfo = state.stack && state.stack.length > 1
      ? ` | LIFO stack: [${state.stack.join(" → ")}]`
      : "";
    lines.push(
      `- Active quest: \`${questPath(state.questId)}\` [${hier}] (${
        fresh ? "fresh" : "SAVE PENDING - update it before compaction"
      }${stackInfo}); manage with /quest, /subquest, /quests, /quest-economy.`,
    );
  } else if (state.pendingRootQuest || state.activeDraft) {
    const draftPath = state.activeDraft
      ? `.pi/quest/future/${state.activeDraft}.md`
      : `.pi/quest/future/<draft>.md`;
    lines.push(
      `- Active quest: [PROVISIONAL ROOT INITIALIZATION] Research required to establish quest identity and plan. Use read/search/bash tools to investigate, determine a concise semantic quest name (e.g. 'persistent-agent-research', 'oauth-login-flow'), and call quest_update_state to initialize the durable quest with your research findings. Original user request is captured in session state. Draft: \`${draftPath}\` id: \`${
        state.questId || "provisional"
      }\`.`,
    );
  } else {
    lines.push(
      "- Active quest: none (substantive requests will automatically receive a persistent quest context in .pi/quest/current/<qid>/).",
    );
  }
  ...
  if (isSteadyAwareness) {
    lines.push(
      ...
      `Guidelines: see AGENTS.md (${
        guidelineFp ? "cached" : "none"
      }) — full invariants already injected via system prompt; compact mode.`,
    );
  } else {
  ...
  if (notes) {
    lines.push("", `## Standing project notes (\`${NOTES_FILE}\`)`, "", notes);
  }
