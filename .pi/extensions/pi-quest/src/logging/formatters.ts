...
export function formatContextFields(context?: QuestLogContext): string {
  ...
  for (const [k, v] of Object.entries(context)) {
    if (
      !priorityKeys.includes(k) && k !== "quest" && v !== undefined &&
      v !== null
    ) {
...
export function formatLogEntry(
  ...
  const questName = sanitizeLogString(
    context?.quest !== undefined ? context.quest : "",
    60,
  );
...
export function parseLogEntry(line: string): QuestLogEntry | null {
  ...
  if (parts.length === 3) {
    ...
    quest = questPart.startsWith("quest=")
      ? questPart.slice("quest=".length)
      : questPart;
