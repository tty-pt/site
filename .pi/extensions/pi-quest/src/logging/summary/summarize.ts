...
function buildFormattedLines(
  ...
  let successfulCompactions = 0,
    failedCompactions = 0,
    inconsistentCompactions = 0;
  for (const cmp of s.compactionsMap.values()) {
  ...
  for (const res of s.resumesMap.values()) {
  ...
  if (s.hasUnresolvedError) {
    terminalVerdictReason = `Unresolved error: ${s.lastError || "unknown"}`;
  } else if (s.hasCriticalReviewFailure) {
    terminalVerdictReason = `Critical review failed without approval`;
  } else if (formattedExtra.lastTestStatus === "FAILED") {
    terminalVerdictReason = `Unrecovered test failure: ${
      formattedExtra.lastFailureReason || formattedExtra.lastFailedCommand ||
      "test failure"
    }`;
  } else if (formattedExtra.unrecoveredFailures.length > 0) {
    terminalVerdictReason = `Unrecovered failure: ${
      formattedExtra.unrecoveredFailures[0].type
    } (${formattedExtra.unrecoveredFailures[0].reason || "unknown"})`;
  } else if (formattedExtra.lastTestStatus === "PASSED") {
    terminalVerdictReason = `All tests verified clean (${
      formattedExtra.lastPassedCommand || "tests passed"
    }), zero unresolved errors`;
  } else if (s.implementationAllowedCount > 0) {
    terminalVerdictReason =
      `Implementation completed with zero unresolved errors`;
  ...
  const lines: string[] = [
    ...
    `Dialogue Turns: ${s.dialogueCount} (thoughts: ${s.thoughtCount}) Sessions: ${
      Array.from(s.piSessionIds).join(", ") || s.opencodeSessionId || "(none)"
    }`,
    ...
    `Tests: ${
      formattedExtra.lastTestStatus === "PASSED"
        ? `PASSED (${formattedExtra.lastPassedCommand || "verified"})`
        : formattedExtra.lastTestStatus === "FAILED"
        ? `FAILED (${
          formattedExtra.lastFailureReason || formattedExtra.lastFailedCommand
        })`
        : "NOT_RUN"
  ...
  if (s.deadlockWarnings.length > 0) {
    lines.push(`Flow Warnings / Deadlocks (${s.deadlockWarnings.length}):`);
    for (const w of s.deadlockWarnings.slice(0, 5)) lines.push(`  - ${w}`);
  }
  if (s.failures.length > 0) {
    lines.push(`Recorded Failures (${s.failures.length}):`);
    for (const f of s.failures.slice(0, 5)) {
      const recStr = f.recovered
        ? ` [RECOVERED via ${f.recoveryAction || "subsequent action"}]`
        : " [UNRECOVERED]";
      lines.push(
        `  - [${f.type}] ${f.code ? `(${f.code}) ` : ""}${
          f.reason || "unknown"
        }${recStr}`,
      );
    }
    if (s.failures.length > 5) {
      lines.push(`  ... and ${s.failures.length - 5} more failures`);
    }
  }
  if (s.hasUnresolvedError) {
    lines.push(`Status: UNRESOLVED ERROR (${s.lastError || "unknown"})`);
  } else if (
    formattedExtra.unrecoveredFailures.length > 0 ||
    formattedExtra.lastTestStatus === "FAILED"
  ) lines.push(`Status: UNRECOVERED FAILURES (${terminalVerdictReason})`);
...
export function summarizeQuestJournalLog(rawOrQid: string): QuestRunSummary {
  ...
  for (const entry of entries) {
    ...
    if (entry.context.filesModified) {
      for (const f of entry.context.filesModified.split(",")) {
        const t = f.trim();
        if (t) s.modifiedFilesSet.add(t);
      }
    } else if (
      entry.context.path &&
      (entry.context.operation === "success" ||
        entry.type === "IMPLEMENTATION_COMPLETED")
    ) {
      if (
        entry.context.tool === "edit" || entry.context.tool === "write" ||
        entry.type === "IMPLEMENTATION_COMPLETED"
      ) s.modifiedFilesSet.add(entry.context.path);
    }
    if (entry.context.completed) {
      s.completedTasksSet.add(entry.context.completed);
    }
    if (
      entry.type === "TURN_START" || entry.type === "TURN_END" ||
      entry.type === "TOOL_ACTIVITY" || entry.type === "TEST_FAILED" ||
      entry.type === "TEST_PASSED" || entry.type === "BUILD_FAILED" ||
      entry.type === "BUILD_PASSED" || entry.type === "GATE_BLOCKED" ||
      entry.type === "GATE_OPENED" || entry.type === "REASSESSMENT_REQUIRED" ||
      entry.type === "REASSESSMENT_COMPLETED" ||
      entry.type === "RESEARCH_COMPLETED" ||
      entry.type === "STATE_UPDATE_ACCEPTED"
    ) {
      s.causalChain.push({
        turn: entry.context.turn,
        correlationId: entry.context.correlationId,
        phase: entry.context.phase || majorPhase || undefined,
        intent: entry.context.intent,
        action: entry.context.action || entry.context.command ||
          entry.context.tool || entry.type,
        result: entry.context.result || entry.context.operation ||
          entry.context.outcome,
        consequence: entry.context.consequence ||
          (entry.type === "GATE_BLOCKED"
            ? "OPERATION_BLOCKED"
            : entry.type === "REASSESSMENT_REQUIRED"
            ? "REASSESSMENT_TRIGGERED"
            : undefined),
        recoveryFor: entry.context.recoveryFor,
    ...
    if (entry.context.recoveryFor) {
      ...
      if (priorIdx !== undefined && s.failures[priorIdx]) {
        s.failures[priorIdx].recovered = true;
        s.failures[priorIdx].recoveryAction = `${entry.type} (${
          entry.context.tool || entry.context.command || entry.message
        })`;
      }
    }
    routeEntry(entry, s);
  ...
  if (s.hasUnresolvedError) {
    ...
    terminalVerdictReason = `Unrecovered test failure: ${
      s.lastFailureReason || s.lastFailedCommand || "test failure"
    }`;
  } else if (s.unrecoveredFailures.length > 0) {
    terminalVerdictReason = `Unrecovered failure: ${
      s.unrecoveredFailures[0].type
    } (${s.unrecoveredFailures[0].reason || "unknown"})`;
  } else if (s.lastTestStatus === "PASSED") {
    terminalVerdictReason = `All tests verified clean (${
      s.lastPassedCommand || "tests passed"
    }), zero unresolved errors`;
  } else if (s.implementationAllowedCount > 0) {
    terminalVerdictReason =
      `Implementation completed with zero unresolved errors`;
  ...
  const formattedLines = buildFormattedLines(s, {
    lastTestStatus: s.lastTestStatus,
    lastFailureReason: s.lastFailureReason,
    lastPassedCommand: s.lastPassedCommand,
    lastFailedCommand: s.lastFailedCommand,
  ...
  return {
    ...
    testVerification: {
      ...
      lastTestCommand: s.lastTestCommand,
      lastPassedCommand: s.lastPassedCommand,
      lastFailedCommand: s.lastFailedCommand,
      lastFailureReason: s.lastFailureReason,
      totalTestsRun: s.totalTestsRun,
      testsPassed: s.testsPassedCount,
      testsFailed: s.testsFailedCount,
    },
    implementationSummary: {
      totalAttempts: s.implementationAttempts,
      allowed: s.implementationAllowedCount,
      blocked: s.implementationBlockedCount,
      blockedGates: Array.from(s.blockedGates),
      completedTasks: Array.from(s.completedTasksSet),
      modifiedFiles: Array.from(s.modifiedFilesSet),
    },
