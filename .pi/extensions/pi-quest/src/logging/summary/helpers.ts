...
export function readQuestLog(
  qidOrPath?: string | ExtensionContext | null,
  maxEntries?: number,
): string {
  let targetPath: string;
  if (!qidOrPath) targetPath = getQuestLogPath();
  else if (typeof qidOrPath !== "string") {
    targetPath = getQuestLogPath(qidOrPath);
  } else if (qidOrPath.endsWith(".log") || qidOrPath.includes("/")) {
    targetPath = qidOrPath;
...
export function parseQuestLogEntries(rawOrPath: string): QuestLogEntry[] {
  ...
  if (
    ...
    (raw.endsWith(".log") || !raw.includes(" | "))
  ) raw = readQuestLog(rawOrPath);
...
export function clearQuestLog(
  qidOrCtx?: string | ExtensionContext | null,
): void {
  let p = "";
  if (
    typeof qidOrCtx === "string" &&
    (qidOrCtx.endsWith(".log") || qidOrCtx.includes("/"))
  ) p = qidOrCtx;
