...
import {
  extractParentFromQuest,
  extractSubQuestsFromQuest,
...
async function collectQuestInfos(
  projectRoot: string,
  requestedQuestId: string | null,
): Promise<QuestFileInfo[]> {
      ...
      if (existsSync(cDir)) {
        ...
        for (const e of entries) {
          if (e.isDirectory()) {
            ...
            try {
              const subEntries = await readdir(nestedDir, {
                withFileTypes: true,
              });
              for (const subFile of subEntries) {
                if (
                  subFile.isFile() && subFile.name.endsWith(".md") &&
                  subFile.name !== "summary.md"
                ) {
                  ...
                  try {
                    ...
                    const declaredSubquests = extractSubQuestsFromQuest(
                      content,
                    );
                    ...
                    const slug = headerMatch
                      ? headerMatch[1].trim()
                      : (subFile.name === "quest.md"
                        ? e.name
                        : subFile.name.replace(/\.md$/, ""));
                    allQuestInfos.push({
                      slug,
                      path: nestedQuest,
                      parent,
  ...
  allQuestInfos.sort((a, b) => {
    if (requestedQuestId) {
      if (a.questId === requestedQuestId && b.questId !== requestedQuestId) {
        return -1;
      }
      if (b.questId === requestedQuestId && a.questId !== requestedQuestId) {
        return 1;
      }
    }
...
> {
  ...
  if (!resolvedRoot && (state?.active || state?.stack?.length)) {
    const activeCandidate = state?.active ||
      (state.stack && state.stack.length > 0 ? state.stack[0] : null);
    if (activeCandidate) {
  ...
  if (!resolvedRoot && rootSlugs.length > 0) {
    ...
    resolutionMethod = rootSlugs.length === 1
      ? "single_unparented_quest"
      : "most_recent_unparented_quest";
    confidence = rootSlugs.length === 1 ? "high" : "medium";
    if (rootSlugs.length > 1) {
      ambiguityDetails = `Multiple potential root quests found: ${
        rootSlugs.join(", ")
      }. Selected most recent: ${resolvedRoot}.`;
    }
  }

  if (resolvedRoot) {
    if (
      state?.active && state.active !== resolvedRoot &&
      questMap.has(state.active)
    ) {
      resolvedActiveSub = state.active;
    } else if (
      selectedRunLog?.activeQuest &&
      selectedRunLog.activeQuest !== resolvedRoot &&
      questMap.has(selectedRunLog.activeQuest)
    ) {
...
export async function resolveActiveRunHierarchy(
  ...
  const { selectedRunLog } = await discoverRunLogs(
    projectRoot,
    requestedQuestId || undefined,
  );
  ...
  if (selectedRunLog) {
    ...
    targetLogPath = resolve(
      searchCurrentDir,
      requestedQuestId || "default",
      "execution.log",
    );
  ...
  const { questMap, childrenMap, effectiveParentMap } = buildQuestMaps(
    allQuestInfos,
  );
  ...
  const { rootSubquestsMap, subquestToRootMap } = computeDescendantMaps(
    childrenMap,
    rootSlugs,
  );

  const rootResolution = await resolveRoot(
    selectedRunLog,
    questMap,
    rootSlugs,
    rootSubquestsMap,
  ...
  const rootPath = resolvedRoot && questMap.has(resolvedRoot)
    ? questMap.get(resolvedRoot)!.path
    : null;
  ...
  if (rootPath && (await fileExists(rootPath))) {
    try {
      ...
      questHash = createHash("sha256").update(rootContent, "utf8").digest("hex")
        .slice(0, 16);
    } catch {}
  }
  ...
  const initialPrompt = rootInfo?.initialPrompt ||
    (state?.prompts && state.prompts.length > 0 ? state.prompts[0] : null);

  // B2/B3 observability: draft/future/compaction flags — disk-authoritative
  let draftCaptured = false;
  let futureCount = 0;
  try {
    if (state?.activeDraft) draftCaptured = true;
    // Disk-authoritative count: FUTURE_DIR + future-archive for resolved quest
    ...
    if (existsSyncFs(futDir)) {
      ...
      futureCount += ents.filter((e) =>
        e.isFile() && e.name.endsWith(".md")
      ).length;
    }
    const qidForArchive = resolvedQuestId || requestedQuestId ||
      state?.questId || null;
    if (qidForArchive) {
      const archDir = resolve(
        projectRoot,
        QUEST_CURRENT_DIR,
        qidForArchive,
        "future-archive",
      );
      if (existsSyncFs(archDir)) {
        const archEnts = await readdir(archDir, { withFileTypes: true }).catch(
          () => [],
        );
        futureCount += archEnts.filter((e) =>
          e.isFile() && e.name.endsWith(".md")
        ).length;
      }
    }
  ...
  try {
    const pid = state?.pendingResume?.compactionId ||
      state?.activeTransaction?.id;
    if (pid) {
      compactionResumeHash = createHash("sha256").update(String(pid), "utf8")
        .digest("hex").slice(0, 12);
    } else {
      // Fallback: disk check — fresh process may have compaction-resume.txt without in-memory state
      const qidForComp = resolvedQuestId || requestedQuestId ||
        state?.questId || null;
      if (qidForComp) {
        ...
        if (existsSyncFs(compPath)) {
          try {
            const content = await readFile(compPath, "utf8");
            compactionResumeHash = createHash("sha256").update(content, "utf8")
              .digest("hex").slice(0, 12);
          } catch {
            compactionResumeHash = createHash("sha256").update(
              "compaction-resume",
              "utf8",
            ).digest("hex").slice(0, 12);
          }
  ...
  const semanticSummaryEnabled =
    typeof state?.semanticSummaryEnabled === "boolean"
      ? state.semanticSummaryEnabled
      : false;
  const thoughtLoggingEnabled =
    typeof state?.thoughtLoggingEnabled === "boolean"
      ? state.thoughtLoggingEnabled
      : false;
  let filteredCount: number | undefined = undefined;
  let opencodeSessionId: string | null = null;
  let startMs: number | null = null;
  let elapsedMaxMs: number | null = null;
  // Typed fallback from session state — disk/logSummary remains authoritative when present
  try {
    filteredCount = 0;
    const ctx = getActiveContext();
    const sid = getSessionId(ctx);
    if (sid) opencodeSessionId = sid;
    const sMs = sid ? sessionStartMap.get(sid) ?? null : null;
    if (sMs !== null) startMs = sMs;
    if (startMs !== null) elapsedMaxMs = Date.now() - startMs;
    else elapsedMaxMs = 0;
  } catch {}
