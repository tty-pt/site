...
export function extractJournalStateFromContent(content: string): ExtractedJournalState {
	...
	const remainingRaw = getSec("remaining work") || getSec("remaining tasks") || getSec("remaining");
	...
	const planRaw = getSec("plan") || getSec("detailed multi-stage execution plan");
	const plan = planRaw ? planRaw.split(/\r?\n/).map((l) => l.trim()).filter((l) => l.length > 0) : [];

	const filesRaw = getSec("files touched") || getSec("files modified");
	const filesTouched = filesRaw ? filesRaw.split(/\r?\n/).map((l) => l.trim()).filter((l) => l.length > 0) : [];
...
export async function generateRunSummary(
	...
	const getSectionText = (key: string): string => {
		...
		if (!body || body === "-" || body === "- [ ]" || body.startsWith(">")) return "";
		return body;
	};
	...
	if (!completed) {
		if (logSummaryInfo && logSummaryInfo.implementationSummary.completedTasks.length > 0) {
			completed = logSummaryInfo.implementationSummary.completedTasks.map((t) => (t.startsWith("- ") ? t : `- ${t}`)).join("\n");
		} else if (logSummaryInfo && logSummaryInfo.implementationSummary.modifiedFiles.length > 0) {
			completed = `- Modified files: ${logSummaryInfo.implementationSummary.modifiedFiles.join(", ")}`;
		} else if (logSummaryInfo && logSummaryInfo.implementationAllowedCount === 0 && logSummaryInfo.implementationAttempts > 0) {
			completed = "- No implementation modifications completed (all attempts blocked by gates).";
		} else {
	...
	if (logSummaryInfo) {
		if (logSummaryInfo.testVerification.status === "PASSED") {
			testStatus = `- Tests verified clean: ${logSummaryInfo.testVerification.lastPassedCommand || "all tests passed"} (total test runs: ${logSummaryInfo.testVerification.totalTestsRun})`;
		} else if (logSummaryInfo.testVerification.status === "FAILED") {
			testStatus = `- Test / build failure: ${logSummaryInfo.testVerification.lastFailureReason || logSummaryInfo.testVerification.lastFailedCommand || "test failure"} (failed tests: ${logSummaryInfo.testVerification.testsFailed})`;
		} else {
			const secTest = getSectionText("test / build status");
			testStatus = secTest && !secTest.toLowerCase().includes("verified clean") ? secTest : "- No test or build verification commands executed during run.";
		}
	} else {
		testStatus = getSectionText("test / build status") || "- No test or build verification recorded.";
	}
	...
	if (logSummaryInfo) {
		lines.push(`- Implementation attempts: ${logSummaryInfo.implementationAttempts} (allowed: ${logSummaryInfo.implementationAllowedCount}, blocked: ${logSummaryInfo.implementationBlockedCount})`);
		if (logSummaryInfo.implementationSummary.modifiedFiles.length > 0) {
			lines.push(`- Modified files: ${logSummaryInfo.implementationSummary.modifiedFiles.join(", ")}`);
		}
	} else {
	...
	if (logSummaryInfo && (logSummaryInfo.failureCount > 0 || logSummaryInfo.blockedGates.length > 0)) {
		...
		if (logSummaryInfo.failures.length > 0) {
			...
			if (logSummaryInfo.failures.length > 8) {
				lines.push(`- ... and ${logSummaryInfo.failures.length - 8} more failures`);
			}
		}
	} else {
	...
	lines.push(
		...
		logSummaryInfo?.terminalVerdictReason ? `- **Verdict Reason**: ${logSummaryInfo.terminalVerdictReason}` : `- **Verdict**: ${finalStatus}`,
		"",
		"## Remaining Work",
		remaining,
		"",
...
export async function generateDiagnosticReport(
	...
	if (hierarchy.logExists && (await fileExists(hierarchy.logPath))) {
		try {
			logSummaryInfo = summarizeQuestJournalLog(hierarchy.logPath);
			const logLines = (await readFile(hierarchy.logPath, "utf8")).split(/\r?\n/).filter((l) => l.trim().length > 0);
			logStats.eventsCount = logLines.length;
		} catch {}
	}

	let journalState: ExtractedJournalState | undefined;
	if (hierarchy.activeRootQuestPath && (await fileExists(hierarchy.activeRootQuestPath))) {
		try {
	...
	const manifest = generateRunManifest(hierarchy, undefined, projectRoot, { status: options?.status, logSummaryInfo });
	const summary = await generateRunSummary(hierarchy, projectRoot, { status: options?.status, logSummaryInfo });
