...
export async function computeStagedFilesHash(
  stagingDir: string,
): Promise<string> {
...
export async function inspectZipEntries(zipPath: string): Promise<string[]> {
  ...
  if (res.exitCode !== 0) {
    throw new Error(
      `Failed to inspect zip entries: ${res.stderr || res.stdout}`,
    );
  }
...
export async function createRunDirectory(
  ...
  const promptContent = hierarchy.initialPrompt ||
  ...
  try {
    ...
    if (existsSync(futSrc)) {
      ...
      if (futEntries.length > 0) {
        ...
        for (const e of futEntries) {
          if (e.isFile() && e.name.endsWith(".md")) {
            try {
              await copyFile(resolve(futSrc, e.name), resolve(futDest, e.name));
            } catch {}
          }
        }
      }
      ...
      if (archEntries.length > 0) {
        ...
        for (const e of archEntries) {
          if (e.isFile() && e.name.endsWith(".md")) {
            try {
              await copyFile(
                resolve(archSrc, e.name),
                resolve(archDest, e.name),
              );
            } catch {}
          }
        }
    ...
    const compSrc = resolve(
      projectRoot,
      QUEST_CURRENT_DIR,
      qId,
      "compaction-resume.txt",
    );
    if (existsSync(compSrc)) {
      try {
        await copyFile(compSrc, resolve(runDir, "compaction-resume.txt"));
      } catch {}
    } else {
    ...
    if (existsSync(promptsSrc)) {
      try {
        await copyFile(promptsSrc, resolve(runDir, "draft-prompts.jsonl"));
      } catch {}
    }
  } catch {}

  const summaryContent = options?.summaryContent ||
    (await generateRunSummary(hierarchy, projectRoot, {
      status: options?.status,
      logSummaryInfo,
      questContent: options?.finalizedQuestContent,
    }));
  ...
  const manifestContent = generateRunManifest(
    hierarchy,
...
async function copyExtensionDirectory(
  sourceDir: string,
  targetDir: string,
): Promise<void> {
...
export async function verifyRunArchive(
  ...
  const hasManifest = entries.includes("run/manifest.txt") ||
    entries.includes("manifest.txt");
  if (!hasManifest) {
    errors.push("Manifest file is missing from run archive (run/manifest.txt)");
  }

  const hasInitialPrompt = entries.includes("run/initial-prompt.txt") ||
    entries.includes("initial-prompt.txt");
  if (!hasInitialPrompt) {
    errors.push(
      "Initial prompt file is missing from run archive (run/initial-prompt.txt)",
    );
  }

  const hasSummary = entries.includes("run/summary.md") ||
    entries.includes("summary.md");
  if (!hasSummary) {
    errors.push(
      "Summary markdown file is missing from run archive (run/summary.md)",
    );
  }
  ...
  if (expected.logExists) {
    ...
    if (
      !entries.includes(logEntry) && !entries.includes(altEntry1) &&
      !entries.includes(altEntry2)
    ) {
      errors.push(
        `Execution log exists on disk but is missing from archive (${logEntry})`,
      );
    }
  }
  ...
  if (nestedZips.length > 0) {
    errors.push(
      `Archive contains recursive zip file(s): ${nestedZips.join(", ")}`,
    );
  }
  ...
  if (nodeModules.length > 0) {
    errors.push(
      `Archive contains node_modules: ${nodeModules.slice(0, 3).join(", ")}`,
    );
  }
...
export async function verifyDiagnosticZip(
  ...
  if (
    !entries.includes("pi-quest/package.json") &&
    !entries.includes("pi-quest/index.ts")
  ) {
    errors.push(
      "pi-quest extension source files are missing from bundle (pi-quest/package.json)",
    );
  }
  ...
  if (!hasInitialPrompt) {
    errors.push(
      "Initial prompt file is missing from archive (run/initial-prompt.txt)",
    );
  }
  ...
  if (!hasSummary) {
    errors.push(
      "Summary markdown file is missing from archive (run/summary.md)",
    );
  }
  ...
  if (needsFuture) {
    const hasFuture = entries.some((e) =>
      (e.includes("future/") || e.includes("future-archive/")) &&
      e.endsWith(".md")
    );
    if (!hasFuture) {
      errors.push(
        "Future draft file missing from bundle (run/future/*.md) despite activeDraft/draftCaptured",
      );
    }
  }
  const hasResumeHash = expected.compactionResumeHash;
  if (hasResumeHash) {
    const hasCompResume = entries.some((e) =>
      e.includes("compaction-resume.txt")
    );
    if (!hasCompResume) {
  ...
  if (staleArchives.length > 0) {
    errors.push(
      `Archive contains historical/stale archive files: ${
        staleArchives.join(", ")
      }`,
    );
  }
...
export async function createRunArchive(
  ...
  const hierarchy = options.hierarchy ||
    (await resolveActiveRunHierarchy(projectRoot, { questId: targetId }));
  ...
  const outputZipPath = resolve(
    options.outputZipPath || join(archiveDir, `${qId}.zip`),
  );

  const stagingDir = await mkdtemp(
    join(tmpdir(), `pi-quest-run-archive-${qId}-`),
  );

  try {
    const currentRunDir = resolve(projectRoot, QUEST_CURRENT_DIR, qId);
    const targetDir =
      (options.finalizedLogContent || options.finalizedQuestContent ||
          !existsSync(currentRunDir))
        ? stagingDir
        : currentRunDir;
    ...
    if (targetDir !== stagingDir) {
      ...
      await copyFile(
        runDirRes.initialPromptPath,
        join(stagingDir, "initial-prompt.txt"),
      );
    ...
    const zipRes = await runShellCommand(
      "zip",
      ["-r", outputZipPath, "."],
      stagingDir,
    );
    if (zipRes.exitCode !== 0) {
      throw new Error(
        `Failed to create run archive zip: ${zipRes.stderr || zipRes.stdout}`,
      );
    }
    ...
    if (!options.skipVerification && !verification.valid) {
      throw new Error(
        `Run archive verification failed: ${verification.errors.join("; ")}`,
      );
    }
...
export async function createUnifiedBundleZip(
  ...
  const outputZipPath = resolve(
    options.outputZipPath || join(projectRoot, "pi-quest-bundle.zip"),
  );

  const targetId = options.questId;
  const hierarchy = await resolveActiveRunHierarchy(projectRoot, {
    questId: targetId,
  });
  const qId = targetId || hierarchy.questId || "quest";
  ...
  try {
    ...
    if (runDirRes.manifestPath !== join(runStagingDir, "manifest.txt")) {
      await copyFile(
        runDirRes.manifestPath,
        join(runStagingDir, "manifest.txt"),
      );
    }
    if (
      runDirRes.initialPromptPath !== join(runStagingDir, "initial-prompt.txt")
    ) {
      await copyFile(
        runDirRes.initialPromptPath,
        join(runStagingDir, "initial-prompt.txt"),
      );
    }
    ...
    if (
      await fileExists(runDirRes.logPath) &&
      runDirRes.logPath !== join(runStagingDir, "execution.log")
    ) {
    ...
    if (zipRes.exitCode !== 0) {
      throw new Error(
        `Failed to create bundle zip: ${zipRes.stderr || zipRes.stdout}`,
      );
    }
