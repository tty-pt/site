...
export function getImplementationBlockReason(targetState?: StoredState, ctx?: ExtensionContext): {
	blocked: boolean;
	code: QuestErrorCode;
	stateName: string;
	reason: string;
	...
	if (!s.active) {
		return { blocked: false, code: QuestErrorCode.IMPLEMENTATION_BLOCKED, stateName: "IDLE", reason: "", requiredAction: "" };
	}
	if (s.pendingSubquestResume && s.pendingSubquestResume !== s.active) {
		return {
			blocked: true,
			...
			reason: `A pending sub-quest continuation exists for '${s.pendingSubquestResume}', but the current authoritative quest state does not establish that continuation as active or completed.`,
			requiredAction: "reconcile the quest hierarchy and durable child/parent state before continuing."
		};
	}
	if (s.activeTransaction && s.activeTransaction.phase === "inconsistent") {
		return {
			...
			reason: "The completed compaction transaction no longer matches the durable checkpoint that was prepared for this transaction.",
			requiredAction: `Investigate the discrepancy in ${questPath(s.questId)}, update the epistemic state, and complete reassessment via quest_update_state({ reassessmentComplete: true, reassessmentConclusion: "..." }) to reconcile the transaction before modifying project code.`
		};
	}
	if (s.activeTransaction && s.activeTransaction.phase === "failed") {
		return {
			...
			requiredAction: `Investigate the failure in ${questPath(s.questId)}, update the epistemic state, and complete reassessment via quest_update_state({ reassessmentComplete: true, reassessmentConclusion: "..." }) to reconcile the transaction before modifying project code.`
		};
	}
	if (s.compactionPending) {
		return {
			...
			requiredAction: "Wait for compaction to complete and deliver its autonomous resumption directive."
		};
	}
	...
	if (s.researchRequired || !s.researchComplete) {
		return {
			...
			requiredAction: `Perform targeted read/search investigation, update ${questPath(s.questId)} with verified understanding and plan, and call quest_update_state({ researchComplete: true }) with medium or high confidence.`
		};
	}
	...
	return { blocked: false, code: QuestErrorCode.IMPLEMENTATION_BLOCKED, stateName: "IMPLEMENTATION_ALLOWED", reason: "", requiredAction: "" };
}

export function syncImplementationPermission(targetState?: StoredState, ctx?: ExtensionContext): boolean {
	const s = targetState || state;
