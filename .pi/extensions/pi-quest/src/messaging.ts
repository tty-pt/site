...
import { logAgentMessageTransition, logEvent, tryLog } from "./logging.ts";
import {
...
export function logError(
  ...
  const errorCode = code ||
    (typeof err?.code === "string" ? err.code : undefined) || "ERROR";
...
export function sendInternalAgentMessage(
  ...
  logAgentMessageTransition(
    ...
    `agent message attempted (${deliverAs})`,
  ...
  if (typeof pi.sendMessage === "function") {
    try {
      ...
      logAgentMessageTransition(
        ...
        `agent message delivery failed (${deliverAs})`,
        { deliverAs, type, correlationId, error: err?.message },
      );
      logError(
        "Failed to send internal custom message",
        err,
        undefined,
  ...
  const marked = text.startsWith(INTERNAL_MESSAGE_PREFIX)
    ? text
    : `${INTERNAL_MESSAGE_PREFIX}\n${text}`;
  ...
  if (sent) {
    logAgentMessageTransition(
      "AGENT_MESSAGE_DELIVERED",
      `agent user message delivered (${deliverAs})`,
      { deliverAs, type, correlationId },
    );
  } else {
    logAgentMessageTransition(
      "AGENT_MESSAGE_FAILED",
      `agent user message failed (${deliverAs})`,
      { deliverAs, type, correlationId },
    );
  }
  return sent;
...
export function reportAgentError(
  ...
  const text = formatAgentErrorMessage(
    options.code,
    message,
    options.requiredNextAction,
    options.details,
  );
  const deliverAs = options.deliverAs || "followUp";
  const messageType = typeof options.code === "string"
  ...
  try {
    ...
    logError(
      "Failed to persist pending agent notification",
      err,
      ctx,
...
export function drainAgentObligations(
  ...
  try {
    ...
    if (
      !Array.isArray(targetState.pendingNotifications) ||
      targetState.pendingNotifications.length === 0
    ) {
      return false;
    }

    const lockKey = getQuestLockKey(
      targetState.questId || targetState.active || "quest",
      targetSessionId,
    );
    ...
    for (const obligation of actionable) {
      ...
      if (delivered) {
        ...
        logAgentMessageTransition(
          ...
          `agent obligation delivered (${deliverAs}): [${
            obligation.code || obligation.kind
          }]`,
          {
        ...
        logAgentMessageTransition(
          "AGENT_MESSAGE_FAILED",
          `agent obligation delivery failed (${deliverAs}): [${
            obligation.code || obligation.kind
          }]`,
          {
            code: String(obligation.code || obligation.kind),
...
export function sendInternalUserMessage(
  pi: ExtensionAPI,
  text: string,
  options?: {
    deliverAs?: "steer" | "followUp" | "nextTurn";
...
export function sendSaveRequest(pi: ExtensionAPI, message: string) {
  ...
  const promptReminder =
    `Original user request -- keep VERBATIM under ## Original request in the quest file:\n"${originalRequestText()}"${
...
export function promptsBlock(): string {
  ...
  const refs = state.refinements.map((r, i) => `Refinement ${i + 1}: ${r}`)
    .join("\n");
...
export function shouldCapturePrompt(text: string): boolean {
  ...
  if (t.startsWith("/")) {
    ...
    tryLog("SYNTHETIC_FILTERED", `synthetic filtered`, {
      syntheticPrefix: "/",
      slice: t.slice(0, 80),
      hash: h,
  ...
  if (lower.startsWith(INTERNAL_MESSAGE_PREFIX.toLowerCase())) {
    const h = createHash("sha256").update(t).digest("hex").slice(0, 12);
    tryLog("SYNTHETIC_FILTERED", `synthetic filtered`, {
      syntheticPrefix: INTERNAL_MESSAGE_PREFIX,
      slice: t.slice(0, 80),
      hash: h,
      classification: "SYNTHETIC",
      quest: "",
  ...
  const contains = [
    ...
    "critical quest journal compaction safety state",
  ];
