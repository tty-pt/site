...
export function calculateAuthoritativeTerminalStatus(
  ...
  if (
    normExplicit === "failed" || normExplicit === "failure" ||
    normExplicit === "terminal_failure"
  ) {
    return "FAILED";
  }
  if (
    normExplicit === "completed" || normExplicit === "complete" ||
    normExplicit === "success"
  ) {
    return "COMPLETED";
  }
  ...
  if (logSummaryInfo) {
    if (
      logSummaryInfo.hasUnresolvedError ||
      logSummaryInfo.hasCriticalReviewFailure
    ) {
      return "FAILED";
    }
    if (
      logSummaryInfo.testVerification &&
      logSummaryInfo.testVerification.status === "FAILED"
    ) {
      return "FAILED";
    }
    if (
      logSummaryInfo.unrecoveredFailures &&
      logSummaryInfo.unrecoveredFailures.length > 0
    ) {
      return "FAILED";
    }
    if (
      logSummaryInfo.implementationAttempts > 0 &&
      logSummaryInfo.implementationAllowedCount === 0 &&
      logSummaryInfo.implementationBlockedCount > 0
    ) {
      return "FAILED";
    }
    if (
...
export async function appendChangelogEntry(
  ...
  const prefix = completed
    ? `Completed \`${rootQuest}\`${idTag}`
    : `Terminal failure for \`${rootQuest}\`${idTag}`;
  const entry = `- ${dateStr} — ${prefix}${
