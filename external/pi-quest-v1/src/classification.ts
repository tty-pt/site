...
export function classifyUserMessage(text: string): UserMessageClassification {
  ...
  if (
    lower.includes("post-compaction") ||
    lower.includes("post-compaction autonomous resumption directive")
  ) {
    tryLog(
      "CLASSIFICATION_RESULT",
      `classification ${UserMessageClassification.CONVERSATIONAL_ACK}`,
  ...
  const matchesConfirmationPattern = confirmPhrases.some((phrase) => {
    ...
    if (
      clean.startsWith(phrase + " ") || clean.endsWith(" " + phrase) ||
  ...
  if (matchesConfirmationPattern) {
    tryLog(
      "CLASSIFICATION_RESULT",
      `classification ${UserMessageClassification.CONFIRMATION}`,
      {
        classification: UserMessageClassification.CONFIRMATION,
        quest: state?.active || "",
      },
  ...
  const isQuestion =
    /^(what|where|who|how|why|is there|are there|can you explain|explain|tell me|show me|which|status|how does|what is|what are)\b/i
      .test(lower) || lower.endsWith("?");
  if (isQuestion && trimmed.length < 250) {
    if (!hasReq) {
      tryLog(
        "CLASSIFICATION_RESULT",
        `classification ${UserMessageClassification.QUESTION_OR_DISCUSSION}`,
        {
          classification: UserMessageClassification.QUESTION_OR_DISCUSSION,
          quest: state?.active || "",
        },
...
export async function acceptRootConfirmation(
  pi: ExtensionAPI,
  ...
  if (!wasImplementable && isImplementable) {
    checkAndEmitGateContinuationSteer(pi, ctx, s.active || undefined, {
...
export function isConfirmationQuestion(
  ...
  const checkOptions = (opts?: any[]) => {
    ...
    for (const opt of opts) {
      const optText =
        (typeof opt === "string" ? opt : opt?.label || opt?.description || "")
          .toLowerCase();
      if (
...
export function handleAskQuestionsResult(
  pi: ExtensionAPI,
  event: any,
  ctx: ExtensionContext,
) {
  ...
  if (
    details &&
    (details.status === "cancelled" || details.status === "unavailable" ||
      details.error)
  ) {
    return;
  }
  ...
  const answers: Array<
    { question: string; header: string; answer: string; options?: any[] }
  > = [];

  if (Array.isArray(details?.answers)) {
    for (const a of details.answers) {
      if (typeof a?.answer === "string") {
        const qIndex = typeof a.questionIndex === "number"
          ? a.questionIndex
          : -1;
        const matchedQ = qIndex >= 0 && Array.isArray(details.questions)
          ? details.questions[qIndex]
          : null;
        answers.push({
          question: typeof a.question === "string"
            ? a.question
            : (matchedQ?.question || ""),
          header: typeof a.header === "string"
            ? a.header
            : (matchedQ?.header || ""),
          answer: a.answer.trim(),
          options: matchedQ?.options ||
            (Array.isArray(event.input?.questions) && qIndex >= 0
              ? event.input.questions[qIndex]?.options
              : undefined),
        });
      }
    }
  ...
  for (const item of answers) {
    ...
    const isConfirmQ = isConfirmationQuestion(
      qText,
      hText,
    ...
    if (
      isConfirmQ && classification === UserMessageClassification.CONFIRMATION
    ) {
      ...
      if (state.active) {
        ...
        triggerReassessment(
          state,
          `User refinement received via ask_questions: "${
