...
export function parseTokenAmount(
  val: unknown,
  defaultVal: number = DEFAULT_CEILING_TOKENS,
): number | null {
  if (typeof val === "number" && !Number.isNaN(val)) {
...
export function truncateQuestName(
  name: string,
  maxLen = MAX_QUEST_NAME_DISPLAY_LENGTH,
): string {
  if (!name) return "";
...
export function formatQuestHierarchy(
  active: string | null,
  stack?: string[],
  maxNameLength = MAX_QUEST_NAME_DISPLAY_LENGTH,
): string {
  if (!active) return "(none)";
  if (!stack || stack.length === 0) {
    return truncateQuestName(active, maxNameLength);
  }
  ...
  for (const item of stack) {
    if (
      item &&
      (cleanStack.length === 0 || cleanStack[cleanStack.length - 1] !== item)
    ) {
...
export function formatQuestShort(
  ...
  let nome: string;
  if (
    state?.stack && Array.isArray(state.stack) && state.stack.length > 1 &&
    state?.active
  ) {
