...
Required: await review verdict.`;
      ...
      if (
        ...
        const draftEscape = draftSlug
          ? `\nDraft plan active: edit \`.pi/quest/future/${draftSlug}.md\` (the \`## Implementation Plan\` section) directly — \`quest_update_state\` is blocked for drafts before reviewer APPROVE.`
          : "";
