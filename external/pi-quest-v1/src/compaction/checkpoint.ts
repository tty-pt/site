...
export function compactionReady(expectedSlug?: string): boolean {
  ...
  if (
    ...
    (state.activeTransaction.phase === "inconsistent" ||
      state.activeTransaction.phase === "failed")
  ) {
    return false;
  }
  const p = questPath(state.questId);
  const hasValidGen = Boolean(
    state.saveGeneration && state.saveGeneration.path === p,
  );
...
export function getCompactionInstructions(
  ...
  const tokenLabel = tokens !== null
    ? ` at ${formatTokens(tokens)} tokens (threshold: ${
...
export function buildPeriodicCheckpointPrompt(
  ...
  const files =
    Array.isArray(opts.filesModified) && opts.filesModified.length > 0
      ? opts.filesModified.slice(-6).join(", ")
      : "(none)";
  const tailBlock = opts.logTail
    ...
    : "";
  if (
    !activeQuest || activeQuest === "provisional" || state.pendingRootQuest ||
    state.activeDraft
  ) {
    const draft = state.activeDraft
      ? `.pi/quest/future/${state.activeDraft}.md`
      : (qp || `.pi/quest/current/${state.questId || "<qid>"}/quest.md`);
    const idTag = state.questId ? ` (id: ${state.questId})` : "";
    const slug = activeQuest && activeQuest !== "provisional"
      ? activeQuest
      : (state.activeDraft || "provisional");
...
export function buildWarningSavePrompt(
  ...
  return buildPeriodicCheckpointPrompt(activeQuest, {
    turnsSinceCheckpoint: state.substantiveTurnsSinceCheckpoint || 0,
  });
}
...
export function validatePhasedPlan(content: string): boolean {
  ...
  const hasNumbered = lines.some((l) =>
    /^\s*\d+\.\s+/.test(l) || /^\s*-\s*\[[ xX]\]/.test(l) || /^\s*-\s+/.test(l)
  );
  ...
  if (/>\s*Paste/i.test(exactBody) || exactBody.trim().length < 30) {
    return false;
  }
  return hasNumbered && (hasVerification || hasSubquest) &&
    exactBody.length > 30;
}
...
export function validateCheckpointMatching(
  ...
  const currentHash = sessionState.saveGeneration?.hash ||
  ...
  const countMismatch = Boolean(
    tx && tx.checkpointSaveCount !== sessionState.saveCount,
  );
