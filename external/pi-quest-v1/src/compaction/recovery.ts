...
import {
  createCompactionResumeObligation,
...
export async function handleUnknownCompaction(
  ...
  let reason: ResumeReason = sessionState.pendingResume?.reason ||
    "normal-compaction";
  ...
  const inconsistentTx: CompactionTransaction = {
    ...
    activeQuest,
    reason,
    stack: Array.isArray(sessionState.stack)
      ? [...sessionState.stack]
    ...
    createdAt: Date.now(),
    error:
      "Unexpected external compaction occurred without a prepared checkpoint transaction.",
    observedSaveCount: sessionState.saveCount,
    observedHash: sessionState.saveGeneration?.hash ||
      sessionState.lastSavedHash || "",
    observedQuestPath: questPath(sessionState.questId),
  };

  logCompactionTransition(
    "COMPACTION_EXTERNAL",
    "external compaction detected without transaction",
    {
      quest: activeQuest,
  ...
  logRecoveryTransition(
    "STATE_INCONSISTENT",
    "state inconsistent due to unexpected external compaction",
    {
      quest: activeQuest,
  ...
  logRecoveryTransition(
    "RECOVERY_STARTED",
    "recovering from unexpected external compaction",
    {
      quest: activeQuest,
      compactionId: externalCompactionId,
    },
  ...
  if (
    ...
    sessionState.activeTransaction.phase !== "resume-pending"
  ) {
  ...
  reportAgentError(
    ...
    `[Quest Journal] RESUME_STATE_INCONSISTENT\n\nAn unexpected external compaction occurred without a verified pre-compaction checkpoint transaction.\n\nDo not assume the pre-compaction state was safely preserved.\n\nRequired action:\nreconstruct the current durable quest state, reconcile the discrepancy with quest_update_state, and continue only from the verified state.`,
    {
      ...
      details: {
        ...
        LiveHash: sessionState.saveGeneration?.hash ||
          sessionState.lastSavedHash || "",
      },
    },
  );
...
function finalizeCompactionSuccess(
  ...
  if (c?.hasUI && typeof tokens === "number" && tokens > 0) {
    c.ui.notify(
      `Economy auto-compaction completed at ${formatTokens(tokens)} tokens.`,
      "info",
    );
  }
  ...
  logResumeTransition(
    ...
    `resume obligation created for '${activeQuest}'`,
    {
      quest: activeQuest,
      compactionId: tx.id,
      id: tx.id,
  ...
  sessionState.pendingResume = createCompactionResumeObligation(
    tx,
    sessionState,
    activeQuest,
    reason,
...
export async function handleCompactionCompleted(
  ...
  const tokens = typeof completedAtTokens === "number"
    ? completedAtTokens
    : calculateCurrentTokens(c);
  ...
  if (
    tx &&
    (tx.phase === "resume-delivered" || tx.phase === "failed" ||
      sessionState.lastDeliveredCompactionId === tx.id)
  ) {
    return;
  }

  const isKnownTransaction = Boolean(
    tx && (tx.phase === "prepared" || tx.phase === "in-flight"),
  );
