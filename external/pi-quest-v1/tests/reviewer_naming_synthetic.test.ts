  ...
  return {
    ...
    sessionManager: {
      ...
      appendCustomEntry: (_t: string, d: any) =>
        branch.push({ type: "custom", customType: "quest_journal", data: d }),
    },
    getContextUsage: () => ({ tokens: 50000, percent: 6 }),
    ui: {
      notify: () => {},
      setStatus: (k: string, v: any) => {
        if (v === undefined) delete status[k];
        else status[k] = v;
      },
      getStatus: (k: string) => status[k],
      getAllStatus: () => ({ ...status }),
      input: async () => "",
      select: async () => null,
    },
...
Deno.test("Reviewer naming and synthetic isolation (T40-T44)", async (t) => {
  ...
  await t.step(
    "T40 synthetic isolation: post-compaction directive not captured",
    () => {
      assert.strictEqual(
        shouldCapturePrompt(
          "⚡ **Post-Compaction Autonomous Resumption Directive** test",
        ),
        false,
      );
      assert.strictEqual(
  ...
  await t.step(
    "T44 synthetic false-positive: user discussion of compaction is captured",
    () => {
      assert.strictEqual(
        shouldCapturePrompt(
          "my compaction warning about context is user discussing compaction tradeoffs",
        ),
        true,
      );
      assert.strictEqual(
        shouldCapturePrompt("context compaction warning: hello").valueOf(),
        false,
      );
    },
  );
  ...
  await t.step(
    "classification defense: post-compaction never becomes refinement",
    () => {
      const c = classifyUserMessage(
        "⚡ **Post-Compaction Autonomous Resumption Directive** please add feature",
      );
  ...
  await t.step(
    "T41 naming: UI shows kind/triggerReason and logs contain triggerReason",
    async () => {
      clearActiveReviews();
      ...
      assert.ok(
        log.includes("triggerReason=no_progress"),
        "log must contain triggerReason",
      );
  ...
  await t.step("UI status contains reviewer and trigger label", () => {
    ...
    assert.strictEqual(
      status,
      undefined,
      `status must be hidden, got: ${status}`,
    );
    assert.ok(
      getActiveReviews().has("rev_ui_1"),
      "review still tracked internally",
    );
    clearActiveReviews();
  });
  ...
  await t.step("draft vs root serialization with cap 1", async () => {
    ...
    const firstWait = new Promise((r) => {
      resolveFirst = r;
    });
    setCustomSubagentRunner(async () => {
      callCount++;
      if (callCount === 1) await firstWait;
      return `VERDICT: PASS\nSEVERITY: NONE\nFINDINGS:\n- None\nREQUIRED ACTIONS:\n- None`;
    });
    const p1 = runCriticalReview(
      mockPi,
      ctx,
      {
        kind: "plan_review",
    ...
    assert.strictEqual(
      p2Res.inProgress,
      true,
      "second must be queued due to cap 1",
    );
  ...
  await rm(".pi/quest/current/naming-quest-41", {
    recursive: true,
    force: true,
  }).catch(() => {});
  await rm(".pi/quest/current/cap-quest-43", { recursive: true, force: true })
    .catch(() => {});
});
