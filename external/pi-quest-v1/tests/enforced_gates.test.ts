...
Deno.test("quest_journal_enforced_gates: complete verification of all 15 behavioral promise gates", async (t) => {
  ...
  await t.step("1. new root quest cannot edit before research", async () => {
    ...
    assert.strictEqual(
      blockRes.block,
      true,
      "Implementation tool must be blocked before research",
    );
    assert.ok(
      blockRes.reason.includes("RESEARCH_PENDING"),
      "Reason must identify RESEARCH_PENDING state",
    );
    assert.ok(
      blockRes.reason.includes("quest_update_state"),
      "Reason must specify how to resolve",
    );
  });
  ...
  await t.step(
    ...
    async () => {
      ...
      assert.strictEqual(
        searchRes?.block,
        undefined,
        "search tool must not be blocked",
      );
      ...
      assert.strictEqual(
        bashRes?.block,
        undefined,
        "bash inspection must not be blocked",
      );
    },
  );
    ...
    async () => {
      const updateRes = await tools["quest_update_state"].execute(
        "call_1",
        {
          ...
          understanding:
            "Audio subsystem uses libxylem with streaming buffers in hyle.",
          assumptions:
            "- Assumption 1: Buffer size 4096 is sufficient.\n- Assumption 2: Zero heap allocations in render loop.",
          openQuestions:
            "No critical blockers remaining; memory and latency constraints verified.",
      ...
      assert.ok(
        !updateRes.content[0].text.includes("refused"),
        "Valid research must be accepted",
      );
    },
  );
  ...
  await t.step(
    ...
    async () => {
      ...
      assert.ok(
        blockRes,
        "Root quest before user confirmation must be blocked",
      );
      assert.strictEqual(
        blockRes.block,
        true,
        "Root quest must be blocked until confirmed",
      );
      assert.ok(
  ...
  await t.step(
    ...
    async () => {
      ...
      assert.strictEqual(
        allowRes?.block,
        undefined,
        "Implementation must be allowed after confirmation",
      );
    },
  );
  ...
  await t.step(
    ...
    async () => {
      ...
      assert.strictEqual(
        allowRes?.block,
        undefined,
        "Conversational ack must not block implementation",
      );
    },
  );
  ...
  await t.step("7. ordinary questions do not become refinements", async () => {
    for (const cb of handlers["before_agent_start"] || []) {
      await cb(
        { prompt: "What is the size of the streaming buffer?" },
        mockCtx,
      );
    }
    ...
    assert.strictEqual(
      allowRes?.block,
      undefined,
      "Ordinary question must not block implementation",
    );
  });
  ...
  await t.step("8. a real refinement triggers reassessment", async () => {
    for (const cb of handlers["before_agent_start"] || []) {
      await cb({
        prompt: "Also add support for Opus audio decoding in the player",
      }, mockCtx);
    }
    ...
    assert.strictEqual(
      blockRes.block,
      true,
      "Implementation must be blocked while reassessment is required",
    );
    assert.ok(
      blockRes.reason.includes("REASSESSMENT_PENDING"),
      "Reason must state REASSESSMENT_PENDING",
    );
  });
  ...
  await t.step(
    ...
    async () => {
      ...
      const updateRes = await tools["quest_update_state"].execute(
        ...
        {
          understanding:
            "Audio player supports Opus decoding via external libopus wrapper.",
          ...
          reassessmentConclusion:
            "Investigated libopus integration; architecture supports it directly without changes to memory model.",
        },
        null,
        null,
        mockCtx,
      ...
      assert.ok(
        !updateRes.content[0].text.includes("refused"),
        "Reassessment resolution must succeed",
      );
      ...
      assert.strictEqual(
        allowRes?.block,
        undefined,
        "Implementation must be allowed after resolving reassessment",
      );
    },
  );
  ...
  await t.step(
    ...
    async () => {
      ...
      assert.strictEqual(
        blockRes.block,
        true,
        "Implementation must remain blocked after restart",
      );
    },
  );
  ...
  await t.step("12. stale quest blocks compaction", async () => {
    ...
    assert.strictEqual(
      cancelRes?.cancel,
      true,
      "Dirty quest must block compaction",
    );
  });
  ...
  await t.step("13. verified save allows compaction", async () => {
    ...
    assert.notStrictEqual(
      allowRes?.cancel,
      true,
      "Clean verified quest must allow compaction",
    );
  });
  ...
  await t.step(
    ...
    async () => {
      ...
      const blockRes = await emitToolCall("edit", {
        path: "mods/song/ring_buffer.c",
      });
      assert.strictEqual(
        blockRes?.block,
        true,
        "Sub-quest must research first",
      );

      await emitToolCall("read", { path: "mods/song/ring_buffer.h" });
      await emitToolResult(
        "read",
        { path: "mods/song/ring_buffer.h" },
        "ring buffer header code",
      );
      ...
      assert.strictEqual(
        allowRes?.block,
        undefined,
        "Sub-quest can implement immediately after research",
      );
    },
  );
  ...
  await t.step(
    ...
    async () => {
      ...
      assert.strictEqual(
        allowRes?.block,
        undefined,
        "Parent restored its mature permission state",
      );
    },
  );
