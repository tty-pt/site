...
async function testSubQuestFeature() {
  ...
  assert.strictEqual(
    commands["subtask"],
    undefined,
    "Command /subtask should NOT be registered",
  );
  assert.strictEqual(
    commands["task-subquest"],
    undefined,
    "Command /task-subquest should NOT be registered",
  );

  // 2. Verify tools registered
  assert.ok(
    tools["quest_subquest"],
    "Tool quest_subquest should be registered",
  );
  ...
  assert.ok(
    toolRes && toolRes.content &&
      toolRes.content[0].text.includes("Created sub-quest"),
    "Tool should succeed",
  );
  ...
  assert.ok(
    updatedParentContent.includes("[[child-remark-quest]]"),
    "Parent quest file should link child sub-quest",
  );
  assert.ok(
    updatedParentContent.includes("Investigate tangent remark from user"),
    "Parent quest file should include sub-quest description",
  );

  // 5. Test command execution `/subquest` with description (slug inferred)
  userMessages = [];
  await commands["subquest"].handler(
    "Implement second follow-up feature",
    mockCtx,
  );

  const parentContentAfterSecondSub = await getQuestContentBySlug(
    "parent-test-quest",
  );
  assert.ok(
    parentContentAfterSecondSub.includes(
      "[[implement-second-follow-up-feature]]",
    ),
    "Parent quest file should link second subquest",
  );
  assert.ok(
    parentContentAfterSecondSub.includes("Implement second follow-up feature"),
    "Parent quest file should have goal",
  );
  ...
  assert.ok(
    widgetText.includes("parent-test-quest"),
    "Widget should list parent quest",
  );
  assert.ok(
    widgetText.includes("↳ child-remark-quest"),
    "Widget should list child-remark-quest nested under parent",
  );
  assert.ok(
    widgetText.includes("↳ implement-second-follow-up-feature"),
    "Widget should list implement-second-follow-up-feature nested under parent",
  );
  ...
  assert.ok(
    statusResult.includes("parent: [[parent-test-quest]]") ||
      notifiedMessages.some((m) => m.includes("parent: [[parent-test-quest]]")),
    "quest-status should show parent quest",
  );
  ...
  const updatedBareParentContent = await getQuestContentBySlug(
    "bare-parent-quest",
  );
  assert.ok(
    updatedBareParentContent.includes("## Sub-Quests"),
    "Parent should have Sub-Quests section added",
  );
  assert.ok(
    updatedBareParentContent.includes("[[bare-child-quest]]"),
    "Parent should link bare-child-quest",
  );
  ...
  assert.ok(
    plan1Res.content[0].text.includes("Kept parent quest active"),
    "switchNow: false should keep parent active",
  );

  // Plan stage 2 sub-quest via command `/subquest --plan`
  await commands["subquest"].handler(
    "--plan Stage 2: Verification and benchmarks",
    mockCtx,
  );
  ...
  assert.ok(
    parentWithPlans.includes("[[planned-sub-stage-1]]"),
    "Parent quest links planned subquest 1",
  );
  assert.ok(
    parentWithPlans.includes("[[stage-2-verification-and-benchmarks]]"),
    "Parent quest links planned subquest 2",
  );
  ...
  const autoCreatedParent = await getQuestContentBySlug(
    "unwritten-parent-quest",
  );
  assert.ok(
    autoCreatedParent.includes("unwritten-parent-quest"),
    "Parent file should be auto-created",
  );
  assert.ok(
    autoCreatedParent.includes("[[unwritten-child-quest]]"),
    "Auto-created parent should link child quest",
  );
  ...
  await commands["quest"].handler("parent-test-quest", mockCtx);
  await commands["quest"].handler(
    "implement-second-follow-up-feature",
    mockCtx,
  );
  const archiveRes = await archiveTool.execute(
    "call_arch",
    { compact: false },
    null,
    null,
  ...
  assert.ok(
    archiveRes &&
      archiveRes.content[0].text.includes(
        "Resumed parent/previous quest 'parent-test-quest'",
      ),
    "Archiving child should resume parent quest via LIFO stack",
  );
  ...
  assert.ok(
    finalParentContent.includes("- [x] [[implement-second-follow-up-feature]]"),
    "Parent quest file should mark completed child sub-quest as [x]",
  );
  ...
  assert.ok(
    prompt.includes("Sub-Quests"),
    "System prompt should mention Sub-Quests",
  );
  assert.ok(
    prompt.includes("quest_subquest"),
    "System prompt should mention quest_subquest tool",
  );
