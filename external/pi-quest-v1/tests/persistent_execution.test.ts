...
Deno.test("quest_journal_persistent_execution: all 12 lifecycle scenarios", async (t) => {
  ...
  await t.step(
    "Test 1 — automatic root quest for substantive prompt",
    async () => {
      await rm(currentDir, { recursive: true, force: true });
      await mkdir(currentDir, { recursive: true });
      ...
      const substantivePrompt =
        "Refactor the authentication system to support passkeys while preserving existing login behavior.";

      // Trigger before_agent_start with substantive prompt
      for (const cb of harness.handlers["before_agent_start"] || []) {
        await cb(
          { prompt: substantivePrompt, systemPrompt: "Base prompt." },
          harness.mockCtx,
        );
      }

      // Verify provisional root quest is initialized
      const statusProvisional = await harness.commands["quest-status"].handler(
        "",
        harness.mockCtx,
      );
      assert.ok(
        statusProvisional &&
          statusProvisional.includes("PROVISIONAL ROOT INITIALIZATION"),
        "Root quest must enter provisional state",
      );
      ...
      await harness.tools["quest_update_state"].execute(
        "call_init",
        {
          ...
          understanding:
            "Authentication system is in mods/auth and uses axil session cookies.",
          assumptions: ["Existing login fallback remains functional"],
          openQuestions: ["None"],
          findings: [
            "Passkey public key credentials can be verified with libcrypto",
          ],
          plan: [
            "1. Add WebAuthn challenge endpoint",
            "2. Add passkey verify handler",
          ],
          planConfidence: "high",
      ...
      const status = await harness.commands["quest-status"].handler(
        "",
        harness.mockCtx,
      );
      assert.ok(
        status.includes(slug),
        `Active quest slug should be active: ${status}`,
      );
      ...
      assert.ok(
        content.includes(substantivePrompt),
        "Original prompt must be persisted in quest file",
      );
    },
  );
  ...
  await t.step("Test 2 — trivial prompt does NOT create a quest", async () => {
    ...
    for (const cb of harness.handlers["before_agent_start"] || []) {
      await cb(
        { prompt: trivialPrompt, systemPrompt: "Base prompt." },
        harness.mockCtx,
      );
    }
    ...
    assert.strictEqual(
      status,
      "No active quest.",
      "Trivial prompt should NOT create an active quest",
    );
  });
  ...
  await t.step("Test 3 — refinement to active quest", async () => {
    ...
    const rootPrompt =
      "Implement song volume normalization across all audio formats";

    // Create root quest
    for (const cb of harness.handlers["before_agent_start"] || []) {
      await cb(
        { prompt: rootPrompt, systemPrompt: "Base prompt." },
        harness.mockCtx,
      );
    }
    ...
    const status1 = await harness.commands["quest-status"].handler(
      "",
      harness.mockCtx,
    );
    ...
    const refinementPrompt =
      "Actually, this must remain compatible with Node 20 and support replaygain tags";
    for (const cb of harness.handlers["before_agent_start"] || []) {
      await cb(
        { prompt: refinementPrompt, systemPrompt: "Base prompt." },
        harness.mockCtx,
      );
    }

    // Verify active quest is STILL the same root quest, but marked dirty (SAVE PENDING)
    const status2 = await harness.commands["quest-status"].handler(
      "",
      harness.mockCtx,
    );
    assert.ok(
      status2.includes(slug) || status2.includes("SAVE PENDING"),
      `Active quest must reflect state: ${status2}`,
    );
    assert.ok(
      status2.includes("SAVE PENDING") || status2.includes(slug),
      "Refinement must mark quest state dirty / SAVE PENDING",
    );
  });

  await t.step(
    "Test 4 — continuous durable memory without artificial checkpoints",
    async () => {
      await rm(currentDir, { recursive: true, force: true });
      await mkdir(currentDir, { recursive: true });
      ...
      for (let i = 0; i < 5; i++) {
        for (const cb of harness.handlers["tool_result"] || []) {
          await cb(
            { toolName: "edit", input: { path: "mods/song/song.c" } },
            harness.mockCtx,
          );
        }
      ...
      const checkpointMsgs = harness.userMessages.filter((m) =>
        (typeof m.msg === "string" ? m.msg : m.msg?.text || "").includes(
          "Quest Incremental Checkpoint",
        )
      );
      assert.strictEqual(
        checkpointMsgs.length,
        0,
        "No artificial checkpoint message should be injected during normal execution",
      );
      ...
      assert.ok(
        status.includes("SAVE PENDING"),
        "Substantive turns must mark quest dirty",
      );

      // Calling quest_mark_saved resets dirty flag
      await harness.tools["quest_mark_saved"].execute(
        "save_call",
        {},
        null,
        null,
      ...
      const cleanStatus = await harness.commands["quest-status"].handler(
        "",
        harness.mockCtx,
      );
      assert.ok(
        cleanStatus.includes("fresh"),
        "quest_mark_saved must clear dirty state",
      );
    },
  );

  await t.step(
    "Test 5 & 7 — periodic checkpoint records state and compaction gate blocks dirty",
    async () => {
      await rm(currentDir, { recursive: true, force: true });
      await mkdir(currentDir, { recursive: true });
      ...
      for (let i = 0; i < 6; i++) {
        for (const cb of harness.handlers["tool_result"] || []) {
          await cb(
            { toolName: "edit", input: { path: `mods/gig/gig${i}.c` } },
            harness.mockCtx,
          );
        }
        for (const cb of harness.handlers["turn_end"] || []) {
      ...
      assert.strictEqual(
        finalSaveMsgs.length,
        1,
        "6 substantive turns must issue periodic checkpoint",
      );
      ...
      assert.strictEqual(
        cancelRes?.cancel,
        true,
        "session_before_compact must block/cancel when dirty",
      );
    },
  );

  await t.step(
    "Test 6 — safe compaction when threshold reached and quest is clean",
    async () => {
      await mkdir(currentDir, { recursive: true });
      ...
      await harness.commands["quest"].handler(rootSlug, harness.mockCtx);
      await harness.commands["quest-economy"].handler(
        "333k 30k",
        harness.mockCtx,
      );
      ...
      assert.notStrictEqual(
        beforeCompactRes?.cancel,
        true,
        "Clean state at threshold must allow compaction",
      );
    },
  );
  ...
  await t.step("Test 8 — post-compaction autonomous resume", async () => {
    ...
    assert.ok(
      harness.userMessages.length > 0,
      "session_compact must send resume directive",
    );
    const lastMsg = harness.userMessages[harness.userMessages.length - 1];
    const msgText = typeof lastMsg.msg === "string"
      ? lastMsg.msg
      : (lastMsg.msg?.text || "");
    assert.ok(
      msgText.includes("Post-Compaction Autonomous Resumption Directive"),
      "Resume directive must be included",
    );
    assert.ok(
      msgText.includes(rootSlug) || msgText.includes("quest.md"),
      "Resume directive must reference active quest file",
    );
    assert.ok(
      msgText.includes("read"),
      "Resume directive must instruct reading the quest file",
    );
  });

  await t.step(
    "Test 9 & 10 — nested subquests, LIFO restoration, and child completion",
    async () => {
      await rm(currentDir, { recursive: true, force: true });
      await mkdir(currentDir, { recursive: true });
      ...
      let status = await harness.commands["quest-status"].handler(
        "",
        harness.mockCtx,
      );
      ...
      status = await harness.commands["quest-status"].handler(
        "",
        harness.mockCtx,
      );
      ...
      const arch1 = await harness.tools["quest_archive"].execute(
        "a1",
        { questName: grandChildSlug, compact: false },
        null,
        null,
        harness.mockCtx,
      );
      assert.ok(
        arch1.content[0].text.includes(
          `Resumed parent/previous quest '${childSlug}'`,
        ),
        "Archiving grandchild should resume child",
      );
      ...
      assert.ok(
        status.includes(childSlug),
        "Child must be active after grandchild archive",
      );

      // Archive child -> should pop LIFO stack and resume root
      const arch2 = await harness.tools["quest_archive"].execute(
        "a2",
      ...
      assert.ok(
        arch2.content[0].text.includes(
          `Resumed parent/previous quest '${rootSlug}'`,
        ),
        "Archiving child should resume root",
      );
      ...
      assert.ok(
        status.includes(rootSlug),
        "Root must be active after child archive",
      );
    },
  );

  await t.step(
    "Test 11 — subquest launch compaction when threshold is exceeded",
    async () => {
      await rm(currentDir, { recursive: true, force: true });
      await mkdir(currentDir, { recursive: true });
      ...
      await harness.tools["quest_subquest"].execute(
        ...
        {
          name: subSlug,
          goal: "Subquest work under high context",
          switchNow: true,
        },
        null,
        null,
      ...
      for (const cb of harness.handlers["turn_end"] || []) {
        await cb(
          { toolResults: [{ toolName: "quest_subquest" }] },
          harness.mockCtx,
        );
      }
      ...
      assert.strictEqual(
        harness.getCompactCount(),
        1,
        "Subquest launch above threshold must trigger launch compaction",
      );
      const lastOpts = harness.getLastCompactOptions();
      assert.ok(
        lastOpts?.customInstructions?.includes(subSlug),
        "Launch compaction instructions must target the child subquest",
      );
      ...
      assert.ok(
        status.includes(subSlug),
        "Active quest must point to child after launch",
      );
    },
  );
  ...
  await t.step("Test 12 — persistence failure handling", async () => {
    ...
    assert.ok(
      res.details?.error || res.content[0].text.toLowerCase().includes("error"),
    ...
    for (const cb of harness.handlers["session_before_compact"] || []) {
      ...
      assert.strictEqual(
        compactRes?.cancel,
        true,
        "Unverified state must cancel compaction",
      );
    }
  });
});
