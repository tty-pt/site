...
Deno.test("quest_journal_ask_questions_confirmation: ask_questions tool result confirmation lifecycle", async (t) => {
  ...
  const rootQuestPath = `${currentDir}/${rootSlug}.md`;
  ...
  // -----------------------------------------------------------------------
  await t.step(
    "1. root quest completes research and enters awaitingUserConfirmation state",
    async () => {
      for (const cb of handlers["before_agent_start"] || []) {
        await cb({
          prompt:
            "Refactor audio streaming buffer and implement lockless ring buffer in player.c",
          systemPrompt: "Base.",
        }, mockCtx);
      }

      await emitToolCall("read", { path: "mods/song/player.c" });
      await emitToolResult("read", { path: "mods/song/player.c" }, [{
        type: "text",
        text: "code",
      }]);
      ...
      await tools["quest_update_state"].execute(
        "call_root_init",
        {
          ...
          understanding:
            "Audio player uses ring buffer with atomic head/tail pointers in mods/song/player.c.",
          assumptions: [
      ...
      assert.ok(
        ...
        "Reason must be CONFIRMATION_PENDING",
      );
    },
  );
  ...
  // -----------------------------------------------------------------------
  await t.step(
    "2. non-confirmation ask_questions answer does NOT clear confirmation gate",
    async () => {
          ...
          questions: [
            {
              ...
              question:
                "Which audio output format would you prefer for default test benchmarks?",
              options: [
                {
      ...
      await emitToolResult(
        "ask_questions",
        nonConfirmEvent.input,
        nonConfirmEvent.content,
        nonConfirmEvent.details,
        false,
      );
      ...
      assert.ok(
        blockRes,
        "Non-confirmation ask_questions must NOT unlock edit",
      );
  ...
  // -----------------------------------------------------------------------
  await t.step(
    "3. negative answer to confirmation question leaves implementation blocked",
    async () => {
      const negativeEvent = {
        toolName: "ask_questions",
        input: {
          questions: [
            {
              header: "Confirmation",
              question:
                "Would you like to proceed with implementing the lockless ring buffer in player.c?",
              options: [
                {
                  label: "Yes, proceed with implementation",
                  description: "Start implementing the changes",
                },
                {
                  label: "No, let me review the plan first",
                  description: "Hold off on changes",
                },
              ],
            },
          ],
      ...
      await emitToolResult(
        "ask_questions",
        negativeEvent.input,
        negativeEvent.content,
        negativeEvent.details,
        false,
      );
  ...
  // -----------------------------------------------------------------------
  await t.step(
    "4. affirmative answer to ask_questions immediately unlocks edit before next tool call",
    async () => {
      ...
      await emitToolResult(
        "ask_questions",
        affirmativeEvent.input,
        affirmativeEvent.content,
        affirmativeEvent.details,
        false,
      );
      ...
      assert.strictEqual(
        allowRes?.block,
        undefined,
        "Edit tool call must be allowed immediately following affirmative ask_questions answer",
      );
    },
  );
  ...
  // -----------------------------------------------------------------------
  await t.step(
    "5. varied affirmative answers (e.g. 'Yes, proceed with finalization and verification') are accepted",
    async () => {
      // Reset to unconfirmed for testing varied phrases
      const anotherSlug = "another-test-quest";
      const anotherPath = `${currentDir}/${anotherSlug}.md`;
      await emitToolCall("read", { path: "mods/song/player.c" });
      await emitToolResult("read", { path: "mods/song/player.c" }, [{
        type: "text",
        text: "code",
      ...
      assert.strictEqual(
        allowRes?.block,
        undefined,
        "Varied affirmative phrase unlocks edit",
      );
  ...
  // -----------------------------------------------------------------------
  await t.step(
    "6. refinement answer via ask_questions triggers reassessment and blocks implementation",
    async () => {
          ...
          questions: [
            {
              ...
              options: [
                {
                  label:
                    "Yes, but also add support for FLAC stream chunking in player.c",
                },
              ],
            },
          ],
      ...
      await emitToolResult(
        "ask_questions",
        refinementEvent.input,
        refinementEvent.content,
        refinementEvent.details,
        false,
      );
      ...
      assert.ok(
        blockRes,
        "Refinement answer must block edit until reassessment is completed",
      );
      assert.strictEqual(blockRes.block, true);
      assert.ok(
        blockRes.reason.includes("REASSESSMENT_PENDING"),
        "Reason must cite REASSESSMENT_PENDING",
      );
    },
  );
