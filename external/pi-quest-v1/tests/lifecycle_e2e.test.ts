...
Deno.test("quest_journal_lifecycle_e2e: complete 12-step autonomous compaction and resumption lifecycle", async () => {
  ...
  assert.ok(
    initialStatus.includes("No active quest"),
    "Step 1: Must start with no active quest",
  );
  ...
  // -----------------------------------------------------------------------
  const objective =
    "Refactor song multi-select filter and implement strict type bounds in mods/song/song.c";
  ...
  assert.ok(
    ...
    "Step 2: Must enter provisional root initialization",
  );
  ...
  await tools["quest_update_state"].execute(
    "call_init",
    {
      ...
      understanding:
        "Song multi-select filter requires type-bound checks in mods/song/song.c.",
  ...
  assert.ok(
    statusAfterPrompt.includes(activeSlug),
    "Step 2: Quest state initialized on disk",
  );
  ...
  assert.ok(
    diskContentInitial.includes("## Goal"),
    "Step 2: Template must include '## Goal'",
  );
  assert.ok(
    diskContentInitial.includes("## Original request"),
    "Step 2: Template must include '## Original request'",
  );
  assert.ok(
    diskContentInitial.includes("## Current Understanding"),
    "Step 2: Template must include '## Current Understanding'",
  );
  assert.ok(
    diskContentInitial.includes("## Exact Next Action"),
    "Step 2: Template must include '## Exact Next Action'",
  );
  ...
  assert.strictEqual(
    incMsgs.length,
    0,
    "Step 4: No artificial incremental checkpoint during normal execution",
  );
  ...
  for (let i = 0; i < 6; i++) {
    for (const cb of handlers["tool_result"] || []) {
      await cb(
        { toolName: "edit", input: { path: `mods/song/song${i}.c` } },
        mockCtx,
      );
    }
    for (const cb of handlers["turn_end"] || []) {
  ...
  assert.strictEqual(
    preCompactMsgs.length,
    1,
    "Step 6: Periodic checkpoint must be sent after 6 turns",
  );
  assert.ok(
  ...
  const markRes = await tools["quest_mark_saved"].execute(
    "call_mark",
    { name: activeSlug },
    null,
    null,
  ...
  assert.ok(
    markRes.content[0].text.includes("verified and marked as saved"),
    "Step 7: Quest must be verified and marked saved",
  );
  ...
  assert.notStrictEqual(
    ...
    true,
    "Step 8: session_before_compact must allow compaction when clean",
  );
  ...
  const resumeDirectives = userMessages.filter((m) =>
    (typeof m.msg === "string" ? m.msg : m.msg?.text || "").includes(
      "Post-Compaction Autonomous Resumption Directive",
    )
  );
  assert.strictEqual(
    resumeDirectives.length,
    1,
    "Step 9: session_compact MUST send the resumption directive",
  );
  assert.ok(
    resumeDirectives[0].options?.deliverAs === "followUp" ||
      !resumeDirectives[0].options,
    "Step 9: Resume directive must be delivered to agent",
  );
  assert.ok(
    resumeDirectives[0].msg.includes(activeSlug) ||
      resumeDirectives[0].msg.includes("quest.md"),
  ...
  assert.ok(
    nextAgentPrompt.includes("Active Quest Resume Context"),
    "Step 11: Next turn system prompt must include Active Quest Resume Context",
  );
  assert.ok(
    nextAgentPrompt.includes("### Execution Snapshot"),
    "Step 11: Resume context must prioritize Execution Snapshot",
  );
  assert.ok(
    nextAgentPrompt.includes("renderEscapedLabel()"),
    "Step 12: Resume context must contain exact technical details from snapshot",
  );
  assert.ok(
    nextAgentPrompt.includes(
      "Open mods/song/ux/song_ux.c and replace raw string formatting",
    ),
    "Step 12: Resume context must contain EXACT NEXT ACTION",
  );
});
...
Deno.test("quest_journal_lifecycle_e2e: subquest launch compaction and archive compaction autonomous resumption", async () => {
  ...
  assert.strictEqual(
    compactInvocationCount,
    1,
    "Subquest launch compaction must trigger ctx.compact",
  );
  ...
  assert.strictEqual(
    childResumeMsgs.length,
    1,
    "Subquest launch session_compact MUST resume the child quest",
  );
  assert.ok(
    childResumeMsgs[0].msg.includes(childSlug) ||
      childResumeMsgs[0].msg.includes(".md"),
  ...
  assert.strictEqual(
    compactInvocationCount,
    1,
    "Subquest archive compaction must trigger ctx.compact",
  );
  ...
  assert.strictEqual(
    parentResumeMsgs.length,
    1,
    "Archive session_compact MUST automatically resume the parent quest",
  );
  assert.ok(
    parentResumeMsgs[0].msg.includes(parentSlug) ||
      parentResumeMsgs[0].msg.includes("quest.md"),
