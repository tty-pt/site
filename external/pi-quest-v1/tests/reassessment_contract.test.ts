...
Deno.test("quest_journal_reassessment_contract: aggregated completion message plus actionable freshness guidance", async (t) => {
  ...
  await t.step(
    "1. incomplete reassessment reports ALL missing contract fields in ONE message",
    async () => {
      ...
      assert.ok(
        note.includes("reassessmentConclusion"),
        "must call out the missing reassessmentConclusion",
      );
      // Low-confidence guidance
      assert.ok(
        note.includes("allowLowConfidence"),
        "must explain low-confidence requirement",
      );
      assert.ok(
        note.includes("planConfidenceReason"),
        "must explain planConfidenceReason requirement",
      );
      // Freshness guidance
      assert.ok(
        note.includes("read") || note.includes("search"),
        "must instruct a fresh read/search",
      );
      assert.ok(
        note.toLowerCase().includes("epoch"),
        "must reference the research/reassessment epoch",
      );

      assert.ok(
        s.reassessmentRequired,
        "reassessmentRequired stays true on rejection",
      );
      assert.strictEqual(
        s.researchComplete,
        false,
        "researchComplete stays false on rejection",
      );
    },
  );

  await t.step(
    "2. a fresh post-trigger read + complete payload completes reassessment (<2 turns)",
    async () => {
      ...
      assert.strictEqual(
        check.sufficient,
        true,
        "a fresh read must satisfy the reassessment evidence gate",
      );
        ...
        handleReassessmentCompletion(
          {
            ...
            planConfidenceReason:
              "Stress test passed 10k iterations after the memory barrier was added.",
            reassessmentConclusion:
              "The missing atomic fence was confirmed and a barrier added; verified with 10k iterations.",
          },
          VALID_EPISTEMIC,
          EMPTY_UPDATES,
          EMPTY_SECTIONS,
      ...
      assert.strictEqual(
        s.reassessmentRequired,
        false,
        "reassessmentRequired clears on completion",
      );
      assert.strictEqual(
        s.researchComplete,
        true,
        "researchComplete set on completion",
      );
      assert.strictEqual(
        s.consecutiveFailures,
        0,
        "consecutiveFailures reset on completion",
      );
      assert.strictEqual(
        s.resolvedReassessmentVersion,
        s.reassessmentVersion,
      ...
      assert.strictEqual(
        perm,
        true,
        "implementation permission must be granted after reassessment resolves",
      );
      assert.strictEqual(
        s.implementationAllowed,
        true,
        "implementationAllowed must be true after reassessment resolves",
      );
    },
  );

  await t.step(
    "3. standalone freshness reason on a stale research-epoch receipt is actionable",
    async () => {
      const { mockPi } = createMockExtensionAPI();
      plugin(mockPi);
      ...
      assert.strictEqual(
        check.sufficient,
        false,
        "research-epoch receipt must not satisfy reassessment",
      );
      assert.ok(check.reason, "reason must be present");
      assert.ok(
        check.reason.includes("read") || check.reason.includes("search"),
        "reason must tell the agent to investigate",
      );
      assert.ok(
        check.reason.toLowerCase().includes("epoch"),
        "reason must reference the epoch",
      );
    },
  );
});
