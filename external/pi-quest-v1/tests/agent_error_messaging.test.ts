...
function createMockExtensionAPI() {
  ...
  return {
    ...
    userMessages,
    appendedEntries,
    setFailSendMessage: (val: boolean) => {
      shouldFailSendMessage = val;
    },
    setFailSendUserMessage: (val: boolean) => {
      shouldFailSendUserMessage = val;
    },
    setFailAppendEntry: (val: boolean) => {
      shouldFailAppendEntry = val;
    },
  };
}
...
Deno.test("quest_journal_agent_error_messaging: uniform model-visible error and enforcement delivery", async (t) => {
  ...
  // -----------------------------------------------------------------------
  await t.step(
    "1. reportAgentError produces structured model-visible message with code and next action",
    () => {
      ...
      const delivered = reportAgentError(
        ...
        "Operation failed because prerequisite was not met.",
        {
          ...
          deliverAs: "steer",
          requiredNextAction:
            "Investigate architecture and update quest state.",
      ...
      assert.strictEqual(
        delivered,
        true,
        "reportAgentError must return true on successful delivery",
      );
      assert.strictEqual(
        api.agentMessages.length,
        1,
        "Must deliver message via primary agent transport",
      );

      const msgText = String(api.agentMessages[0].msg);
      assert.ok(
        msgText.includes(`[Quest Journal] ${QuestErrorCode.RESEARCH_REQUIRED}`),
        "Must include standardized error code",
      );
      assert.ok(
        msgText.includes("Operation failed because prerequisite was not met."),
        "Must explain what happened",
      );
      assert.ok(msgText.includes("Tool: edit"), "Must include details");
      assert.ok(
        msgText.includes("Required next action:"),
        "Must include required next action header",
      );
      assert.ok(
        msgText.includes("Investigate architecture and update quest state."),
        "Must include action text",
      );
    },
  );
  ...
  // -----------------------------------------------------------------------
  await t.step(
    "2. Transport gracefully falls back to sendUserMessage and reports false on total failure without infinite recursion",
    () => {
      const api = createMockExtensionAPI();
      const ctx = createMockContext();
      ...
      const fallbackDelivered = sendInternalAgentMessage(
        api.mockPi as any,
        "Directive text",
        "followUp",
      );
      assert.strictEqual(
        fallbackDelivered,
        true,
        "sendInternalAgentMessage must succeed via fallback",
      );
      assert.strictEqual(
        api.userMessages.length,
        1,
        "Fallback must deliver to userMessages",
      );

      // Both fail -> returns false cleanly
      api.setFailSendUserMessage(true);
      const totalFailure = sendInternalAgentMessage(
        api.mockPi as any,
        "Directive text",
        "followUp",
      );
      assert.strictEqual(
        totalFailure,
        false,
        "sendInternalAgentMessage must return false when all transports fail",
      );
    },
  );
  ...
  // -----------------------------------------------------------------------
  await t.step(
    "3. Blocked edit tool produces model-visible RESEARCH_REQUIRED message",
    async () => {
      const api = createMockExtensionAPI();
      ...
      for (const cb of api.handlers["tool_call"] || []) {
        ...
        assert.strictEqual(
          res?.block,
          true,
          "Tool gate must block mutating edit",
        );
      }
      ...
      assert.ok(
        messages.length >= 1,
        "Agent must receive at least one model-visible message on blocked edit",
      );
      const joined = messages.join("\n\n");
      assert.ok(
        joined.includes(QuestErrorCode.RESEARCH_REQUIRED),
        "Message must contain RESEARCH_REQUIRED error code",
      );
      assert.ok(
        joined.includes("Tool: edit"),
        "Message must identify blocked tool",
      );
      assert.ok(
        joined.includes("Required next action:"),
        "Message must instruct agent on required next action",
      );
    },
  );
  ...
  // -----------------------------------------------------------------------
  await t.step(
    "4. Blocked mutating bash command produces model-visible message",
    async () => {
      const api = createMockExtensionAPI();
      plugin(api.mockPi as any);
      ...
      for (const cb of api.handlers["tool_call"] || []) {
        const res = await cb({
          toolName: "bash",
          input: { command: "sed -i 's/a/b/g' file.c" },
        }, ctx);
        assert.strictEqual(
          res?.block,
          true,
          "Tool gate must block mutating bash command",
        );
      }
      ...
      assert.ok(
        messages.length >= 1,
        "Agent must receive model-visible message on blocked bash",
      );
      const joined = messages.join("\n\n");
      assert.ok(
        joined.includes("Tool: bash"),
        "Message must identify bash tool",
      );
      assert.ok(
        joined.includes("Required next action:"),
        "Message must provide required next action",
      );
    },
  );
  ...
  // -----------------------------------------------------------------------
  await t.step(
    "5. Blocked unknown tool produces model-visible UNKNOWN_TOOL_BLOCKED message",
    async () => {
      const api = createMockExtensionAPI();
      plugin(api.mockPi as any);
      ...
      for (const cb of api.handlers["tool_call"] || []) {
        const res = await cb({
          toolName: "unrecognized_custom_tool",
          input: { foo: "bar" },
        }, ctx);
        assert.strictEqual(
          res?.block,
          true,
          "Unknown tool must be blocked default-deny",
        );
      }
      ...
      assert.ok(
        messages.length >= 1,
        "Agent must receive message on unknown tool block",
      );
      const joined = messages.join("\n\n");
      assert.ok(
        joined.includes(QuestErrorCode.UNKNOWN_TOOL_BLOCKED),
        "Message must contain UNKNOWN_TOOL_BLOCKED code",
      );
      assert.ok(
        joined.includes("Tool: unrecognized_custom_tool"),
        "Message must identify tool name",
      );
    },
  );
  ...
  // -----------------------------------------------------------------------
  await t.step(
    "6. Blocked mutating tool during reassessment produces REASSESSMENT_REQUIRED message",
    async () => {
      const api = createMockExtensionAPI();
      plugin(api.mockPi as any);
      ...
      assert.ok(
        messages.length >= 1,
        "Agent must receive message when edit is blocked during reassessment",
      );
      const joined = messages.join("\n\n");
      assert.ok(
        joined.includes(QuestErrorCode.REASSESSMENT_REQUIRED),
        "Message must contain REASSESSMENT_REQUIRED code",
      );
      assert.ok(
        joined.includes("State: REASSESSMENT_PENDING"),
        "Message must specify REASSESSMENT_PENDING state",
      );
    },
  );
  ...
  // -----------------------------------------------------------------------
  await t.step(
    "7. Blocked mutating tool during confirmation pending produces CONFIRMATION_REQUIRED message",
    async () => {
      const api = createMockExtensionAPI();
      plugin(api.mockPi as any);
      ...
      for (const cb of api.handlers["tool_call"] || []) {
        await cb(
          { toolName: "read", input: { path: "mods/song/song.c" } },
          ctx,
        );
      }
      ...
      assert.strictEqual(
        ...
        true,
        "Must be awaiting user confirmation",
      );
      ...
      assert.ok(
        messages.length >= 1,
        "Agent must receive message when edit is blocked awaiting confirmation",
      );
      const joined = messages.join("\n\n");
      assert.ok(
        joined.includes(QuestErrorCode.CONFIRMATION_REQUIRED),
        "Message must contain CONFIRMATION_REQUIRED code",
      );
      assert.ok(
        joined.includes("State: CONFIRMATION_PENDING"),
        "Message must specify CONFIRMATION_PENDING state",
      );
    },
  );
  ...
  // -----------------------------------------------------------------------
  await t.step(
    "8. Persistence failure produces model-visible PERSISTENCE_FAILURE message",
    async () => {
      const api = createMockExtensionAPI();
      plugin(api.mockPi as any);
      ...
      assert.ok(
        messages.length >= 1,
        "Agent must receive message when persistence fails",
      );
      const joined = messages.join("\n\n");
      assert.ok(
        joined.includes(QuestErrorCode.PERSISTENCE_FAILURE),
        "Message must contain PERSISTENCE_FAILURE code",
      );
      assert.ok(
        joined.includes(
          "Do not assume the current state will survive compaction",
        ),
        "Message must warn agent of durability loss",
      );
      assert.ok(
        joined.includes("Required next action:"),
        "Message must specify recovery action",
      );
    },
  );
  ...
  // -----------------------------------------------------------------------
  await t.step(
    "9. Save verification failure produces model-visible SAVE_VERIFICATION_FAILURE message",
    async () => {
      const api = createMockExtensionAPI();
      plugin(api.mockPi as any);
      ...
      const res = await tools["quest_mark_saved"].execute(
        "call_save_missing",
        { questName: "non-existent-quest-xyz" },
        {},
        () => {},
        ctx,
      );
      assert.strictEqual(
        res.details.success,
        false,
        "Mark saved must fail for missing quest file",
      );

      const messages = getAllModelMessages(api);
      assert.ok(
        messages.length >= 1,
        "Agent must receive model-visible message on save verification failure",
      );
      const joined = messages.join("\n\n");
      assert.ok(
        joined.includes(QuestErrorCode.SAVE_VERIFICATION_FAILURE),
        "Message must contain SAVE_VERIFICATION_FAILURE code",
      );
    },
  );
  ...
  // -----------------------------------------------------------------------
  await t.step(
    "10. Periodic checkpoint does NOT block edits on high token usage (pressure removed)",
    async () => {
      const api = createMockExtensionAPI();
      plugin(api.mockPi as any);
      ...
      for (const cb of api.handlers["tool_call"] || []) {
        ...
        if (res?.block) {
          assert.ok(
            !String(res.reason || "").includes(
              "CRITICAL_COMPACTION_CHECKPOINT_REQUIRED",
            ),
            "Token pressure no longer blocks via CRITICAL",
          );
        }
      }
      ...
      const joined = messages.join("\n\n");
      assert.ok(
        !joined.includes("CRITICAL_COMPACTION_CHECKPOINT_REQUIRED"),
        "Token pressure no longer blocks",
      );
    },
  );
