...
Deno.test("retry_turns: truncated turn (stopReason length, no tools, no text) auto-steers continuation", async () => {
  ...
  assert.strictEqual(
    userMessages.length,
    1,
    "Truncated turn must send one continuation steer",
  );
  assert.ok(
    String(userMessages[0].msg).includes("Continuation Retry"),
    "steer should carry the continuation directive",
  );
});
...
Deno.test("retry_turns: error stopReason with no text is treated as truncation and retried", async () => {
  ...
  assert.strictEqual(
    userMessages.length,
    1,
    "error stopReason with no output should auto-retry",
  );
});
...
Deno.test("retry_turns: clean empty turn (no stopReason) must NOT emit any message", async () => {
  ...
  assert.strictEqual(
    userMessages.length,
    0,
    "clean empty turn must not nag or retry",
  );
  ...
  assert.strictEqual(
    userMessages.length,
    0,
    "empty turn_end event must not nag or retry",
  );
});
...
Deno.test("retry_turns: substantive turn (tools ran) resets the retry budget", async () => {
  ...
  for (const cb of handlers["turn_end"] || []) {
    await cb({
      ...
      toolResults: [{ toolName: "edit", input: { path: "mods/song/song.c" } }],
    }, mockCtx);
  }
  assert.strictEqual(
    userMessages.length,
    0,
    "substantive turn should not emit retry message",
  );
  ...
  assert.strictEqual(
    userMessages.length,
    1,
    "reset budget should fire retry on next truncation",
  );
});
...
Deno.test("retry_turns: budget exhausts and reports CONTINUATION_FAILURE", async () => {
  ...
  try {
    ...
    const slug = "retry-exhaust-quest-" +
    ...
    assert.strictEqual(
      userMessages.filter((m) => String(m.msg).includes("Continuation Retry"))
        .length,
      1,
    );
    ...
    const exhaustMsgs = userMessages.filter((m) =>
      String(m.msg).includes("CONTINUATION_FAILURE")
    );
    assert.strictEqual(
      exhaustMsgs.length,
      1,
      "exhaustion should report a blocking error",
    );
    assert.ok(
      String(exhaustMsgs[0].msg).includes("Exact Next Action"),
      "error should reference the exact next action",
    );
  } finally {
...
Deno.test("retry_turns: maxTurns 0 disables retry entirely", async () => {
  ...
  try {
    ...
    assert.strictEqual(
      userMessages.length,
      0,
      "retry disabled (0) must not emit any message",
    );
  } finally {
    if (prev === undefined) Deno.env.delete("PI_QUEST_RETRY_MAX_TURNS");
    else Deno.env.set("PI_QUEST_RETRY_MAX_TURNS", prev);
...
Deno.test("retry_turns: same-turn re-entry must not fire a duplicate steer", async () => {
  ...
  assert.strictEqual(
    userMessages.length,
    1,
    "same-turn re-entry must not double-fire",
  );
});
