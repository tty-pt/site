...
Deno.test("quest_journal_compaction_dynamics: periodic checkpoint, subquest launch, and post-compaction resume", async () => {
  ...
  for (let i = 0; i < 5; i++) {
    ...
    for (const cb of handlers["turn_end"] || []) {
      await cb({
        toolResults: [{ toolName: "edit", args: { path: `mods/song/${i}.c` } }],
      }, mockCtx);
    }
  }
  userMessages = [];
  ...
  assert.ok(
    userMessages.some((m) =>
      typeof m.msg === "string" && m.msg.includes("Periodic Durable Checkpoint")
    ),
    "6th substantive turn should send periodic checkpoint",
  );
  ...
  assert.ok(
    ...
    "session_compact should send post-compaction resumption directive",
  );
  const postCompactMsg = typeof userMessages[0].msg === "string"
    ? userMessages[0].msg
    : (Array.isArray(userMessages[0].msg)
      ? userMessages[0].msg[0].text
      : userMessages[0].msg.text || "");
  assert.ok(
    postCompactMsg.includes("Post-Compaction Autonomous Resumption Directive"),
    `Post-compaction directive missing: ${postCompactMsg}`,
  );
  ...
  assert.ok(
    systemPrompt.toLowerCase().includes("resume") ||
      systemPrompt.toLowerCase().includes("autonomously"),
    "System prompt should mandate autonomous resumption",
  );
