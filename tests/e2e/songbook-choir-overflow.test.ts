/**
 * E2E test: songbook add with a choir field >= 128 bytes
 *
 * Reproduces the songbook.c:296-298 stack OOB WRITE: choir[128] and
 * choir[choir_len] = '\0' with choir_len from a 200-byte multipart
...
Deno.test("songbook: choir field >= 128 bytes does not crash the server", async () => {
  ...
  try {
    ...
    const title = `Choir Overflow ${Date.now()}`;
    const choir = "X".repeat(200);
    const enc = new TextEncoder();
    const body = multipartBody("----boundary", [
      ["title", enc.encode(title)],
      ["choir", enc.encode(choir)],
      ["csrf_token", enc.encode(token)],
    ]);

    const resp = await fetch(`${BASE}/songbook/add`, {
      method: "POST",
      headers: {
        Cookie: ch,
    ...
    // 303 is the happy path (redirect to /songbook/<id>); anything else
    // must at least not have crashed the daemon.
    const health = await fetch(`${BASE}/`);
    if (health.status !== 200) {
      throw new Error(`Server did not respond 200 after choir overflow (${health.status})`);
    }
