 ...
 * 3. checking the type checkbox (Comunhão) and clicking Apply returns 10 filtered rows
...
Deno.test("song list: filter by type and paginate across pages", async () => {
  ...
  try {
    ...
    const typesP1 = await rowsP1.locator("td:nth-child(2)")
      .allTextContents();
    ...
    const typesP2 = await rowsP2.locator("td:nth-child(2)")
      .allTextContents();
