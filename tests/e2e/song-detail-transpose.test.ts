...
Deno.test("song detail: login → view song → toggle Latin notation → verify display changed", async () => {
  ...
  try {
    ...
    // Toggle the Latin notation checkbox (triggers onchange → form submit)
    const latinCheckbox = page.locator('#transpose-form input[name="l"]');
    await latinCheckbox.waitFor({ state: "visible", timeout: 5000 });

	// Playwright's check() fires both input and change events
	await latinCheckbox.check();

	// Wait for the client-side update to change the chord display
	const deadline = Date.now() + 5000;
	let chordHtml = "";
	while (Date.now() < deadline) {
		chordHtml = await page.innerHTML("#chord-data");
		if (chordHtml !== originalHtml) break;
		await new Promise((r) => setTimeout(r, 100));
	}

	// Verify the chord display changed to Latin notation
	if (!chordHtml.includes("La") && !chordHtml.includes("Do")) {
		throw new Error(
			"Chord display did not show Latin notation after toggling.\n" +
				`Chord HTML: ${chordHtml.slice(0, 100)}`,
		);
	}
  } finally {
    await browser.close();
