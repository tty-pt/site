...
Deno.test({
  ...
  try {
    ...
    for (let i = 0; i < 10; i++) {
      const rm = await page.$('button:has-text("Remove")');
      if (!rm) break;
      await rm.click();
    ...
    // ── 3. Add song A via the "Add Song" dropdown ────────────────────────
    await page.waitForSelector('select[name="song_id"]', { timeout: 5000 });
    await page.selectOption('select[name="song_id"]', repoA);
    await page.click('button:has-text("Add Song")');
    await page.waitForURL(/\/songbook\/[^/]+$/, { timeout: 8000 });
    await waitForText(page, "body", SONG_A_TITLE);

    // ── 4. Add song B via the "Add Song" dropdown ────────────────────────
    await page.waitForSelector('select[name="song_id"]', { timeout: 5000 });
    await page.selectOption('select[name="song_id"]', repoB);
    await page.click('button:has-text("Add Song")');
    await page.waitForURL(/\/songbook\/[^/]+$/, { timeout: 8000 });
    await waitForText(page, "body", SONG_A_TITLE);
