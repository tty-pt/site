 ...
 *   3. POST /gig/<id>/edit with valid data redirects to the view page
 *   4. After save, the view page still shows the gig title
 *
...
Deno.test("gig: register → login → create gig → load edit page → save → view", async () => {
  ...
  try {
    ...
    const editResp = await page.evaluate(
      ...
      { url: `${BASE}/gig/${sbId}/edit`, csrfToken: csrfCookie.value },
    );
