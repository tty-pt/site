...
import plugin, { canImplement, getState, type StoredState } from "../../.pi/extensions/quest-journal.ts";
...
Deno.test("quest_journal_subquest_launch_compaction_resume: subquest launch, compaction, and child resumption lifecycle", async (t) => {
	...
	await t.step("1. Subquest launch with launch compaction consumes pendingSubquestResume and resumes child", async () => {
		...
		const parentPath = `${QUEST_DIR}/${parentSlug}.md`;
		const childPath = `${QUEST_DIR}/${childSlug}.md`;

		// 1. Parent quest is active
		await writeFile(
			parentPath,
			`# Quest: ${parentSlug}\n\n## Goal\nRoot goal\n\n## Current Understanding\nParent context\n\n## Key Assumptions\n- None\n\n## Plan\n1. Launch child subquest\n\n## Plan Confidence\nHigh\n\n## Exact Next Action\nLaunch child subquest\n`,
			"utf8",
		);
		...
		assert.ok(resumeText.includes(`docs/current/${childSlug}.md`), "Continuation must reference child quest file");
	...
	await t.step("3. Compaction failure fallback immediately resumes the pending child", async () => {
		...
		assert.strictEqual(userMessages.length, 1, "Fallback handler must dispatch child resume directive");
		const fallbackMsg = userMessages[0];
		const fallbackText = typeof fallbackMsg.msg === "string" ? fallbackMsg.msg : (fallbackMsg.msg?.text || "");
		assert.ok(fallbackText.includes(`sub-quest \`${childSlug}\``) || fallbackText.includes(`sub-quest **${childSlug}**`), "Fallback continuation must target child");
		assert.strictEqual(fallbackMsg.options?.deliverAs, "followUp", "Fallback directive must use deliverAs: 'followUp'");
