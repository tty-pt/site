...
Deno.test("quest_journal_formation: research-grounded quest formation & verbatim prompt retention", async (t) => {
	...
	const semanticQuestPath = `docs/current/${semanticSlug}.md`;
	const existingSlug = "existing-auth-pipeline";
	const existingQuestPath = `docs/current/${existingSlug}.md`;
	const subSlug = "opus-packet-decoder";
	const subQuestPath = `docs/current/${subSlug}.md`;

	await rm(semanticQuestPath, { force: true });
	await rm(existingQuestPath, { force: true });
	...
	const rawUserPrompt = "Please investigate why high-bitrate FLAC files stutter intermittently during playback in the browser client and optimize the streaming pipeline";

	// -----------------------------------------------------------------------
	// 1. Substantive root prompt does NOT immediately produce a quest file
	// -----------------------------------------------------------------------
	await t.step("1. substantive root prompt does NOT immediately create a quest file on disk", async () => {
		for (const cb of handlers["before_agent_start"] || []) {
			await cb({ prompt: rawUserPrompt, systemPrompt: "Base prompt." }, mockCtx);
		}

		// Verify no file was created on disk merely from slugifying the prompt
		let fileExisted = false;
		try {
			await readFile(`docs/current/please-investigate-why-high-bitrate.md`, "utf8");
			fileExisted = true;
		} catch {}
		assert.strictEqual(fileExisted, false, "Mechanical prompt slug file must NOT be written to disk");

		// Status reports provisional state
		const status = await commands["quest-status"].handler("", mockCtx);
		assert.ok(status.includes("PROVISIONAL ROOT INITIALIZATION"), "Must report provisional root initialization state");
	});

	// -----------------------------------------------------------------------
	// 2. Initial research tools are allowed before quest creation/implementation
	// -----------------------------------------------------------------------
	await t.step("2. investigation & read tools are allowed during provisional orientation", async () => {
	...
	// -----------------------------------------------------------------------
	await t.step("3. direct feature edits remain blocked before quest identity and research complete", async () => {
		const blockRes = await emitToolCall("edit", { path: "mods/song/player.c" });
		assert.ok(blockRes, "Must block implementation edit");
		assert.strictEqual(blockRes.block, true, "Implementation edit is blocked");
		assert.ok(blockRes.reason.includes("PROVISIONAL_RESEARCH_PENDING"), "Reason must identify PROVISIONAL_RESEARCH_PENDING");
	});

	// -----------------------------------------------------------------------
	// 4. Model establishes semantic quest identity and research findings
	// -----------------------------------------------------------------------
	await t.step("4. model establishes semantic quest identity and populates research-grounded quest file", async () => {
		const initRes = await tools["quest_update_state"].execute(
			"call_init_semantic",
			{
				name: semanticSlug,
				goal: "Eliminate FLAC streaming stutter by optimizing audio buffer sizes and prefetch windows",
				status: "Research complete",
				understanding: "Audio player in mods/song uses axil chunking with 4KB ring buffers that underrun at 192kHz/24bit.",
				assumptions: [
				...
				planConfidence: "high",
				planConfidenceReason: "Verified data flow and buffer sizing against 192kHz stream telemetry.",
				exactNextAction: "Present findings and plan to user before editing player.c",
		...
		assert.ok(!initRes.content[0].text.includes("refused"), "Valid research state must be accepted");

		// Verify the file was created at the semantic slug
		const diskContent = await readFile(semanticQuestPath, "utf8");
		assert.ok(diskContent.includes(`# Quest: ${semanticSlug}`), "Quest heading matches semantic name");
		assert.ok(diskContent.includes("Eliminate FLAC streaming stutter"), "Goal is substantive");
		assert.ok(diskContent.includes("Root cause is undersized chunk buffer"), "Research findings are present");
		assert.ok(diskContent.includes("FLAC_CHUNK_SIZE to 64KB"), "Plan steps are present");

		// -------------------------------------------------------------------
		// 5. Verbatim user prompt is preserved in ## Original request
		// -------------------------------------------------------------------
		assert.ok(diskContent.includes(rawUserPrompt), "Original prompt must be preserved verbatim in ## Original request");
	});

	// -----------------------------------------------------------------------
	// 6. First user-facing root interaction asks for confirmation before edit
	// -----------------------------------------------------------------------
	await t.step("6. root quest is blocked awaiting user confirmation after research", async () => {
		const blockRes = await emitToolCall("edit", { path: "mods/song/player.c" });
		...
		assert.strictEqual(allowRes?.block, undefined, "Implementation allowed after confirmation");
	});
	...
	await t.step("7. existing quests resume without being renamed", async () => {
		...
		assert.ok(status.includes(existingSlug), "Existing quest must be activated");
		assert.ok(!status.includes("PROVISIONAL"), "Existing quest must not enter provisional state");

		// Verify file content was not overwritten or renamed
		const checkContent = await readFile(existingQuestPath, "utf8");
		assert.ok(checkContent.includes("Migrate authentication cookies"), "Original goal intact");
		assert.ok(checkContent.includes("Plan Version\n2"), "Original plan version intact");
	});

	// -----------------------------------------------------------------------
	// 8. Subquest naming & behavior remains intact and autonomous
	// -----------------------------------------------------------------------
	await t.step("8. subquest creation and autonomous research/implementation intact", async () => {
		...
		const subBlockRes = await emitToolCall("edit", { path: "mods/song/opus.c" });
		assert.strictEqual(subBlockRes?.block, true, "Sub-quest requires research first");
		...
		const subAllowRes = await emitToolCall("edit", { path: "mods/song/opus.c" });
		assert.strictEqual(subAllowRes?.block, undefined, "Sub-quest implements autonomously");
	});
