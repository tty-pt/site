...
Deno.test("quest_journal_hardening: code fence parsing, timer safety, token calculations, and path consistency", async () => {
	...
	await mkdir("docs/current", { recursive: true });
	...
	assert.ok(updateRes && updateRes.content[0].text.includes("Successfully updated quest state"), "Update state must succeed");
	...
	assert.ok(updatedContent.includes("# This is a bash comment that looks like a markdown heading"), "Bash comments inside code fences must survive");
	assert.ok(updatedContent.includes("## This looks like an H2 section inside code"), "H2 inside code fences must not be treated as a section header");
	assert.ok(updatedContent.includes("## Fake Section Inside Tildes"), "H2 inside tilde fences must not be treated as a section header");
	assert.ok(updatedContent.includes("- [x] Hardening Complete"), "Current status must be properly updated");
	assert.ok(updatedContent.includes("Verify all unit tests pass"), "Next step must be updated");
	...
	const sessionCtxA: any = {
		...
		ui: { notify: () => {}, setStatus: () => {} },
		getContextUsage: () => ({ tokens: 100000, contextWindow: 120000, percent: 83 }),
		compact: (options: any) => {
			if (options.onComplete) options.onComplete();
		},
	};
	...
	const sessionCtxB: any = {
		...
		ui: { notify: () => {}, setStatus: () => {} },
		getContextUsage: () => ({ tokens: 20000, contextWindow: 120000, percent: 16 }),
	};

	const questPathA = "docs/current/timer-quest-a.md";
	const questPathB = "docs/current/timer-quest-b.md";
	await rm(questPathA, { force: true });
	await rm(questPathB, { force: true });
	...
	assert.ok(archiveRes.details.archived === "timer-quest-a", "Archive must target timer-quest-a");
	...
	assert.ok(statusB.includes("timer-quest-b"), "Session B must still have timer-quest-b active after timer callbacks in session A");
