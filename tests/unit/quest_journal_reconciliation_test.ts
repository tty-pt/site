...
import plugin, {
	...
	type StoredState,
} from "../../.pi/extensions/quest-journal.ts";
...
Deno.test("quest_journal_reconciliation: complete verification of durable-state reconciliation and consistency audit", async (t) => {
	...
	// -----------------------------------------------------------------------
	await t.step("1. Concrete failure case: detected as contradictory and inconsistent", () => {
...
`;
		...
		assert.strictEqual(audit.consistent, false, "Concrete failure case must be detected as inconsistent");
		assert.ok(audit.issues.length >= 3, `Expected at least 3 consistency issues, got: ${JSON.stringify(audit.issues)}`);

		// Issue 1: Exact Next Action repeats completed action
		const hasRepeatedAction = audit.issues.some((i) => i.includes("repeats work already recorded as completed"));
		assert.ok(hasRepeatedAction, "Must detect that Exact Next Action repeats completed work from Reassessment");

		// Issue 2: Files Modified is empty despite modified file in Reassessment
		const hasMissingFiles = audit.issues.some((i) => i.includes("Files Modified is empty") || i.includes("i18n_dict.h"));
		assert.ok(hasMissingFiles, "Must detect that Files Modified is empty despite i18n_dict.h being modified");

		// Issue 3: Completed is empty despite completed action in Reassessment
		const hasEmptyCompleted = audit.issues.some((i) => i.includes("Completed section is empty"));
		assert.ok(hasEmptyCompleted, "Must detect that Completed is empty despite completed addition in Reassessment");

		// Issue 4: Plan Version 2 with only initial plan formulated
		const hasPlanVersionMismatch = audit.issues.some((i) => i.includes("Plan Version is 2 but Plan Revisions only lists initial plan"));
		assert.ok(hasPlanVersionMismatch, "Must detect Plan Version 2 without corresponding plan revision rationale");

		// Issue 5: User confirmation claimed as eliminating all uncertainties while unverified assumptions remain
		const hasUserConfIssue = audit.issues.some((i) => i.includes("Open Questions & Uncertainties claims no uncertainties based solely on user"));
		assert.ok(hasUserConfIssue, "Must detect user confirmation improperly claimed as eliminating engineering uncertainties");
	});

	// -----------------------------------------------------------------------
	// 2. Reconciled Durable State Verification
	// -----------------------------------------------------------------------
	await t.step("2. Reconciled durable state: all fields synchronized and consistent", () => {
...
`;
		...
		assert.strictEqual(audit.consistent, true, `Reconciled markdown must be consistent, got issues: ${JSON.stringify(audit.issues)}`);
	...
	// -----------------------------------------------------------------------
	await t.step("3. Live pointer invariant: Exact Next Action cannot repeat completed tasks", () => {
...
`;
		...
		assert.ok(audit1.issues.some((i) => i.includes("repeats work already recorded as completed")));
		...
		assert.strictEqual(audit2.consistent, true, `Advanced live pointer should be consistent, got: ${JSON.stringify(audit2.issues)}`);
	});

	// -----------------------------------------------------------------------
	// 4. Meaningful Plan Versions & Plan Revisions
	// -----------------------------------------------------------------------
	await t.step("4. Meaningful Plan Versions: Version > 1 requires explained revision", () => {
...
`;
		...
		assert.strictEqual(audit1.consistent, false);
		assert.ok(audit1.issues.some((i) => i.includes("Plan Version is 2 but Plan Revisions only lists initial plan")));
		...
		assert.strictEqual(audit2.consistent, true, `Revision explanation must pass audit, got: ${JSON.stringify(audit2.issues)}`);
	});

	// -----------------------------------------------------------------------
	// 5. User Decisions vs Engineering Invariants
	// -----------------------------------------------------------------------
	await t.step("5. User requirement confirmation does not eliminate engineering uncertainties", () => {
...
`;
		...
		assert.ok(audit.issues.some((i) => i.includes("claims no uncertainties based solely on user requirement confirmation")));
	});
...
`;
		...
		assert.strictEqual(audit.consistent, false);
		assert.ok(audit.issues.some((i) => i.includes("Files were modified but Test / Build Status is empty")));
	});

	// -----------------------------------------------------------------------
	// 7. Structured State Reconciliation via quest_update_state
	// -----------------------------------------------------------------------
	await t.step("7. quest_update_state synchronizes completed, filesModified, testStatus, and nextAction", async () => {
		...
		const res: any = await updateTool.execute(
			"call_1",
			{
				...
				completed: ["Added I18N_LOCALE_EN and I18N_LOCALE_PT definitions to i18n_dict.h"],
				...
				exactNextAction: "Run the i18n unit tests and verify locale constants",
			},
			undefined,
			undefined,
			mockCtx,
		...
		const writtenContent = await readFile(`${QUEST_DIR}/reconciliation-test-quest.md`, "utf8");
		assert.ok(writtenContent.includes("Added I18N_LOCALE_EN and I18N_LOCALE_PT"), "Completed text must be written");
		assert.ok(writtenContent.includes("i18n_dict.h"), "Files modified must be written");
		assert.ok(writtenContent.includes("Unit tests pending rerun"), "Test status must be written");
		assert.ok(writtenContent.includes("Run the i18n unit tests and verify"), "Exact next action must be written");

		const audit = auditQuestConsistency(writtenContent);
		assert.strictEqual(audit.consistent, true, `Generated quest file must be internally consistent, got: ${JSON.stringify(audit.issues)}`);
	});

	// -----------------------------------------------------------------------
	// 8. verifyAndMarkSaved integration with Consistency Audit
	// -----------------------------------------------------------------------
	await t.step("8. quest_mark_saved reports consistency audit notices on contradictory markdown", async () => {
		const { mockPi, registeredTools } = createMockExtensionAPI();
		plugin(mockPi as any);
		const mockCtx = createMockContext();

		const contradictoryPath = `${QUEST_DIR}/contradictory-quest.md`;
		await writeFile(
			contradictoryPath,
...
`,
		...
		const markTool = registeredTools.find((t) => t.name === "quest_mark_saved");
		...
		assert.ok(res.content[0].text.includes("Consistency Audit Notice"), "Must report consistency audit notice on contradictory markdown");
		assert.strictEqual(res.details?.consistency?.consistent, false, "Consistency result in details must be false");
	});

	// -----------------------------------------------------------------------
	// 9. Session File Mutation Tracking
	// -----------------------------------------------------------------------
	await t.step("9. Session file mutation tracking identifies omitted modified files", async () => {
...
`,
		...
		for (const cb of handlers["tool_result"] || []) {
			await cb({ toolName: "edit", input: { path: "mods/core/core.c" } }, mockCtx);
		}

		const content = await readFile(questFilePath, "utf8");
		const audit = auditQuestConsistency(content, { recentModifiedFiles: ["mods/core/core.c"] });
		assert.strictEqual(audit.consistent, false, "Audit must detect that modified session file mods/core/core.c is missing from Files Modified");
		assert.ok(audit.issues.some((i) => i.includes("mods/core/core.c")));
	});
